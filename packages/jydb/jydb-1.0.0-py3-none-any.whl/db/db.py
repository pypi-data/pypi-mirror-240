...
class db(object):
    ...
    def __init__(self, config=None, host=None, port=None, user=None, password=None, db=None, charset=None, read_timeout=None, cursor=__cursor):
        if config is None:
            config = {}
        if bool(config):
    ...
    def __connect(self,except_time=0):
        if self.__conn is None:
            try:
                self.__conn = pymysql.connect(host=self.__host, port=self.__port, user=self.__user,password=self.__password, db=self.__db, charset=self.__charset,cursorclass=self.__cursor, read_timeout=self.__read_timeout)
            except Exception as e:
...
if __name__=='__main__':
    ...
    data = db(config).query('select "a" as a,\'b\' as b from level limit  10')
    for d in data:
        print(d)
