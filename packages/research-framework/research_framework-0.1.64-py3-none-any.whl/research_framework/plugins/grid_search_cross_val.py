...
class WandbSeepsAgent(BaseFilterPlugin):
    ...
    def __init__(self, scorer=MetricModel(clazz='F1'), refit=True, filters=[]):
        self.filters = filters
        self.scorer = scorer
        self.refit = refit
    ...
    def fit(self, item:ItemModel):
        ...
        sweep_run = WandbSeepsRun(sweep_id=self.sweep_id, scorer=self.scorer, n_splits=3, test_size=0.3, random_state=43, count=None)
        sweep_run.fit(item)
...
class WandbSeepsRun(BaseFilterPlugin):
    def __init__(self, sweep_id, scorer, n_splits, test_size, random_state, count):
        self.count = count
        self.sweep_id = sweep_id
        self.data = None
        self.n_splits = n_splits
