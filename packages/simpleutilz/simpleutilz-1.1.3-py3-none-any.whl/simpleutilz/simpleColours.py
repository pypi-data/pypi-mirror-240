class SimpleColours: 
    def RGB_to_colour(self, value):
        '''Converts RGB value to a colour string'''
        denaryValue = self.convert_value_to_denary(value, "rgb")
        colour = self.Binary_search(denaryValue)
        return colour

    def hex_to_colour(self, value):
        '''Converts hex value to a colour string'''
        denaryValue = self.convert_value_to_denary(value, "hex")
        colour = self.Binary_search(denaryValue)
        return colour
        
    def RGBA_to_colour(self, value):
        '''Converts RGBA value to a colour string'''
        denaryValue = self.convert_value_to_denary(value, "rgba")
        colour = self.Binary_search(denaryValue)
        return colour

    def binary_to_colour(self, value):
        '''Converts binary value to a colour string'''
        denaryValue = self.convert_value_to_denary(value, "binary")
        colour = self.Binary_search(denaryValue)
        return colour

    def convert_value_to_denary(self, value, whatFormat):
