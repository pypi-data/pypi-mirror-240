...
class ssh接口类:
    ...
    def 有效(self) -> bool:
        ...
        self.主机地址 = str(self.主机地址 if self.主机地址 else '').strip()
        self.用户名 = str(self.用户名 if self.用户名 else '').strip()
        self.密码 = str(self.密码 if self.密码 else '').strip()
        if isinstance(self.端口号, str) and self.端口号.isnumeric():
            self.端口号 = int(self.端口号)
        if not (isinstance(self.端口号, int) and 1 <= self.端口号 <= 65535):
            self.端口号 = 22
        if self.主机名 and self.主机地址 and self.用户名 and self.密码:
            return True
        return False
...
class everything搜索接口类(搜索接口基类):
    ...
    def 搜索(self,
        ...
        参考路径 = str(参考路径 if 参考路径 else '').strip()
        参考路径 = 参考路径.replace('/', '\\').rstrip('\\')
        画板.调试消息(f'参考路径: {参考路径}')
                            ...
                            if everything搜索结果['totalResults'] > 0:
                                for result in everything搜索结果['results']:
                                    ...
                                    if 文档name and '$RECYCLE.BIN' not in 文档name:
                                        ...
                                        文档全名: str
                                        if '\\' in 文档path:
                                            文档path = 文档path.rstrip('\\')
                                            文档全名 = f"{文档path}\\{文档name}"
                                        elif '/' in 文档path:
                                            文档path = 文档path.rstrip('/')
                                            文档全名 = f"{文档path}/{文档name}"
                                        else:
...
class sshFind搜索接口类(搜索接口基类):
    ...
    def __init__(self,
        ...
        if isinstance(ssh接口, ssh接口类):
            super(sshFind搜索接口类, self).__init__(名称='未知' if not ssh接口.主机名 else ssh接口.主机名,
                                               地址=ssh接口.主机地址,
                                               端口号=ssh接口.端口号,
                                               用户名=ssh接口.用户名,
                                               密码=ssh接口.密码)
        else:
            super(sshFind搜索接口类, self).__init__(名称='未知',
                                               地址=地址,
                                               端口号=端口,
                                               用户名=用户名,
                                               密码=密码)
    ...
    def 搜索(self,
        ...
        if self.可用 and 搜索关键字:
            ...
            try:
                ...
                ssh.connect(hostname=self.ssh接口.主机地址,
                            username=self.ssh接口.用户名,
                            password=self.ssh接口.密码,
                            port=self.ssh接口.端口号)
                stdin, stdout, stderr = ssh.exec_command(find指令, timeout=1000)
...
class 交互接口类:
    ...
    @staticmethod
    def 路径格式化(路径: str) -> str:
        """
        将指定的路径中的 ~/ 和 ./ 扩展为绝对路径, 并移除尾部的 / 和 \
        :param 路径: 指定需要处理的路径
        :return: 返回扩展和格式化后的路径
        """
        路径 = str(路径 if 路径 else '').strip()
        ...
        if '/' not in 路径:
            路径 = f"{_os.path.abspath(路径)}"

        # 去除路径右侧的 \\ 或者 /
        路径 = 路径.replace('\\', '/').rstrip('/')
        return 路径
    ...
    @classmethod
    def 指定选择路径(cls, 输入提示: str = None, 搜索接口: 搜索接口基类 = None, 画板: 打印模板 = None) -> str:
        """
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                路径 = cls.路径格式化(用户输入)
                if isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用 and not 搜索接口.存在路径(路径=路径, 画板=画板.副本):
                    # 如果搜索接口可用,则尝试搜索并列出符合条件的路径供用户选择
                    可用路径列表 = 搜索接口.搜索(搜索关键字=用户输入.replace('/', '\\').rstrip('\\'),
                                     搜文档=False,
                                     搜路径=True,
                                     限定数量=50,
                                     画板=画板.副本)
                    if 可用路径列表:
                        if 可用路径列表.总数 > 0:
                            ...
                            # 添加一个额外选项,供用户反悔
                            交互端.添加选项(代号='c', 选项='返回上一步')

                            操作说明: list = [
                                f'您输入的关键字为: {绿字(用户输入)}; 以下列出了符合条件的路径供您选择']
                            if 可用路径列表.截断:
                                操作说明.append(红字(f'搜索结果未完全呈现'))
                    ...
                    # 如果路径有效,或者无法判定路径是否有效,则退出循环
                    break
        return 路径

    @classmethod
    def 指定选择文档(cls, 输入提示: str = None, 搜索接口: 搜索接口基类 = None, 画板: 打印模板 = None) -> str:
        """
        向用户发起交互,以获取用户指定的文档
        :param 输入提示: 提示用户输入时的提示内容
        :param 搜索接口: 协助用户搜索文档的接口
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                文档 = cls.路径格式化(用户输入)
                if isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用 and not 搜索接口.存在文档(文档=文档, 画板=画板.副本):
                    # 如果搜索接口可用, 并且判定文档不存在
                    可用文档列表 = 搜索接口.搜索(搜索关键字=用户输入.replace('/', '\\').rstrip('\\'),
                                     搜文档=True,
                                     搜路径=False,
                                     限定数量=50,
                                     画板=画板.副本)
                    if 可用文档列表:
                        if 可用文档列表.总数 > 0:
                            ...
                            操作说明: list = [
                                f'您输入的关键字为: {绿字(用户输入)}; 以下列出了符合条件的文档供您选择']
                            if 可用文档列表.截断:
                                操作说明.append(红字(f'搜索结果未完全呈现'))
    ...
    def 选择文档(初始路径: str = None,
        ...
        # region 初始化参考路径
        参考路径: str = 交互接口类.路径格式化(初始路径)

        # 提供用户选择文档的交互
        while True:
            if (not 参考路径) or (isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用 and not 搜索接口.存在路径(路径=参考路径, 画板=画板.副本)):
                # if 参考路径不存在, 或者可以判断参考路径不存在
                画板.消息(f"当前路径(本地)为: {交互接口类.路径格式化('.')}")
            ...
            if not 文档列表:
                ...
                # 引导用户重新指定操作路径
                参考路径 = 交互接口类.指定选择路径(输入提示='请指定文档路径(0: 退出操作): ',
                                    搜索接口=搜索接口,
                                    画板=画板)
                if '0' == 参考路径:
                    # 如果用户选择了退出,则退出
            ...
            if '/' in 样本文档:
                ...
                候选文档路径 = 样本文档
            # 把文档名替换为 '', 可得路径
            候选文档路径 = 样本文档.replace(候选文档路径, '')
            候选文档名列表 = [文档.replace(候选文档路径, '').lstrip(路径分隔符).strip() for 文档 in 文档列表]
            ...
            if 'c' in 用户选择.代号 or 'C' in 用户选择.代号:
                ...
                # 处理用户选择的文档
                候选文档路径 = 交互接口类.路径格式化(候选文档路径)
                for 文档 in 用户选择.选项:
                    if 文档:
                        文档全名: str = f"{候选文档路径}/{文档}"
                        选择结果.append(文档全名)
