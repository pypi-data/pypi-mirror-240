    ...
    def 指定选择路径(cls, 输入提示: str = None, 搜索接口: 搜索接口基类 = None, 画板: 打印模板 = None) -> str:
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                if isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用 and not 搜索接口.存在路径(路径=路径,
                                                                          画板=画板.副本):
                    # 如果搜索接口可用,则尝试搜索并列出符合条件的路径供用户选择
                    可用路径列表 = 搜索接口.搜索(搜索关键字=关键字.replace('/', '\\').rstrip('\\'),
                                     搜文档=False,
                                     搜路径=True,
                                     限定数量=500,
                                     画板=画板.副本)
                    if 可用路径列表:
                        if 可用路径列表.总数 > 0:
                            ...
                            if 路径列表:
                                ...
                                画板.消息('不存在满足条件的路径选项, 可能的原因是搜索关键字过于宽泛,您可以提供更多关键字信息以便重试')
    ...
    def 指定选择文档(cls, 输入提示: str = None, 搜索接口: 搜索接口基类 = None, 画板: 打印模板 = None) -> str:
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                if isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用 and not 搜索接口.存在文档(文档=文档,
                                                                          画板=画板.副本):
                    # 如果搜索接口可用, 并且判定文档不存在
                    可用文档列表 = 搜索接口.搜索(搜索关键字=关键字.replace('/', '\\').rstrip('\\'),
                                     搜文档=True,
                                     搜路径=False,
                                     限定数量=500,
                                     画板=画板.副本)
                    if 可用文档列表:
                        if 可用文档列表.总数 > 0:
                            ...
                            if 文档列表:
                                ...
                                # 没有满足约束条件的搜索结果,请用户重新输入, 以便重新搜索
                                画板.消息('不存在满足条件的文档选项, 可能的原因是搜索关键字过于宽泛,您可以提供更多关键字信息以便重试')
