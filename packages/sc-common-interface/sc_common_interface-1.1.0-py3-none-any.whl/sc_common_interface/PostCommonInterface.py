...
def modify_lucky_draw(data):
    ...
    if "winComment" in data:
        winComment = data["winReplyComment"]
    winMessage = "🎁 恭喜您中奖！请与小编联系确认领奖细节！"
    if "winMessage" in data:
        winMessage = data["winReplyComment"]
    message = {
        "winReplyComment":"🎁 恭喜您中奖！请与小编联系确认领奖细节！",
        "winComment":"恭喜 {@winner} 中奖 👏，请与小编联系确认领奖细节！",
        "winMessage":"🎁 恭喜您中奖！请与小编联系确认领奖细节！"
...
def relate_post(data):
    ...
    post_id = jsonpath(available_post_list[0],"$..data.id")[0]
    message = jsonpath(available_post_list[0],"$..data.message")[0]
    permalink_url = jsonpath(available_post_list[0],"$..data.permalink_url")[0]
    status_type = jsonpath(available_post_list[0],"$..data.status_type")[0]
    picture = jsonpath(available_post_list[0],"$..data.picture")[0]
