...
class AbstractGridEntity(LogicalEntity):
    ...
    @property
    def movement_energy(self):
        if self._movement_energy is None:
            self._movement_energy = self.speed
        return self._movement_energy
    
    @movement_energy.setter
    def movement_energy(self, movement_energy):
...
class GridEntity(AbstractGridEntity):
    ...
    def speed(self, value):
        self._speed = value
        self._movement_energy = value
        
    def end_turn(self):
        self._action_history.append(self.actions)
        self.movements = 0
        self.movement_energy = self.speed - abs(self.movement_energy) if self.movement_energy < 0 else self.speed
    ...
    def _update_position(self, terrain_char):
        actor_char = '@'
        self.last_cell.terrain_char = terrain_char
        self.cell.terrain_char = actor_char

    def _enough_energy(self, cell_to_move_to: object = None):
        if self.movement_energy <= 0:
            return 'No energy remaining.'
        if self.movement_energy < self.cell.cost_out:
            return f'Not enough energy to move out of cell {self.cell.designation}.'
        elif self.movement_energy < cell_to_move_to.cost_in:
            return f'Not enough energy to move into cell {cell_to_move_to.designation}.'
        elif self.movement_energy < self.cell.cost_out + cell_to_move_to.cost_in:
            return f'Not enough energy to complete move from cell {self.cell.designation} to cell {cell_to_move_to.designation}.'
        else:
            return True

    def move(self, cell_to_move_to: object = None, teleport: bool = False):
        if not teleport:
            has_energy = self._enough_energy(cell_to_move_to)
            if not has_energy:
                return has_energy
            ...
            if not cell_to_move_to.passable:
                return 'Cell impassable'
            elif cell_to_move_to in [self.grid[cell] for cell in self.cell.adjacent]:
                self._extracted_from_move_10(cell_to_move_to)
            else:
                return 'Invalid move'
        self.vacate()
    ...
    def _extracted_from_move_10(self, cell_to_move_to):
        if not self.movements:
            ...
            self.actions['move'].update({self.movements: {'direction': f'{list(_DIRECTION_MAP.keys())[self.cell.adjacent.index(cell_to_move_to.designation)]}', 'from': f'{self.cell.designation}', 'to': f'{cell_to_move_to.designation}'}})
        energy = self.movement_energy
        cost = self.cell.cost_out
        cost += cell_to_move_to.cost_in
        self.movement_energy -= cost
        self.movements += 1
        print(f'Energy: {energy} - {cost} = {self.movement_energy}')
    ...
    def refresh(self, dt):
        if self.movement_energy > 0 and self.traveling:
            self.move_in_path()
        elif not self.path:
            self.traveling = False
            self.movement_queue = []
