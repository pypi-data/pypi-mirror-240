...
class AstNodeLiteralExpression(AstNodeExpression):
    def __init__(self, token: LexToken, value: Optional[Any] = None, value_type: AstNodetype = AstNodetype.NIL) -> AstNodeLiteralExpression:
        super().__init__(token)
        self.type = AstNodetype.LITERAL_EXPRESSION
        if value is None:
            self._value: Optional[Any] = token.value
            self._value_type: AstNodetype = token.type
        else:
    ...
    def tree(self, indent: str = '') -> None:
        ...
        print(f"{indent}    {self.value} ({self._token.type})")
