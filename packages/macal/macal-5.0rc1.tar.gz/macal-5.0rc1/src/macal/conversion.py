...
def convertToValue(token: LexToken) -> Any:
    if token.type == AstNodetype.INTEGER:
        return int(token.value)
    elif token.type == AstNodetype.FLOAT:
        return float(token.value)
    elif (token.type == AstNodetype.STRING
            or token.type == AstNodetype.STRING_INTERPOLATION_STRING_PART
            or token.type == AstNodetype.STRING_INTERPOLATION_END):
        #return token.value
        return token.value[1:-1]
    elif token.type == AstNodetype.BOOLEAN:
        return token.value == 'true'
    elif token.type == AstNodetype.BOOLEAN_TRUE:
        return True
    elif token.type == AstNodetype.BOOLEAN_FALSE:
        return False
    elif token.type == AstNodetype.NIL:
        return None
    else:
        raise Exception(f"convertToValue(): Unknown type {token.type}")
