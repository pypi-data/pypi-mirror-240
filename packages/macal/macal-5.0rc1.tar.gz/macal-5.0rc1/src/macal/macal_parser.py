...
from .conversion import convertToValue
...
class Parser:
    def __init__(self, tokens: Optional[List[LexToken]] = None, filename: Optional[str] = None) -> Parser:
        ...
        if tokens is None:
            tokens = []
        self.tokens: List[LexToken] = [token for token in tokens if token.type != AstNodetype.WHITESPACE and token.type != AstNodetype.COMMENT]
    ...
    def enforceLiteralType(self, type: AstNodetype, value: Any) -> Any:
        if type == AstNodetype.INTEGER and value != "integer":
            ...
            return float(value)
        elif type in (AstNodetype.STRING,
                          AstNodetype.STRING_INTERPOLATION_STRING_PART,
                          AstNodetype.STRING_INTERPOLATION_END) and value != "string":
    ...
    def get_token(self, index: int) -> LexToken:
        ...
        token.value = self.enforceLiteralType(token.type, token.value)
        return token
    ...
    def expect(self, type: AstNodetype) -> Optional[LexToken]:
        ...
        if token.type == type:
            self.advance()
            return token
        self.error(f"Expected {type} but got {token.type} {token.lineno}, {token.offset}")
    ...
    def parse_interactive(self, tokens: List[LexToken]) -> AstNodeExpression:
        self.tokens = [token for token in tokens if token.type != AstNodetype.WHITESPACE and token.type != AstNodetype.COMMENT]
        self.pos = 0
        self.current_token = self.tokens[self.pos]
        self.node = self.program()
        return self.node
    ...
    def block_statement_list(self, node: AstNodeBlock) -> AstNodeBlock:
        while self.current_token is not None and self.current_token.type != AstNodetype.RIGHT_BRACE and self.current_token.type != AstNodetype.EOF:
            if self.current_token.type != AstNodetype.EOF:
    ...
    def statement(self) -> AstNodeExpression:
        if self.current_token.type == AstNodetype.IF_STATEMENT:
            return self.if_statement()
        if self.current_token.type == AstNodetype.WHILE_STATEMENT:
            return self.while_statement()
        if self.current_token.type == AstNodetype.FOREACH_STATEMENT:
            return self.foreach_statement()
        if self.current_token.type == AstNodetype.SELECT_STATEMENT:
            return self.select_statement()
        if self.current_token.type == AstNodetype.BREAK_STATEMENT:
            return self.break_statement()
        if self.current_token.type == AstNodetype.CONTINUE_STATEMENT:
            return self.continue_statement()
        if self.current_token.type == AstNodetype.RETURN_STATEMENT:
            return self.return_statement()
        if self.current_token.type == AstNodetype.IDENTIFIER:
            return self.assignment_statement()
        if self.current_token.type == AstNodetype.LEFT_BRACE:
            return self.block()
        if self.current_token.type == AstNodetype.PRINT_STATEMENT:
            return self.print_statement()
        if self.current_token.type == AstNodetype.HALT_STATEMENT:
            return self.halt_statement()
        if self.current_token.type == AstNodetype.INCLUDE_STATEMENT:
            return self.include_statement()
        if self.current_token.type == AstNodetype.SWITCH_STATEMENT:
            return self.switch_statement()
        if self.current_token.type == AstNodetype.CONST_STATEMENT:
            return self.const_statement()
        self.error(f"Invalid statement {self.current_token.type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
    ...
    def if_statement(self) -> AstNodeExpression:
        ...
        if self.current_token.type == AstNodetype.ELIF_STATEMENT:
            while self.current_token.type == AstNodetype.ELIF_STATEMENT:
                node.elif_block_list.append(self.elif_statement())
        if self.current_token.type == AstNodetype.ELSE_STATEMENT:
    ...
    def select_statement(self) -> AstNodeExpression:
        ...
        distinct = self.current_token.type == AstNodetype.DISTINCT_STATEMENT
        if distinct:
            self.advance()
        fields: List[AstNodeSelectField] = []
        while self.current_token.type == AstNodetype.IDENTIFIER:
            fieldname = self.current_token
            self.advance()
            alt = None
            if self.current_token.type == AstNodetype.AS_STATEMENT:
                self.advance()
                alt = self.current_token
                self.advance()
            fields.append(AstNodeSelectField(fieldname, alt))
            if self.current_token.type == AstNodetype.COMMA:
                self.advance()
        if len(fields) == 0 and self.current_token.type == AstNodetype.OPERATOR_MULTIPLICATION:
        ...
        if self.current_token.type == AstNodetype.WHERE_STATEMENT:
            self.advance()
            where_expr = self.parse_expression()
        merge = self.current_token.type == AstNodetype.MERGE_STATEMENT
        if merge:
            self.advance()
        self.expect(AstNodetype.INTO_STATEMENT)
        into = self.current_token
        self.advance()
        self.expect(AstNodetype.SEMICOLON)
        return AstNodeSelectStatement(token, fields, from_expr, into, merge, distinct, where_expr)


        self.error(f"Select statement not implemented yet {self.current_token.type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
    ...
    def return_statement(self) -> AstNodeExpression:
        ...
        if self.current_token.type != AstNodetype.SEMICOLON:
    ...
    def library_list(self) -> List[AstNodeExpression]:
        ...
        while self.current_token.type == AstNodetype.COMMA:
            self.advance()
            token = self.current_token
            self.expect(AstNodetype.IDENTIFIER)
    ...
    def switch_statement(self) -> AstNodeExpression:
        ...
        if expr.type not in [AstNodetype.VARIABLE_EXPRESSION, AstNodetype.INDEXED_VARIABLE_EXPRESSION,
                             AstNodetype.FUNCTION_CALL_EXPRESSION, AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION]:
            self.error(f"Invalid switch expression type {expr.type} at: {expr.line}, {expr.column}")
        node = AstNodeSwitchStatement(token, expr)
        self.expect(AstNodetype.LEFT_BRACE)
        while self.current_token.type == AstNodetype.CASE_STATEMENT:
            node.cases.append(self.case_statement())
        if self.current_token.type == AstNodetype.DEFAULT_STATEMENT:
    ...
    def case_statement(self) -> AstNodeExpression:
        ...
        expr = self.parse_expression()
        if expr.type != AstNodetype.LITERAL_EXPRESSION:
            self.error(f"Invalid case expression type {expr.type} at: {expr.line}, {expr.column}")
    ...
    def expression_list(self) -> List[AstNodeExpression]:
        ...
        if self.current_token.type == AstNodetype.RIGHT_PAREN:
            return nodes
    ...
    def expression_array_element(self) -> Any:
        ...
        if (node.type != AstNodetype.LITERAL_EXPRESSION):
            self.error(f"Invalid array element type {node.type} (expected literal, array or record) at: {node.token.lineno}, {node.token.offset}")
        vt = node.value_type
        if (vt == AstNodetype.STRING_INTERPOLATION_STRING_PART
            or vt == AstNodetype.STRING_INTERPOLATION_END):
                self.error(f"Invalid array element type {vt} (expected literal, array or record) at: {node.token.lineno}, {node.token.offset}")
        if vt == AstNodetype.ARRAY or vt == AstNodetype.RECORD:
            return node.value
        return convertToValue(node._token)

    def expression_array(self) -> list:
        if self.current_token.type == AstNodetype.RIGHT_BRACKET:
            self.error(f"Invalid array element, expected literal, got ] at {self.current_token.lineno}, {self.current_token.offset}.")
        array: list = []
    ...
    def expression_record_element(self) -> Tuple[Any, Any]:
        ...
        if key.type != AstNodetype.LITERAL_EXPRESSION:
            self.error(f"Invalid record key type {key.type} (expected literal) at: {key.line}, {key.column}")
        self.expect(AstNodetype.COLON)
        value = self.parse_expression()
        if value.type != AstNodetype.LITERAL_EXPRESSION:
            self.error(f"Invalid record value type {value.type} (expected literal, array or record) at: {value.line}, {value.column}")
        if (key.type == AstNodetype.STRING_INTERPOLATION_STRING_PART
                or key.type == AstNodetype.STRING_INTERPOLATION_END):
            self.error(f"Invalid record key type {key.type} (expected literal) at: {key.line}, {key.column}")
        if (value.type == AstNodetype.LITERAL_EXPRESSION and (value.type == AstNodetype.STRING_INTERPOLATION_STRING_PART
                    or value.type == AstNodetype.STRING_INTERPOLATION_END)):
            self.error(f"Invalid record value type {value.type} (expected literal, array or record) at: {value.line}, {value.column}")
        return (key.value, value.value)

    def expression_record(self) -> dict:
        if self.current_token.type == AstNodetype.RIGHT_BRACE:
            self.error(f"Invalid record element, expected record key, got }} at {self.current_token.lineno}, {self.current_token.offset}.")
        record: dict = {}
    ...
    def expect_assign_operand(self) -> LexToken:
        token = self.current_token
        if (token.type == AstNodetype.OPERATOR_ASSIGNMENT
            or token.type == AstNodetype.OPERATOR_ASSIGNMENT_INC
            or token.type == AstNodetype.OPERATOR_ASSIGNMENT_DEC
            or token.type == AstNodetype.OPERATOR_ASSIGNMENT_MUL
            or token.type == AstNodetype.OPERATOR_ASSIGNMENT_DIV
            or token.type == AstNodetype.OPERATOR_ASSIGNMENT_POW
            or token.type == AstNodetype.OPERATOR_ASSIGNMENT_MOD
            or token.type == AstNodetype.OPERATOR_ARROW):
            self.advance()
            return token
        self.error(f"Invalid assignment operator {token.type} {token.value} {token.lineno}, {token.offset}, {token.lexpos}")

    def assignment_statement(self) -> AstNodeExpression:
        var_expr = self.parse_expression() # Get the variable as an expression.
        if var_expr.type in [AstNodetype.FUNCTION_CALL_EXPRESSION,
                             AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION]:
            self.expect(AstNodetype.SEMICOLON)
            return AstNodeFunctionCallStatement(var_expr._token, var_expr.args)
        if not var_expr.type in [AstNodetype.VARIABLE_EXPRESSION,
                                 AstNodetype.INDEXED_VARIABLE_EXPRESSION]:
            self.error(f"Invalid assignment statement {var_expr.type} {var_expr._token.value} {var_expr._token.lineno}, {var_expr._token.offset}")
        if self.current_token.type == AstNodetype.LEFT_PAREN:
            self.expect(AstNodetype.LEFT_PAREN)
            ...
            self.expect(AstNodetype.SEMICOLON)
            return node
        if self.current_token.type == AstNodetype.NEW_ARRAY:
            self.advance()
        ...
        if op.type == AstNodetype.OPERATOR_ARROW:
    ...
    def function_definition(self, var_expr: AstNodeExpression, op: LexToken) -> AstNodeFunctionDefinition:
        ...
        if self.is_type():
            ...
            self.advance()
        if self.current_token.type == AstNodetype.EXTERNAL_STATEMENT:
            self.advance()
    ...
    def strToType(self, tstr: str) -> AstNodetype:
        if tstr == "integer": return AstNodetype.INTEGER
        if tstr == "float": return AstNodetype.FLOAT
        if tstr == "string": return AstNodetype.STRING
        if tstr == "bool": return AstNodetype.BOOLEAN
        if tstr == "array": return AstNodetype.ARRAY
        if tstr == "record": return AstNodetype.RECORD
        if tstr == "function": return AstNodetype.FUNCTION
        if tstr == "params": return AstNodetype.PARAMS
        if tstr == "variable": return AstNodetype.VARIABLE
        self.error(f"Invalid type {tstr} at {self.current_token.lineno}, {self.current_token.offset}")

    def parse_parameter_list(self) -> List[AstNodeFunctionParameter]:
        param_list: List[AstNodeFunctionParameter] = []
        self.expect(AstNodetype.LEFT_PAREN)
        while self.current_token.type != AstNodetype.RIGHT_PAREN:
            type = AstNodetype.ANY
            if self.is_type():
                type = self.current_token.type #self.strToType(self.current_token.value)
                self.advance()
...
# expression parser
    ...
    def parse_logical_or(self) -> AstNodeExpression:
        ...
        while self.current_token.type == AstNodetype.OR_STATEMENT or self.current_token.type == AstNodetype.XOR_STATEMENT:
            token = self.current_token
            self.advance()
    ...
    def parse_logical_and(self) -> AstNodeExpression:
        ...
        while self.current_token.type == AstNodetype.AND_STATEMENT:
            token = self.current_token
            self.advance()
    ...
    def parse_equality(self) -> AstNodeExpression:
        ...
        while self.current_token.type in [AstNodetype.COMPARETOR_EQUAL, AstNodetype.COMPARETOR_NOT_EQUAL]:
            token = self.current_token
            self.advance()
    ...
    def parse_comparison(self) -> AstNodeExpression:
        ...
        while self.current_token.type in [AstNodetype.COMPARETOR_LESS_THAN, AstNodetype.COMPARETOR_LESS_THAN_EQUAL, AstNodetype.COMPARETOR_GREATER_THAN, AstNodetype.COMPARETOR_GREATER_THAN_EQUAL]:
            token = self.current_token
            self.advance()
    ...
    def parse_addition(self) -> AstNodeExpression:
        ...
        while self.current_token.type in [AstNodetype.OPERATOR_ADDITION, AstNodetype.OPERATOR_SUBTRACTION]:
            token = self.current_token
            self.advance()
    ...
    def parse_multiplication(self) -> AstNodeExpression:
        ...
        while self.current_token.type in [AstNodetype.OPERATOR_MULTIPLICATION, AstNodetype.OPERATOR_DIVISION, AstNodetype.OPERATOR_MODULUS]:
            token = self.current_token
            self.advance()
    ...
    def parse_power(self) -> AstNodeExpression:
        ...
        while self.current_token.type == AstNodetype.OPERATOR_POWER:
            token = self.current_token
            self.advance()
    ...
    def parse_unary(self) -> AstNodeExpression:
        if (self.current_token.type == AstNodetype.OPERATOR_SUBTRACTION
                or self.current_token.type == AstNodetype.OPERATOR_ADDITION
                or self.current_token.type == AstNodetype.NOT_STATEMENT
                or self.current_token.type == AstNodetype.BITWISE_NOT):
            token = self.current_token
            self.advance()
    ...
    def parse_primary(self) -> AstNodeExpression:
        ...
        if self.current_token.type == AstNodetype.IDENTIFIER:
            ...
            self.advance()
            node = None
            if self.current_token.type == AstNodetype.LEFT_BRACKET:
                node = AstNodeIndexedVariableExpression(ident)
                while self.current_token.type == AstNodetype.LEFT_BRACKET:
            ...
            if self.current_token.type == AstNodetype.LEFT_PAREN:
                ...
                return node
            #print(ident)
            #print("self.current_token.type", self.current_token.type, "self.current_token.value", self.current_token.value, "self.current_token.lineno", self.current_token.lineno, "self.current_token.offset", self.current_token.offset)
            return AstNodeVariableExpression(ident)
        if self.current_token.type == AstNodetype.INTEGER:
            token = self.current_token
            self.advance()
            return AstNodeLiteralExpression(token)
        if self.current_token.type == AstNodetype.FLOAT:
            token = self.current_token
            self.advance()
            return AstNodeLiteralExpression(token)
        if self.current_token.type == AstNodetype.STRING:
            token = self.current_token
            self.advance()
            return AstNodeLiteralExpression(token)
        if self.current_token.type == AstNodetype.BOOLEAN_FALSE or self.current_token.type == AstNodetype.BOOLEAN_TRUE or self.current_token.type == AstNodetype.BOOLEAN:
            token = self.current_token
            self.advance()
            return AstNodeLiteralExpression(token)
        if self.current_token.type == AstNodetype.NIL:
            token = self.current_token
            self.advance()
            return AstNodeLiteralExpression(token)
        if self.current_token.type == AstNodetype.STRING_INTERPOLATION_START:
            return self.interpolated_string()
        if self.current_token.type == AstNodetype.STRING_INTERPOLATION_STRING_PART:
            node = AstNodeLiteralExpression(self.current_token)
            self.advance()
            return node
        if self.current_token.type == AstNodetype.NEW_ARRAY or self.current_token.type == AstNodetype.ARRAY:
            node = AstNodeLiteralExpression(self.current_token, [], AstNodetype.ARRAY)
            self.advance()
            return node
        if self.current_token.type == AstNodetype.NEW_RECORD or self.current_token.type == AstNodetype.RECORD:
        ...
        if self.current_token.type == AstNodetype.LEFT_BRACE:
            ...
            self.expect(AstNodetype.RIGHT_BRACE)
            return node
        if self.current_token.type in [AstNodetype.IS_ARRAY_STATEMENT,
        ...
        if self.current_token.type == AstNodetype.TYPE_STATEMENT:
            return self.type_statement()
        self.error(f"Invalid primary expression {self.current_token.type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
    ...
    def interpolated_string(self) -> AstNodeExpression:
        ...
        token.type = AstNodetype.OPERATOR_ADDITION
        token.value = "+"
        node = AstNodeBinaryExpression(token, AstNodeLiteralExpression(self.current_token), None)
        self.advance() # go to right hand side.
        if node.left._token.type == AstNodetype.STRING_INTERPOLATION_END: # if it's just a string without interpolation.
            return node.left
        while self.current_token.type != AstNodetype.STRING_INTERPOLATION_END:
            node.right = self.parse_expression()
            node = AstNodeBinaryExpression(token, node, None)
        if self.current_token.type == AstNodetype.STRING_INTERPOLATION_END:
            node.right = AstNodeLiteralExpression(self.current_token)
            self.advance()
            return node
        self.error(f"Invalid interpolated string {self.current_token.type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
