...
class Parser:
    def __init__(self, tokens: Optional[List[LexToken]] = None, filename: Optional[str] = None) -> Parser:
        ...
        if tokens is None:
            tokens = []
        self.tokens: List[LexToken] = [token for token in tokens if token.token_type != AstNodetype.WHITESPACE and token.token_type != AstNodetype.COMMENT]
    ...
    def expect(self, type: AstNodetype) -> Optional[LexToken]:
        ...
        if token.token_type == type:
            self.advance()
            return token
        self.error(f"Expected {type} but got {token.token_type} {token.lineno}, {token.offset}")
    ...
    def parse_interactive(self, tokens: List[LexToken]) -> AstNodeExpression:
        self.tokens = [token for token in tokens if token.token_type != AstNodetype.WHITESPACE and token.token_type != AstNodetype.COMMENT]
        self.pos = 0
        self.current_token = self.tokens[self.pos]
        self.node = self.program()
        return self.node
    ...
    def block_statement_list(self, node: AstNodeBlock) -> AstNodeBlock:
        while self.current_token is not None and self.current_token.token_type != AstNodetype.RIGHT_BRACE and self.current_token.token_type != AstNodetype.EOF:
    ...
    def statement(self) -> AstNodeExpression:
        ...
        self.error(f"Invalid statement {self.current_token.token_type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
    ...
    def select_statement(self) -> AstNodeExpression:
        ...
        self.error(f"Select statement not implemented yet {self.current_token.token_type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
    ...
    def switch_statement(self) -> AstNodeExpression:
        ...
        if expr.expr_type not in [AstNodetype.VARIABLE_EXPRESSION, AstNodetype.INDEXED_VARIABLE_EXPRESSION,
                             AstNodetype.FUNCTION_CALL_EXPRESSION, AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION]:
            self.error(f"Invalid switch expression type {expr.expr_type} at: {expr.line}, {expr.column}")
    ...
    def case_statement(self) -> AstNodeExpression:
        ...
        if expr.expr_type != AstNodetype.LITERAL_EXPRESSION:
            self.error(f"Invalid case expression type {expr.expr_type} at: {expr.line}, {expr.column}")
    ...
    def expression_array_element(self) -> Any:
        ...
        if (node.expr_type != AstNodetype.LITERAL_EXPRESSION):
            self.error(f"Invalid array element type {node.expr_type} (expected literal, array or record) at: {node.token.lineno}, {node.token.offset}")
        vt = node.value_type
    ...
    def expression_record_element(self) -> Tuple[Any, Any]:
        ...
        if key.expr_type != AstNodetype.LITERAL_EXPRESSION:
            self.error(f"Invalid record key type {key.expr_type} (expected literal) at: {key.line}, {key.column}")
        self.expect(AstNodetype.COLON)
        value = self.parse_expression()
        if value.expr_type != AstNodetype.LITERAL_EXPRESSION:
            self.error(f"Invalid record value type {value.expr_type} (expected literal, array or record) at: {value.line}, {value.column}")
        if (key.expr_type == AstNodetype.STRING_INTERPOLATION_STRING_PART
                or key.expr_type == AstNodetype.STRING_INTERPOLATION_END):
            self.error(f"Invalid record key type {key.expr_type} (expected literal) at: {key.line}, {key.column}")
        if (value.expr_type == AstNodetype.LITERAL_EXPRESSION and (value.expr_type == AstNodetype.STRING_INTERPOLATION_STRING_PART
    ...
    def expect_assign_operand(self) -> LexToken:
        token = self.current_token
        if (token.token_type == AstNodetype.OPERATOR_ASSIGNMENT
            ...
            self.advance()
            return token
        self.error(f"Invalid assignment operator {token.token_type} {token.value} {token.lineno}, {token.offset}, {token.lexpos}")

    def assignment_statement(self) -> AstNodeExpression:
        var_expr = self.parse_expression() # Get the variable as an expression.
        if var_expr.expr_type in [AstNodetype.FUNCTION_CALL_EXPRESSION,
                             AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION]:
            self.expect(AstNodetype.SEMICOLON)
            return AstNodeFunctionCallStatement(var_expr._token, var_expr.args)
        if not var_expr.expr_type in [AstNodetype.VARIABLE_EXPRESSION,
                                 AstNodetype.INDEXED_VARIABLE_EXPRESSION]:
            self.error(f"Invalid assignment statement {var_expr.expr_type} {var_expr._token.value} {var_expr._token.lineno}, {var_expr._token.offset}")
...
# expression parser
    ...
    def parse_logical_or(self) -> AstNodeExpression:
        ...
        while self.current_token.token_type == AstNodetype.OR_STATEMENT or self.current_token.token_type == AstNodetype.XOR_STATEMENT:
            token = self.current_token
            self.advance()
    ...
    def parse_comparison(self) -> AstNodeExpression:
        ...
        while self.current_token.token_type in [AstNodetype.COMPARETOR_LESS_THAN, AstNodetype.COMPARETOR_LESS_THAN_EQUAL, AstNodetype.COMPARETOR_GREATER_THAN, AstNodetype.COMPARETOR_GREATER_THAN_EQUAL]:
            token = self.current_token
            self.advance()
    ...
    def parse_multiplication(self) -> AstNodeExpression:
        ...
        while self.current_token.token_type in [AstNodetype.OPERATOR_MULTIPLICATION, AstNodetype.OPERATOR_DIVISION, AstNodetype.OPERATOR_MODULUS]:
            token = self.current_token
            self.advance()
    ...
    def parse_unary(self) -> AstNodeExpression:
        if (self.current_token.token_type == AstNodetype.OPERATOR_SUBTRACTION
    ...
    def parse_primary(self) -> AstNodeExpression:
        ...
        if self.current_token.token_type == AstNodetype.STRING:
            ...
            self.advance()
            return AstNodeLiteralExpression(token)
        if self.current_token.token_type == AstNodetype.BOOLEAN_FALSE or self.current_token.token_type == AstNodetype.BOOLEAN_TRUE or self.current_token.token_type == AstNodetype.BOOLEAN:
            token = self.current_token
            self.advance()
            return AstNodeLiteralExpression(token)
        ...
        if self.current_token.token_type == AstNodetype.LEFT_BRACE:
            ...
            self.expect(AstNodetype.RIGHT_BRACE)
            return node
        if self.current_token.token_type in [AstNodetype.IS_ARRAY_STATEMENT,
        ...
        self.error(f"Invalid primary expression {self.current_token.token_type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
    ...
    def interpolated_string(self) -> AstNodeExpression:
        ...
        if self.current_token.token_type == AstNodetype.STRING_INTERPOLATION_END:
            ...
            self.advance()
            return node
        self.error(f"Invalid interpolated string {self.current_token.token_type} {self.current_token.value} {self.current_token.lineno}, {self.current_token.offset}")
