...
def option_logs(args):
 ...
 """
    ...
    if check_modifiers('-logs',args,["-last","-tail","-store","-deletelogs","-limit","-limitinfo","-auto","-folder","-level","-loguser","-where","-inputfile","-file","-error","-noScreen","-allLimits","-debuglevel","-var"],help) == False: return

    logId = getParam(args,'-logs')    
    last = getParam(args,'-last')  
    limit = getParam(args,'-limit')    
    limitInfo = isParam(args,'-limitinfo') #True if '-limitinfo' in args else False   
    ...
    var = True if '-var' in args else False   
    var_limit = getParam(args,'-var') 
    if var == True:
        if var_limit == None: var = -1
        else: var = int(var_limit)
    else:
        var = None
    if loguser == None:
