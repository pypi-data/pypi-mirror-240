...
from .plugin_pb2 import PluginRequest, PluginResponse, RegisterReply, PluginEndpoint, ReturnVal as ApiReturnVal
...
class DefaultPluginHost(PluginHost):
    ...
    def Call(self, request: PluginRequest, context):
        try:
            ...
            key = method.replace(f"{self.plugin.root_path()}/", "")
            self.debug(f"key: {key}")
            function = self.route_map[key].function
            ret = function(input)
            self.debug(f"output: {ret}")

            # plugin_response_dict = {
            #     "id": 1234,
            #     "return_val": ApiReturnVal,
            #     "error": ""
            # }
            #
            # return PluginResponse(**plugin_response_dict)
