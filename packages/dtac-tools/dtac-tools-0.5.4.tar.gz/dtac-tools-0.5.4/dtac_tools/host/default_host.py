...
from .plugin_pb2 import PluginRequest, PluginResponse, RegisterReply, PluginEndpoint
...
class DefaultPluginHost(PluginHost):
    ...
    def Call(self, request: PluginRequest, context):
        try:
            ...
            key = request.method.replace(f"{self.plugin.root_path}/", "")
            function = self.route_map[key].function
            output = function(input)
            self.debug(f"output: {output}")
            return_val = ReturnVal.to_grpc(output)
