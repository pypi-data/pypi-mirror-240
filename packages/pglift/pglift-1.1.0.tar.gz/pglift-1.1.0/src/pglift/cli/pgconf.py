...
from .. import instances, postgresql
from ..ctx import Context
from ..models import system
from ..types import ConfigChanges, Status
...
def set_(ctx: Context, instance: system.Instance, parameters: dict[str, Any]) -> None:
    ...
    with ctx.lock:
        ...
        manifest = instances._get(ctx, instance, status)
        manifest.settings.update(parameters)
        changes = instances.configure(
            ctx, manifest, _is_running=status == Status.running
        )
