...
class Query:
    def __init__(self, table, query_string):
        ...
        url_query = unquote(str(query_string))
        self.table_query = url_query.replace("+", "")
    ...
    def parse_group(self, key, val):
        ...
        if len(self.group) == 1:
            group_field = Field(self.group[0])
            group_count_field = Field()
            group_count_field.name = "COUNT(*)"
            group_count_field.alias = 'count'
            self.fields = [group_field, group_count_field]
        return self.group
    ...
    def to_insert_sql(self, data, replace=False):
        ...
        data = [
            "(" + ",".join([f"'{i}'" if i is not None else "NULL" for i in d]) + ")"
