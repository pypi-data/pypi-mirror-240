...
from cscdata.localdata.localAbstract import _DbManager, _DbBase, _TableBase
import cscdata.localdata as localdata
from .utils import remove_file_in_directory, find_file, read_meta
...
class DbManager(_DbManager):
    ...
    h5
    """
    # local_data_path = localdata.LOCAL_DATA_PATH
    ...
    def local_data_path(self):
        return localdata.LOCAL_DATA_PATH
...
class ParquetTable(TableBase):
    def __init__(self,
        ...
        self.metainfo = self._read_meta()
        self.pandas_info = self.metainfo['pandas']
    ...
    @property
    def file(self):
        return self.show_table_list()[0]
    
    def update_time(self):
        return super().update_time()

    def pandas_read(self, columns = None, filters: list[tuple] = None, **kwargs):
        """
        read by pandas
        """
        table_list = self.show_table_list()
        df = pd.read_parquet(table_list,columns = columns, filters=filters, **kwargs)
        return df

    def spark_read(self, spark_session, columns = None, filters= None):
        """
        read by spark
        """
        table_list = self.show_table_list()
    ...
    # @property
    def _read_meta(self,):
        _table_path = self.show_table_list()[0]
        return read_meta(_table_path)
    
    def column_des(self, index = True):
        '''index=False 不带指数轴信息信息'''
        result = self.metainfo['column_des'].copy()
        if not index:
    ...
    def primary_keys(self):
        return self.metainfo['pandas']['index_columns']

    @cached_property
    def index(self):
        ''' 主键 '''
        return self.pandas_info['index_columns']
    
    @cached_property
    def columns(self):
        ''' 字段 '''
        cols = list(self.column_des().keys()).copy()
        idx = self.index
        return [x for x in cols if x not in idx]

    def get_df(self, columns = None, engine = None, filters: list[tuple] = None, index:bool = True, **kwargs):
        ...
        if engine is None :
            ...
            # if columns is None:
            #     columns = slice(None)
            df = self.pandas_read(filters = filters, columns= columns, **kwargs)
            if not index:
                df.reset_index(inplace=True)
        else:
            #default spark
            df = self.spark_read(engine, columns = columns, filters= filters)
    ...
    def get_hist(self, n_day ,day_col = "trade_dt", columns = None, engine = None):
        """ get history data"""
        date_filters = []
        _date_list = localdata._TRADE_DTS.sort(reversed = True)
        ...
        df = self.get_df(engine= engine, columns = columns, filters = date_filters)
        # total day
        return df
    
    # @property
    # def _dsets_structure(self):
    #     """support dset data"""
    #     parser = os.path.basename(os.path.dirname(self.table_path)).split('_')
    #     if len(parser)<3:
    #         return None
    #     else:
    #         return parser[1]
    ...
    # def get_ndns(self, column):
    #     '''从数据库中直接获取ndns mat结构'''
    #     # assert(self.index==['stockcode', 'trade_dt'])
    #     if self._dsets_structure in ['ndns','univ','alf', 'cne5','barra']:
    #         ndns_sorted = True
    #     else:
    #         ndns_sorted = False
    #     ds = self[column]
    #     ndns_df = self.ctx.to_ndns(ds, ndns_sorted)
    #     return ndns_df
    
    # def get_nans(self, column):
    #     '''从数据库中获取nans mat结构'''
    #     assert(self.index==['stockcode', 'ann_dt'])
    #     if self.datastruct in ['nans']:
    #         nans_sorted = True
    #     else:
    #         nans_sorted = False
    #     ds = self[column]
    #     nans_df = self.ctx.to_nans(ds, nans_sorted)
    #     return nans_df
...
class DirectoryTable(TableBase):
    ...
    def show_table_list(self):
        ...
        return [os.path.join(self.table_path, "*.parquet")]
    ...
    def get_df(self, feature_col = None, engine = None, filters: list[tuple] = None, **kwargs):
        """ return df by engine"""
    ...
    def hist(self, n_day, day_col = None, engine = None):
        ...
        for _i in range(n_day):
            date_filters.append((f"{day_col}", "==", _date_list[self.hist_idx_begin]))
            self.hist_idx_begin += 1
        df = self.get_df(engine= engine, filters = date_filters)
        return df
...
class H5DB(DbBase):
    ...
    def create_table(self, table_name, dataset_shape = None, attrs: dict = None, **kwargs):
        """create h5 table with none"""
        if dataset_shape is None:
            raise TypeError("One of data, shape or dtype must be specified")
        with h5py.File(self.db_path , 'w') as f:
            if table_name not in f:
                f.create_dataset(table_name, shape= dataset_shape, **kwargs) #here maxshape best assign as a default value
                if attrs is not None:
                    for _k, _v in attrs.items():
                        f[table_name].attrs[_k] = _v
...
class H5Table:
    ...
    # def nds_to_h5(self, value: union[pd.]):
    #     """save numpy a """
    #     pass

    def to_h5(self, df, dataset_shape= None, attrs = None, **kwargs):
        """
            gen h5 file from base df
            NOTE:
                df : Union[pd.DataFrame ,pd.Series]
                dateset_shape：:tuple[int, int]
        """
        ...
        with h5py.File(self.db_inst.db_path, 'a') as f:
            ...
            if self.table_name not in f:
                if dataset_shape is None:
                    raise TypeError("One of data, shape or dtype must be specified")
                with h5py.File(self.db_path , 'w') as f:
                    if dataset_shape not in f:
                        f.create_dataset(self.table_name, shape= dataset_shape, **kwargs) #here maxshape best assign as a default value
                        if attrs is not None:
                            for _k, _v in attrs.items():
                                f[self.table_name].attrs[_k] = _v
...
class DataAPI(DbManager):
    ...
    def use_db(self, db_name):
        ...
        NOTE:
            ...
            2. if you want change base outer path, using .init
        """
