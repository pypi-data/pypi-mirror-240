...
class ParquetTable(TableBase):
    ...
    def get_df(self, columns = None, engine = None, filters: list[tuple] = None, index:bool = True, **kwargs):
        ...
        NOTE：
            columns: 如果list，返回pd.dataframe， 如果是str， 返回pd.Series
        """
        ...
        if engine is None :
            ...
            # if columns is None:
            #     columns = slice(None)
            df = self.pandas_read(filters = filters, columns= columns_, **kwargs)
            if not index:
            ...
            #default spark
            df = self.spark_read(engine, columns = columns_, filters= filters)

            if not index:
                df = df.reset_index()
            
            # not support pandas to pd.series
            # df = pd.Series(df.select(columns).rdd.flatMap(lambda x: x).collect()) 
