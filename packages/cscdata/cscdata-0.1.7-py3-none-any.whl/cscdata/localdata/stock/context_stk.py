...
from .utils import QtCalendar, read_json, write_json, retry, context_data

today = int(datetime.today().strftime("%Y%m%d"))
yesterday = int((datetime.today() - timedelta(days = 1)).strftime("%Y%m%d"))

class FinContext(CoContext):
    def __init__(self, config = None, start_dt=20070101, end_dt= yesterday):
        super().__init__(start_dt, end_dt, config)
    ...
    def to_feature(self, ds, feature_name, database = None, des=''):
        ...
        json_file = os.path.join(self.user_repo, database, 'feature', f'feature={feature_name}', 'des.json' )
        metadata = {}
    ...
    # @retry(max_attempts=5, delay=1)
    def read_feature(self, feature_name, ndns = False):
        """
            读取非结构化数据
        """
        pth = os.path.join(self.user_repo, self.user_db_name, 'feature', f'feature={feature_name}')
        if not os.path.exists(pth):
            raise Exception(f'{feature_name} not exists')
        p_path = [os.path.join(pth,_i) for _i in os.listdir(pth) if _i.endswith('.parquet')]
        json_file = os.path.join(self.user_repo, self.user_db_name, 'feature', f"feature={feature_name}", 'des.json')
    ...
    def run(self,file):
        ...
        self.init_from_wind()
        self.ndns_idx = self.get_univ_ipo()
