...
class ContextBase(_Context):
    ...
    def __init__(self, config = None):
        ...
        # init from run
        # self.namespace = {}
        self.module = None # single context, single module
    ...
    def _init_config(self):
        ...
        if self.config is None:
            self.config = getattr(self.module, "__config__")
    ...
    def _param_names(self):
        """gloabal params name __xxx__"""
        return [_v for _v, _ in vars(self.module).items() if _v.endswith("__") and _v.startswith("__") ]
    
    @property
    def _func_names(self):
        """function name"""
        return [member[0] for member in inspect.getmembers(self.module) if inspect.isfunction(member[1])] 
    ...
    def _on(self, name):
        ...
        for _n in self._func_names:
            if _n.endswith(name):
                getattr(self.module, _n, None)(self)
    ...
    def run(self, file):
        ...
        module_name = str(os.path.basename(file)).split('.')[0]
...
class CoContext(ContextBase):
    ...
    def __init__(self, start_dt=None, end_dt=None, config = None):
