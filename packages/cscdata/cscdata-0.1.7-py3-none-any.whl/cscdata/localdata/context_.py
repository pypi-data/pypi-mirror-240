...
from .utils import retry, write_json
...
class ContextBase(_Context):
    ...
    def _on(self, name):
        ...
        for _n in self.namesapce:
            if _n.endswith(name):
                self.namespace[_n](self)

    def run(self, file):
        """init some params and exec func"""
        ...
        # user func 
        self._on("_init") # global feature

        for trade_dt in self.trade_dts: # on trade dt 
            self._on("_trade_dt")
            pass
    ...
    def t(self, db_name, table_name):
        """parquet table"""
        return self.con.use_db(db_name).use_table(table_name)
...
class FinContext:
    ...
    def run(self, file):
        ...
        self.save_db_name = __config__["save_db_name"]
    ...
    def hist(self, table_name, n_day columns=None, engine=None):
        """ parquet, get the historical data"""
        return self.ds(table_name).get_hist(n_day, day_col='trade_dt', columns=columns, engine=engine)
    ...
    def parse_read_name(self, name):
        """struct:  db.table.feature"""
        _sp = name.split(".")
        if len(_sp) == 1:
            """read table from default repo db"""
            return (name,)
        elif len(_sp) == 2:
            """read table from user repo db"""
            return (_sp[0], _sp[1])
        elif len(_sp) == 3:
            """read feature from user repo db.table """
