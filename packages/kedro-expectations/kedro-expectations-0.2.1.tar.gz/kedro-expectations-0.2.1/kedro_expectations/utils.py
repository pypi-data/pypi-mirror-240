...
def validate(adjusted_key, suite_name, validation_df):
    ...
    dataframe_asset = get_or_add_dataframe_asset(get_or_add_pandas_datasource(context), name=adjusted_key)
...
def populate_new_suite(input_data: pd.DataFrame, expectation_suite_name: str):
    ...
    data_asset = get_or_add_dataframe_asset(datasource, name=expectation_suite_name.split('.')[0])
    ...
    click.echo('One by one, type the name of the columns you do NOT want to validate.\nOnce you are finished, '
               'type 0 to continue')
    column_to_remove = ""
    ...
    while column_to_remove != "0":
        ...
        if column_to_remove == "0":
            ...
            print(f"The column {column_to_remove} doesn't exist in this dataframe. Try typing again")
        else:
