...
def parse_issue_tracking(subject):
    ...
    # TODO: must add some configuration for issue parsing and url building
    has_tracker_ref = re.compile(r"#(\w+-\d{2,4})")
    if ref := has_tracker_ref.findall(subject):
        issue_id = ref[0].upper()
        url = f"https://jibecompany.atlassian.net/browse/{issue_id}"
