...
from sberpm.miners import ProcessTreeNode, ProcessTreeNodeType
from sberpm.visual import Graph, GraphType, NodeType, utils
...
class GraphvizPainter:
    ...
    def apply(
        ...
        node_color_dict = utils.calc_nodes_colors_by_metric(
            graph, node_style_metric
        )
        edge_width_dict = utils.calc_edges_widths_by_metric(
    ...
    def _add_process_tree_nodes(
        ...
        digraph.node(
            node_id, label, shape=shape, fillcolor=color, style="filled"
        )
