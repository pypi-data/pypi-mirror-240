...
def check_exceptions(code=0, **kwargs):
    ...
    if int(code) == 401:
        ...
        raise BadRequest(message=kwargs.get("error"))
    elif int(code) == 403:
        raise Forbidden(message=kwargs.get("error"))
    elif int(code) == 404:
        raise NotFound(message=kwargs.get("error"))
    elif int(code) == 500:
        raise InternalServerError(message=kwargs.get("error"))
    else:
