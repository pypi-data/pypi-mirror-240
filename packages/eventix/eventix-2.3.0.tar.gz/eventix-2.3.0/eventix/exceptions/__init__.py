...
class BackendFeaturesMissing(WebException):
    ...
    def __init__(self, backend: Backend, features: List[str]) -> None:
        super().__init__(
            f"Backend {backend.__class__.__name__} does not support all features {features}",
            features
        )
...
class WrongTaskStatus(WebException):
    ...
    def __init__(self, uid: str, status: str) -> None:
        super().__init__(
            f"Task {uid} has wrong status: '{status}'",
