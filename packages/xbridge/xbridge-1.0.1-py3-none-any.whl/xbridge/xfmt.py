...
class Any:
    ...
    @staticmethod
    def fromBytes(data: bytes) -> typing.Tuple['Any', int]:
        ...
        if data[0] == XType.StreamInfo.intValue:
...
class List(Any):
    ...
    value: typing.List[Any]

    def __init__(self, items: typing.List[typing.Any]) -> None:
        self.value = [Any.of(item) for item in items]  
    ...
    def fromBytes(data: bytes) -> typing.Tuple['List', int]:
        ...
        while data[off] != 0:
            ...
            item, used = Any.fromBytes(data[off:])
            # print("got item", item)
            value.append(item)
...
class Ret(Any):
    ...
    def __init__(self, id, value: Union[Any, typing.Any]) -> None:
        self.id = id
        self.value = Any.of(value)
...
class Stream(Any):
    ...
    def toBytes(self) -> bytes:
        return XType.StreamInfo.value + Number.of(self.id).toBytes() + List([self.name, self.size]).toBytes()
...
class Err(Any):
    ...
    @staticmethod
    def fromBytes(data: bytes) -> Tuple[Any, int]:
        off = 1
