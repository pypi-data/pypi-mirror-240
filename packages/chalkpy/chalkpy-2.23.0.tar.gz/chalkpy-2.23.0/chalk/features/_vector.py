...
from typing import Any, List, Literal, Sequence, Tuple, Type, Union, cast, overload
...
class Vector:
    ...
    def __init__(self, data: Union[pa.FixedSizeListScalar, np.ndarray, Sequence[float]]):
        if not isinstance(data, pa.FixedSizeListScalar):
            ...
            if pa.types.is_float16(dtype):
                data = np.array(data, np.dtype("float16"))
            data = cast(pa.FixedSizeListScalar, pa.scalar(data, pa.list_(dtype, len(data))))
        if not isinstance(data, pa.FixedSizeListScalar):
            raise TypeError(f"Expected the Vector to be a PyArrow FixedSizeListArray; got {type(data).__name__}.")
    ...
    def __class_getitem__(cls, item: Union[Tuple[Literal["fp32", "fp64", "fp16"], int], int], /) -> Type[Vector]:
        if isinstance(item, int):
            ...
            raise ValueError(
                f"Expected the Vector type argument to be the dimensions (e.g. Vector[1536]) or the dtype and dimensions (e.g. Vector['fp16', 1536]); got {item}"
            )
