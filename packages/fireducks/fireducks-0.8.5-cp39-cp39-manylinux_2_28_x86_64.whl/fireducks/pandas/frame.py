...
class DataFrame(FireDucksPandasCompat):
    ...
    def drop(self, *args, **kwargs):
        ...
        if reason is None:
            if arg.columns is not None:
                ...
                if isinstance(arg.columns, pandas.Index):
        ...
        if reason is None:
            if arg.index is not None:
                if irutils.irable_scalar(arg.index):
                    arg.index = pandas.Index([arg.index])
                if isinstance(arg.index, list):
                    if np.all([irutils.irable_scalar(i) for i in arg.index]):
                        ...
                        arg.index = pandas.MultiIndex.from_tuples(arg.index)
                if isinstance(arg.index, (pandas.Index, pandas.MultiIndex)):
    ...
    def merge(
        ...
        **kwargs,
        ...
        elif not irutils.is_column_name_or_column_names(left_on):
            reason = "left_on is not column name supported by IR"
        elif not irutils.is_column_name_or_column_names(right_on):
            reason = "right_on is not column name supported by IR"
        elif how not in ("left", "right", "inner", "outer"):
            reason = f"unsupported how: {how}"
        elif sort:
            reason = "sort=True"
        elif len(kwargs) != 0:
