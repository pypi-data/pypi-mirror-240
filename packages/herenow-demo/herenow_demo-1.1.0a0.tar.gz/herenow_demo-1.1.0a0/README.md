...
7. Run the container:
```
docker run -p 8000:8000 -d local-herenow-demo-test
```
...
0. Building the app image that will be deployed on the k8s cluster is very similar to the last step for the above local build 
...
2. Once built you should run the container like so:
```
docker build \
...
1. Run the container:
```
docker run -p 8000:8000 -d herenow_demo
```
...
Once this ran I deployed it to a "small" k8s cluster on Digital Ocean in two steps(my key critera was scale the app to 3 nodes in case of a spike, in case of a crash recover, and portability to any infra):
1. T
