...
from .components import CM, OF, THE, VS, Abbv, DocketNo, Prov, StatuteNo, _orth_in, _re
...
def set_attribute_ruler(nlp: spacy.language.Language):
    ...
    for i in WordAttributes.make_attr_rules():
        ruler.add(**i)  # type: ignore

    for i in Prov.make_attr_rules():
        ruler.add(**i)  # type: ignore

    for i in DocketNo.make_attr_rules():
        ruler.add(**i)  # type: ignore

    for i in StatuteNo.make_attr_rules():
        ruler.add(**i)  # type: ignore
    return nlp
