...
from ._pattern import CM, OF, PROV_DIGITS, VS, Label, Rule, _orth_in, _re
...
class Prov(Enum):
    ...
    def make_attr_rules(cls) -> Iterator[dict[str, Any]]:
        ...
        for member in cls:
            ...
            yield {"index": 0, "patterns": p, "attrs": {"POS": "NOUN"}}
            yield {"index": 1, "patterns": p, "attrs": {"POS": "NUM", "LIKE_NUM": True}}
...
class CaseName(Enum):
    ent = {"ENT_TYPE": {"IN": ["ORG", "PERSON", "LAW"]}, "OP": "+"}
...
ENT_CASE_NAME = Rule(label=Label.CaseName, patterns=CaseName.permute_patterns())
...
ENT_GENERIC_DOC = Rule(
    ...
    patterns=[
        [
            {"ORTH": {"IN": ["Writ", "Notice", "Contract", "Motion"]}},
            {"ORTH": {"IN": ["of", "to"]}},
            {"IS_TITLE": True, "OP": "+"},
            {"ORTH": {"IN": ["of", "as", "to"]}, "OP": "*"},
            {"IS_TITLE": True, "OP": "+"},
