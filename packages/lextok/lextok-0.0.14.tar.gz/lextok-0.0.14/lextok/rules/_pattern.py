...
class Label(Enum):
    ...
    def snakecase(self) -> str:
        """Used as the variable creation using spacy's custom attributes e.g. `doc._.provision_nums`"""
        return "_".join(uncamel(self.name)).lower() + "s"

    def get_entities(self, doc: Doc) -> list[str]:
        """Generate doc's entities matching a label."""
        return [ent.text.strip("*, ") for ent in doc.ents if ent.label_ == self.name]

    def get_spans(self, doc: Doc) -> list[str]:
        """Generate doc's span ruler patterns matching a label."""
        return [span.text.strip("*, ") for span in filter_spans(doc.spans[self.name])]
