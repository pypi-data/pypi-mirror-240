...
class CurlAnaly:
    ...
    def curl_analy(self) -> AnalyObj:
        ...
        if isinstance(data[k], str) else data[k]
        ...
        cookies = OrderedDict(
            {k: cookies[k] for k in sorted(cookies)})
...
def curl_differ(curl_str1, curl_str2) -> str:
    ...
    curl_anal_2 = CurlAnaly(curl_str2).curl_analy()  # 数据解析
    # 获取差异数据
    cookies = dict_differ(curl_anal_1.cookies, curl_anal_2.cookies)
    headers = dict_differ(curl_anal_1.headers, curl_anal_2.headers)
    args = dict_differ(curl_anal_1.data, curl_anal_2.data)
