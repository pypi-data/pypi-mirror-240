...
def raise_disconnected_warning(
    ...
    if verbose & (vertices_disconnected == 0) & (edges_removed > 0):
        ...
        warn(
            ...
            f"Use featuremap.utils.disconnected_vertices() to identify them.",
        )
...
def tangent_space_approximation(
    ...
    # dists = featuremap_kwds["graph_dists"]
...
# @numba.njit()
# def smooth_vector_field(
#         data,
#         gauge_vh,
#         knn_index): 
#     # For smaller datasets we can use less iterartions
#     if data.shape[0] <= 5000:
#         s_time_average = round(np.sqrt(data.shape[0]/30))
#     else:
#         s_time_average = round(np.sqrt(data.shape[0]/10))
        
#     for s in range(s_time_average):
#         # print(f's_{s}')
#         gauge_vh_mean = np.zeros(gauge_vh.shape)
#         for i in range(data.shape[0]):
#             # i = 0
#             X_mean = np.zeros((gauge_vh.shape[1], gauge_vh.shape[2]))
#             # X_mean = np.mean(gauge_vh[knn_index[i],:,:], axis=0)
#             X_mean =  mean_axis_0(gauge_vh[knn_index[i],:,:])
#             gauge_vh_mean[i] = X_mean
#         gauge_vh = gauge_vh_mean


# # smooth_vector_field(data, gauge_vh, knn_index)  
...
def tangent_space_embedding(
    ...
    # # T1 = time.time()
    # gauge_vh = gauge_vh
    # rotation_matrix = np.array(gauge_vh)[:,0,:]
    # import umap
    # umap_embedding = umap.UMAP(n_neighbors=30, metric='euclidean', min_dist=0.3).fit_transform(X=rotation_matrix)
    ...
    # gauge_vh_embedding = np.zeros([umap_embedding.shape[0], umap_embedding.shape[1],2])
    # gauge_vh_embedding[:,0,0] = umap_embedding[:, 0]/umap_embedding_norm
    # gauge_vh_embedding[:,0,1] = umap_embedding[:, 1]/umap_embedding_norm
    # # gauge_vh_embedding[:,1,0] = umap_embedding[:, 1]/umap_embedding_norm * (-1)
    # # gauge_vh_embedding[:,1,1] = umap_embedding[:, 0]/umap_embedding_norm
    
    # # Second largest PC
    # rotation_matrix = np.array(gauge_vh)[:,1,:]
    # import umap
    # umap_embedding = umap.UMAP(n_neighbors=30, metric='euclidean', min_dist=0.3).fit_transform(X=rotation_matrix)
    # # plt.scatter(umap_embedding[:,0],umap_embedding[:,1],s=10)
    # # plt.show()
    ...
    # # gauge_vh_embedding is embedding of VH
    # gauge_vh_embedding[:,1,0] = umap_embedding[:, 0]/umap_embedding_norm
    # gauge_vh_embedding[:,1,1] = umap_embedding[:, 1]/umap_embedding_norm
    
    
    ###########################################
    # # Average over NNs in original space.
    # Message passing accross knn network
    #########################################
    knn_index = featuremap_kwds["_knn_indices"]

    gauge_vh = np.array(gauge_vh)
    # s_time_average = round(np.sqrt(data.shape[0]/3)/15)*32
    ...
    # s=0
    # while s < s_time_average:
    #     gauge_vh_mean = np.mean(gauge_vh[knn_index,:,:], axis=1)
    #     gauge_vh = np.array(gauge_vh_mean)
    #     s = s+1
    ...
    if method == 'umap':    
        ...
        # T1 = time.time()
        from sklearn.decomposition import PCA
        pca = PCA(n_components=50)  # for example, reduce to 2 dimensions
        X_pca = pca.fit_transform(rotation_matrix)
        # T2 = time.time()
        # print(f'PCA time is {T2-T1}')
        # The transformed data, X_pca, is now in the PCA space

        # svd = TruncatedSVD(n_components=100, n_iter=5, random_state=42)
        # emb_svd = svd.fit_transform(rotation_matrix)
        # T1 = time.time()
        import umap
        umap_embedding = umap.UMAP(n_neighbors=30,random_state=random_state, metric='euclidean',min_dist=0.5).fit_transform(X=X_pca)
        ...
        spectral_embedding = SpectralEmbedding(n_components=2,).fit_transform(X=X_pca)
        ...
        isomap_embedding = Isomap(n_components=2,eigen_solver='dense', path_method='D', neighbors_algorithm='ball_tree').fit_transform(X=rotation_matrix)
        featuremap_kwds['gauge_v1_emb'] = isomap_embedding

        isomap_embedding_norm = np.linalg.norm(isomap_embedding, axis=1)
    
            
        # gauge_vh_embedding is embedding of VH
        gauge_vh_mean_embedding = np.zeros([isomap_embedding.shape[0], isomap_embedding.shape[1],2])
        gauge_vh_mean_embedding[:,0,0] = isomap_embedding[:, 0]/isomap_embedding_norm
        gauge_vh_mean_embedding[:,0,1] = isomap_embedding[:, 1]/isomap_embedding_norm
        # gauge_vh_embedding[:,1,0] = umap_embedding[:, 1]/umap_embedding_norm * (-1)
        # gauge_vh_embedding[:,1,1] = umap_embedding[:, 0]/umap_embedding_norm
        ...
        # gauge_vh_embedding is embedding of VH
        gauge_vh_mean_embedding[:,1,0] = isomap_embedding[:, 0]/isomap_embedding_norm
        gauge_vh_mean_embedding[:,1,1] = isomap_embedding[:, 1]/isomap_embedding_norm
        
        
    featuremap_kwds["VH_smoothing"] = np.array(gauge_vh).astype(np.float32, copy=True)
    # featuremap_kwds["VH_embedding_original"] = np.array(gauge_vh_embedding).astype(np.float32, copy=True)
...
def variation_embedding(
    ...
    ############################################
    gauge_vh = featuremap_kwds["VH_smoothing"].copy()
    ...
    # VH_temp = gauge_vh
    # if original_data_flag == True:
    # pcVals_project_back = VH_temp
    gene_var_norm = np.linalg.norm(gauge_vh[:, :k, :], axis=1)
    velocity = gene_var_norm
    # adata.obsm['pc_loadings'] = pcVals_project_back[:, :k, :]
    # adata.layers['variation'] = velocity
    featuremap_kwds["variation_pc"] = np.array(velocity).astype(np.float32, copy=True)
    
    pc_loadings_scale = gauge_vh[:, :2, :] /\
        np.linalg.norm(gauge_vh[:, :2, :], axis=1)[:,np.newaxis,:] *\
            gene_var_norm[:,np.newaxis,:]
    # adata.obsm['pc_loadings_scale'] = pc_loadings_scale
    featuremap_kwds["pc_loadings_scale"] = pc_loadings_scale

    ######################################
    # Variation embedding
    ########################################
        
    data_original = featuremap_kwds['variation_pc'].copy()
    ...
    import umap
    emb_variation = umap.UMAP(random_state=random_state, n_neighbors=30,metric=metric,min_dist=0.5, spread=1, n_components=2).fit(emb_svd)
...
def simplicial_set_embedding_with_tangent_space_embedding(
    ...
    T1 = time.time()
    tangent_space_approximation(data, graph, featuremap_kwds)
    T2 = time.time()
    ...
    T1 = time.time()
    tangent_space_embedding(data, featuremap_kwds, random_state)
    T2 = time.time()
    ...
    if output_feat:
        ...
        aux_data["rad_emb"] = re_sum
        featuremap_kwds["rad_emb_no_log"] = np.sqrt(re_sum_without_log) # Variance of Gaussian distribution
    ...
    if variation_emb_flag == True:
        variation_embedding(data,featuremap_kwds,random_state)     
...
class FeatureMAP(BaseEstimator):
    """feature and anisotropic transport enabling Manifold Approximation and Projection
    ...
    output_feat: float (optional, default False)
        ...
        can also be used when featuremap=False to calculate the densities for
        UMAP embeddings.
    ...
    def __init__(
        ...
        self.variation_emb_flag = variation_emb_flag
    ...
    def fit(self, X, y=None):
        ...
        if self.metric == "precomputed" and self._sparse_data:
            ...
            (
                ...
                True,
                self.verbose,
                # self.featuremap or self.output_feat,
                self.output_feat,
    ...
    def fit_transform(self, X, y=None):
        ...
        or a tuple (X_new, r_orig, r_emb) if ``output_feat`` flag is set,
    ...
    def transform(self, X):
        ...
        #         "Transforming data into an existing embedding not supported for FeatureMAP."
        #     )
