...
class Scms(object):
    ...
    def inverse_density_sampling_anisotropic(self, X, rotational_matrix, r_emb, n_samples=2000, n_jobs=1, batch_size=16):
        """
        ...
        """
        if n_jobs == 1:
            logp = [self._kernel_density_estimate_anisotropic(pos,rotational_matrix, r_emb, output_onlylogp=True)
                 for pos in np.array_split(X, np.ceil(X.shape[0] / batch_size))]
        else:
            with Pool(n_jobs) as pool:
                logp = pool.map(
                    lambda pos: self._kernel_density_estimate_anisotropic(pos, rotational_matrix, r_emb, output_onlylogp=True),
    ...
    def inverse_density_sampling(self, X, n_samples=2000, n_jobs=1, batch_size=16):
        ...
        logp = np.concatenate(logp)
        s = -logp + np.random.gumbel(size=logp.shape)
        return np.argsort(-s)[10:n_samples]
    ...
    def _kernel_density_estimate(self, X, output_onlylogp=False, ):
        ...
        msu :
            2D array. Meanshift update based on the probability density function.
        """
        # rand_index = np.random.randint(self.data.shape[0], size=3000)
        # data = self.data[rand_index]
        # adaptive_bw = self.adaptive_bw[rand_index]
        ...
        # # prevent numerical overflow due to large exponentials
        # logc = -d * np.log(np.min(s.adaptive_bw)) - d / 2 * np.log(2 * np.pi)
        # C = (s.adaptive_bw[:, np.newaxis] / np.min(s.adaptive_bw)) ** (-d) * \
        #     np.exp(-1 / 2. * (D / s.adaptive_bw[:, np.newaxis]) ** 2)
        ...
        if output_onlylogp:
            ...
            # gradient of p
            u = vdist(data, X)
            g = np.mean((C / (adaptive_bw[:, np.newaxis] ** 2)).T[:, :, np.newaxis] * u, axis=1)

            # hessian of p
            Z = np.eye(d)
            h = matrix_multiply(
                (C / (n * adaptive_bw[:, np.newaxis] ** 4)).T[:, np.newaxis, :] * u.transpose((0, 2, 1)), u) - \
                as_strided(Z, strides=(0, Z.strides[0], Z.strides[1]), shape=(C.shape[1], Z.shape[0], Z.shape[1])) * \
                (np.sum(C / (n * adaptive_bw[:, np.newaxis] ** 2), axis=0)[:, np.newaxis, np.newaxis])
            # for computation of msu = Cp * g
            Cp = 1. / np.mean(C / (adaptive_bw[:, np.newaxis] ** 2), axis=0)
            # return the kernel density estimate
            return np.mean(C, axis=0).T, g, h, Cp[:, np.newaxis] * g
    def _kernel_density_estimate_anisotropic(self, X, rotational_matrix, r_emb, output_onlylogp=False):
       """
       Estimate density and relevant quantities for data points specified by X
       with kernel density estimation.
       ...
       rotational_matrix: array of shape (X.shape[0], 2, 2)
           two times two rotational matrix determining the orientation of distribution
           
       r_emb: array of shape (X.shape[0], 2)
           embedding radius (without log) in two pricipal directions; determine the scale of distribution
       ...
       # Random sample some data points to speed up calculation
       # np.random.seed(0)
       # rand_index = np.random.randint(self.data.shape[0], size=int(self.data.shape[0]*0.9))
       # data = self.data[rand_index]
       # r_emb = r_emb[rand_index]
       # rotational_matrix = rotational_matrix[rand_index]
       ...
       sigma = np.array([np.diag(r_emb[i])*10 for i in range(r_emb.shape[0])]) # (*10) means scale the sigma
       # sigma = np.tile(np.diag([0.5, 0.5]), (r_emb.shape[0],1,1))
       sigma_square = np.square(sigma)
       # Covariance matrix
       rotational_sigma_square = np.matmul(np.matmul(rotational_matrix.transpose(0,2,1), sigma_square), 
                                           rotational_matrix)

       # rotational_sigma_square = sigma_square
       covariance_det = np.linalg.det(rotational_sigma_square) 
       covariance_inverse = np.linalg.inv(rotational_sigma_square)
       ...
       # X = grid
       m = X.shape[0]
       # evaluate the kernel at each difference, e.g., x - x_i
       u = vdist(data, X) # Vector difference
       # u = vdist(data, X) # Vector difference
       u = u.transpose(1,0,2) # shape (n, m, 2)       
       # rotational_sigma_inverse_tile = np.tile(rotational_sigma_inverse, (m, 1, 1))
       ...
       logc = np.log(np.min(covariance_det)) / 2 - d / 2 * np.log(2 * np.pi)

       exponent_matrix = np.sum(np.matmul(u, covariance_inverse) * u, # shape (n,m,2)
                                axis=2) # shape (n,m)
       # C = (covariance_det[:,np.newaxis] / np.min(covariance_det) **(-1/2)  * \
       #      np.exp(-1 / 2. * exponent_matrix) # C with shape (m, n)
       
       C = (2. * np.pi)**(-d/2) * (covariance_det[:,np.newaxis]) **(-1/2) * \
           np.exp(-1 / 2. * exponent_matrix) # C with shape (m, n)
       ...
       if output_onlylogp:
           # return the kernel density estimate
           return np.log(np.mean(C, axis=0)) + logc
           ...
           # sigma^-1 * (x-x_i)
           covariance_inverse_times_x = np.matmul(covariance_inverse, u.transpose(0,2,1)).transpose(0,2,1) # shape (n, m, 2)
           # c_i * sigma^-1 * (x-x_i)
           C_times_u = np.tile(C, (2,1,1)).transpose(1,2,0) * covariance_inverse_times_x
           g = np.mean(C_times_u, axis=0)# shape (m,d),  each gradient is d-dim vector
           ...
           # outer product (n, m, 2, 1)  (n, m, 1, 2)
           outer_product = np.matmul(covariance_inverse_times_x[:,:,:,np.newaxis], 
                                     covariance_inverse_times_x[:,:,np.newaxis]) # shape (n,m,2,2)
           
           h_term_1 = np.tile(C,(2,2,1,1)).transpose(2,3,0,1) * outer_product # shape (n, m, 2, 2)
           h_term_2 = np.tile(C,(2,2,1,1)).transpose(2,3,0,1) * np.tile(covariance_inverse,(m,1,1,1)).transpose(1,0,2,3) # shape (n, m, 2, 2)
           h = np.mean(h_term_1 - h_term_2, axis=0)
           
           # for computation of msu = Cp * g
           # Cp = 1. / np.mean(C / (self.adaptive_bw[:, np.newaxis] ** 2), axis=0)
           Cp_term = np.mean(h_term_2, axis=0) # h_term_2 happens to be Cp_term
           Cp = np.linalg.inv(Cp_term)
           msu = np.array([Cp[i].dot(g[i]) for i in range(m)])
           # return the kernel density estimate
           return np.mean(C, axis=0), g, h, msu, C
    ...
    def _nlocal_inv_cov_anisotropic(self, X, rotational_matrix, r_emb):
        """
        Computes the hessian of log density function.
        This function also returns other density estimation related
        quantities.
        ...
        # p, g, h, msu = self._density_estimate(X)
        p, g, h, msu = self._kernel_density_estimate_anisotropic(X, rotational_matrix, r_emb)
        return 1. / p[:, np.newaxis, np.newaxis] * h -  (p[:, np.newaxis, np.newaxis]) ** (-2) * matrix_multiply(
            g[:, :, np.newaxis], g[:, :, np.newaxis].transpose((0, 2, 1))), p, g, h, msu
    ...
    def scms_update_anisotropic(self, X, rotational_matrix, r_emb, method='LocInv', stepsize=0.01, ridge_dimensionality=1, relaxation=0):
        """
        Compute the constrained mean shift update of the point at x.
        ...
        if method == 'GradientP':
            # _, g, h, msu = self._density_estimate(X)
            _, g, h, msu = self._kernel_density_estimate_anisotropic(X,rotational_matrix, r_emb)
            eigvals, eigvecs = np.linalg.eigh(-h)
            eigvals = -eigvals
            update = g * stepsize

        elif method == 'GradientLogP':
            h, p, g, _, msu = self._nlocal_inv_cov_anisotropic(X,rotational_matrix, r_emb)
            g = g / p[:, np.newaxis]
    ...
    def scms_anisotropic(self, X, rotational_matrix, r_emb, n_iterations=50, threshold=0, tol=1e-7, method='LocInv', stepsize=0.5, ridge_dimensionality=1,
             relaxation=0, n_jobs=1, batch_size=16):
        """
        Performs subspace constrained mean shift on the entire data set using
        a Gaussian kernel with bandwidth of bw.
        ...
        if n_jobs > 1:
            with Pool(n_jobs) as pool:
                for j in range(n_iterations):
                    ...
                    updates = pool.map(
                        lambda pos: self.scms_update_anisotropic(pos, rotational_matrix, r_emb, method=method, stepsize=stepsize,
                                                     ridge_dimensionality=ridge_dimensionality, relaxation=relaxation),
                        np.array_split(pos, np.ceil(pos.shape[0] / (batch_size * n_jobs))))
