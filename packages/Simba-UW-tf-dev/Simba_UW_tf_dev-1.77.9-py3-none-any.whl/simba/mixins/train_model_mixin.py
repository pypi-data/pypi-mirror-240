...
class TrainModelMixin(object):
    ...
    def read_model_settings_from_config(self,
        ...
        self.save_train_test_frm_info = str_2_bool(read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, 'save_train_test_frm_idx', data_type=Dtypes.STR.value, default_value=False))
    ...
    def check_validity_of_meta_files(self,
        ...
        for config_cnt, path in enumerate(meta_file_paths):
            ...
            if MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value in meta_dict.keys():
                errors.append(check_if_valid_input(name='SAVE TRAIN AND TEST FRAME INDEXES', input=meta_dict[MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value], options=Options.RUN_OPTIONS_FLAGS.value, raise_error=False)[1])
            else:
                meta_dict[MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value] = False
