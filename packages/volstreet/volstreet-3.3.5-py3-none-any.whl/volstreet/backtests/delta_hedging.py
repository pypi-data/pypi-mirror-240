...
class DeltaBackTest(BackTester):
    ...
    def fetch_option_prices(
        ...
        prices = self.fetch_option_prices_from_tsdb(query)
    ...
    def check_option_prices_availability(
        ...
        # Check for call_strike availability
        missing_call_strikes = date_frame_to_merge[
            ~date_frame_to_merge[["timestamp", "call_strike"]]
            .apply(tuple, axis=1)
            .isin(available_options)
        ]

        # Check for put_strike availability
        missing_put_strikes = date_frame_to_merge[
            ~date_frame_to_merge[["timestamp", "put_strike"]]
            .apply(tuple, axis=1)
            .isin(available_options)
        ]

        # Report missing strikes
        if not missing_call_strikes.empty or not missing_put_strikes.empty:
            missing_info = pd.concat(
                [missing_call_strikes, missing_put_strikes], ignore_index=True
            )
            logger.error(
    ...
    def merge_with_option_prices(
        ...
        merged_with_call_prices = date_frame_to_merge.merge(
    ...
    def run_backtest_in_parallel(
        ...
        self.index_prices = index_prices  # Store for use in the subset method
...
def extreme_summary(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    )  # Taking the median of the higher of the two positions. Median to avoid outliers from depressing the percentage return.
