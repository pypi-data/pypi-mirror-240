...
class DeltaPositionMonitor:
    ...
    def process_breach(
        ...
        target_straddle, minimum_unevenness = most_even_delta_strangle(
            self.underlying,
        )
        logger.info(
            f"{current_time()} {self.underlying.name} "
            f"Target straddle: {target_straddle} with unevenness {minimum_unevenness}"
        )
        if (
            (target_straddle is not None)
            and (target_straddle == self.instrument)
            and (minimum_unevenness < (max_multiple * 0.8))
...
def calculate_pnl_for_strategy(
    ...
    if flexible_matching:
        ...
        filtered_orders = [
            order for order in data if order.get("ordertag") == strategy_name
        ]
...
def process_delta_file(data: list[dict]):
    ...
    # Traverse the list of position snapshots
    for entry in data:
        # Check if the strike has changed (position squared up)
        if (
            current_call_strike != entry["call_strike"]
            or current_put_strike != entry["put_strike"]
        ):
            # If this is not the first entry, add the last pnl to the final pnl
            if current_call_strike is not None and current_put_strike is not None:
                final_pnl += last_pnl
            # Update the current strikes
            current_call_strike = entry["call_strike"]
            current_put_strike = entry["put_strike"]

        # Update the last pnl
        last_pnl = entry["pnl"]

    # Add the pnl of the last position before the file ends
    final_pnl += last_pnl
