...
class Dataset:  # (pd.DataFrame):
        ...
        for index, (column_name, column_type) in enumerate(zip(self.data.columns, self.data.dtypes)):
            if column_name not in self.ignore_columns:
                ...
                column_meta = FileColumnMetadata(
                    ...
                    dataType=self.__get_data_type(column_type, column_name, meaning_type),
    ...
    def __get_data_type(self, pandas_data_type, column_name: str, meaning_type: FileColumnMeaningType) -> DataType:
        if meaning_type == FileColumnMeaningType.IPV6_ADDRESS:
            return DataType.BIG_NUMERIC

        if is_integer_dtype(pandas_data_type):
            return DataType.INT
        elif is_float_dtype(pandas_data_type):
            return DataType.DECIMAL
        elif isinstance(self.data[column_name].values[0], Decimal):
            return DataType.BIG_NUMERIC
            # return DataType.DECIMAL
