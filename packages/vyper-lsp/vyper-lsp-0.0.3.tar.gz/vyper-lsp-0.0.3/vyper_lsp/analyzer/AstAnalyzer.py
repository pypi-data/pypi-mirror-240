...
class AstAnalyzer(Analyzer):
    ...
    def signature_help(self, doc: Document, params: SignatureHelpParams) -> None:
        current_line = doc.lines[params.position.line]
        word = get_word_at_cursor(current_line, params.position.character - 2)
        expression = get_expression_at_cursor(current_line, params.position.character - 2)
        print(word, expression, file=sys.stderr)
