...
def insert_rows_into_table(table,rows,schema=None,database=None):
	D=schema;C=table;A=set()
	for B in rows:A.update(B.keys())
	A=list(A);E=', '.join(A);F=', '.join(['%s'for A in A]);G=f'"{D}"."{C}"'if D else C;H=f"INSERT INTO {G} ({E}) VALUES ({F})"
	for B in rows:I=[B.get(A)for A in A];execute_query(H,I,database=database)
