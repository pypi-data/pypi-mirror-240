...
def run(
    ...
    office.pdf.pdf2imgs("test_pdfs", "Image_Processing/test_pngs")
    # Directories
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                a = torch.tensor(20.0, dtype=torch.float32, device="cuda:0")
                # Write results
                k = -1
...
def parse_opt(img_path):
    ...
    parser.add_argument(
        "--save-crop", default="True", help="save cropped prediction boxes"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--hide-labels", default=True, action="store_true", help="hide labels"
    )
    parser.add_argument(
        "--hide-conf", default=True, action="store_true", help="hide confidences"
    )
    parser.add_argument(
...
def segment_molecules(img_path):
    # path to the image to process
    if not os.path.exists("./test_imgs"):
        os.makedirs("./test_imgs")
    office.pdf.pdf2imgs(img_path, "./test_imgs")

    opt = parse_opt("./test_imgs")
    main(opt)
