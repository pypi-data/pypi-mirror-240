...
class NestorInstance:
    ...
    def db_restore_from_s3(
        ...
        if self.db.restore(
            NestorDatabaseCmd.DBCmdTarget.S3, dump_path, s3_secret, bucket, set_password_to_all, verbose
        ):
            print(f"{dump_path} not found, defaulting to {alt_dump_path}")
            return self.db.restore(
                NestorDatabaseCmd.DBCmdTarget.S3, alt_dump_path, s3_secret, bucket, set_password_to_all, verbose
            )
        return 0
