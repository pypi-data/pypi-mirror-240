...
from vios.systemq import (Capture, CompilerContext, Waveform, WaveVStack,
                          _compile, _form_signal, assembly_code,
                          get_all_channels, square, stdlib, wave_eval)
    ...
    for step, operations in instruction.items():
        ...
        for target, cmd in operations.items():
            try:
                kwds = cmd[-1]
                # 重复指令缓存
                if target in bypass and target.endswith(SUFFIX) and (not isinstance(cmd[1], WaveVStack)) and (not isinstance(bypass[target][0], WaveVStack)) and equal(bypass[target][0], cmd[1]):
                    continue
