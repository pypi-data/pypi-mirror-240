...
class AMLPipelineHelperV2(AMLPipelineHelperBase, ABC):
    ...
    def _build_pipeline(self) -> PipelineJob:
        ...
        if self.config.run.publish:
            ...
            _component = self.workspace().components.create_or_update(pipeline_function)
    ...
    def _publish_to_endpoint(self, pipeline: Pipeline):
        ...
        self.workspace().batch_endpoints.begin_create_or_update(endpoint)
        ...
        timeout = 300
        while timeout > 0:
            endpoint = self.workspace().batch_endpoints.get(endpoint_name)
        ...
        deployment = self.workspace().batch_deployments.begin_create_or_update(
            deployment
        )
