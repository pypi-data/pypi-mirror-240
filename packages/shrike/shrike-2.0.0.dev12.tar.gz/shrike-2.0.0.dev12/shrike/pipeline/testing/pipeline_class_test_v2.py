...
from shrike.pipeline import AMLPipelineHelperV2
...
def pipeline_required_subgraphs(pipeline_class):
    ...
    for subgraph_key, subgraph_class in subgraphs_manifest.items():
        if not issubclass(subgraph_class, AMLPipelineHelperV2):
            error_log.append(
                f"In pipeline class module {pipeline_class.__name__}, values in dictionary returned by required_subgraphs() must be subclass of AMLPipelineHelperV2 (under key={subgraph_key}, found object {subgraph_class.__name__})"
            )
            continue
