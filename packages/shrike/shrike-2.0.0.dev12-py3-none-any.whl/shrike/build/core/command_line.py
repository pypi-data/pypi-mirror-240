...
class Command(ABC):
    ...
    def ensure_component_cli_installed(self) -> bool:
        ...
        # Check whether the azure CLI is installed
        ...
        if component_cli_exists:
            log.info("azure CLI exists. Skipping installation.")
            return True
        else:
            log.info(
                f"installing latest azure CLI."
            )
            ...
            cli_install_command = f"extension add -n ml --yes"
