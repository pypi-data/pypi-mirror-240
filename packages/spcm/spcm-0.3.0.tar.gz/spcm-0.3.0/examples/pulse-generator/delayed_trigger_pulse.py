...
# **************************************************************************
#
# 3_delayed_trigger_pulse.py                         (c) Spectrum GmbH
#
# **************************************************************************
#
...
with spcm.Card('/dev/spcm0') as card:
    ...
    card.set(spcm.SPCM_X0_MODE, spcm.SPCM_XMODE_TRIGOUT)  # trigger output for reference on X0
    ...
    lOffset_5us = np.int32(llPulseGenClock * 0.000005)
    card.set(spcm.SPC_XIO_PULSEGEN1_MODE,     spcm.SPCM_PULSEGEN_MODE_SINGLESHOT)
    card.set(spcm.SPC_XIO_PULSEGEN1_LEN,      lLen_1ms)
    card.set(spcm.SPC_XIO_PULSEGEN1_HIGH,     lLen_1ms - 1)
    card.set(spcm.SPC_XIO_PULSEGEN1_DELAY,    lOffset_5us) # delay the pulse for 5us
    card.set(spcm.SPC_XIO_PULSEGEN1_LOOPS,    1) # just once
    card.set(spcm.SPC_XIO_PULSEGEN1_MUX1_SRC, spcm.SPCM_PULSEGEN_MUX1_SRC_UNUSED)
    card.set(spcm.SPC_XIO_PULSEGEN1_MUX2_SRC, spcm.SPCM_PULSEGEN_MUX2_SRC_CARDTRIGGER) # started by card's trigger event
