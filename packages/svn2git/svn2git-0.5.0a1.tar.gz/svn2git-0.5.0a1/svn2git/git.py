    ...
    def push_branches(
        ...
        if large_repository_mode:
            ...
            for idx, commit in enumerate(commits):
                ...
                run_command(f"git push origin {commit}:refs/heads/{self._initial_branch_name}", cwd=self._cwd)
            ...
            for branch in branches.locals:
                ...
                for idx, commit in enumerate(commits):
                    logger.debug(f"Pushing commit {commit} ({idx + 1}/{commits_len})...")
                    run_command(f"git push origin {commit}:refs/heads/{branch}", cwd=self._cwd)

        # Last step -> try to push everything
        run_command(f"git push -u {self._git_remote} --all", cwd=self._cwd)

    def push_tags(self, branches: GitBranches) -> None:
        """
