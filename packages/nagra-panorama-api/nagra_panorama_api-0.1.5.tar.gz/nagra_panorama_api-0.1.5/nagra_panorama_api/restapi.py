...
from .rest_resources import (PanoramaDevicesResourceType,
...
PANORAMA_ERRORS = {
    ...
    4: ('Gateway timeout. A firewall or Panorama module timed out '
    ...
    9: ('Failed precondition. The operation was rejected because '
        'the system is not in a state required '
        'for the execution of the operation.'),
    10:
    ('Aborted because of conflict. A typical cause is a concurrency issue.'),
    11: ('Out of range. The operation was attempted past a valid range. '
         'And example is reaching an end-of-file.'),
    12: ('Not implemented. The operation is disabled, not implemented, '
         'or not supported.'),
    13: ('Internal server error. An unexpected and potentially serious '
    ...
    16: ('Unauthorized. The request does not have '
...
class PanoramaAPI:
    ...
    def _inner_request(self,
        ...
        try:
            ...
            error_occured = (
                not res.ok or data.get('@status', '') != 'success'
    ...
    def _request(self,
                 ...
                 data=None,
                 verify=None):
        data, error = (self._inner_request(method,
                                           url,
                                           params=params,
                                           headers=headers,
                                           data=data,
        ...
        if error and self._verbose:
            log.error('Could not {} {}: {}'.format(method.lower(), url, error))
            return data, error
...
class PanoramaClient:
    ...
    def __init__(self,
        ...
        self.client = client
        self.objects = PanoramaObjectsResourceType(client,
                                                   domain,
                                                   version=version)
        self.policies = PanoramaPoliciesResourceType(client,
                                                     domain,
                                                     version=version)
        self.network = PanoramaNetworkResourceType(client,
                                                   domain,
                                                   version=version)
        self.device = PanoramaDevicesResourceType(client,
                                                  domain,
                                                  version=version)
        self.panorama = PanoramaPanoramaResourceType(client,
                                                     domain,
                                                     version=version)
