...
async def ssh_custom_forward(spawner, port_forward_info):
    ...
    service_url = (
        custom_config.get("backend-services", {})
        .get("tunnel", {})
        .get("urls", {})
        .get("tunnel", "None")
    )
...
async def request_url(spawner, user_options):
    ...
    backend_service = (
        custom_config.get("systems", {}).get(system, {}).get("backend-service", "None")
    )
    url = (
        custom_config.get("backend-services", {})
        .get(backend_service, {})
...
async def request_headers(spawner, user_options):
    ...
    auth = os.environ.get(f"AUTH_{backend_service.replace('-', '_').upper()}")
    headers = {
    ...
    if (
        ...
        send_access_token_key = (
            custom_config.get("backend-services", {})
            .get(backend_service, {})
            .get("send_access_token_key", "Auth-State-access_token")
        )
...
async def pre_spawn_hook(spawner):
    ...
    if not default_version:
        default_version = (
            custom_config.get("services", {}).get(service, {}).get("default_option", "")
        )
    try:
...
def poll_interval(spawner, user_options):
    ...
    if custom_config.get("drf-services", {}).get(drf_service, {}).get("poll", True):
        ...
        poll_interval_randomizer = (
            custom_config.get("drf-services", {})
            .get(drf_service, {})
            .get("poll_interval_randomizer", 0)
        )
