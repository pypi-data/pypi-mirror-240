...
template <class T>
DAGGER::D8connector<T> _create_connector(int nx, int ny, T dx, T dy, T xmin,
                                         T ymin) {
...
template <class T>
DAGGER::D8connector<T>* _create_connector_fromptr(int nx, int ny, T dx, T dy, T xmin,
                                         T ymin) {
...
template <class T>
void _create_connector(int nx, int ny, T dx, T dy, T xmin, T ymin,
                       DAGGER::D4connector<T> &con) {
  con = DAGGER::D4connector<T>(nx, ny, dx, dy, xmin, ymin);
}

template <class T>
void _create_connector_fromptr(int nx, int ny, T dx, T dy, T xmin, T ymin,
                       DAGGER::D4connector<T>* con) {
...
void _create_graph(int nxy, Connector_t &con,
                   DAGGER::graph<T, Connector_t> &gr) {
  gr = DAGGER::graph<T, Connector_t>(con);
}

template <class Connector_t, class T>
void _create_graph_fromptr(int nxy, Connector_t &con,
                   DAGGER::graph<T, Connector_t>* &gr) {
...
std::vector<fT> _generate_perlin_noise_2D(Connector_t &con, fT frequency,
...
out_t generate_perlin_noise_2D(Connector_t &con, fT frequency, int octaves,
                               std::uint32_t seed) {
  std::vector<fT> noise =
