...
class Bench(object):
    ...
    def exec(self, context: Context):
        if context.meter.index == 0 or self._is_meter_pos_operation:
            if self._times(context):
                asb.Application.set(context, 'passed_quantity', 0)

                if not self._exec_main(context):
                    self._fail_repeat(context)

                if int(asb.Application.get(context, 'passed_quantity')) >= context.quantity:
                    self.sleep(context)
    ...
    def _exec_main(self, context: Context):
        ...
        if base.result_judge(self._result):
            asb.Application.set(context, 'passed_quantity', int(asb.Application.get(context, 'passed_quantity')) + 1)
            return True
