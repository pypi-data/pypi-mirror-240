...
class Bench(object):
    ...
    def exec(self, context: Context):
        if context.meter.index == 0 or self._is_meter_pos_operation:
            if self._times(context):
                ...
                passed_quantity = int(asb.Application.get(context, 'passed_quantity'))
                if passed_quantity >= context.quantity:
                    self.sleep(context)
    ...
    def _exec_main(self, context: Context):
        ...
        if base.result_judge(self._result):
            passed_quantity = int(asb.Application.get(context, 'passed_quantity'))
            passed_quantity += 1
            asb.Application.set(context, 'passed_quantity', passed_quantity)
            return True
