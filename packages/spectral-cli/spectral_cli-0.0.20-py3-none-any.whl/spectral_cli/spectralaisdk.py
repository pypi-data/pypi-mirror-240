...
def do_ensure_wallet_exists(config_manager, reconfigure):
    ...
    print([private_key, address, reconfigure])
        ...
        def wrapper(*args, **kwargs):
            ...
            if not alchemy_api_key or reconfigure:           
                ...
                if new_alchemy_api_key:
                    ...
                    return -1
            multisig = config_manager.get('global', 'multisig_wallet_address')
            if config_manager.get('global', 'wallet_address') and not config_manager.get('global', 'multisig_wallet_address'):
                multisig_wallet_address = get_multisig_address(config_manager.get('global', 'wallet_address'))
                if multisig_wallet_address:
                    config_manager.set('global', 'multisig_wallet_address', multisig_wallet_address)
                    config_updated = True
...
def show_wallet():
    ...
    if wallet_address:
        ...
        print("Spectral CLI wallet address is not set. Please run `spectral configure` to set it up.")
...
def fetch_training_data(challenge_id):
    ...
    if challenge_id == '0xB79CDBC5Cd94a807CC5cc761e3eF4A6B9baC8939':
