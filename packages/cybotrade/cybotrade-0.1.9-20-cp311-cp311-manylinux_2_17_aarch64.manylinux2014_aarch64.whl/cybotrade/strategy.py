...
class ControlAPIServer():
    def __init__(self, logger: logging.Logger, port=int):
        self.logger = logger
        self.port = port
        ...
        async def handler(request: web.Request) -> web.StreamResponse:
            try:
                ...
                if "logLevel" not in body:
                    raise Exception("'id' must exists in the body and must be a string")
            ...
            try:
                self.logger.setLevel(level=get_log_level_value(body["logLevel"]))
...
class Strategy:
    ...
    def __init__(
        ...
        self.control_api_server = ControlAPIServer(logger=logging.root, port=3000)
