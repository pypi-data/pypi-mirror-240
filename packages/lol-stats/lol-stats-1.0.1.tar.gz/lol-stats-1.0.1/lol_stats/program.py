...
def index():
    ...
    if df is not None:
        return render_template('index.html', table=df.to_html(classes='data'),table_ready=table_ready,execution_time = round(time.time() - start_time, 2),sample=sample,text=text)
    else:
...
def download():
    ...
    # Write the DataFrame to the in-memory file
...
def merger(tables_list,flag):
    ...
    faker_path=pkg_resources.resource_filename('lol_stats', "data/faker.pkl")

    if flag:

        with open(faker_path, 'rb') as file:
...
if __name__ == '__main__':
    port = os.getenv('LOL_STATS_PORT', 8126)
    print(f"-- PROGRAM STARTED AT PORT {port} --")
    launch(port)