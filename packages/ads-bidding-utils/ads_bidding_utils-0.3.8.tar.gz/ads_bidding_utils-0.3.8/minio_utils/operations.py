...
def load_data_from_minio(minio_client, minio_bucket, minio_data_path, sep=';', compression=None):
    return stream_large_object(minio_client, minio_bucket, minio_data_path, sep=sep)
    # """
...
def stream_large_object(minio_client, bucket_name, object_name, chunk_size=1000000, sep=';'):
    """
    Stream a large object from Minio in chunks and return it as a buffer.

    Parameters:
    - minio_client (Minio): An initialized Minio client.
    - bucket_name (str): Name of the Minio bucket containing the object.
    - object_name (str): Name of the object to stream.
    ...
    try:
        ...
        object_info = minio_client.stat_object(bucket_name, object_name)
        object_size = object_info.size

        # Initialize an empty buffer
        buffer = io.BytesIO()

        # Iterate over chunks of the object and append them to the buffer
        for offset in range(0, object_size, chunk_size):
            length = min(chunk_size, object_size - offset)
            data = minio_client.get_object(bucket_name, object_name, offset=offset, length=length)
    ...
    df = pd.read_csv(buffer, sep=sep, index_col=0)
