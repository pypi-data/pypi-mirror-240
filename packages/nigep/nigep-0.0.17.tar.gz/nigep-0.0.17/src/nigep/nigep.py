...
class Nigep:
    ...
    def __init__(self, execution_name: str, model: Sequential, batch_size: int, input_shape, x_data, y_data, class_mode='categorical', k_fold_n=5):
        ...
        for train_index, test_index in kf.split(self.x_data, self.y_data):
            ...
            try:
                for noise_amount in NOISE_LEVELS:
                    ...
                    train_gen, val_gen = get_train_generator(self.x_data, self.y_data, noise_amount, train_index, self.batch_size, self.class_mode, self.input_shape)
                    ...
                    for noise_amount_testing in NOISE_LEVELS:
                        test_gen = get_test_generator(self.x_data, self.y_data, noise_amount_testing, test_index, self.batch_size, self.class_mode, self.input_shape)
