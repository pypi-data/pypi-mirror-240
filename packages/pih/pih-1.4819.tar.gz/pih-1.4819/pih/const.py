...
class SETTINGS(Enum):
    ...
    TIME_TRACKING_FOR_KAG: StorageVariableHolder = StorageVariableHolder(
        "TIME_TRACKING_FOR_KAG",
        [
            "190",
            "035",
            "058",
    ...
    #
    TIME_TRACKING_REPORT_CANCELED_LOGIN_LIST: IntStorageVariableHolder = (
        StringListStorageVariableHolder(
            "TIME_TRACKING_REPORT_CANCELED_LOGIN_LIST", ["gvi"]
        )
    )

    JOKE_CANCELED_LOGIN_LIST: StringListStorageVariableHolder = (
        StringListStorageVariableHolder(
            "JOKE_CANCELED_LOGIN_LIST", ["vvs", "rob", "smv", "ssm"]
        )
    )
    ...
    MOBILE_HELPER_USER_DATA_INPUT_TIMEOUT: IntStorageVariableHolder = (
        IntStorageVariableHolder(
            ...
            CONST.MOBILE_HELPER.USER_DATA_INPUT_TIMEOUT,
        )
    )
...
class PARAM_ITEMS:
    ...
    ID: ParamItem = ParamItem(FIELD_NAME_COLLECTION.ID, "")
...
class Events(Enum):
    ...
    SERVICE_WAS_STARTED: EventDescription = EventDescription(
        ...
        LogMessageFlags.NORMAL,
        (
            ParamItem("service_name", "Name of service"),
    ...
    NEW_POLIBASE_DOCUMENT_DETECTED: EventDescription = EventDescription(
        ...
        (
            ParamItem("path", "Путь к файлу"),
            ParamItem("person_pin", "Идентификационный номер пациента"),
    ...
    POLIBASE_CREATION_DB_DUMP_COMPLETE: EventDescription = EventDescription(
        "Базы данных Polibase: Завершено создание дампа",
        LogMessageChannels.BACKUP,
        (LogMessageFlags.NORMAL, LogMessageFlags.SAVE),
    )
    ...
    POLIBASE_CREATION_ARCHIVED_DB_DUMP_COMPLETE: EventDescription = EventDescription(
        "Базы данных Polibase: Завершено архивирование дампа",
        LogMessageChannels.BACKUP,
        LogMessageFlags.NORMAL,
    )
    ...
    POLIBASE_PERSON_VISIT_NOTIFICATION_WAS_REGISTERED: EventDescription = (
        EventDescription(
            "Зарегистрировано новое уведомление посещение: {name} ({type_string})",
            LogMessageChannels.POLIBASE,
            LogMessageFlags.SILENCE,
            (
                PARAM_ITEMS.NAME,
    ...
    POLIBASE_PERSON_WITH_INACCESSABLE_EMAIL_WAS_DETECTED: EventDescription = EventDescription(
        ...
        [
            ...
            ParamItem(FIELD_NAME_COLLECTION.EMAIL, ""),
...
EMAIL_CORRECTION: list[tuple[tuple[str], str]] = [
    (("gmail.ru", "email.ru", "gmail"), "gmail.com"),
    (("mail.com", "mail.ruu", "mail"), "mail.ru"),
