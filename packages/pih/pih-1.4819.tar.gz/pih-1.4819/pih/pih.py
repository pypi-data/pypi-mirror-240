...
class Output(OutputBase):
    ...
    def write_line(self, text: str) -> None:
        print(jnl(list(map(lambda item: self.text_before + item, text.splitlines()))))
...
class PIH:
    ...
    class MIO:
        ...
        def create_output(recipient: str | Enum, greeting: bool = True) -> Output:
            ...
            recipient = (
                recipient if isinstance(recipient, str) else EnumTool.get(recipient)
            )
    ...
    class UPDATE:
        ...
        def for_service(
            ...
            if ne(modules) and modules_update and result:
    ...
    class EVENT:
        ...
        def waiting_for_mobile_helper_message_input(
            ...
            def internal_handler(
                ...
                if ne(message) and PIH.DATA.FORMAT.telephone_number(
                    message.sender
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool):
                ...
                if n(action):
                    ...
                    for item in Actions:
                        if (
                            ...
                            or action_or_name
                            in list(map(lambda item: item.lower(), item.value.alias))
                        ):
                            action = item
                            break
                return action
        ...
        class FORMAT:
            ...
            def location_list(
                ...
                location_list: list[str] = value.split(",")[1 if remove_first else 0 :]
                if reversed:
    ...
    class RESULT(ResultTool):
        ...
        class NOTES:
            ...
            @staticmethod
            def get(id: str) -> Result[Note]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(ServiceCommands.get_note, id), Note
                )

            @staticmethod
            def by_label(value: str) -> Result[list[Note]]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(
                        ServiceCommands.get_note_list_by_label, value
                    ),
                    Note,
                )
                    ...
                    PIH.SERVICE.call_command(
                        ...
                        (
                            ...
                            if e(checkable_section_list)
                                ...
                                map(lambda item: item.name, checkable_section_list)
                            ),
                            force_update,
                            all,
                        ),
        ...
        class WORKSTATION:
            ...
            def all_with_prooperty(value: AD.WSProperies) -> Result[list[Workstation]]:
                ...
                return ResultTool.filter(filter_function, PIH.RESULT.WORKSTATION.all())
            ...
            def by_internal_telephone_number(value: int) -> Result[Workstation]:
                ...
                ):  # and result_worksation.accessable and ne(result_worksation.samAccountName):
            ...
            def all() -> Result[list[Workstation]]:
                ...
                return ResultTool.every(
                    every_action,
                    DataTool.to_result(
                        PIH.SERVICE.call_command(ServiceCommands.get_all_workstations),
                        Workstation,
                    ),
                )
        ...
        class PRINTER:
            ...
            def all() -> Result[list[PrinterADInformation]]:
                def filter_by_server_name(
                    ...
                    return DataTool.filter(
                        lambda item: item.serverName == CONST.HOST.PRINTER_SERVER.NAME,
                        printer_list,
                    )
        ...
        class POLIBASE:
            ...
            def persons_pin_by_visit_date(
                date: datetime | date,
                only_operation_department: bool = False,
                test: bool | None = None,
            ) -> Result[list[int]]:
                if test:
        ...
        class USER:
            ...
            def by_group_name(value: str) -> Result[list[User]]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(ServiceCommands.get_users_by_group, value),
                    User,
                )
    ...
    class CHECK:
        ...
        class POLIBASE:
            ...
            def person_exists_by_pin(value: int) -> bool:
                try:
                    return not ResultTool.is_empty(
                        PIH.RESULT.POLIBASE.person_by_pin(value)
                    )
                except NotFound as _:
                    return False
        ...
        class WORKSTATION:
            ...
            def to_workstation(
                ...
                return PIH.MESSAGE.WORKSTATION.by_workstation_name(
                    value.name if isinstance(value, WorkstationDescription) else value,
                    message,
                    timeout,
                    method_type,
                )
    ...
    class CARD_REGISTRY:
        ...
        def set_folder_for_person(
            ...
            result: bool = already_set or PIH.ACTION.POLIBASE.set_card_registry_folder(
                value, person_or_pin, registrator_person
            )
            ...
            if result:
                if e(value):
                    ...
                    event_ds: EventDS | None = ResultTool.get_first_item(
                        PIH.RESULT.EVENTS.get(*event_builder(None, person_or_pin))
                    )
                    ...
                    if nn(event_ds):
                        ...
                        if already_set:
                            PIH.EVENT.send(
                                *event_builder(value, person_or_pin, registrator_person)
                            )
                    else:
                        PIH.EVENT.send(
                            *event_builder(value, person_or_pin, registrator_person)
    ...
    class ACTION:
        ...
        class NOTES:
            @staticmethod
            def create(name: str, title: str, note: Note) -> bool:
                id: str | None = DataTool.rpc_unrepresent(
                    PIH.SERVICE.call_command(ServiceCommands.create_note, (note,))
                )
                if e(id):
                    return False
                return DataTool.rpc_unrepresent(
                    ...
                    if PIH.DATA.FORMAT.polibase_person_card_registry_folder(
                        ...
                        if ResultTool.is_empty(
                            ...
                            PIH.CARD_REGISTRY.set_folder_for_person(
                                value, person_or_pin, registrator_person, True
                            )
                            return True
                        else:
                            return False
