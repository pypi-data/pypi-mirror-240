...
from pih.errors import NotFound, Error
...
from pih.tools import j, nl, js, jnl, b as mb, while_not_do, esc, e, ne, nn, n, i

SPLITTER: str = ":"
FILE_PATTERN_LIST: tuple[str] = (j(("file", SPLITTER)), "<", "file=")
PARAMETER_PATTERN: str = r"\B\$[\w :]+"
LINE: str = "........................................................"
...
class ConsoleAppsApi:
                            ...
                            self.output.write_line(
                                js(
                                    (
                                        "",
                                        A.CT_V.BULLET,
                                        j((self.bold(disk_statistics.name), SPLITTER)),
    ...
    def resources_and_indications_check(
        ...
        with self.output.make_indent(2):
            ...
            if CheckableSections.INDICATIONS in checkable_section_list:
                with A.ER.detect():
                    if self.session.is_mobile:
                        self.output.new_line()
                    self.output.write_line(
                        js(("", A.CT_V.BULLET, self.bold("Показания в помещении КТ")))
                    )
            ...
            if CheckableSections.TIMESTAMPS in checkable_section_list:
                ...
                self.output.separated_line()
                self.output.write_line(
                    js(("", A.CT_V.BULLET, self.bold("Временные метки")))
                )
                with self.output.make_indent(2, True):
            ...
            if CheckableSections.MATERIALIZED_RESOURCES in checkable_section_list:
                ...
                self.output.separated_line()
                self.output.write_line(
                    js(("", A.CT_V.BULLET, self.bold("Материальные ресурсы")))
                )
                with self.output.make_indent(2, True):
            ...
            if CheckableSections.PRINTERS in checkable_section_list:
                ...
                with self.output.make_indent(2, True):
                    ...
                    self.output.separated_line()
                    self.output.write_line(
                        js(("", A.CT_V.BULLET, self.bold("Отчет по принтерам")))
                    )
    ...
    @property
    def arg_as_not_file_filtered(self) -> list[str]:
        return A.D.filter(
            lambda item: not item.startswith(FILE_PATTERN_LIST), self.session.argv
        )

    @property
    def arg_list_include_file(self) -> bool:
        return (len(self.session.argv) - len(self.arg_as_not_file_filtered)) != 0
    ...
    def run_command(
        ...
        def extract_parameters(value: str) -> str:
            ...
            fileless_arg_list: list[str] = self.arg_as_not_file_filtered
            for index, parameter in enumerate(DH.parameters_map):
                DH.parameters_map[parameter][0] = A.D.if_is_in(
                    ...
                    js(
                        (
                            "Введите параметр",
                            self.bold(DH.parameters_map[parameter][1] or parameter),
                        )
                    )
                )
        ...
        if command_type == A.CT_CMDT.CMD:
            ...
            def get_command(value: str | None = None) -> list[str]:
                ...
                return list(filter(lambda item: ne(item), result[0] + result[1]))

            command_text = get_command(js(command_text) if ne(command_text) else None)
            use_psexec_executor: bool = e(
                [
                    value
                    for value in list(map(lambda item: item.lower(), command_text))
            ...
            if use_psexec_executor:
                if e(host):
                    if self.yes_no(
                        ...
                        no_label=self.bold("Или введите название хоста"),
                    ):
                        host = default_host
                    else:
            ...
            if self.session.is_mobile:
                while True:
                    ...
                    command_part_item: str = extract_parameters(
                        command_text or j(command_line_list, nl())
                    )
                    stdout = StringIO()
                    with redirect_stdout(stdout):
                        try:
                            exec(command_part_item)
                        except Exception as exception:
                            self.output.error(
                                j(traceback.format_exception(exception), nl())
                            )
                            with self.output.make_separated_lines():
                                self.output.write_line(command_text)
                            if self.yes_no("Попробовать еще раз"):
                                continue
                            else:
                                break
                    self.output.write_line(stdout.getvalue())
                    break
        elif command_type == A.CT_CMDT.POLIBASE:
            if self.session.is_mobile:
                command_line_list: list[str] = []
                line_count: int = 0
                while e(command_text):
                    if line_count == 0 or self.yes_no(
                        ...
                        no_label=j(
                            (
                                self.bold("Завершить ввод текста запроса"),
                                " - ",
                                A.CT_V.NUMBER_SYMBOLS[0],
                            )
                        ),
                        ...
                        break
                    line_count += 1
                command_text = command_text or jnl(command_line_list)
                command_part_list: list[str] = command_text.split("$\\n")
                ...
                if not command_part_list_is_single:
                    self.output.write_line(
                        js(
                            (
                                "Команда состоит из",
                                self.bold(len(command_part_list)),
                                "запросов",
                            )
                        )
                    )
                    ...
                    while True:
                        try:
                            ...
                            with self.output.make_loading(
                                loading_timeout=0.5, text="Выполнение запроса"
                            ):
                                result = A.R_P.execute(command_part_item)
                            if e(result.data):
                                self.output.good(
                                    "Запрос выполнен"
                                    if command_part_list_is_single
                                    else js(("Запрос", command_part_index, "выполнен"))
                                )
                            else:
                                result.fields = FieldItemList(
                                    A.D.map(
                                        lambda item: FieldItem(item, item),
                                        list(A.R.get_first_item(result).keys()),
                                    )
                                )
                                need_show_result: bool = self.session.is_mobile
                                if len(result) > 1 and (
                                ...
                                if need_show_result:
                                    if result.fields.length() > 1:
                                        ...
                                        if first_field.name == A.CT_P.DBMS_OUTPUT:
                                            ...
                                            self.output.write_line(
                                                j((self.bold(first_field.caption), ":"))
                                            )
                                            A.R.every(
                                                lambda data: self.output.write_line(
                                                    js(
                                                        (
                                                            "",
                                                            A.CT_V.BULLET,
                                    ...
                                    result_title: str = title.split(SPLITTER)[-1]
                                    result_file_path: str = A.PTH.add_extension(
                                        A.PTH.join(A.PTH_MIO.FILES_FOLDER, A.D.uuid()),
                                        A.CT_F_E.EXCEL_NEW,
                                    )
                                    with self.output.make_loading(
                                        loading_timeout=0.5, text="Идёт создание файла"
                                    ):
                                        if A.A_D.save_xlsx(
                            ...
                            if error_catched:
                                ...
                                with self.output.make_separated_lines():
                                    self.output.write_line(command_part_item_source)
                                if self.yes_no("Попробовать еще раз"):
                                    error_catched = None
                                else:
                                    break
                        if nn(error_catched):
                            break
        elif command_type == A.CT_CMDT.DATA_SOURCE:
            if self.session.is_mobile:
                while True:
                    command_line_list: list[str] = []
                    line_count: int = 0
                    ...
                    try:
                        ...
                        result: Result[list[dict[str, Any]]] = A.R_DS.execute(
                            command_part_item
                        )
                        result.fields = FieldItemList(
                            A.D.map(
                        ...
                        self.output.error(error.details)
                        with self.output.make_separated_lines():
                            self.output.write_line(command_text)
                        if not self.yes_no("Попробовать еще раз"):
                            break
            ...
            try:
                ...
                while True:
                    ...
                    if isinstance(recipient, User):
                        prefix = f"Сообщение от {self.user_given_name} ({self.user_description}): {A.D.to_given_name(recipient)}, "
