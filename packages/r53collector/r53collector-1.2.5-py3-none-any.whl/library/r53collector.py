...
sheet_row=[account_id,zone_name,record['Name'].rstrip('.'),record['Type'],is_alias,is_dangling(dns_value),get_dns_value()]
    ...
    print_event(f"[+] Please authorize only if {device_authorization['userCode']} matches the code on your browser 
screen.","yellow",on_color=None,attrs=["bold"],end='',flush=True)
    ...
    def authwait():
        for n in range(1, expires_in // (interval+3) + 1):
            try:
                ...
                print_event("\r[+] ACCESS DENIED!. Program is being terminated...          
","red",on_color=None,attrs=["bold"])
                exit()
            except sso_oidc.exceptions.AuthorizationPendingException:
                if not args.no_verbose:
                    if n==1:
                        print_event("Device yet to be authorized in browser, 
waiting...",color="red",on_color=None,attrs=["blink"],end='', flush=True)
                    else:
                        print_event("\rDevice yet to be authorized in browser, 
waiting...",color="red",on_color=None,attrs=["blink"],end='', flush=True)
                pass
                ...
                for record in record_response['ResourceRecordSets']:
                    ...
                    if args.types and not args.exclude:
                        ...
                        if record['Type'] in dns_types:
                            ...
                            print_event(f"{record['Type']} : {record['Name']} ==> 
{get_dns_value()}","magenta",on_color=None)
