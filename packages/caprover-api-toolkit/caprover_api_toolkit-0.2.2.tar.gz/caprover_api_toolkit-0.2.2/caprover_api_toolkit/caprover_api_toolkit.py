...
infisical_site_url = os.getenv('INFISICAL_SITE_URL')
...
class CaproverAPI:
    ...
    def _resolve_and_remove_app_variables(
        ...
        for service_name, service_definition in _app_data['services'].items():
            if 'environment' in service_definition:
                del service_definition['environment']

        return yaml.dump(_app_data)
    ...
    def _generate_secrets(
        ...
        processed_app_data = yaml.load(raw_app_data, Loader=Loader)

        secrets_service_client = InfisicalClient(token=infisical_token, site_url=infisical_site_url)

        for service_name, service_definition in processed_app_data['services'].items():
            if 'environment' in service_definition:

                infisical_folder_url = infisical_site_url + "/api/v1/folders/"
                infisical_service_token_url = infisical_site_url + "/api/v2/service-token/"

                infisical_service_token = requests.request(
                    "GET",
                    infisical_service_token_url,
                    headers={"Authorization": infisical_token}
                )

                payload = {
                    "environment": "prod",
                    "folderName": service_name,
                    "workspaceId": infisical_workspace_id
                }
                headers = {
                    "X-API-Key": infisical_token,
                    "Authorization": infisical_service_token,
                    "Content-Type": "application/json"
                }

                requests.request("POST", infisical_folder_url, json=payload, headers=headers)

                for env_var, env_value in service_definition['environment'].items():
                    service_definition['environment'][env_var] = app_variables.get(env_value, env_value)

                    secrets_service_client.create_secret(
                        env_var, app_variables.get(env_value, env_value),
                        environment="prod", path="/" + service_name, type="shared"
                    )
    ...
    def deploy_one_click_app(
        ...
        custom_domain: str = None, app_variables: dict = None, automated: bool = False, resource_limits: dict = None,
        resource_reservations: dict = None, **kwargs
    ):
        """
        ...
        resolved_app_data = self._resolve_and_remove_app_variables(
            one_click_app_name=one_click_app_name
        )

        logging.info('Generating secrets and uploading to Infisical')
        ...
        logging.info('Secrets generated and uploaded')
    ...
    def delete_domain(self, app_name: str, custom_domain: str):
        ...
        response = self.session.post(
            self._build_url(CaproverAPI.DELETE_CUSTOM_DOMAIN_PATH),
            headers=self.headers, data=data
        )
        return CaproverAPI._check_errors(response.json())
    ...
    def update_app(
        ...
        repo_info: dict = None, resource_limits: dict = None,
        resource_reservations: dict = None, **kwargs
    ):
        """
        ...
        :return: dict
        """
        if resource_limits and (resource_limits.get('cpu') is None or resource_limits.get('memory') is None):
            raise ValueError("Both CPU and memory limits must be provided together.")

        if resource_reservations and (
            resource_reservations.get('cpu') is None or resource_reservations.get('memory') is None):
            raise ValueError("Both CPU and memory reservations must be provided together.")

        task_resources = {}

        if resource_limits:
            task_resources['Limits'] = {
                'NanoCPUs': resource_limits['cpu'] * 1000000000,
                'MemoryBytes': resource_limits['memory'] * 1024 * 1024
            }

        if resource_reservations:
            task_resources['Reservation'] = {
                'NanoCPUs': resource_reservations['cpu'] * 1000000000,
                'MemoryBytes': resource_reservations['memory'] * 1024 * 1024
            }
