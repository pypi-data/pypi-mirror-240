...
class CaproverAPI:
    ...
    def _generate_secrets(
        ...
        for app_variable in variables:
            variable_id = app_variable['id']
            variable_value = app_variables.get(variable_id, app_variable.get('defaultValue', ''))

            if not variable_value:
                if automated:
                    raise Exception(f'Missing or Invalid value for >>{variable_id}<<')
                else:
                    variable_value = "some default or user provided value"
            ...
            secrets_service_client.create_secret(
                variable_id, variable_value, environment="prod", path="/", type="shared"
            )
