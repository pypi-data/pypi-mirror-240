...
impl PyEventVoxelBuilder {
  ...
  fn py_build(
      ...
      polarity: PyReadonlyArray1<i8>) -> Result<Py<PyArray3<i32>>> {
    ...
    Ok(voxel.to_pyarray(py).to_owned())
  }
}
...
pub fn py_event_voxel_builder(_py: Python<'_>, module: &PyModule) -> PyResult<()> {
  init_static()?;
