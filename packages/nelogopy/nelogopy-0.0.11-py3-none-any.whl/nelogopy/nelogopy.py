...
def run_netlogo(netlogo_home="C:/Program Files/NetLogo 6.2.2"):
    ...
    model = os.path.dirname(__file__)+"/nelogopy.nlogo"
    n=nl4py.NetLogoApp()
    n.openModel(model)
    n.command("setup")
    return n
...
class pyturtle(turtle) :
    def __init__(self, n,x=0,y=0,id_s=0,shape="car",size_shape=4,color=0,name="zn",labelcolor=0,fields={}):

        super().__init__(n)
        self.id_s = id_s
        self.__dict__.update(fields)
        self.create_pyturtle_xyid(n,x,y,id_s,shape,size_shape,color,name,labelcolor)
  
    def create_pyturtle_xyid(self,n,x,y,id_s,shape,size_shape,color,name,labelcolor,):
        c="create-pyturtle-xyid "+list2nllist([x,y,id_s,shape,size_shape,color,name,labelcolor])
        c=n.report(c)
        self.id= int(c[:-2])
        return self