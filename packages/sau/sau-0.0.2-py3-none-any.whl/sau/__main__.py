...
class EC2SAUCollector:
    """_summary_
    Collector class used to scrape ec2 and ebs volume data
    """

    def __init__(self, regions: list, logger: logging.Logger) -> None:
    ...
    def get_stopped_ec2(self, region: str) -> list:
        ...
        """
        self.logger.info("get_stopped_ec2 for region %s", region)
    ...
    def get_unattached_volumes(self, region: str) -> dict:
        ...
        """
        self.logger.info("get_unattached_volumes for region %s", region)
        client = boto3.client("ec2", region_name=region)
        result = []
        ...
        for volume in response["Volumes"]:
            ...
            state = "unattached" if volume["State"] == "available" else volume["State"]
            states[state] += 1
            item = {
                "name": tags.get("Name"),
    ...
    def get_instance_metrics(self) -> dict:
        ...
        for region in self.regions:
            for name, func in funcs.items():
                self.logger.debug("calling func for region %s", region)
                processes.append(
                    {
                        "name": name,
    ...
    def collect(self):
        ...
        if volumes:
            ...
            yield gauge
        self.logger.info("metrics successfully collected")
...
if __name__ == "__main__":
    parser = ArgumentParser(
        ...
        epilog="Created by Emeka Ugwuanyi",
    )
    ...
    REGISTRY.register(EC2SAUCollector(regions=config["regions"], logger=Logging))
