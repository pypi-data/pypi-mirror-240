...
import pandas as pd
from tdfs4ds.utils import execute_query
...
def feature_store_table_creation(entity_id, feature_type, if_exists = 'replace'):
...
def store_feature(entity_id, prepared_features, date_in_the_past_= date_in_the_past, **kwargs):
    ...
    if display_logs: print(target_tables)
    ...
    for i,row in target_tables.iterrows():
        ...
        WHERE     {row.iloc[0]}.{ENTITY_ID}   = UPDATED_FEATURES.{ENTITY_ID}
        ...
        if display_logs: print(f'insert feature values of new {ENTITY_ID} combinations in {row.iloc[1]}.{row.iloc[0]}')
        if tdml.display.print_sqlmr_query:
            print(query_insert)
        execute_query(query_insert)
        if display_logs: print(f'update feature values of existing {ENTITY_ID} combinations in {row.iloc[1]}.{row.iloc[0]}')
        if tdml.display.print_sqlmr_query:
            print(query_update)
        execute_query(query_update)
...
def build_dataset(entity_id, selected_features, view_name,
    ...
    if type(entity_id) == dict:
        ...
        ENTITY_ID_ = ', \n'.join(['B.' + k for k in entity_id])
    elif type(entity_id) == str:
        ENTITY_NAMES = entity_id
        ENTITY_ID = entity_id
        ENTITY_ID_ = 'B.'+entity_id
    ...
    SELECT
        ...
        {ENTITY_ID_},
        B.FEATURE_VALUE
    FROM ({validtime_statement} SEL * FROM {schema}.{feature_catalog_name}) A
    , ({query}) B
    WHERE A.FEATURE_ID = B.FEATURE_ID
    """
...
def upload_features(df, entity_id, feature_names,
    ...
    prepared_features = prepare_feature_ingestion(
        df,
        entity_id,
        feature_names,
...
def register_entity(entity_id):
    ...
    return feature_store_table_name_float,feature_store_table_name_integer
...
def upload_tdstone2_scores(model):
    ...
    #print('feature_names_types :', feature_names_types)
