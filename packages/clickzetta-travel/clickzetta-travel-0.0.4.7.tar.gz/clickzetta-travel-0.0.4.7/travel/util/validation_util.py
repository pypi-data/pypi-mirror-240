...
from migration.util import validation_util, validation_table_util

def count_table_validation(source: Source, destination: Destination, source_table: str, destination_table: str):
    try:
        source_result, destination_result = validation_table_util.count_validation(source_table, destination_table,
                                                                                        source,
                                                                                        destination)
        source_df_result = pd.DataFrame()
        destination_df_result = pd.DataFrame()
        source_df_result['row_count'] = [row[0] for row in source_result]
        destination_df_result['row_count'] = [row[0] for row in destination_result]
