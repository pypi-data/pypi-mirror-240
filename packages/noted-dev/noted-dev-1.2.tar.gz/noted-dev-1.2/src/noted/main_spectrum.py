...
def lhcone_iterate_federations(params, config, row, list_federation, df_cric_database):
    ...
    for i in range(len(list_federation)): 
        ...
        if not df_fts_optimizer.empty: 
            ...
            # If throughput > 0.5 generate an alarm and export it to MONIT Grafana by using a MySQL database. NOTE: this is for LHCONE version
            if throughtput > 0.5: export_noted_alarm_to_monit(params, str(row['Start']) + '_' + row['Alarm'].split(' ')[0] + '_' + row['Device'] + '_' + row['Interface'] + '_LHCONE')
        else: 
