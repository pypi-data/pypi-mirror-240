...
def tile_bursts_overlap_to_xspectra(burst0, burst1, geolocation_annotation, calibration, noise_range, noise_azimuth, tile_width, tile_overlap,
    ...
    xs = list()
    landflag = list()
    periodos_per_tile = list() # storing number of periodo per tile to be sure they are all the same at the end
    ...
    # for i in xndindex(tiles_sizes):
        ...
        variables_list+=[mean_incidence.to_dataset(), ground_heading.to_dataset(), t_tile.to_dataset()]


        DN0 = sub0['digital_number']
        DN1 = sub1['digital_number']

        # ------------- computation of useful variables before Bright Target Removal --------
        # ------------- mean sigma0 and nesz ------------
        sigma0_BT, nesz_BT = compute_mean_sigma0_interp(DN0, calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
        if not np.isfinite(sigma0_BT): # should only append in IW mode. Case when line are badly indexed in noise-range LUT
            sigma0_BT, nesz_BT = compute_mean_sigma0_closest(DN0, burst['linesPerBurst'], calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
        sigma0_BT.attrs.update({'long_name':sigma0_BT.long_name+' including bright targets'})
        nesz_BT.attrs.update({'long_name':nesz_BT.long_name+' including bright targets'})
        variables_list+=[sigma0_BT.to_dataset(name=sigma0_BT.name+'_BT'), nesz_BT.to_dataset(name=nesz_BT.name+'_BT')]
        ...
        if water_only: # Bright target mask is applied only on water 
            ...
            # ---------------- part of the variables to be added to the final dataset ----------------------
            variables_list+=[nv.to_dataset(), sigma0.to_dataset(), sigma0_overlap.to_dataset(), nesz.to_dataset()]
            ...
            N_periodos = np.prod((xspecs.sizes['periodo_line'], xspecs.sizes['periodo_sample'])) # number of periodograms
            periodos_per_tile.append(N_periodos)
            xs_attrs = {'averaged_periodograms':N_periodos}
            xs_attrs.update({'periodo_width_' + d: k for d, k in periodo_width.items()})
            xs_attrs.update({'periodo_overlap_' + d: k for d, k in periodo_overlap.items()})

            [xspecs_m[k].attrs.update(xs_attrs) for k in xspecs_m.keys()] # this line copy the same attribute to all variables of xspecs_m dataset
    ...
    if len(np.unique(periodos_per_tile))>1:
        warnings.warn('Number of averaged periodograms changes over tiles ! attribute value of averaged_periodograms is skipped in output.')
