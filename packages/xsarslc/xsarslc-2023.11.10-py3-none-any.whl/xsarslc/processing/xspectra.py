...
def compute_WV_intraburst_xspectra(dt, polarization, tile_width=None, tile_overlap=None, landmask=None, IR_path=None, **kwargs):
    ...
    if 'xspectra_2tau' in xspectra:
        ...
        xspectra = xr.merge([xspectra, cwaves.to_dataset(), macs.to_dataset()])
...
def compute_macs(xs, k_az_min = -2 * np.pi / 600., k_az_max = 2 * np.pi / 600., k_rg_max = 2 * np.pi / 15., lambda_range_max=[50.]):
    """
    ...
    IMACS = xs.expand_dims({'lambda_range_max_macs':len(lambda_range_max)}).where(np.logical_and(cond_az, cond_rg), drop=True).sum(dim=['freq_line', 'freq_sample'])*dkrg*dkaz
    ...
    # return xr.merge([IMACS.real.rename('macs_Re'), IMACS.imag.rename('macs_Im')])
...
def compute_cwave_parameters(xs, kmin=2*np.pi/600, kmax = 2*np.pi/25, **kwargs):
    ...
    Keywords Args:
        kwargs (): keyword arguments passed to compute_kernel
            save_kernel (bool, optional) : save CWAVE kernel on disk in working directory
    ...
    xxs = xs.where((kk > kmin) & (kk < kmax), drop=True) # selection is operated on xspectra AND coordinates here
    ...
    cwave_parameters = ((kernel.cwave_kernel * xxsmn) * dkrg * dkaz).sum(dim=['freq_sample', 'freq_line'])

    return cwave_parameters.rename('cwave_params')#.to_dataset()


def compute_kernel(krg, kaz, kmin, kmax, Nk=4, Nphi=5, save_kernel=False):
    """
