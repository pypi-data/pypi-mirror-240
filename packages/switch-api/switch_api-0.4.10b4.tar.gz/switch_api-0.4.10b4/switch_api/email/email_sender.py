...
def send_email(
    ...
    if not body:
        sw.pipeline.logger.error("No body was provided")
        return False
    
    if not subject:
        sw.pipeline.logger.error("No subject was provided")
        return False
    
    if to_recipients is None or len(to_recipients) == 0:
        sw.pipeline.logger.error("No recipients were provided")
        return False
    
    sw.pipeline.logger.info(f'Sending email to recipients: {to_recipients}')
    ...
    sw.pipeline.logger.info("Sending request: POST %s", url)
    response = requests.post(url, headers=headers, files=files, data=data)

    if response.status_code != 200:
        sw.pipeline.logger.error(
            f"API Call was not successful. Response Status: {response.status_code}. Reason: {response.reason}. Response: {response.text}")
        return False

    sw.pipeline.logger.info(response.text)
    return True  
