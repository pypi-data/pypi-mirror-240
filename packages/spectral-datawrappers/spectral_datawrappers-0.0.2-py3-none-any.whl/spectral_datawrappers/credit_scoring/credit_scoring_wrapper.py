...
class CreditScoringWrapper(DataWrapperInterface):
    ...
    @staticmethod
    def _config_keys(): return ["url", "api_key"]

    def __init__(self, config: dict) -> None:
        self.config = config
        self.url = config["url"] if "url" in config else settings.CREDIT_SCORING_URL
    ...
    def __validate_request_payload(input: dict):
        ...
        """
        if 'wallet_address' not in input:
            raise ValueError("Missing wallet_address in input")
        if input['wallet_address'] is None:
    ...
    def request(self, input: dict) -> dict:
        ...
        request_url = self.url.format(input["wallet_address"])
