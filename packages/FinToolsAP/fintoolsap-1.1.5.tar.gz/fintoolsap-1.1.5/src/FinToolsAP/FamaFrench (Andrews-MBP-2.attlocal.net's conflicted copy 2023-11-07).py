...
class FamaFrench:
    ...
    def FF_factors(self, factors: list[str] = None, 
                   dfin: pd.DataFrame = None, 
        ...
        if(dfin is None):
            query_start_date = datetime.datetime(1900, 1, 1) if(start_date is None) else start_date
            query_end_date = datetime.datetime.now() if(end_date is None) else end_date
            ccm_df = self.DB.query_DB(self.DB.DBP.CCM, query_start_date, query_end_date)
        else:
            ccm_df = dfin
        ...
        if('mkt' in construct_factors):
            mkt_df = util_funcs.gorup_avg(df = ccm_df, 
                                          gr = 'date', 
                                          vr = ret_type, 
                                          wt = 'me',
                                          name = 'mkt'
    ...
    def sort_mutual_funds(self, funds, char_bkpts, sorting_funcs, rebalnce_freq):
        pass

    def sort_portfolios(self, 
                        stocks: pd.DataFrame, 
                        char_bkpts: dict[str, list[float]], 
                        sorting_funcs, 
                        rebalance_freq: str, 
                        weight_type = 'vw', 
                        sort_month = 7, 
        ...
        for rank_col in rank_cols:
            if('--fail' in rebalance_df[rank_col].unique()):
                if(not suppress):
                    print(f'{config.bcolors.WARNING}There are stocks that could not be sorted in {rank_col}. They will be removed before constructing portfolios.{config.bcolors.ENDC}')
        ...
        rets = None
        if(weight_type == 'vw'):
            rets = fin.groupby(['date', 'port_name']).apply(util_funcs.wavg, ret_typ, 'wt').to_frame().reset_index().rename(columns = {0: ret_typ})
        else:
