...
class pybd_gui(tk.Tk):
    def __init__(self):
        ...
        self.rpi_menu.add_command(label='Generate "no cal" RPi Code',
                command=self.rpi_codegen_no_cal)
    ...
    def add_G_cart_and_sensors(self, *args, **kwargs):
        ...
        if G_cart_name not in self.bd.block_dict:
            ...
            #G_cart = pybd.cart_pendulum_v2(sensor1=line_sense, sensor2=pend_enc, \
            #                            variable_name='G_cartv2',label="$G_{cartv2}$")
    ...
    def rpi_codegen_no_cal(self, *args, **kwargs):
        print("in rpi_codegen function")
        if not self.rpi_output_path:
            self.set_rpi_gen_output_path()
        print("rpi_template_path: %s" % self.rpi_template_path)
