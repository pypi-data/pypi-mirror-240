...
def read_pred_tab(WD):
    ...
    if not os.path.isfile("%s/pred_ls.txt" % WD):
        ...
        try:
            ...
            print("\033[93mWARNING: Unsupported characters by current locale in journal names\n"
                  "Change locale if interested, chars are replaced for now\033[0m\n")
            with open("%s/pred_ls.txt" % WD, 'w', encoding=enc, errors="replace") as ls_file:
                ls_file.write('\n'.join(str(i) for i in ls))
    else:
        ls = []
