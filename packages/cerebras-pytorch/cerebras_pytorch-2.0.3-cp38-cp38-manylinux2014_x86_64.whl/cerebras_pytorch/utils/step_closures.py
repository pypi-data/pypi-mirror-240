...
def checkpoint_closure(closure: Callable) -> Callable:
    ...
    def checkpoint_step_closure(*args, **kwargs):
        ...
        def closure_wrapper(*args, **kwargs):
            if len(backend.run_context_stack) == 0:
                raise RuntimeError(
            ...
            current_step = backend.run_context.iteration + 1
            checkpoint_steps = backend.run_context.checkpoint_steps

            # Only call the function if is an initial checkpoint or on a
            # checkpoint step or if it is the last step
            if backend.run_context.is_pre_initial_step or (
                checkpoint_steps
                and (
                    current_step % checkpoint_steps == 0
                    or backend.run_context.is_final_step
                )
            ):
                closure(*args, **kwargs)
            else:
                logger.debug(
                    ...
                    f"on non-checkpoint step {current_step}."
                )
