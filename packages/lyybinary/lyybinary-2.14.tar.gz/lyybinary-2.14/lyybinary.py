...
def Perfact_Reverse_forfloat_encoding(hex_str, debug=False):
    ...
    if isinstance(hex_str, bytes):
        if debug: print("输入的是bytes")
        hex_str = binascii.hexlify(hex_str).decode()
    else:
        if debug: print("输入的不是bytes")
...
def perfact_hex2float(hex_str, debug=False):
    ...
    if debug: print(f"转换类似“4479fff0”这样的十六进制字符串为浮点数,参数为：{hex_str}")
    #hex_str= 4479fff0
    int_value = int(hex_str, 16)
    if debug: print("int_value=", int_value)
    a = struct.pack('>I', int_value)
    if debug: print("a=", a)
    b = struct.unpack('>f', a)
    f = b[0]
    if debug: print("perfact_hex2float, result=", f)
    return f


def wm_hex_to_date(hex_str, debug=False):
    #['ED', '8B', '34', '01']
    if debug: print("welcome wm_hext-date,输入参数为：", hex_str)
    if isinstance(hex_str, bytes):
        if debug: print("输入的是bytes")
        hex_str = binascii.hexlify(hex_str).decode()
        print(f"输入的是bytes，最初二进制数据为{hex_str}，经过hexlify后的hex_str=", hex_str)
    ...
    if debug: print("result=", reversed_hex, type(reversed_hex))
    if debug: print("reversed_hex=", reversed_hex)
...
def decode_tdx_binary_data(binary_data, debug=False):
    ...
    for i in range(0, len(binary_data), 8):
        ...
        if debug: print("tdx start, original date_bytes=", date_bytes)  #b'\xe5\x8b4\x01'
        dateint = wm_hex_to_date(date_bytes, debug=debug)
        if debug: print("int_date_==", dateint)
        decoded_data.append(dateint)
        # 解析定点数

        fixed_point_bytes = binary_data[i + 4:i + 8]
        if debug: print("original fixed_point_bytes=", fixed_point_bytes)
        hex_string = ''.join([f'{byte:02x}' for byte in fixed_point_bytes])
        if debug: print("converted hex=", hex_string)  #00008040，顺序同二进制存放，所以要先分组逆序
        resvered_hex_string = Perfact_Reverse_forfloat_encoding(hex_string, debug=debug)
        if debug: print("resvered hex string = ", resvered_hex_string)
...
def get_lastdate_tdx_sinal(code, tdx_path=r"D:/Soft/_Stock/通达信开心果202310/T0002/signals/signals_user_999", debug=False):
...
def dayfile_from_vipdoc(tdx_vipdoc_path, code):
    market_dict = {0: "sz", 1: "sh"}
    market_code = int(code.startswith("6"))
    mark = market_dict[market_code]
    fname = f"{mark}{code}.day"
    fname = tdx_vipdoc_path + os.sep + mark + os.sep + fname
