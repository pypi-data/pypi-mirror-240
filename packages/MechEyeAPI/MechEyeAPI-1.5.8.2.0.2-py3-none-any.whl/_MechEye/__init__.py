...
if (
    ...
    package_dir = Path(importlib.util.find_spec("_MechEye").origin).parent
    pyd_name = "_MechEye.cp" + str(sys.version_info.major) + str(sys.version_info.minor) + "-win_amd64.pyd"
    pyd_files = list(package_dir.glob(pyd_name))
    assert len(pyd_files) == 1
    ctypes.WinDLL(str(pyd_files[0]), winmode=0)

try:
    from _MechEye._MechEye import (  # pylint: disable=import-error,no-name-in-module
        __version__,
        Device,
        DeviceInfo,
        ErrorStatus,
        ...
        LaserSettings,
        UhpSettings,
        color_from_numpy_array,
    )
except ImportError as ex:

    def __missing_sdk_error_message():
        error_message = """Failed to import the MechEye Python C-module, please verify that:
 - MechEye SDK is installed
 - MechEye SDK version is matching the SDK version part of the MechEye Python version """
        if platform.system() != "Windows":
            return error_message
        return (
            error_message
            + """
 - MechEye SDK libraries location is in system PATH"""
        )
