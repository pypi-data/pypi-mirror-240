...
def draw_monthly_leaderboard() -> go.Figure:
    ...
    JOIN actor a on e.actor_key = a.key
    WHERE 
        to_timestamp(e.created) >= date_trunc('month', current_date)
        AND e.type in ('validate','send','client_approve','reject')
        AND a.type = 'research'
    ...
    pivot_df = pivot_df[["name", "reject", "validate", "send", "client_approve"]]
    pivot_df["approval_rating"] = round(pivot_df["client_approve"] / pivot_df["send"], 2).fillna(0)
    ...
    # Now create the figure using display_df
...
def draw_annual_leaderboard() -> go.Figure:
    ...
    JOIN actor a on e.actor_key = a.key
    WHERE 
        to_timestamp(e.created) >= date_trunc('year', current_date)
        AND e.type in ('validate','send','client_approve','reject')
        AND a.type = 'research'
    GROUP BY a.name, e.type
    """
