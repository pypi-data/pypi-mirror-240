...
def draw_monthly_leaderboard() -> go.Figure:
    window = 'month'
    statement = f"""
    SELECT 
        a.name,
    ...
    WHERE 
        ...
        AND e.type in ('validate','send','client_approve','client_conflict','reject')
        AND a.type = 'research'
    ...
    pivot_df = pivot_df[["name","validate", "reject",  "send", "client_approve","client_conflict"]]
    pivot_df["client_total"] = pivot_df["client_approve"] + pivot_df["client_conflict"]
    pivot_df = pivot_df.sort_values(by="validate", ascending=False)
    ...
    display_df["approval_rating"] = round(display_df["client_total"] / display_df["send"], 2).fillna(0)
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(values=list(display_df.columns),
                    align='left'),
        cells=dict(values=[display_df[col] for col in display_df.columns],  # Use display_df to include the totals
                align='left'))
    ])

    fig.update_layout(
        title_text='Monthly Leaderboard',
        title_font=dict(size=24)
    )
    return fig
    
def draw_annual_leaderboard() -> go.Figure:
    window = 'year'
    statement = f"""
    SELECT 
        a.name,
