    ...
    def import_command(self, data_config, file_path, update_artifact, service_name='', version_name='', icon='',
        ...
        if data_config.get(constant.ARTIFACT):
            ...
            support_artifact_relation_types = [constant.ARTIFACT_RELATION, constant.FILE_ARTIFACT_RELATION,
                                               constant.ACR_IMAGE_ARTIFACT_RELATION, constant.HELM_CHART_ARTIFACT_RELATION]
        ...
        if len(data_list.body.services) == 0:
            ...
            if constant.SUPPLIER_DEPLOY_METADATA in deploy_metadata_config:
                ...
                if constant.SUPPLIER_TEMPLATE_CONFIGS in supplier_deploy_metadata:
                    for supplier_deploy_metadata in supplier_deploy_metadata.get(constant.SUPPLIER_TEMPLATE_CONFIGS):
                        template_path = os.path.join(os.path.dirname(file_path), supplier_deploy_metadata.get(constant.URL))
                        supplier_deploy_metadata[constant.URL] = self._replace_file_path_with_url(template_path)
            ...
            if constant.SUPPLIER_DEPLOY_METADATA in deploy_metadata_config:
                ...
                supplier_deploy_metadata = deploy_metadata_config.get(constant.SUPPLIER_DEPLOY_METADATA)
                if constant.SUPPLIER_TEMPLATE_CONFIGS in supplier_deploy_metadata:
                    for supplier_deploy_metadata in supplier_deploy_metadata.get(
                            constant.SUPPLIER_TEMPLATE_CONFIGS):
                        if supplier_deploy_metadata[constant.NAME] in name_url_mapping:
                            template_path = os.path.join(os.path.dirname(file_path),
                                                         supplier_deploy_metadata.get(constant.URL))
                            result_template = file.check_file_repeat(
                                name_url_mapping[supplier_deploy_metadata[constant.NAME]],
                                template_path)
                            if result_template:
                                supplier_deploy_metadata[constant.URL] = name_url_mapping.get(
                                    supplier_deploy_metadata[constant.NAME])
                            else:
                                supplier_deploy_metadata[constant.URL] = self._replace_file_path_with_url(template_path)
                        else:
                            supplier_deploy_metadata[constant.URL] = self._replace_file_path_with_url(
                                template_path)
