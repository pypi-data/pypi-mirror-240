...
def sparse_pauli_z(n, L):
    ...
    if 0 <= n < L:
        ...
        result = sparse.csc_array((diag, (row_col_indices, row_col_indices)), shape=(2**L, 2**L), dtype=complex)
        return result
    else:
        raise ValueError("Index n must fulfill 0 <= n < L")
...
# Sparse Ground state
# ---------------------------------------------------------------------------------------
def sparse_ising_ground_state(L: int, h_t: float, h_l: float = 1e-7, J: float = 1, k: int = 1) -> sparse.csc_array:
    """
    exact_initial_state
...
def exact_evolution_sparse(
    ...
    # total
    mag_exact_tot.append(
        (psi_exact.T.conjugate() @ mag_tot_op @ psi_exact)[0,0].real
    )
    ...
    for trott in range(trotter_steps):
        ...
        # total
        mag_exact_tot.append(
            (psi_new.T.conjugate() @ mag_tot_op @ psi_new)[0,0].real
        )
