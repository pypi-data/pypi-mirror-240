...
class FSParams(dict):
    ...
    def __repr__(self) -> str:
        repr = super().__repr__()
        return repr if self.__len__() > 0 and self.loaded_default_repr != repr else ''
...
class FlexSrc(FSIndirectObject):
    ...
    def try_load_configs(self):
        self.config.update(get_yamlfile_contents(Path(os.path.expanduser('~')) / ('.' + CONFIG_FILE)))
        self.config.update(get_yamlfile_contents(Path(os.path.expanduser('~')) / CONFIG_FILE))
    ...
    def prepare_cache_dir(self):
        if self.config[REPO_CACHE_DIR] is None:
            with tempfile.TemporaryDirectory() as d:
                self.config[REPO_CACHE_DIR] = Path(d).parent / self.__class__.__name__ / 'repo'
        if self.config[DATA_CACHE_DIR] is None:
            with tempfile.TemporaryDirectory() as d:
                self.config[DATA_CACHE_DIR] = Path(d).parent / self.__class__.__name__ / 'data'
    ...
    def try_load_fsr_yaml(self):
        if os.path.isdir(Path(self.root_dir) / self.target)\
            ...
            self.config.update(get_yamlfile_contents(Path(self.root_dir) / self.target / FSR_FILE))
    ...
    def load(self):
        ...
        if self.loaded_params_str != params_str:
            ...
            code = compile(get_file_contents(self.config[OBJECT_LOADER_PATH]), self.config[OBJECT_LOADER_PATH], 'exec')
            globals_storage = {}
            ...
            objects = eval(f"{self.config[OBJECTS_FUNC]}()", globals_storage, locals_storage)
            self.clear()
            self.update(objects)
            # END: reflect objects
    ...
    def load_default_params(path=FSR_FILE):
        if FLEXSRC_CURRENT_ROOT_DIR not in globals():  # if is invoked from external procedure
            path = Path(path)
            if not path.is_absolute():
                import inspect
                parent = Path(inspect.currentframe().f_back.f_code.co_filename).parent
                path = parent / path
