...
class _GeometryDescr(_BaseDescr):
    ...
    Args:
        bbox: a bbox attribute that will be set when the geometry is updated. This is important for keeping geometry/bbox in sync on objects where the bbox is always derived from a geometry field.
...
class _ListDescr(_BaseDescr):
    ...
    def _validate(self, obj: object, value: list) -> None:
        ...
        if self.max_len is not None and len(value) > self.max_len:
            raise ValueError(
                f"{self.public_name} must be less than or equal to {self.max_len} elements in length"
            )
        if self.min_len is not None and len(value) < self.min_len:
            raise ValueError(
                f"{self.public_name} must be greater than or equal to {self.min_len} elements in length"
            )

        if self.item_type is not None:
            if len(value) > 0:
                print(f"In Validation {self.item_type = }")
                if isinstance(self.item_type, (tuple, list)):
                    _type = self.item_type[0]
                else:
                    _type = self.item_type
...
class _TimeDeltaDescr(_BaseDescr):
    ...
    def _validate(self, obj: object, value: object) -> str:
        ...
        raise ValueError(
            f"must be a string, datetime.timedelta or numpy.timedelta64, got {value.__class__.__name__}"
        )
...
class _URLDescr(_StringDescr):
    """A URL string that is checked to be a valid URL. Inherits from :class:`_StringDescr<geodesic.descriptors._StringDescr>`
    for all methods except validate.
