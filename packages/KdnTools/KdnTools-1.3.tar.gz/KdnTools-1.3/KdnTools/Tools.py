...
class Tools:
        ...
        def view_data(self, table_name, page_size=50):
            try:
                ...
                for page in range(1, total_pages + 1):
                    ...
                    if len(data) == 0:
                        logging.info(f"No data found in the '{table_name}'.")
                    else:
                        table = PrettyTable()
    ...
    class Data:
        def __init__(self, db_location, tools_instance):
            ...
            self.db_manager = tools_instance.DatabaseManager(db_location)
    ...
    def __init__(self, db_location):
        ...
        console_handler.setLevel(logging.INFO)  # Change log level as needed
...
if __name__ == "__main__":
    db_location = "your_database.db"  # Replace with your actual database location
    app = Tools(db_location)

    try:
        while True:
            choice = app.User().choice("Choose an option:",
                                       ["Exit", "Manage Database", "Search and Analyze Text", "Generate Random Number"])

            if choice == 1:
                break  # Exit the program

            elif choice == 2:
                app.DatabaseManager(db_location).view_data("your_table")  # Replace with your actual table name

            elif choice == 3:
                app.Data(db_location, app).search_and_display_results()

            elif choice == 4:
                num1 = int(input("Enter the lower bound: "))
                num2 = int(input("Enter the upper bound: "))
                print(app.RandNumber(num1, num2))
