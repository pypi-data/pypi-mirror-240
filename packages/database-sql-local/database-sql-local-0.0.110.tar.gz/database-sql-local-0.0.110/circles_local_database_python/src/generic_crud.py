...
class GenericCRUD:
    ...
    def insert(self, table_name: str = None, json_data: dict = None) -> int:
        ...
        try:
            ...
            logger.exception(GenericCRUD._log_error_message(GenericCRUD,message="Error inserting json_data", sql_statment=insert_query), object=e)
            logger.end()
            raise
    ...
    def update_by_id(self, table_name: str = None, id_column_name: str = None, id_column_value: Any = None,
        ...
        if id_column_name:  # id_column_value can be empty
            ...
            self.update_by_where(table_name=table_name, where=where, update_data=json_data,
    ...
    # Old name: update
    def update_by_where(self, table_name: str = None, update_data: dict = None, where: str = None,
                        params: tuple = None, limit: int = 100, order_by: str = None) -> None:
        """Updates data in the table by WHERE.
        Example:
        "UPDATE table_name SET A=A_val, B=B_val WHERE C=C_val AND D=D_val"
        translates into:
        update_by_where(table_name="table_name", update_data={"A": A_val, "B": B_val}, where="C=%s AND D=%s", params=(C_val, D_val)"""
        logger.start(object={"table_name": table_name, "json_data": str(update_data), "where": where,
                             "params": str(params), "limit": limit})
        table_name = table_name or self.default_table_name
        self._validate_json_data(update_data)
        self._validate_table_name(table_name)
        try:
            validate_none_select_table_name(table_name)
            set_values = ', '.join(f"{k}=%s" for k in update_data.keys()) + ("," if update_data else "")
            if not where:
            ...
            self.cursor.execute(update_query, tuple(update_data.values()) + params)
            self.connection.commit()
            logger.end("Data updated successfully.")
        except Exception as e:
            logger.exception(GenericCRUD._log_error_message(GenericCRUD,message="Error updating json_data", sql_statment=update_query), object=e)
            logger.end()
            raise
    ...
    def delete_by_where(self, table_name: str = None, where: str = None, params: tuple = None) -> None:
        ...
        try:
            ...
            logger.exception(GenericCRUD._log_error_message(GenericCRUD, message="Error while deleting", sql_statment=update_query), object=e)
            logger.end()
            raise
    ...
    def select_multi_tuple_by_where(self, view_table_name: str, select_clause_value: str = "*",
        ...
        try:
            ...
            logger.exception(GenericCRUD._log_error_message(GenericCRUD,message="Error selecting json_data", sql_statment=select_query), object=e)
            logger.end()
            raise
    ...
    def _log_error_message(self, message: str, sql_statment: str) -> str:
        return f"{message} - SQL statement: {sql_statment}"