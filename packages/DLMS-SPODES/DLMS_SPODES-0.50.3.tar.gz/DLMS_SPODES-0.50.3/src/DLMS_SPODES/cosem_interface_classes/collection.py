...
class Collection:
    ...
    def copy(self, ldn: octet_string.LDN = None) -> Self:
        ...
        for obj in islice(self.__container.values(), self.__const_objs, len(self.__container)):
    ...
    def pop(self, class_id: ClassID) -> list[InterfaceClass]:
        ...
        for obis, obj in self.__container.items():
            if obj.CLASS_ID == class_id:
                o.append(obis)
        for obis in o:
            ret.append(self.__container.pop(obis))
        return ret
    ...
    def try_remove(self, logical_name: cst.LogicalName) -> bool:
        ...
        match self.get(logical_name.contents):
            case None:
                return False
            case ic.COSEMInterfaceClasses() as obj if (tuple(self.__container.keys()).index(obj.logical_name.contents) >= self.__const_objs):
                self.__container.pop(obj.logical_name.contents)
                return True
            case _:
