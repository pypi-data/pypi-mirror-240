...
class FixedAssets:
    ...
    def __init__(self, current_period: str = None, previous_period: str = None):
        ...
        self.reporting_date_string = self.sht['c1'].options(dates=self.date_handler).value
    ...
    def define_excel_file_name(self):
        if self.current_period:
            ...
            print(self.reporting_date_object)
        for item in Path("../Fixed assets/DataSource_Backups").rglob("*"):
            pattern = r'\d+\b'
    ...
    def backup_excel_file():
        shutil.copy(
            ...
            dst='../Fixed assets/DataSource_Backups/!Амортизация_помесячно_БУ_2023_' + datetime.now().strftime(
                "%d-%m-%Y") + '.xlsb'
        )
...
if __name__ == '__main__':
    wkb_path = str(Path(__file__).resolve()).split(".")[0] + '.xlsm'
