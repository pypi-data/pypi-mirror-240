...
argparser.add_argument("-q", "--quick", action="store_true", help="不联网检查dspawpy版本")
...
try:
    ...
    cmd = 'pip install prompt_toolkit loguru dspawpy'
    if os.system(cmd) != 0:
        raise ImportError(f'无法自动配置依赖库，请手动安装后再试 $ {cmd}')
    
def online_check(dv, df):
    latest_version = None
    try:  # 如果正常导入dspawpy，应当有 requests库（pymatgen和mp-api的依赖）
        print("正在联网检查dspawpy版本... 使用 -q 参数启动此程序可跳过检查")
        response = get(f"https://pypi.org/pypi/dspawpy/json", timeout=3)
        ...
        if latest_version:
            if latest_version != dv:
                ...
                print(f" 导入的dspawpy模块文件路径为（如果不符合预期，请检查环境变量)：\n {df}\n")
            else:
                print(f"\n         成功导入 {dv}（最新版）dspawpy\n")
        else:
        ...
        if n == "1": # 环境部署比较特殊，不要进入主程序
            while True:  # 子菜单选择
                ...
                if n not in ["0", "1", "2"]:
                    ...
                    print('请在终端执行以下命令加载HZW集群上已部署好的环境,然后再次运行此程序：\n  source /data/hzwtech/profile/dspawpy.env')
                    return 1, 1
                elif n == "2":
                    cmd = "pip install dspawpy loguru prompt_toolkit --user -i https://pypi.tuna.tsinghua.edu.cn/simple && pip install -U dspawpy loguru prompt_toolkit -i https://pypi.org/simple --user"
                    while True:
                        yn = input(f"即将执行\n {cmd}\n (y/n)? ")
                        if yn.lower() == "y":
                            if os.system(cmd) == 0:
...
def main():
    ...
    if task_number[0] == 2:
        ...
        if task_number[1] != 6:
            xlims = check_lims("输入x轴范围（先小后大，以空格分隔，直接回车可跳过设置）：")
            ylims = check_lims("输入y轴范围（先小后大，以空格分隔，直接回车可跳过设置）：")
            outfile = prompt("输出图片路径（包含文件名）：", completer=pc)
        if task_number[1] == 1:  # tdos
            ...
            tasks.s5_6(dosdata, dos_plotter)
...
def until_give_valid_input(user_prompt, valid_input, completer=None):
    while True:
        n = prompt(user_prompt, completer=completer)
        if n in valid_input:
            return n
        else:
            print("!!! 输入错误，请重试")
...
class Tasks:
    ...
    def s2(
        ...
        infile = prompt("待解析文件路径（包含文件名，支持h5, json, pdb, as, hzw等格式）：", completer=pc)
        outfile = prompt(
            "输出文件路径（包含文件名，支持pdb, xyz, dump, json, as, hzw等格式）：", completer=pc
        )
    ...
    def s3_1(
        ...
        if task == "potential":
            if infile.endswith(".h5"):
                ...
                if len(keys) == 0:
                    raise ValueError(f"未检测到数据集，请检查文件{infile}路径或者任务类型是否正确！")
                elif len(keys) == 1:
                    subtype = keys[0]
                else:
                ...
                raise ValueError(f"仅支持h5和json格式，不支持{infile.split('.')[-1]}格式的数据文件！")

        outfile = prompt("输出文件路径（包含文件名，支持.cube和.vasp格式）：", completer=pc)
    ...
    def s3_2(
        ...
        individuals = []
        while True:
            individual = prompt("体系各组分电荷密度文件路径，(包含文件名，直接回车表示停止输入）:", completer=pc)
            if individual == "":
                break
    ...
    def s4_1(self, infile, ylims, outfile):
        ...
        if is_metal:
            ...
            while True:
                shift = input("此为非金属体系，是否要将能量零点从价带顶平移至费米能级？(y/n)")
    ...
    def s4_6(
        ...
        if infile1.endswith(".json"):
            infile3 = prompt(
                "待解析system.json文件路径（与wannier.json所在文件夹一致，包含文件名）：", completer=pc
            )
    ...
    @logger.catch
    def s5_6(self, dos_data, outfile):
    ...
    def s7(
        ...
        key = until_give_valid_input(
            f'数据类型（可选 {_list}）：',
            _list,
            completer=WordCompleter(_list),
        )
        _list2 = ['X', 'Y', 'Z', 'XY', 'YZ', 'ZX']
        index = until_give_valid_input(
            f"指定数据 {_list2}：",
            _list2,
        ...
        plot_optical(datafile=infile, key=key, index=int(index))
    ...
    def s8_2(
        ...
        """绘制NEB势垒"""
        infile = prompt("待解析neb.h5/neb.json路径（包含文件名）或NEB文件夹：", completer=pc)
        outfile = prompt("输出图片路径（包含文件名）：", completer=pc)
    ...
    def s8_4(
        ...
        """输出插值链"""
        preview = until_give_valid_input("预览插值生成的NEB链（无需完成计算）（y/n）：", ["y", "n"])
        directory = prompt("NEB计算文件夹：", completer=pc)
        if preview == "y":
            step = 0
        else:
            step = int(input("第几个离子步（从1开始）："))
        dst = prompt("输出到文件夹（直接回车表示当前文件夹）：", completer=pc)
        json_or_xyz = until_give_valid_input("输出文件格式（json/xyz）：", ["json", "xyz"])
        logger.info("正在处理，请稍等...")
    ...
    def s10_3(
        ...
        timestep = input(f"输入时间步长（fs），直接回车将尝试从{infile}中自动读取，失败则此数值将设为1.0：")
