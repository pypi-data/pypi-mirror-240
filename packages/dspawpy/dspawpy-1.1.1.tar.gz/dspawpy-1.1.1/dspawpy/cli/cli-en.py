...
=== 1 environment activation ===
  ...
  1: activate installed dspawpy environment on HZW machine
  2: install dspawpy environment
...
try:
    ...
    if os.system(cmd) != 0:
        raise ImportError(f'Failed to install dependencies, please manually try this command $ {cmd}')
...
class Tasks:
    ...
    def s2(
        ...
        infile = prompt(
            "Please enter the path to the file (including file name, support h5, json, pdb, as, hzw, ...): ", completer=pc
        )
        outfile = prompt(
    ...
    def s7(
        ...
        outfile = prompt(
            "Please enter the path to the output file (include file name): ",
            completer=pc,
        )
        logger.info("Processing...")
    ...
    def s8_4(
        ...
        if preview == "y":
            ...
            step = int(input("Ionic step index (start from 1)"))
