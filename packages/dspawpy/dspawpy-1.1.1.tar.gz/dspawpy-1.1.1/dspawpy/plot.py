...
def dump_bs_raw(ax, xycsv="band_xy.csv", tickcsv="band_ticks.csv"):
    """仅支持无磁性体系（暂时不能区分自旋上下）！
    传入一个能带图的ax对象，将数据点xy坐标保存到 band_xy.csv 中，
    将xticks和xticklabels保存到 band_ticks.csv 文件中
    ...
    xycsv : str
        保存数据点xy坐标的csv文件名
    tickcsv : str
        保存xticks和xticklabels的csv文件名

    Examples
    --------
    >>> from dspawpy.plot import dump_bs_raw
    ...
    # xticklabel "Text(2.755552578251739, 0, 'X')" -> "X"
    xticklabels = [label.get_text() for label in xticklabels]

    df1 = pd.DataFrame({"x": xs, "y": ys})
    df2 = pd.DataFrame({"xticks": xticks, "xticklabels": xticklabels})
    df1.to_csv(xycsv)
    print(f"==> {xycsv}")
    df2.to_csv(tickcsv)
    print(f"==> {tickcsv}")
...
def dump_dos_raw(ax, xycsv="band_xy.csv"):
    # get all x data and y data
    xs = []
    ys = []
    ls = []
    for line in ax.get_lines():
        ...
        y = line.get_ydata()
        l = line.get_label()
        if l.startswith("_child"):
            continue
        xs.extend(x.tolist())
        ys.extend(y.tolist())
        ls.extend([l] * len(x))
    df1 = pd.DataFrame({"x": xs, "y": ys, "label": ls})
    df1.to_csv(xycsv)
    print(f"==> {xycsv}")
