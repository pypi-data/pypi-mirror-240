...
class custom_agent:
    def __init__(
        ...
        if self.load_memory:
            # self.__create_retriever_and_vectordb(embeddings_model=self.embeddings_model)
        ...
        if agent_type:
            ...
            self.__start_routine_func(self._update_summary, 300)
            self.__start_routine_func(self._update_status, 300)
            self.__start_routine_func(self._update_place, 300)
...
chat_summ: {self.chat_hist_summary }"""
        ...
        **kwargs,
        ...
        if self.vector_database_type == Vector_Database_Type.PINECONE:
            ...
            if index_collection_name in [col.name for col in client.list_collections()]:
                ...
                collection = client.create_collection(index_collection_name)
    ...
    def interview(self, user, query):
        ...
        if not (self.start_interview):
            ...
            self.__start_routine_func(self._update_chat_hist, 300)
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit the functions with input parameters for concurrent execution
            guard = executor.submit(self.__guarding, question=query)
