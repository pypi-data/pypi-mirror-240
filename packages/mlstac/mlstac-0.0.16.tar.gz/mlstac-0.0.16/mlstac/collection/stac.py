...
class Provider(pydantic.BaseModel):
    ...
    roles: Optional[
        List[Literal["licensor", "producer", "processor", "host"]]
    ] = None        
...
class STACCollection(pydantic.BaseModel):
    ...
    def check_id(cls, v):
        return Id(field=v).field
    
    @pydantic.field_validator("description")
    def check_description(cls, v):
        return Description(field=v).field
    
    @pydantic.field_validator("license")
    def check_license(cls, v):
        return License(field=v).field
    
    @pydantic.field_validator("extent")
    def check_extent(cls, v):
        return Extent(**v).model_dump()
    
    @pydantic.field_validator("links")
    def check_links(cls, v):
        return Links(**v).model_dump()

    @pydantic.field_validator("title")
    def check_title(cls, v):
        return Title(field=v).field
    
    @pydantic.field_validator("keywords")
    def check_keywords(cls, v):
        return Keywords(field=v).field
    
    @pydantic.field_validator("providers")
    def check_providers(cls, v):
        return Providers(field=v).model_dump()["field"]
