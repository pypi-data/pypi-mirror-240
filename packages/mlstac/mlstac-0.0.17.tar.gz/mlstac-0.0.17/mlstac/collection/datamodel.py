...
from mlstac.collection.spectral import (
    _add_landsat1, _add_landsat2, _add_landsat3,
    _add_landsat4_mss, _add_landsat4_tm,
    _add_landsat5_mss, _add_landsat5_tm,
    _add_landsat7, _add_landsat8, _add_landsat9,
    _add_sentinel2, _add_eo1_ali, _add_aster,
    _add_modis
)

from mlstac.collection.stac import STACCollection, Link
from typing import List, Optional, Literal, Dict, Union
from mlstac.utils import finder
from mdutils import MdUtils, Html


KEY_ORDER = (
    "id", "type", "stac_version", "description",
    "links", "stac_extensions", "title", "keywords",
    "providers", "extent", "license", "ml_version", 
    "ml_train", "ml_validation", "ml_test", "ml_task",
    "ml_labels", "ml_curator", "ml_authors",
    "ml_reviewers", "ml_dimensions", "ml_spectral", 
    "ml_split_strategy", "ml_raw_data_url",
    "ml_discussion_url", "ml_size", 
    "ml_metadata_train", "ml_metadata_validation",
    "ml_metadata_test"
)
...
class Collection(STACCollection):
    ...
    def add_dimension(
        self,
        tensor: Literal["input", "target", "extra"],
        **kwargs: dict
    ) -> None:
    ...
    def add_sentinel2(self, bands: str = "all"):
        ...
        return _add_sentinel2(self, bands=bands)
    
    def add_landsat1(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat1(self, bands=bands)
    
    def add_landsat2(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat2(self, bands=bands)
    
    def add_landsat3(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat3(self, bands=bands)
    
    def add_landsat4_mss(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat4_mss(self, bands=bands)
    
    def add_landsat4_tm(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat4_tm(self, bands=bands)
    
    def add_landsat5_mss(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat5_mss(self, bands=bands)

    def add_landsat5_tm(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat5_tm(self, bands=bands)
    
    def add_landsat7(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat7(self, bands=bands)
    
    def add_landsat8(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat8(self, bands=bands)

    def add_landsat9(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_landsat9(self, bands=bands)

    def add_eo1_ali(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_eo1_ali(self, bands=bands)

    def add_aster(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_aster(self, bands=bands)

    def add_modis(self, bands: str = "all"):
        self.ml_spectral = None
        return _add_modis(self, bands=bands)
    ...
    def add_sensor(self, sensor: str):
        ...
        self.ml_spectral = SpectralBands(
            bands=self.ml_spectral.bands,
            axis=self.ml_spectral.axis,
            sensor=sensor
        )
    ...
    def auto(self, path: Union[str, pathlib.Path], checksum: bool = True):
        if isinstance(path, str):
            path = pathlib.Path(path)

        # Get the size of the dataset
        all_files = finder(path, pattern=".*safetensors\.gz$", recursive=True, full_names=True)
        ...
        # Save the metadata
        self.create_parquet(path=path, checksum=checksum)
    ...
    def create_checksums(self, path: Union[str, pathlib.Path]):
        if isinstance(path, str):
            path = pathlib.Path(path)

        # Get the checksums of the dataset
        ...
        # Get the checksums of the dataset
        train_checksums = train_db["checksum"].to_list()
        validation_checksums = validation_db["checksum"].to_list()
        test_checksums = test_db["checksum"].to_list()
        
        self.checksums = {
            "train": train_checksums,
            "validation": validation_checksums,
            "test": test_checksums
        }
        ...
        for index, value in enumerate(dataset["geotransform"]):
            ...
            if index == 0:
                train_xmin, train_ymin, train_xmax, train_ymax = (
                    geotransform[0], geotransform[3], geotransform[0], geotransform[3]
                )
            else:
    ...
    def create_extent(self, path: Union[str, pathlib.Path]):
        ...
        dates_start_train = train_db["start_datetime"].str.to_datetime(format="%Y-%m-%dT%H:%M:%S")
        dates_start_val = validation_db["start_datetime"].str.to_datetime(format="%Y-%m-%dT%H:%M:%S")
        dates_start_test = test_db["start_datetime"].str.to_datetime(format="%Y-%m-%dT%H:%M:%S")

        self.extent = {
            "spatial": {
                "bbox": [
                    train_bbox, validation_bbox, test_bbox
                ]
            }, 
            "temporal": {
                "interval": [ 
                    [min(dates_start_train).isoformat(), max(dates_start_train).isoformat()],
                    [min(dates_start_val).isoformat(), max(dates_start_val).isoformat()],
                    [min(dates_start_test).isoformat(), max(dates_start_test).isoformat()]
                ]
            }
        }
    ...
    def create_parquet(self, path: Union[str, pathlib.Path], checksum: bool = True):
        if isinstance(path, str):
            path = pathlib.Path(path)
    ...
    def add_documentation(self, path: Union[str, pathlib.Path]):
        ...
        # Add code snippet ---------------------------------------
        url = self.ml_train.link[:self.ml_train.link.strip("/").rfind("/")] + "/main.json"
        mdFile.new_line("```python")
        mdFile.new_line(f"import mlstac\nsecret = '{url}'")
        mdFile.new_line(f"train_db = mlstac.load(secret, framework='torch', stream=True, device='cpu')")
        mdFile.new_line("```")
        ...
        mdFile.new_paragraph("Sensor: " + self.ml_spectral.sensor, bold_italics_code='b')
        mdFile.new_paragraph("ML-STAC Task: " + ", ".join(self.ml_task), bold_italics_code='b')
        mdFile.new_paragraph(f"Data raw repository:  " + mdFile.new_inline_link(link=self.ml_raw_data_url), bold_italics_code='b')
        mdFile.new_paragraph(f"Dataset discussion:  " + mdFile.new_inline_link(link=self.ml_discussion_url), bold_italics_code='b')
        mdFile.new_paragraph(f"Review mean score:  {self.get_review_mean_score()}", bold_italics_code='b')
        mdFile.new_paragraph(f"Split_strategy:  " + self.ml_split_strategy, bold_italics_code='b')
        mdFile.new_paragraph(f"Paper:  " + mdFile.new_inline_link(link=self.ml_paper), bold_italics_code='b')
        ...
        mdFile.new_table(columns=3, rows=len(self.providers)+1, text=list_of_strings, text_align='center')
        ...
        mdFile.new_table(columns=3, rows=len(self.ml_curators.curators)+1, text=list_of_strings, text_align='center')

        # Add Reviewers --------------------------------------------
        if len(self.ml_reviewers.reviewers) > 0:
            ...
            mdFile.new_table(columns=4, rows=len(self.ml_reviewers.reviewers)+1, text=list_of_strings, text_align='center')

        # Add Labels ----------------------------------------------
        if len(self.ml_labels.labels) > 0:
            ...
            mdFile.new_table(columns=2, rows=len(self.ml_labels.labels)+1, text=list_of_strings, text_align='center')
        ...
        for k, v in self.ml_dimensions.model_dump().items():
            if len(v["dimensions"]) > 0:
                ...
                mdFile.new_table(columns=3, rows=len(v["dimensions"])+1, text=list_of_strings, text_align='center')

        # Add Spectral Bands --------------------------------------    
        if len(self.ml_spectral.bands) > 0:
            mdFile.new_header(level=2, title=f"Spectral Bands")
            list_of_strings = ["Name", "Common Name", "Description", "Center Wavelength", "Full Width Half Max", "Index"]
            for p in self.ml_spectral.bands:
                list_of_strings.extend([p.name, p.common_name, p.description, p.center_wavelength, p.full_width_half_max, p.index])
            mdFile.new_table(columns=6, rows=len(self.ml_spectral.bands)+1, text=list_of_strings, text_align='center')
