...
""")
    def sort_by_body_size(lines):
        ...
        for line in lines:
            parsed_lines.append({
                "text":line,
                "body_size":float(parse_line(line)["body_bytes_sent"])
                })
    ...
    with open(f'./{args.file}', 'r') as f:
        ...
        if args.large != None and args.large.isnumeric():
            for line in sort_by_body_size(final_lines)[:int(args.large)]:
                print(f"{str(round(float(parse_line(line)['body_bytes_sent'])/1024,2))}KB {parse_line(line)['ip_address']} {parse_line(line)['host']} {parse_line(line)['request']}")
            return
