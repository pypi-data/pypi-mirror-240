...
class DelayedModel:
    ...
    def _instance(self):
        ...
        try:
            assert self._delayed_obj
        except AssertionError as e:
            log(f"Model relationship error. Most likely failed to clean up dangling reference.\nModel: {self._delayed_model}\npk: {self._delayed_pk}\nResult: {self._delayed_model.get(self._delayed_pk)}")
            raise e
        return self._delayed_obj
    ...
    def __getattribute__(self, name):
        ...
        return getattr(self._instance(), name, self._delayed_model)

    def __getattr__(self, attr):
        log(self._instance(), self._delayed_pk)
    ...
    def __repr__(self):
        return f"<DelayedModel {self._delayed_model.__name__} {self._delayed_pk}:{self._delayed_obj}>"
