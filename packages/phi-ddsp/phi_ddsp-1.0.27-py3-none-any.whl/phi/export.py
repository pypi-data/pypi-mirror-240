...
def export(config, model_directory, output_directory, device):
    ...
    torch.jit.save(
        scripted_model,
        path.join(config["train"]["model_name"], config["train"]["model_name"], f"ddsp_{name}_pretrained.ts"),
    )

    impulse = ddsp.reverb.build_impulse().reshape(-1).numpy()
    sf.write(
        path.join(config["train"]["model_name"], config["train"]["model_name"], f"ddsp_{name}_impulse.wav"),
        impulse,
        config["data"]["sampling_rate"],
    )

    with open(
            path.join(config["train"]["model_name"], config["train"]["model_name"], f"ddsp_{name}_config.yaml"),
            "w",
    ) as config_out:
