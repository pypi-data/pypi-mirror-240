#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""Proxmox Backup RDX Datastore module
"""
...
class Datastore():
    ...
    def maintenance_mode(self, datastore_maintenance_mode):
        """Set the maintenance mode on the datastore

        :param datastore_maintenance_mode: Set the datastore mode. 
                                           Accept only to value : "online", "offline"
        :type datastore_maintenance_mode: str
        """
        if datastore_maintenance_mode in ['online', 'offline']:
            self._maintenance_mode = datastore_maintenance_mode
        else:
    ...
    def enable(self):
        """Enable datastore
        """
        if self._maintenance_mode() == 'offline':
            subprocess.run(['proxmox-backup-manager',
                            'datastore',
                            'update',
                            self.name,
    ...
    def disable(self):
        """Disable datastore
        """
        if self._maintenance_mode() == 'online':
            subprocess.run(['proxmox-backup-manager',
                            'datastore',
                            'update',
                            self.name,
    ...
    def eject(self, period):
        ...
        if period == 'daily':
            ...
            if date.strftime("%d-%m-Y") == utils.last_sunday_of_month(date).strftime("%d-%m-Y"):
                status = 'RDX tape ejected'
            else:
                status = 'RDX tape not ejected'
    ...
    def get(datastore_name):
        ...
        # json_result = json.loads('[{"maintenance-mode":"offline", \
        #                             "name":"RDX", \
        #                             "notify-user":"gitsadmin@pam", \
        #                             "path":"/mnt/rdx"}, \
        #                            {"comment":"", \
        #                             "gc-schedule":"14:00", \
        #                             "keep-daily":7, \
        ...
        #                             "name":"EspaceClefs", \
        #                             "notify-user":"gitsadmin@pam", \
        #                             "path":"/pbs/espaceclefs", \
        #                             "prune-schedule":"11:00"}]'
        #                         )
