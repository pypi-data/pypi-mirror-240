...
class AsyncFuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, tied_operands=None, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_674')) else
      _ods_ir.AttrBuilder.get('anonymous_674')(function_type, context=_ods_context))
    if tied_operands is not None: attributes["tied_operands"] = (tied_operands if (
        issubclass(type(tied_operands), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('Util_TiedOpStorageAttr')) else
          _ods_ir.AttrBuilder.get('Util_TiedOpStorageAttr')(tied_operands, context=_ods_context))
...
class ParameterLoadOp(_ods_ir.OpView):
  ...
  def __init__(self, result, result_timepoint, source_key, source_offset, result_size, *, source_scope=None, await_timepoint=None, affinity=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
...
def parameter_load(result, result_timepoint, source_key, source_offset, result_size, *, source_scope=None, await_timepoint=None, affinity=None, loc=None, ip=None) -> _Sequence[_SubClassValueT]:
  return _get_op_result_or_op_results(ParameterLoadOp(result=result, result_timepoint=result_timepoint, source_key=source_key, source_offset=source_offset, result_size=result_size, source_scope=source_scope, await_timepoint=await_timepoint, affinity=affinity, loc=loc, ip=ip))
...
class ResourceConstantsOp(_ods_ir.OpView):
  ...
  def __init__(self, results_, result_timepoint, values, result_sizes, *, affinity=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_640')) else
      _ods_ir.AttrBuilder.get('anonymous_640')(values, context=_ods_context))
    if affinity is not None: attributes["affinity"] = (affinity if (
        issubclass(type(affinity), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('Stream_AffinityAttr')) else
          _ods_ir.AttrBuilder.get('Stream_AffinityAttr')(affinity, context=_ods_context))
