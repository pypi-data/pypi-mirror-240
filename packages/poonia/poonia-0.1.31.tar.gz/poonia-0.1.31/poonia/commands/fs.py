...
def sub(regex, text, replacement, count=0, evaluate=False):
    def extract(func, iterable):
        ...
        while i < len(s):
            if s[i] == '$':
                if s[i+1:i+2].isnumeric():
                    n = ''.join(extract(lambda c: c.isnumeric(), s[i+1:]))
                    if buf:
                        out.append(''.join(buf))
                        buf.clear()
                    out.append(int(n))
    ...
    replace_group, parsed = parse(replacement)
    ...
    return re.sub(regex, capture, text, count=count, flags=re.MULTILINE)
...
def rename(paths, rename_dirs, regex, regex_eval):
    ...
    if regex or regex_eval:
        ...
        try:
            renamed = sub(match, original, replacement, evaluate=eval)
