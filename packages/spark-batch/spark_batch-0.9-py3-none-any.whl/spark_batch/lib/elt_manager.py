...
class EltManager:
    ...
    def ingest_full(self, source_objects, target_object, source_customSchema=None, target_customSchema=None) :    
        ...
        while True:
            ...
            if source_df is None :
                self.logger.error(f"ETL/FL Done : [ {targetInfo} / True (0, 0) / {datetime.datetime.now() - start_time} ]")
        ...
        self.logger.info(f"ETL/FL Done : [ {targetInfo} / {valid} ({offset}, {target_df.count()}) / {datetime.datetime.now() - start_time} ]")
    ...
    def ingest_increment(self, source_objects, target_object, source_inc_query, target_condition,  
        ...
        # 데이터가 없으면 종료
        if source_df is None :
            self.logger.error(f"ETL/IC Done : [ {targetInfo} / True (0, 0) / {datetime.datetime.now() - start_time} ]")
            return (None, None, True)
        ...
        self.logger.info(f"ETL/IC Done : [ {targetInfo} / {valid} ({source_read_size},{target_read_size}) / {datetime.datetime.now() - start_time} ]")
