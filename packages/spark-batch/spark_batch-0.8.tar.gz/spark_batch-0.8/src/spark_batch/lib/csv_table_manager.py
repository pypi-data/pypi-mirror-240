...
class CSVTableManager:
    def __init__(self, spark, s3_endpoint, s3_access_key, s3_secret_key, archive_bucket, archive_dpath, bucket, dpath):
        self.spark = spark
        ...
        self.s3_client = boto3.client('s3', endpoint_url=s3_endpoint, aws_access_key_id=s3_access_key, aws_secret_access_key=s3_secret_key)
    ...
    def loadTables(self, file_names, bucket=None, dpath=None, customSchema=None, header=True, infer_schema=True):
        file_names = parseSourceObject(file_names)

        for file_name in file_names:
            df = self.loadTable(file_name, header, infer_schema)
    ...
    def queryTable(self, query, table_names=None, dpath=None, customSchema=None, header=True, infer_schema=True):
        self.logger.debug(("queryTable = ", query, table_names))

        
        if table_names is not None:
            self.loadTables(table_names, header, infer_schema)
    ...
    def archive(self, file_name, bucket=None, dpath=None):
        ...
        try:
            ...
            self.logger.info(("Archived ", "s3a://{sBucket}/{sKey} > s3a://{archive_bucket}/{destination_key}"))
        except Exception as e:
