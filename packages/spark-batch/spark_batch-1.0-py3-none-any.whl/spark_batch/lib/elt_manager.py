...
class EltManager:
    ...
    def _getSourceInfo(self, source_objects) :
        return (
            ...
            else f"{self.source_type} {self.source_topic}/{source_objects}"
        )
    ...
    def _getTargetInfo(self, target_object) :
        return (
            ...
            else f"{self.target_type} {self.target_topic}/{target_object}"
        )
    
    # Single tables full load
    def ingest_full(self, source_objects, target_object, source_customSchema=None, target_customSchema=None, count=True) :    
        ...
        if count is True:
            size = self.source_tm.loadTable(sourceTable).count()
            self.logger.info(f"Source count = {size} / expected loop {math.ceil(size / self.chunk_size)}")
        ...
        while True:
            ...
            self.logger.info(f"Source Loading Chunk : {sourceInfo} / offset={offset}, self.chunk_size={chunk_read_size} / elipsed={timer.tab():.2f}")
        ...
        self.logger.info(f"ETL/FL Done : [ {targetInfo} / {valid} ({offset}, {target_df.count()}) / {timer.elapsed():.2f} ]")
    ...
    def ingest_increment(self, source_objects, target_object, source_inc_query, target_condition,  
        ...
        self.logger.info(f"ETL/IC Done : [ {targetInfo} / {valid} ({source_read_size},{target_read_size}) / {timer.elapsed():.2f} ]")
