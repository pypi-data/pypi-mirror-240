    ...
    def crea_base(self):
        try:
            ...
            with conn:
                ...
                conn.execute('''CREATE TABLE Informe ( id INTEGER PRIMARY KEY, fecha DATE DEFAULT CURRENT_DATE, punto_equilibrio REAL DEFAULT 0, total_compra_fiat REAL DEFAULT 0, total_venta_fiat REAL DEFAULT 0, tipo_fiat TEXT DEFAULT '', total_compra_cripto REAL DEFAULT 0, total_venta_cripto REAL DEFAULT 0, tipo_cripto TEXT DEFAULT '', open BOOLEAN DEFAULT(TRUE) );''')
                ...
                conn.execute('''CREATE TRIGGER crea_comprobante AFTER INSERT ON Transacciones BEGIN INSERT INTO Comprobante (path) VALUES (''); UPDATE Transacciones SET id_comprobante = (SELECT last_insert_rowid() FROM Comprobante) WHERE id = new.id; END;''')
