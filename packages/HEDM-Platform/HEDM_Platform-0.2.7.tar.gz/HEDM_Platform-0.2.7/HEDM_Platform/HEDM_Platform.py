...
class HEDM_Platform:
    ...
    #         # Determine the slice to read (if any empty images should be removed)
    #         slice_to_read = slice(self.empty_images, None) if self.empty_images > 0 else slice(None)

    #         # Open the output HDF5 file
    #         with h5py.File(self.output_file, 'w') as output_file:
    #             # Create a new dataset with the same shape (minus any empty images) and dtype as the input dataset
    #             # Adjust the shape if empty images are to be removed
    #             output_shape = (input_dataset.shape[0] - self.empty_images,) + input_dataset.shape[1:] if self.empty_images > 0 else input_dataset.shape
                
    #             # Store it under the '/images' path in the output file
    #             output_dataset = output_file.create_dataset('/images', shape=output_shape, dtype=input_dataset.dtype)

    #             # Initialize a variable for the current slice start position
    #             current_slice_start = 0

    #             # Define the chunk size (number of images to process at a time)
    #             # This should be tuned based on the available memory and the size of the images
    #             chunk_size = 100  # example chunk size

    #             # Loop over the dataset in chunks
    #             for i in range(0, output_shape[0], chunk_size):
    #                 # Calculate the end position of the current slice
    #                 current_slice_end = min(i + chunk_size, output_shape[0])
    #                 # Read a chunk from the input dataset
    #                 data_chunk = input_dataset[slice_to_read][i:current_slice_end]
    #                 # Write the chunk to the output dataset
    #                 output_dataset[current_slice_start:current_slice_end] = data_chunk
    #                 # Update the slice start position for the next iteration
    ...
    def hdf5_to_hdf5(self):
        ...
        with h5py.File(self.input_file, 'r') as input_file:
            ...
            # Open the output HDF5 file
            with h5py.File(self.output_file, 'w') as output_file:
                # If we have empty frames to skip, we need to create a virtual dataset
                if self.empty_images > 0:
                    # Calculate the shape of the output dataset
                    num_frames = input_dataset.shape[0] - self.empty_images
                    output_shape = (num_frames,) + input_dataset.shape[1:]

                    # Create a virtual source from the input dataset
                    virtual_source = h5py.VirtualSource(input_dataset)

                    # Apply the selection to the virtual source
                    virtual_source = virtual_source[self.empty_images:]

                    # Create a virtual layout to map the virtual source to the output without the empty frames
                    layout = h5py.VirtualLayout(shape=output_shape, dtype=input_dataset.dtype)

                    # Map the non-empty frames to the virtual layout
                    layout[:] = virtual_source

                    # Create the virtual dataset in the output file
                    output_dataset = output_file.create_virtual_dataset('/images', layout, fillvalue=0)
                else:
                    # If there are no empty frames, we can just copy the dataset
                    input_file.copy(input_dataset, output_file, '/images')
    ...
    def integrate_images_in_hdf5(self, input_hdf5_path, output_hdf5_path, integration_factor=10):
        ...
        with h5py.File(input_hdf5_path, 'r') as input_file:
            # Assuming the dataset name is 'images'
            ...
            if num_images % integration_factor != 0:
                num_output_images += 1  # Account for the last incomplete batch
            
            # Open the output HDF5 file
            with h5py.File(output_hdf5_path, 'w') as output_file:
                # Calculate the output dataset dimensions
                out_dim = (num_output_images,) + input_dataset.shape[1:]
                
                # Create the output dataset
                output_dataset = output_file.create_dataset('/images', out_dim, dtype=np.int32)
                ...
                for i in range(num_images):
                    # Accumulate image intensities
                    accumulated_image += input_dataset[i].astype(np.int32)
                    count += 1
                    
                    # Check if it's time to save the accumulated image
                    if count == integration_factor or (i == num_images - 1 and count != 0):
                        # Save the accumulated image to the output dataset
                        output_dataset[i // integration_factor] = accumulated_image
                        # Reset the accumulation buffer
                        accumulated_image.fill(0)
                        count = 0
