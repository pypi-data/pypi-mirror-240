...
def session_data_dir(tmp_path_factory):
    yield tmp_path_factory.mktemp("session_data")
...
@pytest.fixture(scope="session")
def tmp_env_vars(
    session_out_dir, session_data_dir, redis_password, flowdb_test_creds, test_jwt_token
):
    out = dict()
    out["HOST_PROJECT_DIR"] = str(Path(__file__).parent)
    out["HOST_NOTEBOOK_OUT_DIR"] = str(session_out_dir)
    out["HOST_DATA_DIR"] = str(session_data_dir)
