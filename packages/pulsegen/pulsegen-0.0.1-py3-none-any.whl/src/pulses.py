...
def pi_gate(w,cp,ctx) : 
  w["drag_scale"] = cp["drag_scale"] ; 
  w["drag_detuning"] += cp["drag_detuning"] ;  
  w["vz"] += cp["pi_phase_offs"] ; 
  w["amp"]= cp["piamp"]; 

def pi2gate(w,cp,ctx) : 
  w["drag_scale"] += cp["drag2scale"] ;  
  w["drag_detuning"] += cp["drag_detuning"] ;  
  w["amp"]= cp["piamp"]*cp["pi2ratio"]/2 ; 
...
class F_p(I_p):
  __mnr_type__="F_p"; 
  def will(self,w,cp,ctx): w["amp"]= cp["piamp"];  w["freq"] = 0 ; 
