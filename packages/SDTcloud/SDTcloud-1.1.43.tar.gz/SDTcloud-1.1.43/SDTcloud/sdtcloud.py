...
class SDTcloud():
    ...
    def getDataFromTimeScaleDB(self, database, table, query_condition):
        ...
        if isinstance(query_condition, Dict):
            raise AttributeError('query_condition parameter type is dictionary')
        ...
        try:
            ...
            query_result = cur.execute(text('SELECT {0} FROM {1}.public.{2} {3} LIMIT {4};'.format(field_query, database, table, range_query if range_query else '', limit_count if limit_count else 1000)))
            results = []
