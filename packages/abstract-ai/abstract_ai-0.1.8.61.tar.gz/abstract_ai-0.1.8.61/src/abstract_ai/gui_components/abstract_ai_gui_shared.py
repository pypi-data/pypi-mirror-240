...
multi_line_slider = MultilineSlider()
# Get screen size
# Get screen size
screen_width, screen_height = make_component("Window").get_screen_size()
height=0.70
width=0.80
max_size=None
...
def get_right_click(key=None):
    return multi_line_slider.get_right_click(key=key)
    
def get_response():
    output_layout = make_component("Multiline",right_click_menu=get_right_click(text_to_key('response')),key=text_to_key('response'),**expandable(size=(None, 5)))
...
def get_feedback():
    ...
    for i,title in enumerate(["request_chunks",'abort','additional_responses','suggestions', 'notation','other']):
        if title in ["request_chunks",'abort','additional_responses']:
            ...
            component = make_component("Multiline",right_click_menu=get_right_click(text_to_key(text=title,section='feedback')),key=text_to_key(text=title,section='feedback'),**expandable(size=(None, 5)))
...
def get_urls():
    return [
        ...
        [make_component("Multiline",right_click_menu=get_right_click(text_to_key('url text')),key=text_to_key('url text'),**expandable(size=(None, 5)))],
    ]
def abstract_browser_layout(section=None,extra_component=None):
    ...
    return AbstractBrowser().get_scan_browser_layout(section=section,extra_buttons=extra_buttons)+[[make_component("Multiline",right_click_menu=get_right_click(text_to_key('file text',section=section)),key=text_to_key('file text',section=section),**expandable(size=(None, 5)))]]
