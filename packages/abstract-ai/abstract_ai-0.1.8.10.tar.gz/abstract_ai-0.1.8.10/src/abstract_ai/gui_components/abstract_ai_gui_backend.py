...
from .abstract_ai_gui import get_total_layout
from .abstract_ai_gui_shared import instructions_keys, all_token_keys, test_options_keys
...
def get_prompt_section_number(event_type,prompt_number,prompt_data):
    if prompt_number>len(prompt_data)-1:
        prompt_number = len(prompt_data)-1
    if event_type == 'back':
        if prompt_number>0:
...
def get_prompt_section_sub_number(event_type,prompt_number,prompt_data):
    if prompt_data:
        if event_type == 'back':
            if prompt_number>0:
                prompt_number = prompt_number - 1
...
#PromptManagement
    ...
    def update_prompt_mgr(self,prompt_data_list=None,chunk_token_distributions=None,completion_percentage=None,notation=None,chunk_number=None,chunk_type="CODE"):
        """
        initializes the prompt manager with the information needed for querying the GPT AI model.
	"""
        ...
        self.prompt_mgr = PromptManager(instruction_mgr=self.instruction_mgr,
                                   ...
                                   prompt_data=prompt_data_list or self.prompt_data_list,
                                   request=self.request_data_list,
                                   notation=self.notation,
                                   chunk_number=chunk_number or self.chunk_number,
        ...
        if len(self.prompt_data_list) == 0:
            self.chunk_text_number_display=0
        else:
            self.chunk_text_number_display = 1
        self.window_mgr.update_value(key=text_to_key("chunk number"),value=0)
        self.window_mgr.update_value(key=text_to_key("chunk section number"),value=0)
        #self.window_mgr.update_value(key='-CHUNK_TEXT_NUMBER-',value=self.chunk_text_number_display)
        self.update_chunk_info(0,0)
    ...
    def update_prompt_data(data):
        self.history_mgr.add_to_history(name=self.chunk_history_name,data=self.prompt_data_list)
        self.prompt_data_list[self.get_prompt_data_section_number()] = data
        self.window_mgr.update_value(key='-PROMPT_DATA_DATA-',value=data)
    ...
    def update_request_data(data):
        ...
        self.request_data_list[self.get_prompt_data_section_number()] = data
        self.window_mgr.update_value(key='-REQUEST-',value=data)
        return self.request_data_list
    
    def update_chunk_info(self,chunk_iteration=0,chunk_number=0):
        if self.chunk_token_distributions:
            if chunk_iteration < len(self.chunk_token_distributions) and chunk_iteration >=0:
                self.chunk_token_distribution = self.chunk_token_distributions[int(chunk_iteration)][int(chunk_number)]
    ...
    def adjust_chunk_display(self,num):
        """
        manages the GUI's chunk navigation buttons 'Back' and 'Forward'.
        It ensures that the navigation is within the limits of the total chunks and updates the GUI with the correct chunk.
 	"""
    ...
    def get_chunk_display_numbers(self):
        """
        This helper method retrieves the number of chunks that need to be displayed on the GUI.
 	"""
    ...
    def determine_chunk_display(self,event):
        """
        determines the chunk to be displayed based on the chunk navigation buttons 'Back' or 'Forward'.
 	"""
    ...
    def add_to_chunk(self,content):
 	...
 	"""
        ...
        if self.prompt_data_list[self.get_prompt_data_section_number()]:
            content = self.prompt_data_list[self.get_prompt_data_section_number()]+'\n\n'+content
    ...
    def chunk_event_check(self):
 	...
 	"""
        if self.event in ['-CHUNK_TEXT_BACK-','-CHUNK_TEXT_FORWARD-']:
            ...
            self.update_prompt_mgr(prompt_data_list=self.update_prompt_data(data),completion_percentage=completion_percentage)
        else:
            return False
        return True
...
#InstructionManagement
    ...
    def update_instruction_mgr(self):
 	...
 	"""
        ...
        self.update_bool_text_instructions()
    def update_bool_text_instructions(self):
        for key in self.instruction_pre_keys:
...
#ResponseManagement
	...
	"""
        ...
        if done == True:
            ...
            self.window_mgr.update_value(key='-QUERY_COUNT-', value=f"chunk {i_query+1} of {max(chunk,1)} being sent")
    ...
    def listbox_event_check(self):
        ...
        if file_path and os.path.isfile(file_path):
            try:
                contents = read_file_contents(file_path)  # Assuming read_file_contents is defined to handle different file types
...
#WindowManagement
    ...
    def perform_event_checks(self):
        ...
        if self.event == text_to_key('prompt_data data'):
            self.prompt_data_list[prompt_number] = self.window_mgr.get_values()[text_to_key('prompt_data data')]
            
        if self.event == text_to_key('request'):
            self.request_data_list[prompt_number] = self.window_mgr.get_values()[text_to_key('request')]
            
        elif self.event == '-ADD_QUERY-':
            self.fill_lists()
            
        elif self.event in [text_to_key('instructions section back'),
            ...
            prompt_number=get_prompt_section_number(self.event.split('_')[-1][:-1].lower(),prompt_number,self.prompt_data_list)
            for header in ['instructions','request','prompt_data']:
                self.window_mgr.update_value(text_to_key(f"{header} section number"),prompt_number)
            self.update_bool_text_instructions()
            self.window_mgr.update_value(text_to_key('prompt_data data'),self.prompt_data_list[prompt_number])
            self.window_mgr.update_value(text_to_key('request'),self.request_data_list[prompt_number])
            
        elif self.event in [text_to_key("chunk section forward"),
            ...
            chunk_token_distribution_number = self.get_chunk_token_distribution_number()
            chunk_number = 0
            if "SECTION" in self.event:
                chunk_token_distribution_number=get_prompt_section_number(self.event.split('_')[-1][:-1].lower(),chunk_token_distribution_number,self.chunk_token_distributions)
                self.window_mgr.update_value(text_to_key("chunk section number"),chunk_token_distribution_number)
                self.window_mgr.update_value(text_to_key("query section number"),chunk_token_distribution_number)
                
            else:
                chunk_number = self.get_chunk_number()
                chunk_number= get_prompt_section_number(self.event.split('_')[-1][:-1].lower(),chunk_number,self.chunk_token_distributions[chunk_token_distribution_number])            
                self.window_mgr.update_value(text_to_key("chunk number"),chunk_number)
                self.window_mgr.update_value(text_to_key("query number"),chunk_number)
            self.window_mgr.update_value(text_to_key('chunk sectioned data'),self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]['chunk']['data'])
            self.window_mgr.update_value(key='-QUERY-',value=self.prompt_mgr.create_prompt(chunk_token_distribution_number=chunk_token_distribution_number,chunk_number=chunk_number))
