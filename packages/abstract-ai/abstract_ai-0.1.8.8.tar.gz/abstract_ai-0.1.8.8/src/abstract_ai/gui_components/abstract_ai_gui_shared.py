...
def get_response():
    output_layout = make_component("Multiline",key=text_to_key('response'),**expandable(size=(None, 5)))
...
def utilities():
    ...
    layout.append(make_component("Tab",'RESPONSES', abstract_browser_layout(section='responses',extra_component = make_component("Frame","responses",ensure_nested_list(collate_responses))),key=text_to_key(text='response tab'),**expandable(size=(50, 100)))),
...
def get_urls():
    return [
        ...
        [make_component("Multiline",key=text_to_key('url text'),**expandable(size=(None, 5)))],
    ]
def abstract_browser_layout(section=None,extra_component=[]):
    extra_buttons = [make_component("Button",'CHUNK_DATA',key=text_to_key(text='add file to chunk',section=section),enable_events=True),
                     make_component("Frame",'chunk title',layout=[[make_component("Input",key=text_to_key(text='chunk title',section=section),size=(20,1)),extra_component]])]
    return AbstractBrowser().get_scan_browser_layout(section=section,extra_buttons=extra_buttons)+[[make_component("Multiline",key=text_to_key('file text',section=section),**expandable(size=(None, 5)))]]
