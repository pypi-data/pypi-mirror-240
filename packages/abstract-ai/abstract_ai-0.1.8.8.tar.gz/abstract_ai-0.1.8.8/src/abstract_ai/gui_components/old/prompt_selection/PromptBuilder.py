...
class PromptManager:
    ...
    def create_prompt_guide(self,
        ...
        def get_chunk_header(chunk,total_chunks,data_chunk):
            ...
            if total_chunks in [0,'0']:
                ...
                chunk_display = int(chunk)+1
            return f'this is chunk {chunk_display} of {total_chunks}'+'\n\n'+f'{data_chunk}'
        chunk=self.chunk
        ...
        prompt_as_previous=get_for_prompt('previouisly generated prompt',prompt_as_previous)
        values_ls = [instructions,request,
                     bot_notation,
                     generated_title,
                     request_chunks,
                     chunks_prompt,
        ...
        for value in values_ls:
            if value:
                prompt_data_source+=f"{get_delimeters()}{value}"
    ...
    def create_prompt(self,dist_number=None,bot_notation=None,generate_title=None,chunk_descriptions=None,request_chunks=None,prompt_as_previous=None,token_adjustment=None):
        ...
        if prompt_as_previous:
            ...
            self.prompt_as_previous = chunk_data_by_type(prompt_as_previous, available,chunk_type=None,reverse=reverse)
        ...
        self.prompt =self.create_prompt_guide(token_dist_prompt=token_dist_prompt,bot_notation=bot_notation,generate_title=generate_title,request_chunks=request_chunks,prompt_as_previous=self.prompt_as_previous)
