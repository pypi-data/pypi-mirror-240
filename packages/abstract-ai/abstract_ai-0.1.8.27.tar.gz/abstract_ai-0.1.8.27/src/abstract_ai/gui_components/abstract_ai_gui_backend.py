...
from .abstract_gui_navigation import AbstractNavigationManager
...
class GptManager:
    ...
    def __init__(self):
 	...
 	"""
        ...
        self.navigation_mgr= AbstractNavigationManager(selfs=self,window_mgr=self.window_mgr)
    ...
    def initialize_keys(self):
 	...
 	"""
        ...
        self.sub_navigation_keys = [text_to_key("chunk back"),
        ...
        self.section_navigation_keys = [text_to_key('instructions section back'),
        ...
        self.section_navigation_keys = self.section_navigation_keys+self.sub_navigation_keys
        ...
        self.chunk_display_keys=self.get_bool_and_text_keys(all_token_keys)
...
#PromptManagement
    ...
    def update_prompt_mgr(self,prompt_data_list=None,request_data_list=None,chunk_token_distributions=None,completion_percentage=None,notation=None,chunk_number=None,chunk_type="CODE"):
        ...
        initializes the prompt manager with the information needed for querying the GPT AI model.
	"""
        for i in range(len(self.request_data_list)):
            if self.prompt_data_list[-1] == '' and self.request_data_list[-1] == '':
                self.prompt_data_list[:-1]
                self.request_data_list[:-1]
            else:
                break
        ...
        distributions = self.prompt_mgr.chunk_token_distributions
        if distributions:
            if len(distributions)-1<=chunk_token_distribution_number:
                chunk_token_distribution_number =0
            chunk_distribution = distributions[chunk_token_distribution_number]
            if chunk_distribution:
                if len(chunk_distribution)-1 <= chunk_number:
                    chunk_number = 0
    ...
    def update_chunk_info(self,chunk_token_distribution_number=0,chunk_number=0):
        if self.prompt_mgr.chunk_token_distributions:
            chunk_token_distribution_number=int(chunk_token_distribution_number)
    ...
    def chunk_event_check(self):
 	...
 	"""
        ...
        if self.event in self.section_navigation_keys:
...
#ResponseManagement
    ...
    def listbox_event_check(self):
        ...
        if self.script_event_js['found'] == '-FILES_LIST-':
            selected_files = self.values[self.script_event_js['-FILES_LIST-']]
            if selected_files:
                file_path = selected_files[0]
                event_triggered = True
        elif self.script_event_js['found'] == '-BROWSER_LIST-':
            directory = self.values[self.script_event_js['-DIR-']]
            selected_files = self.values[self.script_event_js['-BROWSER_LIST-']]
            if selected_files:
