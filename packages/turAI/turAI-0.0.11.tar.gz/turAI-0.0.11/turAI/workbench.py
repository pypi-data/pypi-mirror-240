...
from turAI.common import Record, UserError, normpath_with_actual_case
from turAI.config import try_load_configuration
from turAI.config_ui import ConfigurationDialog
from turAI.custom_notebook import CustomNotebookPage
from turAI.editors import EditorNotebook, is_local_path
from turAI.languages import tr
from turAI.misc_utils import (
    copy_to_clipboard,
    get_menu_char,
    running_on_linux,
    running_on_mac_os,
...
from turAI.running import BackendProxy, Runner
from turAI.shell import ShellView
from turAI.ui_utils import (
    AutomaticNotebook,
...
OBSOLETE_PLUGINS = [
    "turAIcontrib.pi",
    "turAIcontrib.micropython",
    "turAIcontrib.circuitpython",
    "turAIcontrib.microbit",
    "turAIcontrib.esp",
    "turAIcontrib.rpi_pico",
]


class Workbench(tk.Tk):
    """
    turAI's main window and communication hub.
    ...
    def __init__(self) -> None:
        ...
        tk.Tk.__init__(self, className="turAI")
        ...
        self._active_ui_mode = os.environ.get("turAI_MODE", self.get_option("general.ui_mode"))
    ...
    def _make_sanity_checks(self):
        ...
        if bad_home_msg:
            messagebox.showwarning(
                ...
                bad_home_msg + "\nThis may cause problems for turAI.",
                master=self,
            )
    ...
    def _init_configuration(self) -> None:
        self._configuration_manager = try_load_configuration(turAI.CONFIGURATION_FILE)
        self._configuration_pages = []  # type: List[Tuple[str, str, Type[tk.Widget], int]]

        self.set_default("general.single_instance", turAI.SINGLE_INSTANCE_DEFAULT)
    ...
    def update_debug_mode(self):
        os.environ["turAI_DEBUG"] = str(self.get_option("general.debug_mode", False))
        turAI.set_logging_level()
    ...
    def _init_window(self) -> None:
        ...
        # even if they were True when turAI was last run
    ...
    def _init_icon(self) -> None:
        ...
        if running_on_linux() and ui_utils.get_tk_version_info() >= (8, 6):
            self.iconphoto(True, self.get_image("turAI.png"))
        else:
            icon_file = os.path.join(self.get_package_dir(), "res", "turAI.ico")
            try:
    ...
    def _load_plugins(self) -> None:
        # built-in plugins
        import turAI.plugins  # pylint: disable=redefined-outer-name

        self._load_plugins_from_path(turAI.plugins.__path__, "turAI.plugins.")  # type: ignore

        # 3rd party plugins from namespace package
        # Now it's time to add plugins dir to sys path
        sys.path.append(turAI.get_sys_path_directory_containg_plugins())
        try:
            import turAIcontrib  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(turAIcontrib.__path__, "turAIcontrib.")
    ...
    def _create_server_socket(self):
        if running_on_windows():
            ...
            with open(turAI.get_ipc_file_path(), "w") as fp:
                fp.write(str(port) + "\n")
                fp.write(secret + "\n")

        else:
            server_socket = socket.socket(socket.AF_UNIX)  # @UndefinedVariable
            server_socket.bind(turAI.get_ipc_file_path())
            secret = ""

        os.chmod(turAI.get_ipc_file_path(), 0o600)
    ...
    def _support_ukraine(self, event=None) -> None:
        webbrowser.open("https://github.com/turAI/turAI/wiki/Support-Ukraine")
    ...
    def _post_backend_menu(self):
        from turAI.plugins.micropython.uf2dialog import (
            show_uf2_installer,
        ...
        if self.winfo_screenwidth() / self.winfo_screenheight() > 2:
            ...
            # Tk will adjust x properly with single monitor, but when turAI is maximized
    ...
    def _publish_command(
        ...
        if sequence:
            if not skip_sequence_binding:
                ...
                # https://github.com/turAI/turAI/issues/1347
        ...
        def on_click(event):
            ...
            tk.messagebox.showinfo(
                ...
                tr(
                    ...
                    + "Restart turAI to start working in regular mode.\n\n"
    ...
    def update_image_mapping(self, mapping: Dict[str, str]) -> None:
        """Was used by turAI-pi. Not recommended anymore"""
    ...
    def get_package_dir(self):
        """Returns turAI package directory"""
        return os.path.dirname(sys.modules["turAI"].__file__)
    ...
    def in_debug_mode(self) -> bool:
        return os.environ.get("turAI_DEBUG", False) in [
            "1",
            1,
            "True",
            True,
        ...
        if self._toolbar.winfo_ismapped():
            for group_frame in self._toolbar.grid_slaves(0):
                for button in group_frame.pack_slaves():
                    if turAI._runner is None or button.tester and not button.tester():
    ...
    def _guard_font_size(self, size: int) -> int:
        # https://bitbucket.org/plas/turAI/issues/164/negative-font-size-crashes-turAI
        MIN_SIZE = 4
        MAX_SIZE = 200
        if size < MIN_SIZE:
            return MIN_SIZE
    ...
    def destroy(self) -> None:
        try:
            ...
            if self._is_server() and os.path.exists(turAI.get_ipc_file_path()):
                os.remove(turAI.get_ipc_file_path())
    ...
    def open_url(self, url):
        m = re.match(r"^turAI-editor://(.*?)(#(\d+)(:(\d+))?)?$", url)
        if m is not None:
    ...
    def get_temp_dir(self, create_if_doesnt_exist=True):
        path = os.path.join(turAI_USER_DIR, "temp")
    ...
    def show_notebook_drop_targets(self):
        from turAI.custom_notebook import NotebookTabDropTarget
