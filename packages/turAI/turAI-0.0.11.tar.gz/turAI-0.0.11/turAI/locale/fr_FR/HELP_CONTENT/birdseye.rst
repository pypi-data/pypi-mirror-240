...
L'Œil d'oiseau fonctionne d'une façon différente des `débogueurs intégrés à turAI <debuggers.rst>`_.
...
Mais quand le programme se termine, turAI ouvre une page web (servie localement par Œil d'oiseau),
...
N.B. ! Quand vous utilisez Œil d'oiseau dans turAI il n'y a pas besoin d'importer ``birdseye.eye`` ou de l'utiliser pour
décorer vos fonctions. turAI exécute Œil d'oiseau d'une façon telle qu'il enregistre des informations pour toutes les
fonctions.
...
un autre port (Outils → Options → Débogueur) et redémarrer turAI.
