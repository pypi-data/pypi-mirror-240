...
if __name__ == "__main__":
    ...
    # make sure turAI folder is in sys.path during startup
    ...
    if turAI_container not in sys.path:
        # We're running with non-turAI interpreter or env
    ...
    if sys.platform == "darwin":
        try:
            ...
            print(
                "\nNB! Potential problems detected, see\nhttps://github.com/turAI/turAI/wiki/MacOSX#catalina\n",
                file=sys.stderr,
            )

    if not sys.version_info > (3, 8):
        print(
            "This version of turAI only supports Python 3.8 and later.\n"
    ...
    from turAI.plugins.cpython_backend.cp_back import MainCPythonBackend

    turAI.prepare_turAI_user_dir()
    turAI.configure_backend_logging()
    print(PROCESS_ACK)

    if using_temp_augmented_sys_path:
        # Don't make turAI container available for user programs.
        # NB! Need to do it before constructing the backend as it would clean the main scope and the flag would be gone.
        from logging import getLogger

        assert sys.path[0] == turAI_container
        getLogger(__name__).info("Removing temporary %r from sys.path", turAI_container)
        del sys.path[0]
