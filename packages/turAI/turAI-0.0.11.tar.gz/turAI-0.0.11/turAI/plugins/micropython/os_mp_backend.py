...
from turAI.backend import SshMixin
from turAI.common import PROCESS_ACK, BackendEvent, serialize_message
from turAI.plugins.micropython.bare_metal_backend import LF, NORMAL_PROMPT
...
from turAI.plugins.micropython.mp_common import PASTE_SUBMIT_MODE

# Can't use __name__, because it will be "__main__"
logger = getLogger("turAI.plugins.micropython.os_mp_backend")
...
class UnixMicroPythonBackend(MicroPythonBackend, ABC):
    ...
    def _cmd_execute_system_command(self, cmd):
        ...
        self._execute("__turAI_helper.os.system(%r) or None" % cmd_line)
        # TODO: report returncode

    def _cmd_get_fs_info(self, cmd):
        script = """__turAI_helper.os.system("stat -f -c '%b %f %a %S' {path}") or None""".format(
            path=cmd.path
        )
    ...
    def _sync_time(self):
        self._show_error("WARNING: Automatic time synchronization by turAI is not supported.")

    def _get_utc_timetuple_from_device(self) -> Union[tuple, str]:
        out, err = self._execute("__turAI_helper.os.system('date -u +%s')", capture_output=True)
        if err:
            return err
...
class LocalUnixMicroPythonBackend(UnixMicroPythonBackend):
    def _create_connection(self, run_args=[]):
        from turAI.plugins.micropython.subprocess_connection import SubprocessConnection
...
class SshUnixMicroPythonBackend(UnixMicroPythonBackend, SshMixin):
    ...
    def _create_connection(self, run_args=[]):
        ...
        from turAI.plugins.micropython.ssh_connection import SshProcessConnection
