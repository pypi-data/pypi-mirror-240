...
import turAI
from turAI import report_time
from turAI.backend import UploadDownloadMixin, convert_newlines_if_has_shebang
...
from turAI.misc_utils import find_volumes_by_name
from turAI.plugins.micropython.connection import MicroPythonConnection
from turAI.plugins.micropython.mp_back import (
    EOT,
    NORMAL_MODE_CMD,
    PASTE_MODE_CMD,
...
from turAI.plugins.micropython.mp_common import (
    PASTE_SUBMIT_MODE,
    RAW_PASTE_SUBMIT_MODE,
    RAW_SUBMIT_MODE,
)
from turAI.plugins.micropython.webrepl_connection import WebReplConnection
...
logger = getLogger("turAI.plugins.micropython.bare_metal_backend")
...
class BareMetalMicroPythonBackend(MicroPythonBackend, UploadDownloadMixin):
    ...
    def _check_prepare(self):
        out, err = self._execute(
            "__turAI_helper.builtins.print('__turAI_helper' in __turAI_helper.builtins.dir())",
            capture_output=True,
        )
        if out.strip() == "True":
            return
    ...
    def _get_helper_code(self):
        ...
        if self._read_block_size and self._read_block_size > 0:
            ...
            from turAI.plugins.micropython.serial_connection import OUTPUT_ENQ
            ...
            result += indent(
                ...
                def controlled_print(cls, *args, sep=" ", end="\\n"):
                    for arg_i, arg in enumerate(args):
                        ...
                        data = __turAI_helper.builtins.str(arg)
                        
                        for i in __turAI_helper.builtins.range(0, __turAI_helper.builtins.len(data), cls._print_block_size):
    ...
    def _infer_write_block_size(self):
        ...
        # https://github.com/turAI/turAI/issues/2143
    ...
    def _fetch_builtin_modules(self):
        script = "__turAI_helper.builtins.help('modules')"
    ...
    def _sync_time(self):
        ...
        if self._using_microbit_micropython():
            ...
            specific_script = dedent(
                """
                from rtc import RTC as __turAI_RTC
                __turAI_RTC().datetime = {ts}
                del __turAI_RTC
            """
            ).format(ts=tuple(now))
        else:
            ...
            specific_script = dedent(
                """
                from machine import RTC as __turAI_RTC
                try:
                    __turAI_RTC().datetime({datetime_ts})
                except:
                    __turAI_RTC().init({init_ts})
                finally:
                    del __turAI_RTC
        ...
        script = (
            ...
            try:
            %s
                __turAI_helper.print_mgmt_value(True)
    ...
    def _get_utc_timetuple_from_device(self) -> Union[tuple, str]:
        if self._using_microbit_micropython():
            ...
            specific_script = dedent(
                """
                from rtc import RTC as __turAI_RTC
                __turAI_helper.print_mgmt_value(__turAI_helper.builtins.tuple(__turAI_RTC().datetime)[:6])
                del __turAI_RTC
                """
            )
        else:
            specific_script = dedent(
                ...
                try:
                    ...
                    __turAI_temp = __turAI_helper.builtins.tuple(__turAI_RTC().datetime())
                    # remove weekday from index 3
                    __turAI_helper.print_mgmt_value(__turAI_temp[0:3] + __turAI_temp[4:7])
                    del __turAI_temp
                except:
                    __turAI_helper.print_mgmt_value(__turAI_helper.builtins.tuple(__turAI_RTC().now())[:6])
                del __turAI_RTC
                """
            )
        ...
        script = dedent(
            ...
            try:
                try:
                    ...
                    __turAI_helper.print_mgmt_value(__turAI_helper.builtins.tuple(__turAI_localtime()))
                    del __turAI_localtime
                except:
                    # some CP boards
                    from rtc import RTC as __turAI_RTC
                    __turAI_helper.print_mgmt_value(__turAI_helper.builtins.tuple(__turAI_RTC().datetime))
                    del __turAI_RTC
            except __turAI_helper.builtins.Exception as e:
                __turAI_helper.print_mgmt_value(__turAI_helper.builtins.str(e))
        """
    ...
    def _clear_repl(self):
        """NB! assumes prompt and may be called without __turAI_helper"""
    ...
    def _submit_code_via_raw_paste_mode(self, script_bytes: bytes) -> None:
        ...
        # https://github.com/turAI/turAI/issues/1545
    ...
    def _process_output_until_active_prompt(
        ...
        Note that this Ctrl+A may have been issued even before turAI connected to
        the device.
        ...
        output or the starting EOT from turAI expression (or, in principle, even
        the second raw-REPL EOT or terminating turAI expression EOT)
        ...
        if cmd.get("source"):
            ...
            if cmd.get("populate_argv", False):
                ...
                argv_updater = textwrap.dedent(
                    f"""
                try:
                    import sys as _turAI_sys
                    _turAI_sys.argv[:] = {argv}
                    del __turAI_sys
                except:
                    pass
                """
    ...
    def _cmd_get_fs_info(self, cmd):
        if self._using_microbit_micropython():
            used = self._evaluate(
                dedent(
                    ...
                    __turAI_helper.print_mgmt_value(
                        __turAI_helper.builtins.sum([__turAI_helper.os.size(name) for name in __turAI_helper.os.listdir()])
                    )  
                    """
                )
            )
            ...
            result = self._evaluate(
                dedent(
                    """
                __turAI_stat = __turAI_helper.os.statvfs(%r)
                __turAI_total = __turAI_stat[2] * __turAI_stat[0]
                __turAI_free = __turAI_stat[3] * __turAI_stat[0]

                __turAI_helper.print_mgmt_value({
                    "total" : __turAI_total,
                    "used" : __turAI_total - __turAI_free,
                    "free": __turAI_free,
                })  
        ...
        if not self._supports_directories():
            ...
            self._execute_without_output(
                ...
                for __turAI_path in %r: 
                    __turAI_helper.os.remove(__turAI_path)
    ...
    def _read_file_via_serial(
        ...
        self._execute_without_output(
            "__turAI_fp = __turAI_helper.builtins.open(%r, 'rb')" % source_path
        )
        if hex_mode:
        ...
        while True:
            ...
            if hex_mode:
                block = binascii.unhexlify(
                    self._evaluate("__temp_hexlify(__turAI_fp.read(%s))" % block_size)
                )
            else:
                block = self._evaluate("__turAI_fp.read(%s)" % block_size)
    ...
    def _write_file_via_serial(
        ...
        out, err = self._execute(
            dedent(
                ...
                __turAI_path = '{path}'
                __turAI_written = 0
                __turAI_fp = __turAI_helper.builtins.open(__turAI_path, 'wb')
            """
        ...
        if hex_mode:
            self._execute_without_output(
                dedent(
                    """
                from binascii import unhexlify as __turAI_unhex
                def __W(x):
                    global __turAI_written
                    __turAI_written += __turAI_fp.write(__turAI_unhex(x))
                    __turAI_fp.flush()
                    if __turAI_helper.builtins.hasattr(__turAI_helper.os, "sync"):
                        __turAI_helper.os.sync()
            """
                )
            )
            ...
            self._execute_without_output(
                ...
                def __W(x):
                    global __turAI_written
                    __turAI_written += __turAI_fp.write(x)
            """
                )
            )
        else:
        ...
        bytes_received = self._evaluate("__turAI_written")
    ...
    def _get_fs_mount_label(self):
        ...
        result = self._evaluate(
            dedent(
                """
            try:
                from storage import getmount as __turAI_getmount
                try:
                    __turAI_result = __turAI_getmount("/").label
                finally:
                    del __turAI_getmount
            except __turAI_helper.builtins.ImportError:
                __turAI_result = None 
...
def launch_bare_metal_backend(backend_class: Callable[..., BareMetalMicroPythonBackend]) -> None:
    ...
    try:
        if args["port"] is None:
            ...
            from turAI.plugins.micropython.serial_connection import (
