...
4.1.1 (2023-06-11)
==================
* Fix warning for the cases where turAI avoids user module shadowing a library module (#384)
...
* Enable turAI to open and save files with zenity 3.91+ (#2786) by @befeleme
...
* Ensure turAI's backends don't get confused by other installations, #2616
...
* Add turAI version as environment value for user programs, #2494 by @kr-g
...
* Update several deps in turAI + Python bundles (asttokens, mypy, pylint, esptool, paramiko)
...
* Enhance Linux installer script (the .bash-file) to create a venv and pip-install turAI if run on a platform for which there is no binary bundle available. This means you can now use Linux installer also on Raspberry Pi, #1736
...
* Use isolated mode for launching turAI. This includes hiding user-site packages from turAI's GUI process (not from user programs). Plug-ins now get installed under turAI's data folder, #2038, #1651
...
* Merge "The same interpreter which runs turAI" and "Alternative Python 3 interpreter or virtual environment" back-ends into "Local Python 3"
...
* Make sure turAI's dialogs are treated as dialogs in macOS and Linux, #1790
...
3.3.14 (2021-08-01)
===================
* Fix problems in org.turAI.turAI.appdata.xml
* Allow assigning Python coloring for Python-like files, #1927
* Increase the initial top coordinate of turAI's main window. Fixes problem of turAI's title bar being hidden by the system menubar, #1925

3.3.13 (2021-07-25)
===================
* Update org.turAI.turAI.appdata.xml
...
* Prepare turAI for Flatpak, by Jordan Williams, #1900 and #1912
...
* Update dependencies in turAI+Python bundles
...
* Reduce memory usage by turAI's MicroPython helper (store last REPL value in global _ instead of storing N last values in a list), #1797, #1798
...
* Don't show full error info when ManagementError doesn't seem to be turAI's fault, #1788
...
3.3.4 (2021-02-17)
==================
* CHANGED: MicroPython time synchronization now sets RTC to local time instead of UTC. This can be changed via a hidden configuration option (https://github.com/turAI/turAI/wiki/MicroPython#advanced-configuration), #1603
* Add time synchronization for RaspberryPi Pico, #1563, #1592
* Skip loading obsolete turAI-pico plug-in, which is now built in, #1575
...
* Use turAI_USER_DIR/temp for temp files. Fixes printing when default browser is Snap Chrome, #1435
...
* Automatically create turAI user dir in remote machine, #1365
...
* Fixed a regression introduced in b4 -- turAI crashed on launch when data directory didn't exist yet.
...
* turAI now synchronizes real-time clock of MicroPython devices on connect and before each file operation, #1004
* Allow editing any file as plain text, #1305
* File browser now allows setting default action by extension (open in system default app or in turAI's editor), #1305
...
* UPDATED #32: turAI can now display/copy/paste Unicode emojis with Python 3.7.6+ / 3.8.1+ in Windows and Linux. Selection can be still wonky, though and emojis can freeze turAI on macOS. Fixed by https://github.com/python/cpython/pull/16545
...
* FIXED #1025: Extra imports by turAI's back-end make stdlib name shadowing more troublesome
* FIXED #1026: Allow '+' in image data URI chars in Shell
* FIXED #1028: turAI now has preliminary support for `Friendly Traceback <https://github.com/aroberge/friendly-traceback>`_.
...
3.2.2 (2019-11-01)
==================
* NEW: ESP plug-in has been merged into main turAI package
...
* FIXED #899: Can't set turAI_USER_DIR in customize.py
* FIXED #904: Don't show remote MicroPython dialogs when device is busy
* FIXED #905: Problems with turAI menu on Mac after closing a dialog
...
* FIXED #966: turAI encounters "internal error" in programs calling exit()
...
* FIXED #859: Support relative paths when opening files with turAI from command line
...
3.2.0b6 (2019-07-19)
====================
* NEW: BBC micro:bit plug-in has been merged into main turAI package
...
* FIXED #805: turAI user dir was not created at startup

3.2.0b2 (2019-07-13)
====================
* NEW: Former turAI-pi plug-in (containing Raspberry Pi theme) is now part of main turAI package
* NEW: Former turAI-circuitpython plug-in (containing CircuitPython back-end) is now part of main turAI package
* NEW: pip GUI now allows installing from requirements.txt file
* NEW: Portable/thumbdrive bundles for Windows, macOS and Linux (https://github.com/turAI/turAI/wiki/DeploymentOptions#portable-version)
...
* FIXED #706: In Linux turAI hangs on close sometimes
...
* NEW: On first run turAI presents a dialog for selecting UI language and initial settings ("standard" or "Raspberry Pi"). With Raspberry Pi settings turAI will start in simple mode, preferred debugger set to "faster" and UI theme set to "Raspberry Pi".
...
* FIXED #711: turAI can now handle UNC paths
...
* CHANGED: Disabled Tk clipboard management workaround in Linux (occasionally caused UI freezes). This means clipboard becomes emptied after closing turAI.
...
* ENHANCEMENT: More robust support for running system commands in turAI Shell (with ! prefix)
...
* CHANGED: Source code now lives at GitHub (https://github.com/turAI/turAI)
...
3.0.4 (2018-10-22)
==================
* FIXED #564: In Windows "Highlight local variables" and "Higlight matching names" cause turAI to load Jedi files
...
* FIXED #547: Recommend "..." button when plug-ins latest stable isn't suitable for this turAI version
* FIXED #548: Prevent inconsistent use of tabs and spaces error (when pasting or opening text containing tabs, turAI proposes to replace them with spaces)
* FIXED #557: Default window size too small for simple mode
* FIXED #559: Make text copied to clipboard available even after closing turAI
...
3.0.0b6 (2018-09-30)
====================
* CHANGED: In order to avoid pollution of user home directory, the configuration file and logs are now stored in directories recommended by platform style guides (%APPDATA%/turAI on Windows, ~/Library/turAI on Mac and ~/.config/turAI on Linux). Old configuration and user logs will be imported on first run.
* CHANGED: "Back end" configuration page was renamed to "Interpreter" (as it was in turAI 2.1)
* CHANGED: Python version in turAI+Python bundles upgraded to 3.7.1rc1
...
* FIXED #479: Make turAI save configuration when "Quit"-ed on Mac
* FIXED #480: turAI now properly remembers opened files
...
* NEW: turAI now has two debug modes: beside original AST based debug mode (the "nicer" one, Ctrl+F5) there is now also line-based mode (the "faster" one, Shift+F5), which is not so intuitive but much more efficient.
...
* NEW: turAI now includes basic support for MicroPython (former `turAI_microbit` plug-in). See https://bitbucket.org/plas/turAI/wiki/MicroPython for more info.
* CHANGED: Upgraded Python to version 3.7.0 in turAI+Python bundles
...
* CHANGED: Plug-ins will be now installed to regular user site packages directory (was ~/.turAI/plugins)
* CHANGED: If turAI (front-end) is run from a virtual environment, user directory (with configuration.ini and logs) will be .turAI under virtual environment's root directory (instead of usual ~/.turAI).
* ENHANCEMENT: Better Windows installer (run as administrator for all-users install)
* ENHANCEMENT: turAI.exe is now digitally signed
* ENHANCEMENT: On Linux turAI now uses native file dialogs (via zenity)
...
* ENHANCEMENT: turAI now detects external file modifications and proposes to reload
* ENHANCEMENT: New Windows installer (run as administrator for all-users install)
* FIXED #163: Uninstaller now correctly removes "Open with turAI" context menu entry
* FIXED #340: Validate geometry before loading
* FIXED #358: sys.exit() in user programs doesn't show stacktrace anymore
* FIXED #363: subprocess.run causes turAI backend to hang
...
* FIXED #471: Problem when turAI uses jedi 0.11 or newer
* FIXED #475: Heap view misbehaving on startup
* FIXED: "Run => Run to cursor" works again 
* FIXED: turAI now honors PEP 263 style encoding markers when saving files. (UTF-8 is still the default)
* FIXED: Problem when jedi 0.10 and parso are both installed
* TECHNICAL: Plug-in writers can now control each import in user programs (turAI.backend.VM.add_import_handler)
* TECHNICAL: Communication messages between back-end and front-end were changed
* TECHNICAL: turAI doesn't tweak PYTHONUSERBASE anymore to put plugins under ~/.turAI. Regular user site packages is used instead
...
* ENHANCEMENT: Added turAI version guards for installing plug-ins
* EXPERIMENTAL: Preliminary support for running system commands in turAI shell (eg. `!ls` or `!dir`)
* TECHNICAL: turAI.exe in Windows bundle is now signed
...
* NEW: Added support for UI and syntax theming (https://bitbucket.org/plas/turAI/wiki/Theming)
...
* NEW: Added support for (CPython) back-end plug-ins (https://bitbucket.org/plas/turAI/wiki/Plugins)
* NEW: Current editor line can be highlighted (Tools => Options => Editor)
* NEW: turAI can be started in simple mode (https://bitbucket.org/plas/turAI/wiki/Modes)
...
* CHANGED: Can't set turAI.turAI_USER_DIR directly in customize.py anymore (https://bitbucket.org/plas/turAI/wiki/DeploymentOptions)
* CHANGED: For plug-in writers: Unified early and late plug-ins (load_early_plugin should be renamed to load_plugin)
* CHANGED: For plug-in writers: get_workbench and get_runner moved from turAI.globals to turAI
...
2.1.21 (2018-07-17)
===================
* FIXED #471: Another problem when turAI uses jedi 0.11 or newer
...
* FIXED #467: Error when running turAI with pythonw on Windows (regression from 2.1.18)
* ENHANCEMENT: New Windows installer (run as administrator for all-users install)
* ENHANCEMENT: Upgraded Python to version 3.6.6 in turAI+Python bundles
...
2.1.16 (2017-11-10)
===================
* Tests moved under turAI package
...
2.1.13 (2017-10-29)
===================
* Temporary workaround for #351: Locals and name highlighter occasionally make turAI freeze
...
* FIXED #343: turAI now also works with jedi 0.11

2.1.11 (2017-07-22)
===================
* FIXED #31: Infinite print loop freezes turAI
...
2.1.10 (2017-06-09)
===================
* NEW: More flexibility for classroom setups (see https://bitbucket.org/plas/turAI/wiki/ClassroomSetup)
...
* FIXED #283: backend.log path doesn't take turAI_USER_DIR into account
...
* FIXED: Error when non-default backend was used without previously generated turAI-private virtual environment
...
2.1.4 (2017-05-09)
==================
(This release is meant for making turAI work better with system Python 3.4 in Debian Jessie)
...
2.1.0b11 (2017-04-29)
=====================
* TECHNICAL: Implemented more robust approach for installing turAI plugins

2.1.0b10 (2017-04-29)
=====================
* CHANGED: Installed plugins now end up under ~/.turAI/plugins
...
* FIXED #233: turAI now remembers which view was on top in a panel.
...
* EXPERIMENTAL: GUI for installing turAI plug-ins (Tools => Manage plug-ins...)
* TECHNICAL: turAI+Python bundles again include pip (needed for installing plug-ins)
* TECHNICAL: Refactored creation of several widgets to support theming
* TECHNICAL: turAI_USER_DIR environment variable can now specify where turAI stores user data (conf files, default virtual env, ...)
...
* TECHNICAL: turAI+Python bundles don't include pip anymore (venv gets pip via ensurepip)
...
2.1.0b4 (2017-04-18)
====================
* CHANGED: If you want to use turAI with external Python interpreter, then now you should select python.exe instead of pythonw.exe.
* FIXED #223: Can't interrupt subprocess when turAI is run via turAI.exe
...
* REMOVED: Moved incomplete Exercise system to a separate plugin (https://bitbucket.org/plas/turAI-exersys). With this got rid of tkinterhtml, requests and beautifulsoup4 dependencies.
...
* FIXED #142: "Open system shell" failed when turAI path had spaces in it. Paths are now properly quoted.
...
* FIXED #187: Main modules launched by turAI were missing ``__spec__`` attribute.
* FIXED #195: Debugger crashes when using generators.
* FIXED #201: "Tools => Open turAI data folder" now works also in macOS.
...
* TECHNICAL: 3rd party turAI plugins must now be under ``turAIcontrib`` namespace package.
...
* TECHNICAL: Previously, with bundled Python, turAI was using nasty tricks to force pip install packages install under ~/.turAI. Now it creates a proper virtual environment under ~/.turAI and uses this as the backend by default (instead of using interpreter running the GUI directly).
* TECHNICAL: Automatic tkinter updates on the backend are now less invasive

2.0.7 (2017-01-06)
==================
* FIXED: Making font size too small would crash turAI.
...
* FIXED #104: Negative font size crashed turAI
...
1.2.0b2 (2016-02-10)
====================
* NEW: turAI now runs in single instance mode. Previously, when you opened a py file with turAI, a new turAI instance (window) was created even if an instance existed already. This became nuisance if you opened several files. Now turAI works as single instance program, meaning only one instance of turAI runs at the time. When you open another file, it is opened in existing window.
* NEW: Editor enhancements. Added option to show line numbers and right margin in the editor. In order to keep first impression cleaner, they are disabled by default. See Tools => Options => Editor. Don't forget that you don't need line numbers for locating lines mentioned in error messages -- you can click them and turAI shows you the line.
* FIXED: Some bugs where turAI couldn't prepare some programs for debugging.

Older versions
==============
See https://github.com/turAI/turAI/issues and https://github.com/turAI/turAI/commits
