...
from turAI import get_runner, get_workbench, misc_utils, tktextext
from turAI.common import InlineCommand, UserError, get_dirs_children_info
from turAI.languages import tr
from turAI.misc_utils import get_menu_char, running_on_mac_os, running_on_windows, sizeof_fmt
...
class BaseFileBrowser(ttk.Frame):
    ...
    def update_node_data(self, node_id, name, data):
        ...
        if data["isdir"]:
            ...
            elif self.should_open_name_in_turAI(name):
    ...
    def on_double_click(self, event):
        ...
        if kind == "file":
            if self.should_open_name_in_turAI(name):
                self.open_file(path)
            else:
    ...
    def add_first_menu_items(self, context):
        ...
        if context == "button":
            ...
            if selected_kind == "dir":
                ...
                self.menu.add_command(
                    label=tr("Open in turAI"), command=lambda: self.open_file(selected_path)
                )
    ...
    def open_extension_dialog(self, extension: str) -> None:
        ...
        turAI_choice = tr("Open in turAI's text editor")

        current_index = (
            1 if get_workbench().get_option(get_file_handler_conf_key(extension)) == "turAI" else 0
        )

        choice = ask_one_from_choices(
            title=tr("Configure %s files") % extension,
            question=tr(
                "What to do with a %s file when you double-click it in turAI's file browser?"
            )
            % extension,
            choices=[system_choice, turAI_choice],
        ...
        get_workbench().set_option(
            ...
            "system" if choice == system_choice else "turAI",
        )
        # update icons
        self.refresh_tree()
    ...
    def should_open_name_in_turAI(self, name):
        ext = self.get_extension_from_name(name)
        return get_workbench().get_option(get_file_handler_conf_key(ext), "system") == "turAI"
...
class BaseRemoteFileBrowser(BaseFileBrowser):
    ...
    def open_path_with_system_app(self, path):
        messagebox.showinfo(
            ...
            + "\n\n"
            + tr(
                "If it is a text file, then you can configure it to open in turAI "
...
class DialogRemoteFileBrowser(BaseRemoteFileBrowser):
    ...
    def should_open_name_in_turAI(self, name):
        # In dialog, all file types are to be opened in turAI
        return True
