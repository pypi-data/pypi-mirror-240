...
from turAI.ui_utils import select_sequence, show_dialog
from turAI.workdlg import WorkDialog
...
class Runner:
    ...
    def _init_commands(self) -> None:
        ...
        try:
            import turAI.plugins.debugger  # @UnusedImport
        ...
        if not running_on_mac_os():  # on Mac Ctrl+C is not used for Copy.
            ...
            if isinstance(widget, tk.Text):
                ...
                try:
                    ...
                    # i.e. it may be from another application when turAI has nothing selected
                    ...
                    if isinstance(selection, str) and len(selection) > 0:
                        ...
                        # ie. there may be no selection in turAI actually.
    ...
    def _handle_backend_termination(self, returncode: int) -> None:
        ...
        try:
            faults_file = os.path.join(turAI_USER_DIR, "backend_faults.log")
    ...
    def _check_alloc_console(self) -> None:
        if sys.executable.endswith("pythonw.exe"):
            ...
            try:
                ...
                # See https://bitbucket.org/plas/turAI/issues/508/cant-allocate-windows-console-when
                # Without flush the console window becomes visible, but turAI can be still used
    ...
    def using_venv(self) -> bool:
        from turAI.plugins.cpython_frontend import LocalCPythonProxy
...
class BackendProxy(ABC):
    ...
    def destroy(self, for_restart: bool = False):
        """Called when turAI no longer needs this instance
        (turAI gets closed or new backend gets selected)
        """
        ...
...
class SubprocessProxy(BackendProxy, ABC):
    ...
    def _get_environment(self):
        ...
        # see https://github.com/turAI/turAI/issues/808
        env["PYTHONUNBUFFERED"] = "1"

        env["PYTHONSAFEPATH"] = "1"

        # Let back-end know about plug-ins
        env["turAI_USER_DIR"] = turAI_USER_DIR
        env["turAI_FRONTEND_SYS_PATH"] = repr(sys.path)

        env["turAI_LANGUAGE"] = get_workbench().get_option("general.language")
        env["turAI_VERSION"] = get_version()

        if turAI.in_debug_mode():
            env["turAI_DEBUG"] = "1"
        elif "turAI_DEBUG" in env:
            del env["turAI_DEBUG"]
        return env
        ...
        while True:
            ...
            if data == "":
                ...
                try:
                    ...
                    # then the suffix can be turAI message
...
def get_environment_overrides_for_python_subprocess(target_executable):
    ...
    # exe is bundled for turAI.
    # In remaining cases it is user's responsibility to avoid
    # calling turAI with environment which may be confusing for
    ...
    if os.path.samefile(
        ...
        # (If they are set then it's most likely because current exe is in turAI bundle)
