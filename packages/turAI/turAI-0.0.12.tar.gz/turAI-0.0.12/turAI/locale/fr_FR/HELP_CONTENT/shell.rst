...
Tout comme l'invite REPL officielle de Python, le Shell de turAI accepte des expressions et des commandes Python, qu'elles soient
sur une seule ligne ou sur plusieurs. Si on presse la touche Entrée, turAI utilise quelques heuristiques pour prédire
si on veut lancer la commande ou continuer à taper la commande sur la ligne suivante.
Si vous voulez lancer la commande mais que turAI vous offre une nouvelle ligne, vérifiez si vous avez oublié
...
Si on sélectionne "Lancer => Lancer le script courant" ou qu'on presse la touche F5, on voit que turAI insère une commande
...
(facilement) s'exprimer par des commandes Python. Les commandes magiques de turAI ont d'habitude
...
préfixer la commande par ``!`` (p. ex. ``!pytest mon-script.py``) et de la saisir dans le Shell de turAI.
...
du terminal. Le Shell de turAI ne peut pas en faire autant, mais il supporte cependant quelques trucs et astuces.
