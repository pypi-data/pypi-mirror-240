...
import turAI
from turAI import get_runner, get_workbench, running, tktextext, ui_utils
from turAI.common import DistInfo, InlineCommand, normpath_with_actual_case, path_startswith
from turAI.languages import tr
from turAI.misc_utils import construct_cmd_line, levenshtein_distance
from turAI.running import InlineCommandDialog, get_front_interpreter_for_subprocess
...
from turAI.workdlg import SubprocessDialog
    ...
    def _show_package_info(self, name, data, error_code=None):
        ...
        if info.get("requires_dist", None):
            ...
            for item in requires_dist:
                ...
                # avoid introducing a new dependency as turAI is included in
...
class BackendPipDialog(PipDialog):
    ...
    def _confirm_install(self, package_data):
        ...
        if name.lower().startswith("turAI"):
            return messagebox.askyesno(
                tr("Confirmation"),
                tr(
                    "Looks like you are installing a turAI-related package.\n"
                    + "If you meant to install a turAI plugin, then you should\n"
...
class PluginsPipDialog(PipDialog):
    ...
    def _conflicts_with_turAI_version(self, req_strings):
        import pkg_resources
        ...
        try:
            ...
            for req_string in req_strings:
                ...
                if req.project_name == "turAI" and turAI.get_version() not in req:
    ...
    def _confirm_install(self, package_data):
        ...
        other_version_text = tr(
            ...
            + "which is compatible with current turAI version. "
            + "Click on '...' button to choose the version to install."
        )

        if name.lower().startswith("turAI-") and not reqs:
            showerror(
                tr("turAI plugin without requirements"),
                (
                    "Looks like you are trying to install an outdated turAI\n"
                    + "plug-in (it doesn't specify required turAI version\n"
            ...
            conflicts = self._conflicts_with_turAI_version(reqs)
            if conflicts:
                showerror(
                    tr("Unsuitable requirements"),
                    tr("This package requires different turAI version:")
                    + "\n\n  "
                    + "\n  ".join(conflicts)
                    + "\n\n"
    ...
    def _get_target_directory(self):
        if self._use_user_install():
            target = turAI.get_sys_path_directory_containg_plugins()
    ...
    def _create_widgets(self, parent):
        ...
        banner_msg = (
            tr(
                "This dialog is for managing turAI plug-ins and their dependencies.\n"
        ...
        banner_msg += "\n" + tr(
            "NB! You need to restart turAI after installing / upgrading / uninstalling a plug-in."
        )
    ...
    def _run_pip_with_dialog(self, command: str, args: Dict, title: str) -> Tuple[int, str, str]:
        ...
        proc = running.create_frontend_python_process(
            ...
            environment_extras={"PYTHONUSERBASE": turAI.get_user_base_directory_for_plugins()},
        )
