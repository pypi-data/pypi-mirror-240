...
from turAI.backend import MainBackend, logger
...
_REPL_HELPER_NAME = "_turAI_repl_print"

_CONFIG_FILENAME = os.path.join(turAI.turAI_USER_DIR, "backend_configuration.ini")
...
class MainCPythonBackend(MainBackend):
    def __init__(self, target_cwd, options):
        ...
        self._main_dir = os.path.dirname(sys.modules["turAI"].__file__)
        ...
        # required in shell mode and also required to overwrite turAI location dir
    ...
    def _init_command_reader(self):
        ...
        # https://github.com/turAI/turAI/issues/1363
        pass
    ...
    def _handle_immediate_command(self, cmd: ImmediateCommand) -> None:
        ...
        was reverted for CPython https://github.com/turAI/turAI/issues/1363.
    ...
    def _load_plugins(self):
        # built-in plugins
        try:
            import turAI.plugins.backend  # pylint: disable=redefined-outer-name
        except ImportError:
            # May happen eg. in ssh session
            return

        self._load_plugins_from_path(turAI.plugins.backend.__path__, "turAI.plugins.backend.")

        # 3rd party plugins from namespace package
        try:
            import turAIcontrib.backend  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(turAIcontrib.backend.__path__, "turAIcontrib.backend.")
    ...
    def _cmd_FastDebug(self, cmd):
        self.switch_env_to_script_mode(cmd)
        from turAI.plugins.cpython_backend.cp_tracers import FastTracer
    ...
    def _cmd_Debug(self, cmd):
        ...
        from turAI.plugins.cpython_backend.cp_tracers import NiceTracer
    ...
    def export_value(self, value, max_repr_length=5000):
        ...
        try:
            ...
            # See https://bitbucket.org/plas/turAI/issues/584/problem-with-turAIs-back-end-obj-no
            rep = "??? <repr error>"
...
def format_exception_with_frame_info(e_type, e_value, e_traceback, shorten_filenames=False):
    """Need to suppress turAI frames to avoid confusion"""
        ...
        if tb is not None:
            ...
            for entry in traceback.extract_tb(tb):
                ...
                if (
                    ...
                    and "turAI/backend" not in entry.filename.replace("\\", "/")
                    and (
...
def in_debug_mode():
    return os.environ.get("turAI_DEBUG", False) in [1, "1", True, "True", "true"]
