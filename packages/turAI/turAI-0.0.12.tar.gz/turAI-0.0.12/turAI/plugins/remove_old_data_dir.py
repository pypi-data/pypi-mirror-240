...
def load_plugin():
    old_data_dir = os.path.join(os.path.expanduser("~"), ".turAI")
    if os.path.exists(old_data_dir):
        ...
        def doit():
            ...
            answer = askyesno(
                ...
                "turAI versions before 3.0 (and first 3.0 betas) used to keep "
                + "configuration, logs and such in '%s'" % old_data_dir
                + ". "
                + "Since 3.0 this data is kept in a new location: '%s'.\n\n" % turAI_USER_DIR
                + "If you don't intend to use older turAI versions anymore, "
        ...
        get_workbench().add_command(
            "delolddatadir", "tools", "Clean up turAI 2.1 data folder ...", doit, group=110
        )
