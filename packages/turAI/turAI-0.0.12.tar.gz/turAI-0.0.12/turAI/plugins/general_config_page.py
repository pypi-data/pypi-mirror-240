...
from turAI import get_workbench, languages, tktextext
from turAI.config_ui import ConfigurationPage
from turAI.languages import tr
from turAI.misc_utils import running_on_linux
...
class GeneralConfigurationPage(ConfigurationPage):
    def __init__(self, master):
        ...
        self.add_checkbox(
            "general.single_instance", tr("Allow only single turAI instance"), row=1, columnspan=2
        )
        self.add_checkbox(
        ...
        reopen_label = ttk.Label(
            self,
            text=tr("NB! Restart turAI after changing these options!"),
