...
from turAI.common import normpath_with_actual_case
from turAI.misc_utils import get_menu_char, running_on_windows
from turAI.plugins.micropython import BareMetalMicroPythonProxy, list_serial_ports
from turAI.plugins.micropython.base_flashing_dialog import (
    BaseFlashingDialog,
    TargetInfo,
    family_code_to_name,
)
from turAI.plugins.micropython.mp_front import get_serial_port_label
from turAI.running import get_front_interpreter_for_subprocess
from turAI.ui_utils import EnhancedBooleanVar, MappingCombobox
...
class ESPFlashingDialog(BaseFlashingDialog):
    ...
    def get_variants_url(self) -> str:
        return f"https://raw.githubusercontent.com/turAI/turAI/master/data/{self.firmware_name.lower()}-variants-esptool.json"
    ...
    def prepare_work_get_options(self) -> Dict[str, Any]:
        ...
        port_was_used_in_turAI = (
            isinstance(proxy, BareMetalMicroPythonProxy) and proxy._port == target.port.device
        )
        if port_was_used_in_turAI and self._work_needs_disconnect():
        ...
        return {
            ...
            "port_was_used_in_turAI": port_was_used_in_turAI,
        }
    ...
    def perform_core_operation(
        ...
        if self._work_needs_disconnect() and work_options["port_was_used_in_turAI"]:
