...
import turAI
from turAI import get_runner, get_shell, get_workbench, running, ui_utils
...
from turAI.misc_utils import running_on_mac_os, running_on_windows
from turAI.plugins.backend_config_page import BackendDetailsConfigPage
from turAI.running import WINDOWS_EXE, SubprocessProxy, get_front_interpreter_for_subprocess
from turAI.terminal import run_in_terminal
from turAI.ui_utils import askdirectory, askopenfilename, create_string_var
...
class LocalCPythonProxy(SubprocessProxy):
    ...
    def _get_launch_cwd(self):
        ...
        empty_dir = os.path.join(turAI.turAI_USER_DIR, "leave_this_empty")
        ...
        if force or get_runner().is_waiting_toplevel_command():
            ...
            if not self._expecting_response_for_gui_update:
                try:
                    ...
                    # https://github.com/turAI/turAI/issues/966
    ...
    def get_pip_gui_class(self):
        from turAI.plugins.cpython_frontend.cp_pip_gui import LocalCPythonPipDialog
...
class LocalCPythonConfigurationPage(BackendDetailsConfigPage):
    def __init__(self, master):
        ...
        extra_text = tr("NB! turAI only supports Python %s and later") % "3.8"
