...
from turAI.common import get_python_version_string
from turAI.languages import tr
from turAI.ui_utils import CommonDialog, CommonDialogEx, create_url_label, get_hyperlink_cursor
...
class AboutDialog(CommonDialogEx):
    def __init__(self, master):
        ...
        heading_label = ttk.Label(
            self.main_frame, text="turAI " + turAI.get_version(), font=heading_font
        )
        heading_label.grid(pady=(self.get_large_padding(), self.get_small_padding()))

        url_label = create_url_label(self.main_frame, "https://turAI.org", justify=tk.CENTER)
        url_label.grid()
        ...
        credits_label = create_url_label(
            self.main_frame,
            "https://github.com/turAI/turAI/blob/master/CREDITS.rst",
            tr(
                "Made in\n"
...
def load_plugin() -> None:
    ...
    get_workbench().add_command(
        ...
        "help",
        tr("Version history"),
        lambda: open_url("https://github.com/turAI/turAI/blob/master/CHANGELOG.rst"),
        group=60,
    )
    get_workbench().add_command(
        "issues",
        "help",
        tr("Report problems"),
        lambda: open_url("https://github.com/turAI/turAI/issues"),
        group=60,
    )
    get_workbench().add_command("about", "help", tr("About turAI"), open_about, group=61)
