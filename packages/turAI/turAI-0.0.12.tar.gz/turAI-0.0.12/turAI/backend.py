...
from turAI.common import (  # TODO: try to get rid of this
...
class BaseBackend(ABC):
    ...
    def _report_internal_exception(self, msg: str) -> None:
        user_msg = "PROBLEM IN turAI'S BACK-END: " + msg
        if sys.exc_info()[1]:
            err_msg = "\n".join(
        ...
        user_msg += ".\nSee " + turAI.BACKEND_LOG_MARKER + " for more info."
    ...
    def _handle_normal_command(self, cmd: CommandToBackend) -> None:
        ...
        if handler is None:
            ...
            try:
                ...
                # Exceptions must be caused by turAI or plugins code, because the ones
...
def get_ssh_password_file_path():
    from turAI import turAI_USER_DIR

    return os.path.join(turAI_USER_DIR, "ssh_password")
