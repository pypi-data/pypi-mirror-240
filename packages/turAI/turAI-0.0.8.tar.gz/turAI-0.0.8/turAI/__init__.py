...
from turAI.common import is_private_python, is_virtual_executable
...
BACKEND_LOG_MARKER = "turAI's backend.log"
...
def _compute_turAI_user_dir():
    if os.environ.get("turAI_USER_DIR", ""):
        return os.path.expanduser(os.environ["turAI_USER_DIR"])
    elif is_portable():
        ...
        return os.path.join(sys.prefix, ".turAI")
    elif sys.platform == "win32":
        return os.path.join(_get_roaming_appdata_dir(), "turAI")
    elif sys.platform == "darwin":
        return os.path.expanduser("~/Library/turAI")
    else:
        # https://specifications.freedesktop.org/basedir-spec/latest/ar01s02.html
        data_home = os.environ.get(
            "XDG_CONFIG_HOME", os.path.expanduser(os.path.join("~", ".config"))
        )
        return os.path.join(data_home, "turAI")
...
def is_portable():
    ...
    portable_marker_path = os.path.join(os.path.dirname(sys.executable), "portable_turAI.ini")
    shared_marker_path = os.path.join(os.path.dirname(sys.executable), "shared_turAI.ini")
...
def get_version():
    global _turAI_VERSION
    if _turAI_VERSION:
        return _turAI_VERSION
    try:
        package_dir = os.path.dirname(sys.modules["turAI"].__file__)
        with open(os.path.join(package_dir, "VERSION"), encoding="ASCII") as fp:
            _turAI_VERSION = fp.read().strip()
            return _turAI_VERSION
    except Exception:
        return "0.0.0"


turAI_USER_DIR = _compute_turAI_user_dir()
CONFIGURATION_FILE = os.path.join(turAI_USER_DIR, "configuration.ini")
...
def get_ipc_file_path():
    ...
    ipc_dir = os.path.join(base_dir, "turAI-%s" % username)
...
def _check_welcome():
    from turAI import misc_utils

    if not os.path.exists(CONFIGURATION_FILE) and not misc_utils.running_on_rpi():
        from turAI.config import ConfigurationManager
        from turAI.first_run import FirstRunWindow
        ...
        if win.ok and sys.platform == "darwin":
            ...
            if macos_app_path:
                ...
                # or saving a file (https://github.com/turAI/turAI/issues/2860).
                # Let's restart.
...
def _get_macos_app_path() -> Optional[str]:
    ...
    if orig_argv[0].endswith("turAI.app/Contents/MacOS/turAI"):
        return orig_argv[0][: -len("/Contents/MacOS/turAI")]
...
def launch():
    ...
    if sys.executable.endswith("turAI.exe"):
        # otherwise some library may try to run its subprocess with turAI.exe
        # NB! Must be pythonw.exe not python.exe, otherwise Runner thinks console
        # is already allocated.
        sys.executable = sys.executable[: -len("turAI.exe")] + "pythonw.exe"

    set_dpi_aware()

    try:
        runpy.run_module("turAI.customize", run_name="__main__")
    except ImportError:
        pass

    prepare_turAI_user_dir()
    ...
    if _should_delegate():
        try:
            ...
            print("Delegated to an existing turAI instance. Exiting now.")
            return 0
        except Exception:
            import traceback
...
def prepare_turAI_user_dir():
    if not os.path.exists(turAI_USER_DIR):
        os.makedirs(turAI_USER_DIR, mode=0o700, exist_ok=True)
        ...
        if os.path.isdir(template_dir):
            ...
            copy_contents(template_dir, turAI_USER_DIR)
...
def get_backend_log_file():
    return os.path.join(turAI_USER_DIR, "backend.log")


def get_frontend_log_file():
    return os.path.join(turAI_USER_DIR, "frontend.log")
...
def _get_orig_argv() -> Optional[List[str]]:
    try:
        ...
        try:
            ...
            # See https://github.com/turAI/turAI/issues/2206
            # and https://bugs.python.org/issue40910
            # This symbol is not available in turAI.exe built agains Python 3.8
            return None
...
def _configure_logging(log_file, console_level=None):
    ...
    main_logger = logging.getLogger("turAI")
    contrib_logger = logging.getLogger("turAIcontrib")
    ...
    main_logger.info("turAI version: %s", get_version())
    ...
    fault_out = open(os.path.join(turAI_USER_DIR, "frontend_faults.log"), mode="w")
    faulthandler.enable(fault_out)


def get_user_base_directory_for_plugins() -> str:
    return os.path.join(turAI_USER_DIR, "plugins")


def get_sys_path_directory_containg_plugins() -> str:
    from turAI.misc_utils import get_user_site_packages_dir_for_base
...
if TYPE_CHECKING:
    ...
    import turAI.workbench
    from turAI.running import Runner
    from turAI.shell import ShellView
    from turAI.workbench import Workbench
...
def set_logging_level(level=None):
    ...
    logging.getLogger("turAI").setLevel(level)
...
def in_debug_mode() -> bool:
    ...
    return (
        os.environ.get("turAI_DEBUG", False)
        in [
            "1",
            1,
            "True",
...
report_time("After loading turAI module")
