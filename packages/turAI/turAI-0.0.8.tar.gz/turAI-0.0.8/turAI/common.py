...
MESSAGE_MARKER = "\x02"
OBJECT_LINK_START = "[object_link_for_turAI=%d]"
OBJECT_LINK_END = "[/object_link_for_turAI]"
REMOTE_PATH_MARKER = " :: "
PROCESS_ACK = "OK"
...
def serialize_message(msg: Record, max_line_length=65536) -> str:
    ...
    # default (safe) window size in Paramiko (https://github.com/turAI/turAI/issues/1680)
    msg_str = ascii(msg)
    ...
    if os.name == "nt":
        try:
            ...
            if result.casefold() != norm_name.casefold():
                ...
                # see eg. https://github.com/turAI/turAI/issues/925
        ...
        # Hopefully only correct case comes into turAI (eg. via open dialog)
        return os.path.normpath(name)
...
def get_windows_lnk_target(lnk_file_path):
    ...
    script_path = os.path.join(os.path.dirname(turAI.__file__), "res", "PrintLnkTarget.vbs")
...
def execute_with_frontend_sys_path(function: Callable) -> Any:
    ...
    try:
        frontend_sys_path = ast.literal_eval(os.environ["turAI_FRONTEND_SYS_PATH"])
        assert isinstance(frontend_sys_path, list)
        logger.info("Using turAI_FRONTEND_SYS_PATH %s", frontend_sys_path)
    except Exception as e:
        logger.debug("Could not get turAI_FRONTEND_SYS_PATH", exc_info=e)
        frontend_sys_path = []
...
def is_private_python(executable):
    result = os.path.exists(os.path.join(os.path.dirname(executable), "turAI_python.ini"))
