...
import turAI.memory
from turAI import get_runner, get_workbench, ui_utils
from turAI.common import InlineCommand
from turAI.languages import tr
from turAI.memory import MemoryFrame
from turAI.misc_utils import shorten_repr
from turAI.tktextext import TextFrame
from turAI.ui_utils import CustomToolbutton, ems_to_pixels
...
class ObjectInspector(ttk.Frame):
    ...
    def _show_object_by_id(self, object_id, via_navigation=False):
        ...
        if self.winfo_ismapped() and self.object_id != object_id:
            ...
            self._set_title("object @ " + turAI.memory.format_object_id(object_id))
    ...
    def set_object_info(self, object_info):
        ...
        if object_info is None or "error" in object_info:
            ...
            self._set_title(
                ...
                + " @ "
                + turAI.memory.format_object_id(object_info["id"])
            )
...
class ElementsInspector(turAI.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        turAI.memory.MemoryFrame.__init__(
    ...
    def set_object_info(self, object_info):
        ...
        for element in object_info["elements"]:
            ...
            self.tree.set(node_id, "id", turAI.memory.format_object_id(element.id))
            self.tree.set(
                node_id, "value", shorten_repr(element.repr, turAI.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            index += 1
...
class DictInspector(turAI.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        turAI.memory.MemoryFrame.__init__(
    ...
    def set_object_info(self, object_info):
        ...
        for key, value in object_info["entries"]:
            node_id = self.tree.insert("", "end")
            self.tree.set(node_id, "key_id", turAI.memory.format_object_id(key.id))
            self.tree.set(
                node_id, "key", shorten_repr(key.repr, turAI.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            self.tree.set(node_id, "id", turAI.memory.format_object_id(value.id))
            self.tree.set(
                node_id, "value", shorten_repr(value.repr, turAI.memory.MAX_REPR_LENGTH_IN_GRID)
            )
...
class AttributesFrame(turAI.memory.VariablesFrame):
    def __init__(self, master):
        turAI.memory.VariablesFrame.__init__(self, master)
    ...
    def show_selected_object_info(self):
        ...
        if repr_str == "<bound_method>":
            from turAI.plugins.micropython import MicroPythonProxy
