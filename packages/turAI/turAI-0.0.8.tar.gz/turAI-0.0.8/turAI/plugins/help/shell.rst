...
Python commands
---------------
Just like the official Python REPL, turAI's Shell accepts Python expressions and statements, both
single-line and multiline. If you press ENTER, then turAI uses some heuristics to predict
whether you wanted to submit the command or continue typing the command on the next line. 
If you want to submit the command but turAI offers you a new line, then check whether you forgot
to close some parentheses.   

Magic commands
--------------
If you select *Run →  Run current script* or press F5, then you'll see how turAI inserts a command
...
(easily) expressed as Python commands. turAI's magic commands usually have
...
prefix the command with ``!`` (eg. ``!pytest my-script.py``) and enter it into turAI's shell.
...
screen. turAI's Shell is not so capable, but it does support a couple of simpler tricks.
