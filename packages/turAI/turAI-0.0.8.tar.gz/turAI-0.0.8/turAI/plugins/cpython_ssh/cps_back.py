...
logger = getLogger("turAI.plugins.cpython_ssh.cps_back")
...
class SshCPythonBackend(BaseBackend, SshMixin):
    ...
    def _start_main_backend(self) -> RemoteProcess:
        env = {"turAI_USER_DIR": "~/.config/turAI", "turAI_FRONTEND_SYS_PATH": "[]"}
        self._main_backend_is_fresh = True

        cp_launcher_file = (
            self._get_remote_program_directory()
            + "/"
            + "turAI/plugins/cpython_backend/cp_launcher.py"
        )
        args = [
    ...
    def _get_remote_program_directory(self):
        return f"/tmp/turAI-backend-{turAI.get_version()}-{self._user}"
    ...
    def _upload_main_backend(self):
        ...
        if self._get_stat_mode_for_upload(launch_dir) and not turAI.get_version().endswith("-dev"):
            # don't overwrite unless in dev mode
            return

        ensure_posix_directory(
            launch_dir + "/turAI/plugins/cpython_backend",
            self._get_stat_mode_for_upload,
            self._mkdir_for_upload,
        )

        import turAI.ast_utils
        import turAI.backend
        import turAI.jedi_utils
        import turAI.plugins.cpython_backend.cp_back

        # Don't want to import cp_back_launcher and cp_tracers

        local_context = os.path.dirname(os.path.dirname(turAI.__file__))
        for local_path in [
            turAI.__file__,
            turAI.common.__file__,
            turAI.ast_utils.__file__,
            turAI.jedi_utils.__file__,
            turAI.backend.__file__,
            turAI.plugins.cpython_backend.__file__,
            turAI.plugins.cpython_backend.cp_back.__file__,
            turAI.plugins.cpython_backend.cp_back.__file__.replace("cp_back.py", "cp_launcher.py"),
            turAI.plugins.cpython_backend.cp_back.__file__.replace("cp_back.py", "cp_tracers.py"),
        ]:
