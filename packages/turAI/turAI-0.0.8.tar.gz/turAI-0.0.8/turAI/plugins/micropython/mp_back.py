...
from turAI import BACKEND_LOG_MARKER, get_backend_log_file, report_time
from turAI.backend import MainBackend
from turAI.common import (
    OBJECT_LINK_END,
    OBJECT_LINK_START,
    BackendEvent,
    CommandToBackend,
...
from turAI.plugins.micropython.connection import MicroPythonConnection
...
EOT = b"\x04"
MGMT_VALUE_START = b"<turAI>"
MGMT_VALUE_END = b"</turAI>"
...
class MicroPythonBackend(MainBackend, ABC):
    ...
    def _prepare_after_soft_reboot(self, clean=False):
        ...
        # See https://github.com/turAI/turAI/issues/1877
        # self._execute_without_output(
        #     dedent(
        #         """
        #     for key in __turAI_helper.builtins.dir(__turAI_helper.builtins):
        #         if not key.startswith("__"):
        #             __turAI_helper.builtins.globals()[key] = None
        #     """
        #     ).strip()
        # )
            ...
            class __turAI_helper:
                ...
                def repr(cls, obj):
                    try:
                        ...
                        return "<could not serialize: " + __turAI_helper.builtins.str(e) + ">"
    ...
    def _fetch_sys_path(self):
        if not self._supports_directories():
            ...
            return self._evaluate("__turAI_helper.sys.path")
    ...
    def _fetch_epoch_year(self):
        ...
        val = self._evaluate(
            dedent(
                """
            try:
                from time import localtime as __turAI_localtime
                __turAI_helper.print_mgmt_value(__turAI_helper.builtins.tuple(__turAI_localtime(%d)))
                del __turAI_localtime
            except __turAI_helper.builtins.Exception as e:
                __turAI_helper.print_mgmt_value(__turAI_helper.builtins.str(e))
        """
                % Y2000_EPOCH_OFFSET
            )
        )
    ...
    def _update_cwd(self):
        if not self._using_microbit_micropython():
            ...
            self._cwd = self._evaluate("__turAI_helper.getcwd()")
    ...
    def _evaluate(self, script):
        ...
        try:
            ...
            prefix = "__turAI_helper.print_mgmt_value("
            suffix = ")"
    ...
    def _cmd_cd(self, cmd):
        if len(cmd.args) == 1:
            ...
            path = cmd.args[0]
            self._execute("__turAI_helper.chdir(%r)" % path)
            self._update_cwd()
            return {}
        else:
    ...
    def _cmd_get_globals(self, cmd):
        if cmd.module_name == "__main__":
            globs = self._evaluate(
                "{name : (__turAI_helper.repr(value), __turAI_helper.builtins.id(value)) for (name, value) in __turAI_helper.builtins.globals().items() if not name.startswith('__')}"
            )
        else:
            globs = self._evaluate(
                dedent(
                ...
                __turAI_helper.print_mgmt_value(
                    {name : (__turAI_helper.repr(__turAI_helper.builtins.getattr(__mod_for_globs, name)), 
                             __turAI_helper.builtins.id(__turAI_helper.builtins.getattr(__mod_for_globs, name)))
                        in __turAI_helper.builtins.dir(__mod_for_globs) 
    ...
    def _cmd_get_object_info(self, cmd):
        ...
        self._execute(
            dedent(
                """
                if __turAI_helper.builtins.id(__turAI_helper.object_info) not in __turAI_helper.inspector_values:
                    __turAI_helper.inspector_values[__turAI_helper.builtins.id(__turAI_helper.object_info)] = __turAI_helper.object_info
            """
            )
        )
        ...
        if match:
            ...
            if "." in prefix:
                ...
                names = self._evaluate(
                    "__turAI_helper.builtins.dir({obj}) if '{obj}' in __turAI_helper.builtins.locals() or '{obj}' in __turAI_helper.builtins.globals() else []".format(
                        obj=obj
                    )
                )
            else:
                names = self._evaluate("__turAI_helper.builtins.dir()")
        else:
            names = []
            prefix = ""
    ...
    def _find_basic_object_info(self, object_id, context_id):
        ...
        result = self._evaluate(
            dedent(
                """
                for __turAI_helper.object_info in (
                        [__turAI_helper.last_non_none_repl_value]
                        + __turAI_helper.builtins.list(__turAI_helper.builtins.globals().values()) 
                        + __turAI_helper.builtins.list(__turAI_helper.inspector_values.values())):
                    if __turAI_helper.builtins.id(__turAI_helper.object_info) == %d:
                        __turAI_helper.print_mgmt_value({
                            "repr" : __turAI_helper.builtins.repr(__turAI_helper.object_info),
                            "type": __turAI_helper.builtins.str(__turAI_helper.builtins.type(__turAI_helper.object_info))
                        })
                        break
                else:
                    __turAI_helper.object_info = None
                    __turAI_helper.print_mgmt_value(None)
            """
                % object_id
            )
        )
        ...
        if result is not None:
            ...
            return self._evaluate(
                dedent(
                    """
                __turAI_helper.context_value = __turAI_helper.inspector_values.get(%d, None)
                
                if __turAI_helper.context_value is None:
                    __turAI_helper.object_info = None
                    __turAI_helper.print_mgmt_value(None)
                else:
                    __turAI_helper.context_children = [
                         __turAI_helper.builtins.getattr(__turAI_helper.context_value, name)
                         for name in __turAI_helper.builtins.dir(__turAI_helper.context_value)
                    ]
                    if __turAI_helper.builtins.isinstance(__turAI_helper.context_value, (set, tuple, list)):
                        __turAI_helper.context_children += __turAI_helper.builtins.list(__turAI_helper.context_value)
                    elif __turAI_helper.builtins.isinstance(__turAI_helper.context_value, __turAI_helper.builtins.dict):
                        __turAI_helper.context_children += __turAI_helper.builtins.list(__turAI_helper.context_value.values())
                    
                    for __turAI_helper.object_info in __turAI_helper.context_children:
                        if __turAI_helper.builtins.id(__turAI_helper.object_info) == %d:
                            __turAI_helper.print_mgmt_value({
                                "repr" : __turAI_helper.repr(__turAI_helper.object_info),
                                "type": __turAI_helper.builtins.str(__turAI_helper.builtins.type(__turAI_helper.object_info))
                            })
                            break
                    else:
                ...
                __turAI_helper.context_value = None
                __turAI_helper.context_children = None
            """
    ...
    def _get_object_attributes(self, all_attributes):
        """object is given in __turAI_helper.object_info"""
        atts = self._evaluate(
            "{name : ("
            "   __turAI_helper.builtins.id(__turAI_helper.builtins.getattr(__turAI_helper.object_info, name)),"
            "    __turAI_helper.repr(__turAI_helper.builtins.getattr(__turAI_helper.object_info, name))"
            ") for name in __turAI_helper.builtins.dir(__turAI_helper.object_info)}"
        )
        return {
    ...
    def _get_object_info_extras(self, type_name: str, repr_str: str):
        ...
        if type_name in ("list", "tuple", "set"):
            items = self._evaluate(
                "[(__turAI_helper.builtins.id(x), __turAI_helper.repr(x)) for x in __turAI_helper.object_info]"
            )
            return {"elements": [ValueInfo(x[0], x[1]) for x in items]}
        elif type_name == "dict":
            items = self._evaluate(
                "[((__turAI_helper.builtins.id(key), __turAI_helper.repr(key)), (__turAI_helper.builtins.id(__turAI_helper.object_info[key]), "
                "__turAI_helper.repr(__turAI_helper.object_info[key]))) for key in __turAI_helper.object_info]"
            )
            return {
                "entries": [
    ...
    def _mkdir(self, path: str) -> None:
        ...
        self._execute_without_output("__turAI_helper.os.mkdir(%r)" % path)
    ...
    def _delete_sorted_paths(self, paths):
        self._execute_without_output(
            dedent(
                """
            def __turAI_delete(path):
                if __turAI_helper.os.stat(path)[0] & 0o170000 == 0o040000:
                    for name in __turAI_helper.listdir(path):
                        child_path = path + "/" + name
                        __turAI_delete(child_path)
                    __turAI_helper.rmdir(path)
                else:
                    __turAI_helper.os.remove(path)
            
            for __turAI_path in %r: 
                __turAI_delete(__turAI_path)
    ...
    def _get_stat(
        ...
        stat = self._evaluate(
            dedent(
                """
            try:
                __turAI_helper.print_mgmt_value(__turAI_helper.os.%s(%r))
            except __turAI_helper.builtins.Exception:
                __turAI_helper.print_mgmt_value(None)
            """
            )
            % (func, path)
    ...
    def _get_dir_children_info(
        ...
        if self._supports_directories():
            raw_data = self._evaluate(
                ...
                __turAI_result = {} 
                try:
                    __turAI_names = __turAI_helper.listdir(%r)
                except __turAI_helper.builtins.OSError:
                    __turAI_helper.print_mgmt_value(None) 
                else:
                    for __turAI_name in __turAI_names:
                        if not __turAI_name.startswith(".") or %r:
                            try:
                                __turAI_result[__turAI_name] = __turAI_helper.os.stat(%r + __turAI_name)
                            except __turAI_helper.builtins.OSError as e:
                                __turAI_result[__turAI_name] = __turAI_helper.builtins.str(e)
                    __turAI_helper.print_mgmt_value(__turAI_result)
            """
                )
            ...
            raw_data = self._evaluate(
                "{name : __turAI_helper.os.size(name) for name in __turAI_helper.os.listdir()}"
            )
        else:
            return None
    ...
    def _replace_last_repl_value_variables(self, source: str) -> str:
        ...
        for node in reversed(load_nodes):
            lines[node.lineno - 1] = (
                ...
                + "__turAI_helper.builtins.globals().get('_', __turAI_helper.last_non_none_repl_value)"
    ...
    def _add_expression_statement_handlers(self, source):
        try:
            ...
            from turAI.ast_utils import mark_text_ranges
            ...
            marker_prefix = "__turAI_helper.print_repl_value("
            marker_suffix = ")"
    ...
    def _avoid_printing_expression_statements(self, source):
        # temporary solution for https://github.com/turAI/turAI/issues/1441
        try:
            root = ast.parse(source)
