...
import turAI
from turAI import get_runner, get_workbench, rst_utils, tktextext, ui_utils
...
from turAI.misc_utils import levenshtein_damerau_distance, running_on_mac_os
from turAI.ui_utils import CommonDialog, get_hyperlink_cursor, scrollbar_style
    ...
    def __init__(self, error_info):
        ...
        self.suggestions = [
            Suggestion(
                ...
                "Let turAI developers know",
                "Click on the feedback link at the bottom of this panel to let turAI developers know "
                + "about your problem. They may add support for "
                + "such cases in future turAI versions.",
                1,
            )
        ]
...
class FeedbackDialog(CommonDialog):
    def __init__(self, master, main_file_path, all_snapshots):
        ...
        intro_label = ttk.Label(
            ...
            + self._get_since_str()
            + ".\n\n"
            + "In order to improve this feature, turAI developers would love to know how "
        ...
        self.include_turAI_id_var = tk.IntVar(value=1)
        include_turAI_id_check = ttk.Checkbutton(
            main_frame,
            variable=self.include_turAI_id_var,
            onvalue=1,
            offvalue=0,
            text="Include turAI's installation time (allows us to group your submissions)",
        )
        include_turAI_id_check.grid(
    ...
    def _collect_submission_data(self):
        ...
        submission = {
            ...
            "turAI_version": turAI.get_version(),
        ...
        if self.include_turAI_id_var.get():
            submission["turAI_timestamp"] = get_workbench().get_option(
        ...
        def do_work():
            try:
                handle = urllib.request.urlopen(
                    "https://turAI.org/store_assistant_feedback.php",
...
def format_file_url(filename, lineno, col_offset):
    s = "turAI-editor://" + rst_utils.escape(filename).replace(" ", "%20")
