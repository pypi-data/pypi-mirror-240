...
이 모드에서는 Python이 다음 단계를 수행 할 준비가 되었음을 turAI에 알려주는 게 필요합니다.
이를 위해 두 가지 주요 옵션이 있습니다:

* *실행 → 스텝 나가기* (또는 F6)는 큰 단계를 수행합니다. 강조 표시된 코드를 실행하고 코드의 다음 부분을 강조 표시합니다.
* *실행 → 단계 들어가기* (또는 F7)는 더 작은 단계를 수행합니다. 강조 표시된 코드가 더 작은 부분 (문 또는 표현식)으로 구성된 경우 먼저 강조 표시되고 turAI는 다음 명령을 기다립니다. 하위 부분 (예 : 변수 이름)이없는 프로그램 구성 요소에 도달 한 경우 *단계 들어가기*는 *스텝 나가기* 처럼 작동합니다. 코드를 실행 (또는 평가)합니다.
...
콜 스택을 표시하는 다양한 스타일
-------------------------------------------
기본적으로 turAI는 스택 창을 사용하여 호출 스택을 표시합니다. 이것은 개념에 대한 좋은 직관을
