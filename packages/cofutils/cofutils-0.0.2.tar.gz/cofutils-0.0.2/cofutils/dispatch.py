...
def auto_gen(config_file, template_file):
    ...
    def autogen_from_template(config_list, template_file, ipx):
        ...
        merge_dict = {k:v for each in config_list for k,v in each.items() }
        with open(template_file, 'r') as file:
            lines = file.readlines()

        modified_lines = [compile(line, merge_dict) if line.startswith('%') else line for line in lines]
        modified_lines = [ '_'.join(line.strip('\n').split('&'))+f"={ipx}\n"if line.startswith('&') else line for line in modified_lines]
        return ''.join(modified_lines)

    with open(config_file) as f:
        config_list = json.load(f)
    cluster_config=config_list[0]
    cluster_ip_list = cluster_config["CLUSTER_IPS"]
    
    ##########################################
    # generate launch-all.sh!
    ##########################################
    NNODES = cluster_config['NNODES']
    
    current_dir = os.getcwd()
    launch_all_content = f'mpirun -np {NNODES} \\\n\
    --hostfile {current_dir}/tmp/hostfile  \\\n\
...
exit 0'
    ...
    for host_idx,host_ip in enumerate(cluster_ip_list[:NNODES]):
        script_content = autogen_from_template(config_list, template_file, host_idx)
...
def main():
    ...
    parser.add_argument('--template','-T', type=int, default=None, help='provide the path of template .sh file')
