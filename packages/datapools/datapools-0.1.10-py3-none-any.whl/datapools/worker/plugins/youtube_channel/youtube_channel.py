...
class YoutubeChannelPlugin(BasePlugin):
    ...
    async def _process_video(self, url, page, context):
        ...
        channel_uri = page.locator("ytd-channel-name a:visible").filter(has_not=page.get_by_role("link", name="ManageEngine", exact=True))
        ...
        thread = Thread(target=self._download_thread, args=(url, storage_id, download_state))
        thread.start()
