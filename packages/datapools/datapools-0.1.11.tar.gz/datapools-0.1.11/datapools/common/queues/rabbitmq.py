...
class RabbitmqQueue(Stoppable):
    ...
    async def _try_reopen_channel(self, message: aio_pika.IncomingMessage):
        if message.__channel.is_closed:
            logger.info(f"mark_done: reopening closed {message.__channel=}")
            await message.__channel.reopen()

    async def mark_done(
        self, message: aio_pika.IncomingMessage, try_reopen_channel=True
    ):
        if try_reopen_channel:
            await self._try_reopen_channel(message)
        await message.ack()
