...
class _BaseGridObject(_AbstractGridObject):
    def __init__(self, grid: Any, name: AnyStr, object_type: AnyStr, cell: Union[AnyStr, type(Cell)] = None):
        ...
        self._views = {
            ...
            'Near': {f'{i}': f'A{" " if getattr(self, "category")[0] not in "AEIOUaeiou" else "n "}{getattr(self, "category").lower()}' for i in range(8)}, 'Far': {f'{i}': f'A{" " if getattr(self, "category")[0] not in "AEIOUaeiou" else "n "}{getattr(self, "category").lower()}' for i in range(8)}}
