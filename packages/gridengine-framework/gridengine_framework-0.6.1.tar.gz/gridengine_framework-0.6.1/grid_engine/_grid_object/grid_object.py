...
class _AbstractGridObject(ABC):
    ...
    def __view__(self, From: int = None):
...
class _BaseGridObject(_AbstractGridObject):
    def __init__(self, grid: Any, name: AnyStr, object_type: AnyStr, cell: Union[AnyStr, type(Cell)] = None):
        ...
        self.cell = init_cell
        self._views = {'Close': {f'{i}': self.description for i in range(8)}, 'Near': {f'{i}': f'A{" " if getattr(self, "category")[0] not in "AEIOUaeiou" else "n "}{getattr(self, "category").lower()}'}, 'Far': {f'{i}': f'A{" " if getattr(self, "category")[0] not in "AEIOUaeiou" else "n "}{getattr(self, "category").lower()}'}}
