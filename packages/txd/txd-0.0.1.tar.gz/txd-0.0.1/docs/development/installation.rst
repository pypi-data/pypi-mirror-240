...
The latest released version of :obj:`txd` can be installed from `PyPI <https://pypi.org/project/txd/>`_ using `pip`.

.. code-block:: console

   $ python3 -m pip install txd
...
.. code-block:: console
   ...
   $ python3 -m pip install git+https://github.com/bernardodionisi/txd.git

Or from a specific branch.

.. code-block:: console

   $ python3 -m pip install git+https://github.com/bernardodionisi/txd.git@branch
...
fashion from a local folder. This allows any changes in the current directory to be immediately seen upon the next `import txd`.

Clone the repo wherever you'd like.

.. code-block:: console

    $ git clone https://github.com/bernardodionisi/txd.git

Install the local folder using the `-e` or `--editable` flag.

.. code-block:: console

    $ python3 -m pip install -e txd/
