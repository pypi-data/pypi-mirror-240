...
class LakalaPay(AsyncPayCore):
    ...
    async def pay(
        ...
        **kwargs,
        ...
        content = self._handler_response(response)
        return PayResult(
            code=0,
            msg="success",
            trade_no=content["data"]["out_order_id"],
            prepay_data=json.loads(content["data"]["wap_pay_request"]),
        )
    ...
    async def refund(
        ...
        **kwargs,
        ...
        return CancelResult(
            code=0,
            msg="success",
            refund_fee=content["data"]["refund_amount"],
            out_trade_no=content["data"]["client_sn"],
            trade_no=content["data"]["out_order_id"],
        )
    ...
    def _handler_response(self, response: httpx.Response):
        try:
