...
from quant1x.trader import market, utils, context, thinktrader, config
...
def auto_trader() -> int:
    ...
    while True:
        ...
        if today != trade_date:
            ...
            # 如果状态还没调整
            logger.error('today={}, trade_date={}, 当前日期为交易日', today, trade_date)
            ctx.switch_date()
            date_updated = True
        # 3.3.2 早盘策略
        head_trading(ctx, trader)
        # 3.3.3 盘中策略
        tick_trading(ctx, trader)
        # 3.3.4 尾盘策略
...
def head_trading(ctx: QmtContext, trader: thinktrader.ThinkTrader):
    ...
    for idx, stock in df.iterrows():
        ...
        strategy_code = '0'
        strategy_name = '0号策略'
        logger.warning('{}: 证券名称={}, 证券代码={}, date={}, strategy_code={}, price={},vol={}', strategy_code, security_name, security_code, order_date, strategy_code, buy_price, buy_num)
        order_id = trader.buy(security_code, buy_price, buy_num, f'{strategy_code}:{strategy_name}', order_remark)
...
def tick_trading(ctx: QmtContext, trader: thinktrader.ThinkTrader):
    """
    盘中交易
    :param ctx: 上下文
    :param trader: 交易实例
...
def positions_sell(ctx: QmtContext, trader: thinktrader.ThinkTrader):
    ...
    for position in positions:
        ...
        # 控制台输出持仓记录
        logger.warning("sell: %s stock %s holding: %d can_ask: %d, open_price:%f, market_value: %f" % (op_flag, security_code,
                                                                                        position.volume,
