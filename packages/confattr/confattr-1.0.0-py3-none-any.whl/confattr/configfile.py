...
class ConfigFile:
	...
	def __init__(self, *,
		...
		if config_instances is None:
			config_instances = Config.iter_instances()  # this does not return a list or tuple and that is important for sorting
		self.config_instances = {i.key: i for i in self.iter_config_instances(config_instances, ignore)}
	...
	def iter_config_instances(self,
		...
		Sort the resulting list if :paramref:`~confattr.configfile.ConfigFile.iter_config_instances.config_instances` is not a :class:`list` or a :class:`tuple`.
	...
	def set_save_default_arguments(self, kw: 'SaveKwargs') -> None:
		...
		'''
		kw.setdefault('config_instances', list(self.config_instances.values()))
...
class Set(ConfigFileCommand):
	...
	def iter_config_instances_to_be_saved(self,
		...
		'''
		for config in self.config_file.iter_config_instances(config_instances, ignore):
