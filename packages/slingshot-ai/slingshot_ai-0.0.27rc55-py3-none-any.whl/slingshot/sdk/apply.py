...
from slingshot.schemas import HasAutoscalingParams, HasComponentConfig, ProjectManifest, requested_requirements_from_str
...
class ApplyService:
    ...
    async def create_component_spec(
        ...
        component_spec_id_response = await self._sdk.create_component(
            ...
            config_variables=(
                component_spec.get_component_config() if isinstance(component_spec, HasComponentConfig) else None
            ),
            ...
            min_replicas=component_spec.get_min_replicas()
            if isinstance(component_spec, HasAutoscalingParams)
            else None,
            max_replicas=component_spec.get_max_replicas()
            if isinstance(component_spec, HasAutoscalingParams)
            else None,
    ...
    async def update_component_spec(
        ...
        await self._sdk.update_component(
            ...
            machine_size=machine_size,
            config_variables=component_spec.get_component_config()
