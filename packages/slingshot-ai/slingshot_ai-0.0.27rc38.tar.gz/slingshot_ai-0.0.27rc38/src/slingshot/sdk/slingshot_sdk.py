...
from ..schemas import CreateServiceAccountResponse, LogLine, Response, RunConfiguration, SshResult
...
class SlingshotSDK:
    ...
    async def start_app_ssh(self, spec_id: str) -> Response[SshResult]:
        """Starts SSH access for an app."""
        project_id = await self._get_current_project_id_or_raise()
        return await self._api.start_app_ssh(spec_id=spec_id, project_id=project_id)

    async def start_run_ssh(self, run_id: str) -> Response[SshResult]:
