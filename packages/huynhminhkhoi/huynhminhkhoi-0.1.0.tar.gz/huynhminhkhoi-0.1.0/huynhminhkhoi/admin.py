...
class Admin:
    ...
    def add_key(self, key, methods):
        ...
        try:
            add = requests.get(f'https://khoidzbantumlum.prokunkhoi.repl.co/addkey?admin={self.admin}&key={key}&loaikey={methods}').json()
            check = add['status']
            if check == 'success':
                return add['message']
            else:
    ...
    def remove(self, key):
        try:
            add = requests.get(f'https://khoidzbantumlum.prokunkhoi.repl.co/remove?admin={self.admin}&key={key}').json()
            check = add['status']
            if check == 'success':
                return add['message']
            else:
