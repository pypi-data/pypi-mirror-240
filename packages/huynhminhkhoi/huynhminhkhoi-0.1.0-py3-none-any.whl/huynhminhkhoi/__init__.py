...
class Api:
    ...
    def get_key(self, ip):
        ...
        self.ip = ip
        request_key = requests.post('https://khoidzbantumlum.prokunkhoi.repl.co/get', json = {'keyword':'Khoidz', 'name': self.name, 'ip': ip, 'token': token, 'long_url': long_url}).json()
        return request_key
    def check_key(self, ip, key):
        request_key = requests.post('https://khoidzbantumlum.prokunkhoi.repl.co/check', json = {'keyword':'Khoidz', 'name': self.name, 'ip': ip, 'key': key}).json()
        return request_key
