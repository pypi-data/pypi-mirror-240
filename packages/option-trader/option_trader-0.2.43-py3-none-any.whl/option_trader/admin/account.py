...
class account():
    ...
    def create_position_summary(self, symbol, exp_date, strategy, row, q):
        ...
        margin = q * 100 * row[position_summary.MARGIN]
        cash = q * 100 * row[position_summary.OPEN_PRICE]    
    ...
    def update_position_summary(self):
        ...
        for uuid in uuid_list:       
            ...
            if strategy == st.UNPAIRED:
                ...
                data = (last_price, pl, gain, last_quote_date, last_price, uuid)                                
                cursor = self.db_conn.cursor()    
                cursor.execute(sql, data) 
                self.db_conn.commit()   
                continue
...
if __name__ == '__main__':
    ...
    winwin_user = mysite.create_user('chrishua')

    winwin_acct= winwin_user.get_account('Individual')

    winwin_acct.open_new_strategy_positions()
