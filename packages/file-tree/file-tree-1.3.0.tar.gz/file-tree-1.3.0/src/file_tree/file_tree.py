...
from .template import Placeholders, Template, is_singular
...
class FileTree:
    ...
    def _add_actual_template(
        ...
        for key in keys:
            if key in self._templates:
                if not overwrite:
                    raise ValueError(
                        f"{self._templates[key]} already exists with short name '{key}', so cannot add {template}"
                    )
    ...
    def template_keys(self, only_leaves=False):
        ...
        return {
            key for key, template in self._templates.items() if template not in parents
        }
    ...
    def filter_templates(
        ...
        return FileTree(
            new_templates, self.placeholders.copy(), return_path=self.return_path
        )
            ...
            if isinstance(key, frozenset):
                ...
                for linked_key, linked_value in value.items():
                    placeholder_lines.append(
                        f"{linked_key} = {', '.join([str(v) for v in linked_value])}"
                    )
