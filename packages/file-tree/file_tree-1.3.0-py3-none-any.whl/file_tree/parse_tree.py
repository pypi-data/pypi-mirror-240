...
def add_line_to_tree(
    tree: FileTree, line: str, current: List[Tuple[int, str]], to_link: List[List[str]]
    ...
    if stripped[:2] == "->":
        ...
        if short_names is not None and len(short_names) == 0:
            raise ValueError(
                "Empty short names are not allowed (they are used internally for the top-level directory"
            )
