...
class FinetuningRunDisplayItem(MCLIDisplayItem):
    ...
    def from_finetuning_run(cls, finetuning_run: Finetune) -> List[MCLIDisplayItem]:
        run_rows = []
        label = "End Time"
        status = finetuning_run.status.display_name
        eta = "-"
        ...
        run_rows.append(
            MCLIDisplayItem({
                'name': finetuning_run.name,
                'User': finetuning_run.created_by,
                'Status': status,
                'Reason': finetuning_run.reason or "-",
                'Created Time': format_timestamp(finetuning_run.created_at),
                label: eta if label == "ETA" else format_timestamp(finetuning_run.completed_at),
            }))
        return run_rows
