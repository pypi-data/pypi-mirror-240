...
from mcli.cli.m_delete.delete import delete_deployment, delete_finetuning_run, delete_run, delete_secret
...
_finetuning_run_description = Description("""
Delete a finetuning run or set of finetuning runs that match some conditions.
""")
_finetuning_run_simple = CLIExample(example='mcli delete finetuning-run <my-finetuning-run>',
                                    description='Delete a specific finetuning run by name')
_finetuning_run_example_status = CLIExample(example='mcli delete finetuning-runs --status failed,completed',
                                            description='Delete all failed and completed finetuning runs')
_finetuning_run_examples = [
    _finetuning_run_simple,
    CLIExample(example='mcli delete finetuning-run <my-finetuning-run>',
               description='Delete a specific finetuning run'),
    _finetuning_run_example_status,
    CLIExample(example='mcli delete finetuning-runs --status failed,completed',
               description='Delete all failed and completed finetuning runs'),
]
...
def configure_argparser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    finetuning_run_parser = subparsers.add_parser(
        'ft',
        aliases=['finetuning-runs', 'finetuning-run'],
        help='Delete one or more finetuning runs',
        description=_finetuning_run_description,
        epilog=get_example_text(*_finetuning_run_examples),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )

    finetuning_run_parser.add_argument(
        dest='name_filter',
        nargs='*',
        metavar='FINETUNE',
        default=None,
        help='String or glob of the name(s) of the finetuning runs to delete',
    )

    configure_finetuning_submission_filter_argparser('delete', finetuning_run_parser)
    finetuning_run_parser.set_defaults(func=delete_finetuning_run)
    add_common_args(finetuning_run_parser)
