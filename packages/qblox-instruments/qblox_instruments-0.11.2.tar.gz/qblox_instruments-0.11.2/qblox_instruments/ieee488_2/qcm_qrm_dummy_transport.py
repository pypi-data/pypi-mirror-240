...
class QcmQrmDummyTransport(DummyTransport):
    ...
    def __init__(
        ...
        self._acq_scope_cfg = {}
        self._sequencer_cfg = {}
        self._sequencer_state = {}
        if self.is_qcm_type:
            self._channelmap = {'0': [[0, 2], [1, 3]], '1': [[0, 2], [1, 3]], '2': [[0, 2], [1, 3]], '3': [[0, 2], [1, 3]], '4': [[0, 2], [1, 3]], '5': [[0, 2], [1, 3]]}
            self._acq_channelmap = {}
        else:
            self._channelmap = {'0': [[0], [1]], '1': [[0], [1]], '2': [[0], [1]], '3': [[0], [1]], '4': [[0], [1]], '5': [[0], [1]]}
            self._acq_channelmap = {'0': [[0], [1]], '1': [[0], [1]], '2': [[0], [1]], '3': [[0], [1]], '4': [[0], [1]], '5': [[0], [1]]}
        self._awg_waveforms = {}
        self._acq_weights = {}
        self._acq_metadata = {}
        ...
        self._cmds["SEQuencer#:ACQ:ALISt:ACQuisition:DATA:DELete"] = self._del_acq_acquisition_data
    ...
    def set_dummy_binned_acquisition_data(
        ...
        if "scope" not in self._acq_acquisitions_dummy[sequencer][acq_index_name]:
            self._acq_acquisitions_dummy[sequencer][acq_index_name]["acq"]["scope"] = self._initial_scope_acquisition_data()
    ...
    def _get_sequencer_state(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        self._data_out = self._sequencer_state.get(sequencer_idx, "IDLE;")
    ...
    def _add_acq_acquisition(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        if "scope" not in self._acq_acquisitions[cmd_params[0]][cmd_args[0]]["acq"]:
            self._acq_acquisitions[cmd_params[0]][cmd_args[0]]["acq"]["scope"] = self._initial_scope_acquisition_data()
    ...
    def _del_acq_acquisition(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        if cmd_params[0] in self._acq_acquisitions:
            if cmd_args[0].lower() == "all":
                ...
                if (cmd_args[0] in self._acq_acquisitions[cmd_params[0]] and
                    cmd_args[0] in self._acq_metadata[cmd_params[0]]):
    ...
    def _get_channelmap(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        ----------
        """
        self._bin_out = self._encode_bin(json.dumps(self._channelmap[cmd_params[0]]).encode('utf-8'))
    ...
    def _get_acq_channelmap(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        ----------
        """
        self._bin_out = self._encode_bin(json.dumps(self._acq_channelmap[cmd_params[0]]).encode('utf-8'))
    ...
    def _arm(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        """
        sequencer_idx = cmd_params[0]
        self._sequencer_state[sequencer_idx] = "ARMED;"
    ...
    def _start(self, cmd_params: list, cmd_args: list, bin_in: bytes) -> None:
        ...
        if sequencer_idx_str in self._acq_acquisitions_dummy:
            self._acq_acquisitions[sequencer_idx_str] = deepcopy(self._acq_acquisitions_dummy[sequencer_idx_str])
        self._acq_scope_acquisition = deepcopy(self._acq_scope_acquisition_dummy)
        self._sequencer_state[sequencer_idx] = "STOPPED;ACQ_BINNING_DONE,"
    ...
    # ------------------------------------------------------------------------
    def _set_initial_binned_acquisition_data(self, sequencer: int, acq_index_name: Optional[str] = None):
        """
        ...
        def _set_initial_for_index(sequencer, name, bins):
            self._acq_acquisitions[sequencer][name]["acq"]["bins"] = (
                [{"valid": False, "int": [0, 0], "thres": 0, "avg_cnt": 0}] * bins
            )
