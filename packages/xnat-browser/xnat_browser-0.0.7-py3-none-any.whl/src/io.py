...
from src.constants import QUERY_SELECTIONS
...
class QueryProtocol(Protocol):
    ...
    def query_one(self, selector: str | type[QueryType], expect_type: type[QueryType] | None = None) -> QueryType:
        ...


async def load_search(filename: Path | None, app: QueryProtocol) -> None:
    if filename is None:
        return
    ...
    await app.set_query_constraints([xnat.search.Constraint(*x) for x in data.constraints])


def save_search(filename: Path | None, app: QueryProtocol) -> None:
    if filename is None:
        return
...
def save_result(filename: Path | None, query_result: pd.DataFrame | None) -> None:
