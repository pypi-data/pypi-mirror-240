...
class Citros:
    ...
    @contextmanager
    def modified_docker_config(self, disable_creds_store=True):
    ...
    def _create_citrosignore(self):
        if not Path(self.CITROS_REPO_DIR, '.citrosignore').exists():
            with open(Path(self.CITROS_REPO_DIR, '.gitignore'), 'w') as file:
                # file is populated by user
                pass


    def get_package_ignore_list(self):
