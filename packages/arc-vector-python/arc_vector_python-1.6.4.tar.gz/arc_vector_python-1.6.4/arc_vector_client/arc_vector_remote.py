...
from arc_vector_client._pydantic_compat import construct
from arc_vector_client.client_base import ArcVectorBase
from arc_vector_client.connection import get_async_channel, get_channel
from arc_vector_client.conversions import common_types as types
from arc_vector_client.conversions.common_types import get_args_subscribed
from arc_vector_client.conversions.conversion import (
    GrpcToRest,
    RestToGrpc,
    grpc_payload_schema_to_field_type,
)
from arc_vector_client.http import ApiClient, SyncApis, models
from arc_vector_client.parallel_processor import ParallelWorkerPool
from arc_vector_client.uploader.grpc_uploader import GrpcBatchUploader
from arc_vector_client.uploader.rest_uploader import RestBatchUploader
from arc_vector_client.uploader.uploader import BaseUploader


class ArcVectorRemote(ArcVectorBase):
    def __init__(
        self,
    ...
    def _init_grpc_channel(self) -> None:
        if self._closed:
            raise RuntimeError("Client was closed. Please create a new ArcVectorClient instance.")
    ...
    @classmethod
    def _try_argument_to_rest_selector(cls, points: types.PointsSelector) -> models.PointsSelector:
        if isinstance(points, list):
            _points = [
                GrpcToRest.convert_point_id(idx) if isinstance(idx, grpc.PointId) else idx
                for idx in points
            ]
            points_selector = construct(models.PointIdsList, points=_points)
        elif isinstance(points, grpc.PointsSelector):
            points_selector = GrpcToRest.convert_points_selector(points)
        elif isinstance(points, get_args(models.PointsSelector)):
            points_selector = points
        elif isinstance(points, models.Filter):
            points_selector = construct(models.FilterSelector, filter=points)
        elif isinstance(points, grpc.Filter):
            points_selector = construct(
                models.FilterSelector, filter=GrpcToRest.convert_filter(points)
            )
        else:
            raise ValueError(f"Unsupported points selector type: {type(points)}")
        return points_selector
