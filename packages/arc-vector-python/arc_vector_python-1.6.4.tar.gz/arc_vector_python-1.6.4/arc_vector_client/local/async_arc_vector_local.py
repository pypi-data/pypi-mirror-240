...
from arc_vector_client._pydantic_compat import to_dict
...
from arc_vector_client.http import models as rest_models
from arc_vector_client.http.models.models import RecommendExample
from arc_vector_client.local.local_collection import LocalCollection

META_INFO_FILENAME = "meta.json"


class AsyncArcVectorLocal(AsyncArcVectorBase):
    """
    Everything ArcVector server can do, but locally.

    Use this implementation to run vector search without running a ArcVector server.
    Everything that works with local ArcVector will work with server ArcVector as well.

    Use for small-scale data, demos, and tests.
    If you need more speed or size, use ArcVector server.
    """

    def __init__(self, location: str) -> None:
        """
        Initialize local ArcVector.
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Storage folder {self.location} is already accessed by another instance of ArcVector client. If you require concurrent access, use ArcVector server instead."
            )
    ...
    async def create_collection(
        ...
        **kwargs: Any,
        ...
        collection = LocalCollection(
            rest_models.CreateCollection(vectors=vectors_config), location=collection_path
        )
    ...
    async def create_payload_index(
        ...
        **kwargs: Any,
        ...
        logging.warning(
            "Payload indexes have no effect in the local ArcVector. Please use server ArcVector if you need payload indexes."
        )
        return self._default_update_result()
    ...
    async def create_snapshot(
        ...
        raise NotImplementedError(
            "Snapshots are not supported in the local ArcVector. Please use server ArcVector if you need full snapshots."
        )
    ...
    async def create_shard_snapshot(
        ...
        raise NotImplementedError(
            "Snapshots are not supported in the local ArcVector. Please use server ArcVector if you need snapshots."
        )
    ...
    async def lock_storage(self, reason: str, **kwargs: Any) -> types.LocksOption:
        raise NotImplementedError(
            "Locks are not supported in the local ArcVector. Please use server ArcVector if you need full snapshots."
        )
