...
class QuantizationSearchParams(BaseModel, extra="forbid"):
    ...
    rescore: Optional[bool] = Field(
        default=None,
        description="If true, use original vectors to re-score top-k results. Might require more time in case if original vectors are stored on disk. If not set, arc_vector decides automatically apply rescoring or not.",
    )
...
class SnapshotRecover(BaseModel, extra="forbid"):
    location: str = Field(
        ...,
        description="Examples: - URL `http://localhost:8080/collections/my_collection/snapshots/my_snapshot` - Local path `file:///arc_vector/snapshots/test_collection-2022-08-04-10-49-10.snapshot`",
    )
