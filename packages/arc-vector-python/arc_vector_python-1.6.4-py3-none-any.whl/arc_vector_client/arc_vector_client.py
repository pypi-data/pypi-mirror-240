...
from arc_vector_client import grpc as grpc
from arc_vector_client.client_base import ArcVectorBase
from arc_vector_client.conversions import common_types as types
from arc_vector_client.http import ApiClient, SyncApis
from arc_vector_client.local.arc_vector_local import ArcVectorLocal
from arc_vector_client.migrate import migrate
from arc_vector_client.arc_vector_fastembed import ArcVectorFastembedMixin
from arc_vector_client.arc_vector_remote import ArcVectorRemote


class ArcVectorClient(ArcVectorFastembedMixin):
    """Entry point to communicate with ArcVector service via REST or gPRC API.
    ...
    All methods in ArcVectorClient accept both gRPC and REST structures as an input.
    ...
    .. note::
        ...
        - :py:attr:`ArcVectorClient.grpc_points`
        - :py:attr:`ArcVectorClient.grpc_collections`
        - :py:attr:`ArcVectorClient.rest`

    .. note::
        If you need async, please consider using Async Implementations of ArcVectorClient.

        - :class:`arc_vector_client.async_arc_vector_client`

    Args:
        location:
            If `:memory:` - use in-memory ArcVector instance.
        ...
        api_key: API key for authentication in ArcVector Cloud. Default: `None`
        prefix:
            If not `None` - add `prefix` to the REST URL path.
            Example: `service/v1` will result in `http://localhost:6333/service/v1/{arc_vector-endpoint}` for REST API.
            Default: `None`
        timeout:
            Timeout for REST and gRPC API requests.
            Default: 5.0 seconds for REST and unlimited for gRPC
        host: Host name of ArcVector service. If url and host are None, set to 'localhost'.
            Default: `None`
        path: Persistence path for ArcVectorLocal. Default: `None`
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # we will need to pop them from **kwargs. Otherwise, they might be passed to ArcVectorRemote as httpx kwargs.
        # Httpx has specific set of params, which it accepts and will raise an error if it receives any other params.

        self._client: ArcVectorBase

        if location == ":memory:":
            self._client = ArcVectorLocal(location=location)
        else:
            if path is not None:
                self._client = ArcVectorLocal(location=path)
            else:
                if location is not None and url is None:
                    url = location
                self._client = ArcVectorRemote(
                    url=url,
                    port=port,
                    grpc_port=grpc_port,
    ...
    def close(self, **kwargs: Any) -> None:
        """
        Closes the connection to ArcVector
        """
    ...
    def grpc_collections(self) -> grpc.CollectionsStub:
        ...
        """
        if isinstance(self._client, ArcVectorRemote):
    ...
    def recommend(
        ...
        **kwargs: Any,
        ...
        """Recommend points: search for similar points based on already stored in ArcVector examples.

        Provide IDs of the stored points, and ArcVector will perform search based on already existing vectors.
        ...
        Args:
            ...
            positive:
                ...
                point.
                If there are more than one example, ArcVector will attempt to search for similar to all of them.
    ...
    def recommend_groups(
        ...
        **kwargs: Any,
        ...
        """Recommend point groups: search for similar points based on already stored in ArcVector examples
    ...
    def overwrite_payload(
        ...
        **kwargs: Any,
        ...
        `Set payload`::
            ...
            arc_vector_client.overwrite_payload(
                collection_name="test_collection",
                wait=True,
                payload={
                    "key": "value"
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            location: URL of the snapshot
                ...
                - Local path `file:///arc_vector/snapshots/test_collection-2022-08-04-10-49-10.snapshot`
    ...
    def migrate(
        self,
        dest_client: ArcVectorBase,
        collection_names: Optional[List[str]] = None,
        batch_size: int = 100,
        recreate_on_collision: bool = False,
    ) -> None:
        """Migrate data from one ArcVector instance to another.

        Args:
            dest_client: Destination ArcVector instance either in local or remote mode
