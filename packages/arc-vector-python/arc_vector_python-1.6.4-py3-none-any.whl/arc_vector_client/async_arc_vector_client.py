...
from arc_vector_client.async_client_base import AsyncArcVectorBase
from arc_vector_client.async_arc_vector_fastembed import AsyncArcVectorFastembedMixin
from arc_vector_client.async_arc_vector_remote import AsyncArcVectorRemote
from arc_vector_client.conversions import common_types as types
from arc_vector_client.http import AsyncApiClient, AsyncApis
from arc_vector_client.local.async_arc_vector_local import AsyncArcVectorLocal


class AsyncArcVectorClient(AsyncArcVectorFastembedMixin):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._client: AsyncArcVectorBase
        if location == ":memory:":
            self._client = AsyncArcVectorLocal(location=location)
        elif path is not None:
            self._client = AsyncArcVectorLocal(location=path)
        else:
            if location is not None and url is None:
                url = location
            self._client = AsyncArcVectorRemote(
                url=url,
                port=port,
                grpc_port=grpc_port,
    ...
    def grpc_collections(self) -> grpc.CollectionsStub:
        ...
        """
        if isinstance(self._client, AsyncArcVectorRemote):
