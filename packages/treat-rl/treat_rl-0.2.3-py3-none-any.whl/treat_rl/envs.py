...
class DiseaseTreatmentEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if np.random.rand() < remission_prob:
            ...
            return self._combine(self.current_symptoms, action), reward, True, {"treatment": action, "disease_pre_treatment": self.current_disease, "remission": True}
        ...
        if self.use_disease_rewards:
            reward -= self.diseases[self.current_disease]['base_cost']
        ...
        if self.use_symptom_rewards:
            # Calculate the reward based on the current symptoms
            reward -= (self.current_symptoms.sum() / self.n_symptoms) * (self.remission_reward / self.max_visits)
        ...
        return self._combine(self.current_symptoms, action), reward, terminated, {"treatment": action, "disease_pre_treatment": self.current_disease, "remission": False}

    def sample_symptoms(self):
        # Baseline symptoms for a healthy individual
        baseline_symptom_level = np.random.uniform(0, 0.05, self.n_symptoms)
