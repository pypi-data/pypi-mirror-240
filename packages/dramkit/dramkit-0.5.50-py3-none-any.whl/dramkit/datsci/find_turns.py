...
def find_turns(series: Union[pd.Series, np.ndarray, List[Union[float, int]]],
    ...
    if t_min:
        ...
        def _to_del(df, k1, k2, k3, k4, rev):
            '''
            ...
            if k4 < df.shape[0] and k1 < 0: # 开头位置特殊处理
                ...
                if k3-k2 >= t_max+1: # 间隔大于t_max
                    ...
                    if df.loc[k2, 'label'] == 1:
                        if v2 >= v4: # 若影响前后趋势，保留
                            return False, None
                        elif _max_dif_ok(v2, v3, rev): # 差值满足条件，保留
                            return False, None
                        else:
                            return True, k2
                    ...
                    if df.loc[k2, 'label'] == 1:
                        if v2 >= v4: # 若影响前后趋势，保留
                            return False, None
                        elif _min_dif_ok(v2, v3, rev): # 不满足差值条件，删除
                            return True, k2
                        else:
                            return False, None
                    elif df.loc[k2, 'label'] == -1:
        ...
        def _del_t_min(df, rev):
            ...
            while k2 < df.shape[0]:
                if df.loc[k2, 'label'] == 0:
                    ...
                    to_del, idxs = _to_del(df, k1, k2, k3, k4, rev)
                    if to_del:
                        df.loc[idxs, 'label'] = 0
        ...
        def _check2point(df):
            ...
            _del = False
            if k3-k2 < t_min+1: # 间隔小于t_min
                if not _max_dif_ok(v2, v3, False): # 差值不满足条件，删除
                    df['label'] = 0
                    _del = True
            if not (k3-k2 >= t_max+1): # 间隔大于等于t_min小于t_max
                if _min_dif_ok(v2, v3, False): # 不满足差值条件，删除
                    df['label'] = 0
                    _del = True
            if plot_process:
            ...
            while can_del and k < df.shape[0]:
                ...
                if df.loc[k, 'label'] != 0:
                    ...
                    if k3-k2 >= t_max+1:
                        ...
                        if not _max_dif_ok(v2, v3, rev): # 差值不满足条件，删除
                            df.loc[k2, 'label'] = 0
                        else:
                            can_del = False
                    else: # 间隔大于等于t_min小于t_max
                        if _min_dif_ok(v2, v3, rev): # 不满足差值条件，删除
                            df.loc[k2, 'label'] = 0
                        else:
                            can_del = False
                    if plot_process:
        ...
        # 起止位置单点循环筛除
        df.index = range(df.shape[0])
        df = _del_begin(df, False)
        df.index = range(df.shape[0]-1, -1, -1)
        df = _del_begin(df, True)
