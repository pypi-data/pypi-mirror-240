...
print("Begining LoginInput process...")
...
def get_instance_info_from_server():
    print("Sending POST request to get valid instances")
    response = requests.post(constants.GRAPH_URL, #headers={'x-auth-token': constants.X_AUTH_TOKEN},
                             json={'query': queries.GET_INSTANCES}, verify=False, timeout=10)
    print(response)
...
def get_login_from_user(source_instance_name, target_instance_name, namespace_name):
    ...
    for i in range(2):
        ...
        namespace_input = namespace_name #input("Enter namespaceName: ")
        print(f"The submitted namespace name for {instance_input} is {namespace_name}")
