...
def useMLmodel(argsMLmodel,prog,notes=''):
    ...
    if prog == 'mlatomf':
        ...
        import AIQM1_cmd
        AIQM1_cmd.AIQM1Cls(argsMLmodel).forward()
        #print(charges,mults)
...
# Andersen thermostat (NVT ensemble)
def andersen_thermostat(Natoms,mass,velocity,nu,dt,temp):
    p = nu*dt 
    ...
    for iatom in range(Natoms):
        ...
        if ri < p:
            v_modulus = temp / cal2J / 1000 * R_const / mass[iatom][0] / 1822.888515 / (0.024188432 / bohr2angstrom)**2 / hartree2kcal
            v_modulus = np.sqrt(v_modulus)
            vel[iatom][0] = v_modulus * np.random.randn(1)[0]
            vel[iatom][1] = v_modulus * np.random.randn(1)[0]
            vel[iatom][2] = v_modulus * np.random.randn(1)[0]

    return vel

# Nose-Hoover chain (NHC) half integration step 
# NVT ensemble
def Nose_Hoover_chain2(KE,velocity,dt,xi,vxi,Q,Natoms,T,dist_unit):
    ...
    G2 = (Q[0]*vxi[0]*vxi[0]*abc-avg_en)/Q[1]
    vxi[1] = vxi[1] + G2*dt_4
    vxi[0] = vxi[0] * np.exp(-vxi[1]*dt_8)
    G1 = (2*KE-3*Natoms*avg_en)/Q[0]
    vxi[0] = vxi[0] + G1*dt_4 
    vxi[0] = vxi[0] * np.exp(-vxi[1]*dt_8)

    xi[0] = xi[0] + vxi[0]*dt_2 
    xi[1] = xi[1] + vxi[1]*dt_2 
    s = np.exp(-vxi[0]*dt_2) # Scalar factor
    velocity = velocity * s 
    KE = KE*s**2
...
def Nose_Hoover_chain_old(KE,velocity,dt,xi,vxi,Q,Natoms,T,dist_unit):
