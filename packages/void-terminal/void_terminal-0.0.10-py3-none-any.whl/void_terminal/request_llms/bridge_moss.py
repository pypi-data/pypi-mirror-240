...
load_message = "MOSS has not been loaded yet，Loading takes some time。Attention，Depending on`config.py`Configuration，MOSS consumes a lot of memory（CPU）Or video memory（GPU），May cause low-end computers to freeze..."
...
class GetGLMHandle(Process):
    ...
    def check_dependency(self): # Main process execution
        try:
            ...
            self.info = """
            Lack of dependencies for MOSS，If you want to use MOSS，In addition to the basic pip dependencies，You also need to run`pip install -r request_llms/requirements_moss.txt`and`git clone https://github.com/OpenLMLab/MOSS.git request_llms/moss`Install dependencies for MOSS。
            """
            self.success = False
        return self.success
    ...
    def moss_init(self): # Subprocess execution
        ...
        # This code is from https://github.com/OpenLMLab/MOSS/blob/main/moss_cli_demo.py
        import argparse
        import os
        import platform
        import warnings
    ...
    def run(self): # Subprocess execution
        ...
        try:
            ...
            self.child.send('[Local Message] Call MOSS fail, unable to load MOSS parameters normally。')
            raise RuntimeError("Unable to load MOSS parameters normally!")
