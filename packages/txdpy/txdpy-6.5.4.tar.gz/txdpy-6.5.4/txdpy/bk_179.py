...
def optstr(s):
    if type(s)==str:
        ...
        return [str(s).strip().strip('.').strip('。').replace('（', '(').replace('）', ')').replace('\t', '').replace('\n', '').replace(
            '\r', '').replace(' ', '').replace('，', ',').replace('：', ':').replace('；',';').replace('\xa0', ' ') for s in s]
    else:
        return s
...
def gen_excel(data:list,save_path,header=None):
    ...
    for row_index, row_data in enumerate(data):
        ...
        for col_index, cell_data in enumerate(row_data):
            sheet.write(row_index, col_index, optstr(str(cell_data) if cell_data else cell_data))
