...
from .util import *
from ..util import apply_tags_to_audio

from typing import Union
import httpx, logging, tempfile, io, aiofiles
...
class Deezer:
    def __init__(self, arl, debug) -> None:
        self.arl = arl
                ...
                for key in fallback_qualities:
                    ...
                    async with httpx.AsyncClient(headers=networking_settings.HTTP_HEADERS, cookies=await self.get_cookies()) as client:
                        ...
                        if not fallback or (response.status == 200 and int(response.headers.get("Content-Length", 0)) > 0):
                            return (url, key) 
    async def download_track(self, track, download_dir=None, quality=None, fallback=True, filename=None, 
                             renew=False,with_metadata=True, with_lyrics=True, 
                             tag_separator=", ", **kwargs):
        ...
        title = tags["title"]
        artist = tags['artist']
        if self.debug: logging.info(f'[DEEZER]: Download started [{artist} - {title}]')
                ...
                async with aiofiles.open(download_path, 'wb') as f:
                    ...
                    for chunk in data_iter:
                        try:
                            ...
                            if self.debug: logging.error(f"[DEEZER] DL-ERROR: (bad chunk) - {type(chunk)}")
                        i += 1
            else:
                audio = io.BytesIO()
                audio.seek(0)
        ...
        if with_metadata:
            if self.debug: logging.info(f'[DEEZER]: Tag editing [{artist} - {title}]')
            if ext.lower() == ".flac":
                pass #self._write_flac_tags(download_path, track, tags=tags)
            else:
                if download_dir:
                    await self._write_mp3_tags(audio=download_path, 
                                               track=track, tags=tags)
                else:
                    audio = await self._write_mp3_tags(audio=audio, 
                                               track=track, tags=tags, return_bytes=True)
        ...
        return {
            ...
            'lyric': lyric_check[1] if with_lyrics and lyric_check[0] else None
        }
    ...
    """
        TAGS
    """
    async def _write_mp3_tags(self, audio:Union[str, io.BytesIO], track, tags=None, return_bytes:bool=False):
        track = track["DATA"] if "DATA" in track else track

        if not tags:
            tags = await self.get_track_tags(track)

        _task = await asyncio.get_event_loop().run_in_executor(None, 
                                    lambda: apply_tags_to_audio(audio, tags, return_bytes))
        result = await _task
        return result
