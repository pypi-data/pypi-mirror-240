...
def get_args():
    ...
    # Subparser for listing avaialble off-the-shelf workflows
...
def list_ots_workflows(args):
    ...
    group = gl.groups.get(76478892, include_subgroups=True)
...
def get_ots(args):
    ...
    """
    raft_cfg = load_raft_cfg()                                                                      
    local_ots = pjoin(raft_cfg['filesystem']['projects'], args.project_id, args.workflow) 
    ots_repo = 'https://gitlab.com/landscape-of-effective-neoantigens-software/nextflow/workflows'
    Repo.clone_from(pjoin(ots_repo, args.workflow, "{}-{}".format(args.workflow, args.species)),
                    pjoin(local_ots),
                    branch=args.branch)
    return(local_ots)
...
def load_ots_ref(args, ref_url, ots_ref_dir, postproc):
    ...
    try:
        ...
        if ref in postproc.keys() and postproc[ref] == 'gunzip':
            subprocess.run("gunzip {}".format(pjoin(ots_ref_dir, ref)), shell=True, check=False) 
        elif ref in postproc.keys() and postproc[ref] == 'tar -xvf':
            subprocess.run("tar -xvf {}".format(pjoin(ots_ref_dir, ref)), shell=True, check=False) 
           ...
           if line_idx == 0:
               ...
               for required_column in required_columns:
                   if required_column not in line:
                       sys.exit("Missing column {} from the header. Add this column and re-run.".format(required_column)) 
               for nice_to_have_column in nice_to_have_columns:
                   if nice_to_have_column not in line:
                       print("Missing column {} from the header. This may be needed for some workflows".format(nice_to_have_column)) 
...
def main():
    ...
    if 'project_id' in args and args.command not in ['init-project', 'load-project', 'copy-parameters', 'run-ots', 'available-workflows']:
