...
def format_json_key_values(json_data, indent=0):
    ...
    for key, value in json_data.items():
        ...
        # If the value is a dictionary (nested JSON), recursively format its key-value pairs
        if isinstance(value, dict):
            formatted_string += format_json_key_values(value, indent + 1)
        else:
            # Append the value with additional indentation
            formatted_string += '    ' * (indent + 1) + str(value) + "\n"
