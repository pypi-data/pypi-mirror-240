...
class SearchString(Generic[const.Data]):
    ...
    def _any_match(
        ...
        return re_pattern in text.lower() if isinstance(re_pattern, str) else bool(re_pattern.search(text))
    ...
    def match(self, text: str | list[str]) -> bool:
        ...
        False
        """
        return self._match_sentence(text) if isinstance(text, str) else self._match_sentences(text)
    ...
    def matched_text_highlighted(self) -> str | None:
        ...
        return ' '.join([self.highlight(sentence) for sentence in self.matched_sentences])
