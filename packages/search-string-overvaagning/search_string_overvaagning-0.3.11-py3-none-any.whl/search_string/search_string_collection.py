...
class Trie(Generic[Data]):
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, Trie) and self.sub_tries == other.sub_tries and self.end_tokens == other.end_tokens
...
class SearchStringCollection(Generic[Data]):
    ...
    def _match_sentences(self, sentences: list[str]) -> list[SearchString[Data]]:
        ...
        global_matched = {
            (ss_id, part_id) for ss_id, part_id in global_matched_raw if self.search_strings[ss_id].is_global(part_id)
        }
    ...
    def find_all(self, text: str | list[str]) -> list[SearchString[Data]]:
        ...
        """
        return self._match_sentences(text) if isinstance(text, list) else self._match_sentence(text)
    ...
    def find_all_sliding_window(
        ...
        *,
        ...
        []
        """
        return self.find_all([text[start_idx : start_idx + window_size] for start_idx in range(0, len(text), step_size)])
