...
def test_waterstandlocationset(loc_sets):
    ...
    assert loc_sets.waterstand_loc.csv_filename == expected_csvfile
    ...
    assert isinstance(loc_sets.waterstand_loc.geo_df_original, pd.DataFrame)
    assert LocSetSharedCols.z in loc_sets.waterstand_loc.geo_df_original
    assert not loc_sets.waterstand_loc.geo_df_original[LocSetSharedCols.z].hasnans
