...
# def conv_legend_label(label_def1,replace_V_for_Vac_in,legend_KV_in):
def conv_legend_label(label_def1,str_for_Vac_in,legend_KV_in):
    if label_def1=="Hole":
        if legend_KV_in:
        ...
        for t1 in label_def2.split("+"):
            t2=t1.split("_")
            # if t2[0]=="Vac" and replace_V_for_Vac_in:
            if t2[0]=="Vac":
                t2[0]=str_for_Vac_in
        ...
        ##### Space-filling adjustment: \! = -3/18em, \; = +5/18em,  \: = +4/18em, \, =+3/18 em
        if legend_KV_in:
            label_charge="?"
...
def plot_Edef_eFermi(filename_in,root_outfiles,plot_params_in={}):
    ...
    for lab1 in labels[2:]:
        ...
        leg1,leg1_noQ=conv_legend_label(lab1,plot_params_in["str_replacing_Vac"],plot_params_in["legend_KV"])
        dict1["label_for_legend"]=leg1_noQ
        dict1["label_for_legend_withQ_withID"]=leg1
        dict1["label_withoutQ"]=leg1_noQ
        """
        if "+" in def_label1:
            def_label3="["
...
def plot_defect_densities(plot_params_in={}):
    ...
    if not "dens_ylabel" in plot_params_in.keys():
        if plot_params_in["dens_unit_cm3"]:
            ...
            fout_sc=open(root_outfiles+"_scaleFactor_plotDens.txt","w")
            fout_sc.write("   Scale factor for plotting = "+str(plot_params_in["dens_scale"])+"\n")
            fout_sc.write("     Size-dependent quantities (e.g. densities) in summarized plots  are devided by this value.\n")
            fout_sc.close()
    if not "Edef_x_lower_limit" in plot_params_in.keys():
        plot_params_in["Edef_x_lower_limit"]=0.0
    if not "Edef_x_upper_limit" in plot_params_in.keys():
    ...
    for ieq1,eq1 in enumerate(eq_conditions):
        ...
        for k1,d1 in datalist1.items():
            ...
            leg1,leg1_noQ=conv_legend_label(label_def1,plot_params_in["str_replacing_Vac"],plot_params_in["legend_KV"])
...
#######################################################################################
        ...
        if dens_highest2<plot_params_in["dens_y_lower_limit"]:
            ...
            print(f"    dens_y_lower_limit = {dens_lowest1}")
            # plot_params_in["dens_y_lower_limit"]=dens_highest/10.0
            # plot_params_in["dens_y_upper_limit"]=dens_highest/10.0
        defects_list_screened={}
        for k1,d1 in datalist4.items():
            #print(k1,d1)
            #if k1.find("density_")==0:
