...
class TestInputValidation:
    ...
    def test_chat_completion_correct_input(self, mocked_request):
        ...
        mocked_request.assert_called_once_with(expected_model_input)
    ...
    def test_chat_completion_wrong_input(self, mocked_request):
        ...
        with pytest.raises(FoundationModelAPIException) as error:
            ChatCompletion.create(model=CHAT_COMPLETION_MODEL_NAME + "-fake")
        assert "validation error" in str(error.value), "wrong model name should raise validation error"
    ...
    def test_completion_wrong_input(self, mocked_request):
        ...
        with pytest.raises(FoundationModelAPIException) as error:
            Completion.create(model=COMPLETION_MODEL_NAME + "-fake")
        assert "validation error" in str(error.value), "wrong model name should raise validation error"
    ...
    def test_embedding_wrong_input(self, mocked_request):
        ...
        with pytest.raises(FoundationModelAPIException) as error:
            Embedding.create(model=EMBEDDING_MODEL_NAME + "-fake")
        assert "validation error" in str(error.value), "wrong model name should raise validation error"
