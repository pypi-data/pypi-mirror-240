        ...
        for index, column in enumerate(table_columns):
            ...
            if is_number_type(column, type_mapping):
                ...
                if (source.name.lower() == 'clickzetta' or source.name.lower() == 'doris'
                        or source.name.lower() == 'mysql' or source.name.lower() == 'postgresql'):
        ...
        for index, column in enumerate(table_columns):
            ...
            if is_number_type(column, type_mapping):
                ...
                if destination.name.lower() == 'clickzetta' or destination.name.lower() == 'doris' \
                        or destination.name.lower() == 'mysql' or destination.name.lower() == 'postgresql':
...
def data_diff_validation(source_table: str, destination_table: str, source: Source, destination: Destination):
    try:
        source_db_url = None
        destination_db_url = None
        source_db_type = source.name.lower()
        destination_db_type = destination.name.lower()
        if source_db_type == 'postgresql' or destination_db_type == 'postgresql':
        ...
        elif source_db_type == 'mysql' or destination_db_type == 'mysql':
            if source_db_type == 'mysql':
                config = source.get_connection_params()
                source_db_url = f"mysql://{config['user']}:{config['password']}@{config['host']}:{config['port']}/{config['database']}"
            if destination_db_type == 'mysql' or destination_db_type == 'doris':
                config = destination.get_connection_params()
                destination_db_url = f"mysql://{config['user']}:{config['password']}@{config['host']}:{config['port']}/{config['database']}"
        elif source_db_type == 'clickzetta' or destination_db_type == 'clickzetta':
            if source_db_type == 'clickzetta':
                config = source.get_connection_params()
            ...
            if destination_db_type == 'clickzetta':
                config = destination.get_connection_params()
            ...
            raise Exception(f"Unsupported database type {source_db_type} or {destination_db_type} in data-diff-validation")
        ...
        return diff_tables(source_diff_table, destination_diff_table, algorithm=Algorithm.HASHDIFF)
    except Exception as e:
