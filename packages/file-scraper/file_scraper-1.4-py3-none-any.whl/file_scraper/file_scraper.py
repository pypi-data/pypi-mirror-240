...
import datetime, sys, os, json, jq, regex as re, base64, threading, concurrent.futures, subprocess, urllib.parse, bs4
...
def validate_template_decode(obj):
	...
	for key in obj:
		if subkey in obj[key]:
			if not isinstance(obj[key][subkey], str) or len(obj[key][subkey]) < 1 or obj[key][subkey].lower() not in ["base64", "url"]:
				obj[key].pop(subkey, None)
			else:
...
class FileScraper:
	...
	def run(self):
		...
		if not self.__results:
			...
			self.__results = jquery(self.__results, "sort_by(.file | ascii_downcase)[]")
	...
	def __grep(self, res):
		...
		for key in self.__template:
			...
			try:
				response = res.decode(self.__res_enc).replace("\\n", "\n")
	...
	def __validate_matched(self, matched, subtemplate):
		...
		for match in matched:
			...
			if "decode" in subtemplate:
				try:
					if subtemplate["decode"] == "base64":
						decoded = base64.b64decode(match).replace(b"\x00", b"").replace(b"\r", b"").decode(self.__url_enc).strip()
						if decoded:
							tmp["decoded"].append(decoded)
					elif subtemplate["decode"] == "url":
						decoded = urllib.parse.unquote(match, encoding = self.__url_enc).replace("\x00", "").replace("\r", "").strip()
						if decoded:
							tmp["decoded"].append(decoded)
				except Exception:
					continue
	...
	def __set_html_content(self):
		...
		for key in self.__template:
			if "collect" in self.__template[key]:
				tmp = []
				for array in jquery(self.__results, (".[].matched.\"{0}\" | select(. != null)").format(key)):
					tmp.extend(array)
				if tmp:
...
class Validate:
	...
	def __basic(self):
		...
		print("File Scraper v1.4 ( github.com/ivan-sincek/file-scraper )")
		print("")
	...
	def __advanced(self):
		...
		print("    Specify 'default' to use a built-in list")
	...
	def __parse_excludes(self, value, special):
		...
		for entry in value.split(","):
			...
			if not entry:
				...
				tmp = [const + e for e in ["car", "css", "gif", "jpeg", "jpg", "mp3", "mp4", "nib", "ogg", "otf", "png", "storyboard", "strings", "svg", "ttf", "webp", "woff", "woff2", "xib"]]
				break
			else:
	...
	def __validate(self, key, value):
		...
		if len(value) > 0:
			...
			# --------------------
				...
				if os.path.isdir(self.__args["template"]):
					self.__error("Template file is a directory")
				elif os.path.isfile(self.__args["template"]):
	...
	def run(self):
		...
		# --------------------
			...
			if self.__args["directory"] is None or self.__args["out"] is None or not self.__check(argc):
				self.__error("Missing a mandatory option (-d, -o) and/or optional (-t, -e, -i, -th)", True)
		# --------------------
		else:
			self.__error("Incorrect usage", True)
		# --------------------
