...
class QuantumUnsignedInteger(QuantumNumeric):
    kind: Literal["quint"]
...
class QuantumFixedRealBase(QuantumNumeric):
    fraction_places: Expression = pydantic.Field()


class QuantumFixedReal(QuantumFixedRealBase):
    kind: Literal["qfixed"]
...
class QuantumUnsignedFixedReal(QuantumFixedRealBase):
...
def register_info_to_quantum_type(reg_info: RegisterArithmeticInfo) -> QuantumType:
    result: QuantumType
    if reg_info.fraction_places > 0:
        fraction_places = Expression(expr=f"{reg_info.fraction_places}")
        if reg_info.is_signed:
            result = QuantumUnsignedFixedReal(fraction_places=fraction_places)
        else:
            result = QuantumUnsignedFixedReal(fraction_places=fraction_places)
    else:
        if reg_info.is_signed:
            result = QuantumUnsignedInteger()
        else:
            result = QuantumUnsignedInteger()
...
def quantum_var_to_register(name: str, qtype: QuantumType) -> RegisterUserInput:
    ...
    fraction_places = (
        ...
        if isinstance(qtype, QuantumFixedRealBase)
        and qtype.fraction_places.is_evaluated()
        else 0
    )
