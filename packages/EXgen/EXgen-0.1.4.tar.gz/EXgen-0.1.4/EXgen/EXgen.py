...
class EXgen:
    ...
    def _init_task_variables(self) -> None:
        for i_task in range(self.n_tasks):
            ...
            if "netlist_pool" in self.tasks[i_task].keys():
                netlist_ind = self.tasks[i_task]["group_netlist_pool_map"][self.group_ind]
                self.tasks[i_task]["netlist"] = self.tasks[i_task]["netlist_pool"][netlist_ind]
    ...
    def _extend_tasks_list(self) -> None:
        ...
        for i_task in range(self.n_tasks):
            ...
            if "netlist_pool" in self.tasks[i_task].keys():
                ...
                self.tasks[i_task]["group_netlist_pool_map"] = np.random.randint(0, n_netlists, self.n_groups)
    ...
    def _val_to_latex(self, val, precision, resmode=False):
        ...
        if resmode: # do from first significant place : 
            ...
            signed_precision = np.log10(np.abs(val))
    ...
    def _move_pdfs(self):
        ...
        files = os.listdir()
        pdf_files = [f for f in files if f[-3:] == "pdf"]

        for f in pdf_files:
            shutil.move(f, f"{self.pdf_dir}{f}")
