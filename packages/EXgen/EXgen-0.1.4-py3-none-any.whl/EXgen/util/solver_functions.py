...
def electric_field_1(task):
    ...
    task["insert_figpaths_solution"].append(figpath)
...
def electric_field_mpq_1(task):
    ...
    plot_e_field_and_charges(xx, yy, E, Plist, task["insert_figpaths"][0])
...
def norton_equivalent(task):
    ...
    net_full.draw(task["insert_figpaths"][0], style="european")
...
def thevenin_helmholtz_one_v_one_i(task):
    ...
    equations = {"Uth_1": net_th_1.V1.V.expr['dc'],
                 "Uth_2": net_th_2.V1.V.expr['dc'], 
                 "Ri":  net_th.Z1.R.expr,
                 }
    ...
    solutions["URL"] = solutions["Uth"]*(solutions["Ri"]/(solutions["Ri"] + task["variables"]["RL"]))

    solutions["IRL"] = solutions["Uth"]/solutions["Ri"]
