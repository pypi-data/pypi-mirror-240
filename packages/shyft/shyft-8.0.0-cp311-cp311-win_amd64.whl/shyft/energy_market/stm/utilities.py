...
from shyft.energy_market.core import XyPointCurve, XyPointCurveWithZ, TurbineOperatingZone, TurbineDescription, XyPointCurveWithZList
...
def create_t_turbine_description(t0: time, efficiency_curves: List[XyPointCurveWithZ]) -> t_turbine_description:
    ...
    te = TurbineOperatingZone(efficiency_curves)
...
def create_t_turbine_description_pelton(t0: time,
                                        turbine_operating_zones: List[TurbineOperatingZone]) -> t_turbine_description:
    """ to ease construction """
    r = t_turbine_description()
    td = TurbineDescription(turbine_operating_zones)
    r[t0] = td
    return r
