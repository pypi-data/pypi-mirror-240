...
class ClimRCatData(UserData):
    ...
    def __init__(
        ...
        *,
        ...
        self._gdf, self._gdf_bounds = _get_shp_file(shp_file=self._gdf, cat_grid=self._cat_grid, is_degrees=is_degrees)
    ...
    def prep_interp_data(self, key: str, poly_id: int) -> AggData:
        ...
        bounds = np.asarray(bbox.buffer(2 * max(x_cell_size, y_cell_size)).bounds)  # type: ignore
...
class ODAPCatData(UserData):
    ...
    def __init__(
        ...
        *,  # all arguments below must be defined as a keyword argument
        ...
        logger.info("  - checking latitude bounds")
        is_intersect, is_degrees, is_0_360 = _check_for_intersection(cat_params=cp, cat_grid=cg, gdf=self._gdf)
        self._gdf, self._gdf_bounds = _get_shp_file(shp_file=self.f_feature, cat_grid=cg, is_degrees=is_degrees)
        self._check_id_feature()
        self._rotate_ds = bool((not is_intersect) & is_degrees & (is_0_360))
