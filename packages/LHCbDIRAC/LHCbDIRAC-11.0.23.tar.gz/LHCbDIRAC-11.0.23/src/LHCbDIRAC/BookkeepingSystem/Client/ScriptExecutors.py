...
def executeFileMetadata(dmScript):
    ...
    for lfn in lfnMetadata:
        ...
        if full:
            gLogger.notice(f"{sep}{'FileName'.ljust(lenItem)} {lfn}")
            sep = "\n"
...
def executeFilePath(dmScript):
    ...
    if groupBy and groupBy not in dirMetadata:
        ...
        progressBar = ProgressBar(
            len(lfnList), title=f"Getting {groupBy} from BK for {len(lfnList)} files", chunk=chunkSize
        )
        ...
        if summary:
            ...
            for groupStr in paths["Successful"]:
                pathSummary["Successful"][groupStr] = f"{len(paths['Successful'][groupStr])} files"
            if paths["Failed"]:
        ...
        chunkSize = 2
        progressBar = ProgressBar(
            len(directories), title=f"Getting metadata for {len(directories)} directories", chunk=chunkSize
        )
        success = {}
        failed = set()
        ...
        for dirName in success:
            ...
            if full:
                ...
                if groupBy in bkDict:
                    ...
                    gLogger.notice(f"Invalid metadata item: {groupBy}")
                    gLogger.notice(f"Available are: {str(list(bkDict))}")
                    diracExit(1)
                else:
        ...
        if groupBy:
            if summary:
                ...
                if failed:
                    pathSummary["Failed"] = {
                        path: f"Directory not in BK ({len(directories[path])} files)" for path in failed
                    }
                else:
    ...
    if printList:
        ...
        gLogger.notice(",".join(sorted(item.replace(f"{groupBy} ", "") for item in res["Value"]["Successful"])))
...
def executeFixLuminosity(dmScript):
    ...
    for evtType in evtTypes:
        gLogger.notice(f"**** Event type {evtType}")
        ...
        if not runChecked:
            ...
            if runNotFinished:
                gLogger.notice(f"Found {len(runNotFinished)} runs that are not Finished: {','.join(runNotFinished)}")
            runFinished = sorted(
                run for run in res["Value"]["Successful"] if res["Value"]["Successful"][run]["Finished"] == "Y"
            )
            if runFinished:
                gLogger.notice(f"Found {len(runFinished)} runs that are Finished")
            else:
        ...
        for run in runFinished:
            ...
            if doIt:
                if result is not None:
                    gLogger.notice(f"Update done {'with errors' if result else 'successfully'}")
            ...
            if full:
                ...
                for lfn in okResult:
                    fullResult["Value"].setdefault("Successful", {})[lfn] = {
                        desc: f"Replica-{meta['GotReplica']}" for desc, meta in okResult[lfn].items()
                    }  # can be an iterator
...
def executeGetFiles(dmScript, maxFiles=20):
    ...
    if output:
        fd.close()
        gLogger.notice(f"\nList of {len(fileDict)} files saved in", output)
    ...
    if optionsFile:
        ...
        gLogger.notice(f"\n{len(fileDict)} files in options file {optionsFile}")
...
def executeFileSisters(dmScript, level=1):
    ...
    if not prod:
        ...
        for dirName in directories:
            ...
            if not prod:
                gLogger.error(f"Error: could not get production number for {dirName}")
            else:
                    ...
                    if sister not in lfns and (not sameType or metadata["FileType"] in (lfnTypes[lfn] for lfn in lfns)):
                        ...
                        if full:
                            ...
                            resValue[resItem].setdefault(allLfns, {}).update(
                                {sister: f"Replica-{metadata['GotReplica']}"}
                            )
                        found = True
                for lfn in lfns:
                    if not found:
...
def executeGetStats(dmScript):
    ...
    for prod in prodList:
        ...
        if prod:
            ...
            gLogger.notice(f"For {len(lfns)} LFNs:")
        elif bkQuery:
            ...
            if len(processingPasses) > 1:
                ...
                gLogger.notice(f"   and {len(processingPasses)} processing passes :", ", ".join(processingPasses))
            else:
                ...
                for processingPass in processingPasses:
                    ...
                    for item in info:
                        ...
                        try:
                            ...
                            gLogger.exception(f"Exception for {str(list(metadata))}", lException=e)
                progressBar.endLoop()
            if lfns:
                    ...
                    if res["OK"]:
                        for lfn, metadata in res["Value"]["Successful"].items():  # can be an iterator
                            try:
                                ...
                                gLogger.exception(f"Exception for {lfn}", str(list(metadata)), lException=e)
                    else:
        ...
        for name, value in zip(paramNames, records):
            if name == "NbofFiles":
                ...
                size = value
                sizePerEvt = f"({size / nevts / 1000.0:.1f} kB per evt)" if nevts and nDatasets == 1 else ""
        ...
        if triggerRate:
            ...
            if runs:
                ...
                gLogger.notice(f"{'Total Luminosity'.ljust(tab)}: {totalLumi:.3f} {lumiUnit}")
                gLogger.notice(f"{'Run duration'.ljust(tab)}: {fullDuration:.2f} hours ({len(runs)} runs)")
                gLogger.notice(f"{'Trigger rate'.ljust(tab)}: {rate}")
                rate = (
                    f"{size / 1000000.0 / fullDuration / 3600.0:.1f} MB/second"
                    if fullDuration
                    else "Run duration not available"
                )
                gLogger.notice(f"{'Throughput'.ljust(tab)}: {rate}")
                collBunches = 0.0
                result = {}
                ...
                if fullDuration and collBunches:
                    ...
                    gLogger.notice(f"{'Colliding bunches'.ljust(tab)}: {collBunches:.1f} on average")
                    gLogger.notice(f"{'Trigger per bunch'.ljust(tab)}: {triggerRate / collBunches:.2f} events/s/bunch")
                if listFills:
                    gLogger.notice(
                        "List of fills: ",
                        ",".join(
...
def executeRunInfo(item):
    ...
    if not getRanges:
        ...
        timeGap = 365
    if activity:
        progressBar = ProgressBar(1, title=f"Getting run list for activity {activity}", step=20)
        runsList = BKQuery(f"/LHCb/{activity}//Real Data/RAW").getBKRuns()
        progressBar.endLoop(message=f"Obtained {len(runsList)} runs")
        if runsDict:
            runsList = sorted(set(runsList) & set(runsDict["RunNumber"]))
            gLogger.notice(f"Only {len(runsList)} runs in range {runRange}")
    elif not runsDict:
        Script.showHelp()
        diracExit(1)
    else:
        ...
        runsList.remove(0)
        if len(runRange) < 30:
            progressBar = ProgressBar(1, title=f"Getting runs for run range {runRange} ", step=20)
        else:
            progressBar = ProgressBar(1, title=f"Getting runs for {len(runsList)} runs ", step=20)
    ...
    if dqFlag:
        nruns = len(runsList)
        chunkSize = 10
        progressBar = ProgressBar(len(runsList), title=f"Getting DQFlag for {len(runsList)} runs ", chunk=chunkSize)
        ...
        if len(runsList) != nruns:
            gLogger.notice(f"Only {len(runsList)} runs have DQ flag in {','.join(dqFlag)}")
    ...
    if getRanges and force:
        ...
        counted = "files"
        runDict = {}
        progressBar = ProgressBar(len(runsList), title=f"Getting {item} for {len(runsList)} runs ", step=20)
        ...
        for run in runList:
            ...
            if runValue == itemValue and firstRun is None:
                ...
                if lastRun != firstRun:
                    ...
                    rangeStr = f"{firstRun}:{lastRun}"
                else:
                    rangeStr = f"{firstRun}"
                if lastRunDesc:
                    rangeStr += f" ({lastRunDesc})"
                if getRanges:
                    rangesDict[rangeStr] = f"{count} {counted}"
                    # Initialize count of files
                    count = 0
                else:
...
def _getJobsEISFromAncestors(lfnList):
    ...
    gLogger.verbose(f"\nGet EIS for {len(jobDict)} jobs like {str(list(jobDict.items())[0])}")
    gLogger.verbose(f"\t{len(set(lfnList))} unique files")
    ancToCheck = set()
    ancestors = {}
    topFiles = 0
...
def _getEventInputStat(lfns):
    ...
    if jobLfns:
        progressBar = ProgressBar(len(jobLfns), title=f"Getting ancestors for {len(jobLfns)} jobs", chunk=1)
...
def executeRejectionStats(dmScript):
    ...
    for stream in streams:
        ...
        if jobLfns:
            if not evtInputStatOK:
                ...
                progressBar = ProgressBar(len(lfns), title=f"Getting metadata for {len(jobLfns)} jobs", chunk=chunkSize)
    ...
    if multiStream:
        gLogger.notice(f"*** WARNING *** {len(fileTypes)} streams with as many jobs as files, rescaled input stat")
    ...
    for stream in streams:
        ...
        if len(fileTypes) > 1:
            gLogger.notice(f"For {stream} stream{'' if byStream else 's'}:")
        gLogger.notice(tab + "Event stat: %d on %d files" % (eventStat, len(lfnsByStream[stream])))
        if prInput:
            gLogger.notice(
                tab + "EventInputStat: %d" % eventInputStat,
                f"from {len(jobs[stream])} jobs" if jobs[stream] != lfnsByStream[stream] else "",
            )
        gLogger.notice(tab + f"Retention: {100.0 * eventStat / eventInputStat:.2f} %")
