...
class Network:
    ...
    def getServer(self, type):
        if type == "api":
            ...
            return f"https://messengerg2c{randint(2, 6)}.iranlms.ir"
        else:
    ...
    def request(self, method:str, input:dict={}, tmpSession:bool=False, apiVersion:int=6):
        ...
        while True:
            ...
            if result["status"] == "OK":
                if not self.sessionData: result["data"]["tmp_session"] = self.crypto.auth
                return result["data"]
    ...
    def upload(self, file:str and bytes, fileName:str=None, chunkSize:int=131072):
        ...
        if isinstance(file, str):
            if Tools.checkLink(url=file):
                file:bytes = self.http.request("GET", file).data
                mime:str = Tools.getMimeFromByte(file)
            ...
            mime = Tools.getMimeFromByte(file)
