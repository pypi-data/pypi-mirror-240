...
class Formatter:
    def __init__(self, workdir=DOWNLOAD_FOLDER, file_path=None):
    ...
    async def osm2osw(self) -> Response:
        convert = OSM2OSW(pbf_file=self.file_path, workdir=self.workdir, prefix='final')
        result = await convert.convert()
        self.generated_files = result.generated_files
        return result

    def osw2osm(self) -> Response:
        convert = OSW2OSM(zip_file_path=self.file_path, workdir=self.workdir, prefix='final')
