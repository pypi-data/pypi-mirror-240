...
def emailSend(title, context, mane_mails: list or tuple = ('1321443305@qq.com',), **kwargs):
    sender = Sender('1650444933@qq.com', 'uqymsjljcoxhccbe',
                    'smtp.qq.com', True, encoding='utf-8')
    return customize_emailSend(sender, title, context, mane_mails, **kwargs)


# 报错通知装饰器
def raiseEmailFunc(title, mane_mails: list or tuple = ('1321443305@qq.com',)):
    def temp1(func):
        def temp(*args, **kwargs):
            try:
                return func(*args, **kwargs)
            except Exception as e:
                emailSend(title, traceback.format_exc(), mane_mails=mane_mails)
                raise e
