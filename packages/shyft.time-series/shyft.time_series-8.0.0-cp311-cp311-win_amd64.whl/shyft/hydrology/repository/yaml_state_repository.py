...
import yaml
from shyft.hydrology.repository.interfaces import StateInfo, StateRepository
    ...
    def _load_state_from_yaml_file(self, filename):
        def yaml_load(f):
            if '5.0.0' < yaml.__version__ <= '5.1.2':
                return yaml.full_load(f)  # nosec
            elif yaml.__version__ >= '5.2':
                return yaml.load(f, yaml.Loader)  # nosec
            return yaml.load(f)  # nosec
