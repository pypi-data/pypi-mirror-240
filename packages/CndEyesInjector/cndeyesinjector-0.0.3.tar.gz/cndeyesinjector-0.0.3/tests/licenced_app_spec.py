...
file_path_base = "licenced_apps"
with description('LicencedApp') as self:
    with before.each:
        unstub()
        ...
        self.instance = self.instance_parent.licenced_app

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._licenced_app.LicencedApp)).to(equal(True))
    ...
    with context("get", "get"):
        with it("shoud get"):
            with Mocker(real_http=True) as m:
                ...
                expect(result).to(equal([{'id': 530, 'name': 'Jedha', 'max': 123, "renew_date": "2023-01-01", "renew_before": "2023-01-01", 'active': False}]))
    ...
    with context("create", "create"):
        with it("shoud create"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.create(self.instance.data("Abcd", "Vador", "2024-05", "2024-01", True))
    ...
    with context("patch", "patch"):
        with it("shoud update"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.update(id, self.instance.data("----", "Vador", "2024-05", "2024-01", True))
