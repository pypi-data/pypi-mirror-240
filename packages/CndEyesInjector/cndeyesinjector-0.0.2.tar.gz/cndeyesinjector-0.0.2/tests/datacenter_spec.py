...
file_path_base = "datacenters"
with description('Datacenter') as self:
    with before.each:
        unstub()
        ...
        self.instance = self.instance_parent.datacenter

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._datacenter.Datacenter)).to(equal(True))
    ...
    with context("get", "get"):
        with it("shoud get"):
            with Mocker(real_http=True) as m:
                ...
                expect(result).to(equal([{'id': 530, 'name': 'Jedha'}]))
    ...
    with context("create", "create"):
        with it("shoud create"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.create(self.instance.data("Abcd"))
