...
file_path_base = "services"
with description('Service') as self:
    with before.each:
        unstub()
        ...
        self.instance = self.instance_parent.service

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._service.Service)).to(equal(True))
    ...
    with context("get", "get"):
        with it("shoud get"):
            with Mocker(real_http=True) as m:
                ...
                expect(result).to(equal([{'id': 530, 'name': 'Jedha', "app_id": 123, "description": "ma description", "organization_id": 456}]))
    ...
    with context("create", "create"):
        with it("shoud create"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.create(self.instance.data("Abcd", "Vador", 123, "ma description", 456))
    ...
    with context("patch", "patch"):
        with it("shoud update"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.update(id, self.instance.data("----", "Vador", 123, "ma description", 456))
