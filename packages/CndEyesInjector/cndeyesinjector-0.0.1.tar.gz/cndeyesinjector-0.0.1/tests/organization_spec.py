...
with description('Organization') as self:
    with before.each:
        ...
        self.instance = self.instance_parent.organization

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._organization.Organization)).to(equal(True))
    ...
    with context("get", "get"):
        with it("shoud get"):
            with Mocker(real_http=True) as m:
                ...
                expect(result).to(equal([{'id': 530, 'name': 'Jedha', 'guid': 'guid123', 'active': True}]))
    ...
    with context("create", "create"):
        with it("shoud create"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.create(self.instance.data("Abcd", 'guid', True))
    ...
    with context("patch", "patch"):
        with it("shoud update"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.update(id, self.instance.data("----", 'guid', True))
