...
file_path_base = "environments"
with description('Environment') as self:
    with before.each:
        unstub()
        ...
        self.instance = self.instance_parent.environment

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._environment.Environment)).to(equal(True))
    ...
    with context("create", "create"):
        with it("shoud create"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.create(self.instance.data("Abcd", 'dev', 'nonprod', 'domain.com', True))
    ...
    with context("patch", "patch"):
        with it("shoud update"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.update(id, self.instance.data("----", 'dev', 'nonprod', 'domain.com', True))
