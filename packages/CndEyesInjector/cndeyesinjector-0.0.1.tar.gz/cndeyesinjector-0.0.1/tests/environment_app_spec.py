...
file_path_base = "environment_apps"
with description('EnvironmentApp') as self:
    with before.each:
        unstub()
        ...
        self.instance = self.instance_parent.environment_app

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._environment_app.EnvironmentApp)).to(equal(True))
