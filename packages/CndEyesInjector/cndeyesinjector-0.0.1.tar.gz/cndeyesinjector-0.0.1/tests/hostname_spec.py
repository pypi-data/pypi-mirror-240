...
file_path_base = "hostnames"
with description('Hostname') as self:
    with before.each:
        unstub()
        ...
        self.instance = self.instance_parent.hostname

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, cnd_eyes_injector._hostname.Hostname)).to(equal(True))
    ...
    with context("get", "get"):
        with it("shoud get"):
            with Mocker(real_http=True) as m:
                ...
                expect(result).to(equal([{'id': 530, 'name': 'Jedha', 'domain_id': 123}]))
    ...
    with context("create", "create"):
        with it("shoud create"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.create(self.instance.data("Abcd", 678))
    ...
    with context("patch", "patch"):
        with it("shoud update"):
            with Mocker(real_http=True) as m:
                ...
                result = self.instance.update(id, self.instance.data("----", 678))
