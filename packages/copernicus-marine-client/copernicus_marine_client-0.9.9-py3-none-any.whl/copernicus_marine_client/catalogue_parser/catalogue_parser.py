...
async def async_fetch_items_from_collection(
    session: aiohttp.ClientSession, collection: pystac.Collection
) -> List[pystac.Item]:
    items = []
...
async def async_fetch_collection(
    ...
    async with session.get(
        ...
        json_collection = await resp.json()
        try:
...
async def async_fetch_childs(
    session: aiohttp.ClientSession, child_links: List[pystac.Link]
) -> Iterator[Optional[Tuple[pystac.Collection, List[pystac.Item]]]]:
    tasks = []
    for link in child_links:
        tasks.append(
            asyncio.ensure_future(
                async_fetch_collection(session, link.absolute_href)
            )
        )
...
async def async_fetch_catalog(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            json_catalog = await resp.json()
            catalog = pystac.Catalog.from_dict(json_catalog)
            if collection_titles_filter is not None:
                collection_titles_filter_list: List[
                    str
                ] = collection_titles_filter
                child_links = list(
                    filter(
            ...
            childs = await async_fetch_childs(session, child_links)
