...
class nn:
    ...
    def add(self, layer=None, activation=None, optimizer=None, **kw):
        ...
        #     print("增加ReLU层。")
            ...
            self.model.add_module('avgpooling' + str(self.layers_num), torch.nn.AvgPool2d(kw['kernel_size']))
    ...
    def load_tab_data(self, data_path,batch_size=32, train_val_ratio=1.0, shuffle=True,y_type='long'):
        from .load_data import TabularDataset
        self.dataset_type = "tab"

        dataset = TabularDataset(data_path,y_type=y_type)
        self.dataset_size = int(len(dataset))
    ...
    def convert(self, checkpoint=None,out_file="convert_model.onnx",backend="ONNX", ):
        # if checkpoint is not None:
