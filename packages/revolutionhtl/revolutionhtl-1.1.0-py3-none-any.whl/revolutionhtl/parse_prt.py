...
def _parse(path, f_value, singletones, fastaspath, mode= 'normal', N_max= 2000):
    ...
    if singletones:
        singletones= identify_singletones(OGs_table, fastaspath)
    return df, OGs_table, singletones
...
def identify_singletones(df, fastaspath):
    """
    ...
    for file in fastaspath:
        ...
        DD.update({x:file for x in fgenes})
    singletones= genes_fa - genes_og
    singletones= [[x, DD[x]] for x in singletones]
    singletones= pd.DataFrame(singletones, columns= ['gene', 'file'])
    return singletones.sort_values('file')
