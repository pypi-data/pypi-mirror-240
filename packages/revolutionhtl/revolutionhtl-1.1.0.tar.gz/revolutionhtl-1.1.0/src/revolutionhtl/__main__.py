...
desc_def= """REvolutionH-tl : Reconstruction of Evolutionaty Histories TooL | Steps :
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--evalue', default= e_default, type= float,
                        help= f'[float | Parameter for step 1 | Default: {e_default}]'
                       )
    mk_def= 'makeblastdb'
    ...
    parser.add_argument('-k_hits', '--max_target_seqs', default= kh_def, type= int,
                        help= f'[float | Parameter for step 1 | Default: {kh_def}] Maximum number of alignment hits per gene againist a fixed species.')

    bhh_def= 'target'
    parser.add_argument('-bh_heuristic', '--besthit_heuristic',
                        help= f'[str | Parameter for step 2 | Default: {bhh_def}] Indicates how to normalize bit-score. Normalize by sequence lenght: query, target, alignment, smallest. No normalization: row.',
                        type= str,
                        choices= ['query', 'target', 'alignment', 'smallest', 'row'],
                        default= bhh_def,
                       )

    parser.add_argument('-f', '--f_value',
                        help= f'[float | Parameter for step 2 | Defualt: {_default_f}] Number between 0 and 1. Defines the adaptative threshhold for best-hit election as: f*max_bit_score.',
                        type= float,
                        default= _default_f,
                       )
    ...
    parser.add_argument('-streeh_repeats', '--stree_heuristic_repeats',
                        help= f'[int | Parameter for step 4] Specifies how many times run the MaxConsistentTriples heuristic. (default: {sr_def})',
                        type= int,
                        default= sr_def
                       )
    ...
    parser.add_argument('-singletones',
    ...
    parser.add_argument('-Nm', '--N_max',
                        help= f'[int | Default: {nm_def}] Maximum number of genes in a orthogroup, bigger orthogroups are splitted. If 0, no orthogroup is splitted.',
                        type= int,
                        default= nm_def,
                       )
    ...
    logfile= f'{args.output_prefix}_log.txt'
    ...
    if 2 in args.steps:
        ...
        if args.singletones:
            if type(args.fastas)==str:
                ...
                raise MissedData('Step 2 needs a value for the parameter --fastas whenerver you use the -singletones flag.')
        else:
            fasta_files= None
            text= ''
        ...
        df_hits, OGs_table, singletones= _parse(alignment_hits, args.f_value, args.singletones, fasta_files, args.besthit_heuristic, N_max= args.N_max)
        ...
        if args.singletones:
            opath= args.output_prefix+'.singletones.tsv'
            singletones.to_csv(opath, sep='\t', index= False)
            print(f'Singletones successfully written to {opath}')
