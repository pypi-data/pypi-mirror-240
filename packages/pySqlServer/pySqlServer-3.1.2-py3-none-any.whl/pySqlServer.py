...
class PySQL():
    ...
    @_log_decorator
    def update_table(self, table_name, schema='dbo', update_key=None, update_value=None):
        """
            function to update table values with a dictionary.
        ...
        QUERY += ' SET ' + ' , '.join([f" {i} = {update_key[i]} " for i in update_key.keys()])
        QUERY += ' WHERE ' + ' and '.join([f" {i} = '{update_value[i]}' " for i in update_value.keys()])
