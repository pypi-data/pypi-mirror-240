...
from CRISPR_TAPE.shared_functions import clean_inputs, get_codon_index, PAMposition, analyse_text, notes, pamcolumn, remove_pam, correct_distance, list_search, get_count, reverse_complement, find_sign_change
...
def specific_function(spec_amino,
    ...
    sys.stderr.write('\nThe PAM you have selected is ' + motif + '\n')
    ...
    # get the distance of the gRNAs from the start base of the amino acid
    start_gRNAs = 
    efjoijofe
    ...
    if not len(gRNA) == 0:
        ...
        # get the index of the sign change in the distances
        slice_index = find_sign_change(list(gRNA["Distance from Amino Acid (bp)"]))
        if slice_index:
            upperguides = gRNA.iloc[:slice_index+1,]
            downerguides = gRNA.iloc[slice_index+1:,]
        elif gRNA["Distance from Amino Acid (bp)"][0] < 0:
            upperguides = gRNA
        else:
            downerguides = gRNA
    else:
