...
class HttpConnectionManager(betterproto.Message):
    ...
    ork.http_connection_manager.v3.HttpConnectionManager.HcmAccessLogOptions.ac
    cess_log_flush_interval>`. Note that if both this field and
    :ref:`access_log_flush_interval <envoy_v3_api_field_extensions.filters.netw
    ork.http_connection_manager.v3.HttpConnectionManager.HcmAccessLogOptions.ac
    cess_log_flush_interval>` are specified, the former (deprecated field) is
    ignored.
    """
    ...
    """
    .. attention:: This field is deprecated in favor of
    :ref:`flush_access_log_on_new_request <envoy_v3_api_field_extensions.filter
    s.network.http_connection_manager.v3.HttpConnectionManager.HcmAccessLogOpti
    ons.flush_access_log_on_new_request>`. Note that if both this field and
    :ref:`flush_access_log_on_new_request <envoy_v3_api_field_extensions.filter
    s.network.http_connection_manager.v3.HttpConnectionManager.HcmAccessLogOpti
    ons.flush_access_log_on_new_request>` are specified, the former (deprecated
    field) is ignored.
    """
...
class HttpFilter(betterproto.Message):
    ...
    by setting per filter configuration in the route configuration. A disabled
    filter will be treated as enabled in the following cases: 1. Valid per
    filter configuration is configured in the ``typed_per_filter_config`` of
    route config. 2. :ref:`FilterConfig
    <envoy_v3_api_msg_config.route.v3.FilterConfig>` is configured in the
    ``typed_per_filter_config`` of route config and the    :ref:`disabled of
    FilterConfig <envoy_v3_api_field_config.route.v3.FilterConfig.disabled>`
    is set to false. Terminal filters (e.g. ``envoy.filters.http.router``)
