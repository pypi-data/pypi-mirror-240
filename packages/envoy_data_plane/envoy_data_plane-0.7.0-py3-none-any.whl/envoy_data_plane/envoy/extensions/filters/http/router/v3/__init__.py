...
class Router(betterproto.Message):
    ...
    filters for the upstream filter chain. These filters will be applied for
    all requests that pass through the router. They will also be applied to
    shadowed requests. Upstream filters cannot change route or cluster.
    Upstream filters specified on the cluster will override these filters. If
    using upstream filters, please be aware that local errors sent by upstream
    filters will not trigger retries, and local errors sent by upstream filters
    will count as a final response if hedging is configured. [#extension-
