...
class CertificateValidationContext(betterproto.Message):
    ...
    chain. Only one of ``trusted_ca`` and ``ca_certificate_provider_instance``
    may be specified. [#next-major-version: This field and watched_directory
    below should ideally be moved into a separate sub-message, since there's no
    point in specifying the latter field without this one.]
    """
    ...
    not include the leaf, so a depth of 1 allows the leaf and one CA
    certificate. If a trusted issuer appears in the chain, but in a depth
    larger than configured, the certificate validation will fail. See
    `BoringSSL SSL_CTX_set_verify_depth
    <https://commondatastorage.googleapis.com/chromium-boringssl-
    docs/ssl.h.html#SSL_CTX_set_verify_depth>`_ If you use OpenSSL, its
    behavior is different from BoringSSL, this will define a limit on the
    number of certificates between the end-entity and trust-anchor
    certificates. Neither the end-entity nor the trust-anchor certificates
    count against depth. See `OpenSSL SSL set_verify_depth <https://www.openssl
    .org/docs/man1.1.1/man3/SSL_CTX_set_verify_depth.html>`_. Trusted issues
    are specified by setting :ref:`trusted_ca <envoy_v3_api_field_extensions.tr
    ansport_sockets.tls.v3.CertificateValidationContext.trusted_ca>`
    """
...
class CommonTlsContext(betterproto.Message):
    ...
    certificates and support SNI-based selection. Only one of
