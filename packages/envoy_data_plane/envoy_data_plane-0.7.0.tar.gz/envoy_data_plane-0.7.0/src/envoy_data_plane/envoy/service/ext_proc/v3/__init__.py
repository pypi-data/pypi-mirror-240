...
class ProcessingResponse(betterproto.Message):
    ...
    ProcessingResponse message. [#next-free-field: 11]
    """
...
class HttpHeaders(betterproto.Message):
    ...
    header keys are case-insensitive. The ``headers`` encoding is based on the
    runtime guard envoy_reloadable_features_send_header_raw_value setting. When
    it is true, the header value is encoded in the :ref:`raw_value
    <envoy_v3_api_field_config.core.v3.HeaderValue.raw_value>` field. When it
    is false, the header value is encoded in the :ref:`value
...
class HttpTrailers(betterproto.Message):
    ...
    envoy_reloadable_features_send_header_raw_value setting. When it is true,
...
class CommonResponse(betterproto.Message):
    ...
    ``trailers`` encoding is based on the runtime guard
    envoy_reloadable_features_send_header_raw_value setting. When it is true,
    the header value is encoded in the :ref:`raw_value
    <envoy_v3_api_field_config.core.v3.HeaderValue.raw_value>` field. When it
    is false, the header value is encoded in the :ref:`value
...
class HeaderMutation(betterproto.Message):
    ...
    or ``host`` headers will be ignored. The ``set_headers`` encoding is based
    on the runtime guard envoy_reloadable_features_send_header_raw_value
    setting. When it is true, the header value is encoded in the
