...
def cli(parser: argparse.ArgumentParser = parser) -> None:
    ...
    if any((not not args.output, not not args.stdout, not not args.stderr)) and not args.folder:
    ...
    if args.output is not None:
        manage_output(res, args.output, args.mode, args.folder)
...
def manage_input(wc: WhisperClient, input: str, folder: bool, video: bool, modes, no_skip, interval) -> Optional[list]:
    if folder:
        if video:
