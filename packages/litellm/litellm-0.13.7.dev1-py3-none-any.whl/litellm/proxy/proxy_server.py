...
def run_redis_worker():
    command = ["python3", "-m", "rq", "worker", "--url", 
               f"redis://{os.getenv('REDIS_USERNAME', 'default')}:{os.environ['REDIS_PASSWORD']}@{os.environ['REDIS_HOST']}:{os.environ['REDIS_PORT']}"]
    
    # Set the environment variable OBJC_DISABLE_INITIALIZE_FORK_SAFETY to "YES"
    env = {"OBJC_DISABLE_INITIALIZE_FORK_SAFETY": "YES"}
    
    with open(os.devnull, 'w') as devnull:
        process = subprocess.Popen(command, stdout=devnull, stderr=devnull, env=env)
    ...
    try: 
        ...
        if queue is None: 
            try: 
                ...
                raise e
                # print(e) 
                # raise Exception(f"Install the following packages to use this: `pip install rq` and `pip install redis`")
            queue = Queue(connection=Redis(host=os.environ["REDIS_HOST"], port=os.environ["REDIS_PORT"], password=os.environ["REDIS_PASSWORD"]))
            # run_redis_worker()

        print(f"ENQUEUING JOB!!")
        job = queue.enqueue(llm_router.completion, kwargs={}, retry=Retry(max=3))
...
@router.get("/router/chat/result/{run_id}")
async def router_result(request: Request, run_id: str):
    from rq import Job
    from redis import Redis
    job = Job.fetch(id='my_id', connection=Redis(host=os.environ["REDIS_HOST"], port=os.environ["REDIS_PORT"], password=os.environ["REDIS_PASSWORD"]))
