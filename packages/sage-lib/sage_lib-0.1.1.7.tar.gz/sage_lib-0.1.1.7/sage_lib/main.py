...
def generate_xyz_from_outcar(path, all_subfolders=False, verbose=False):
    """
    ...
    read_files(partition=DP, path=path, all_subfolders=all_subfolders)

    DP.export_configXYZ()
    
def generate_vacancy(path, all_subfolders=False, verbose=False):
    """
...
def generate_disassemble_surface(path, steps=5, final_distance=5.0, atoms_to_remove=None, all_subfolders=False, verbose=False):
    """
    ...
    read_files(partition=SSG, path=path, all_subfolders=all_subfolders)
...
def generate_dimers(path=None, labels:list=None, steps:int=10, vacuum:float=18.0, all_subfolders=False, verbose=False):
    """
    ...
    read_files(partition=VSG, path=path, all_subfolders=all_subfolders)
...
def read_files(partition, path=None, source='VASP', all_subfolders=False, verbose=False):
...
def add_arguments(parser):
    ...
    parser.add_argument('--all-subfolders', default=False, action='store_true', help='Read from all subfolders under the specified path')
...
def main():
    ...
    print("All Subfolders flag:", args.all_subfolders)
    # Handle execution based on the specified sub-command
    if   args.command == 'xyz':
        generate_xyz_from_outcar(args.path, all_subfolders=args.all_subfolders, verbose=args.verbose)
    elif args.command == 'vacancy':
        generate_vacancy(args.path, all_subfolders=args.all_subfolders, verbose=args.verbose)
    elif args.command == 'disassemble':
        generate_disassemble_surface(args.path, steps=args.steps, final_distance=args.final_distance, atoms_to_remove=args.atoms_to_remove, all_subfolders=args.all_subfolders, verbose=args.verbose)
    elif args.command == 'dimer':
        generate_dimers(path=args.path, labels=args.labels, steps=args.steps, vacuum=args.vacuum, all_subfolders=args.all_subfolders, verbose=args.verbose)
