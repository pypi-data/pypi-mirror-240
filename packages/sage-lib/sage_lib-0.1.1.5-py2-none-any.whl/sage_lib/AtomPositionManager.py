...
class AtomPositionManager(FileManager):
    ...
    def add_atom(self, atomLabels:str, atomPosition:np.array, atomicConstraints:np.array=np.array([1,1,1])):
        self._atomPositions = np.append(self.atomPositions, np.atleast_2d(atomPosition), axis=0)
        self._atomLabelsList = np.append(self.atomLabelsList, atomLabels)
        self._atomicConstraints = np.append(self.atomicConstraints, np.atleast_2d(atomicConstraints), axis=0)
    ...
    def export_as_xyz(self, file_location:str=None, save_to_file:str='w', verbose:bool=False) -> str:
        ...
        c_str = ' '.join(['T' if val else 'F' for val in self.pbc])
        xyz_content += f'Lattice="{lattice_str}" Properties=species:S:1:pos:R:3:masses:R:1:DFT_forces:R:3 DFT_energy={self._E}  pbc="{pbc_str}"\n'
