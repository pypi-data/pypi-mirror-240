...
def rates(
    ...
    plt.savefig(directory / f"{directory.name}-violin.svg")
    ...
    plt.savefig(directory / f"{directory.name}-violin-trimmed.svg")
    ...
    plt.savefig(directory / f"{directory.name}-trace.svg")

    # plot interactive trace
    output_file(filename=directory / f"{directory.name}-trace.html")
    with no_browser():
