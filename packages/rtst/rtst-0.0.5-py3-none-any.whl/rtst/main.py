...
def process_speech(service="google", source_language="ru-RU", target_language="en-US"):
    print("Please speak into the microphone...")
    with sr.Microphone() as source:
        r.adjust_for_ambient_noise(source)
        audio = r.listen(source)
    if service == "google":
        transcription = google_speech_recognize(r, audio, language=source_language)
    elif service == "azure":
        transcription = azure_speech_recognize(audio, language=source_language)
    elif service == "aws":
        transcription = aws_speech_recognize(audio, language=source_language)
    elif service == "huggingface":
        transcription = huggingface_speech_recognize(audio, language=source_language)
    else:
        print(f"Error: Unsupported speech recognition service '{service}'")
        return
