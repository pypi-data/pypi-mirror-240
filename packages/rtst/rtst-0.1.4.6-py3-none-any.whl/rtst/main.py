...
def process_speech(service='google'):
    ...
    response = recognize_speech_from_mic(r, source, service, language)
    
    if response["success"]:
        detected_lang = LANGUAGES.get(language.split('-')[0], "Unknown language")
        print(f"Transcribed Text: {response['transcription']}")
        print(f"Detected Language: {detected_lang}")

        # Translation logic only if the detected language is different from 'en'
        if detected_lang != 'en':
            translation = translator.translate(response["transcription"], src=detected_lang, dest='en')
            response["translation"] = translation.text
            print(f"Translated Text: {response['translation']}")
            speak_text(response["translation"])
        else:
