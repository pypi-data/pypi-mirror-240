...
def process_speech(service="google"):
    ...
    with sr.Microphone() as source:
        recognizer.adjust_for_ambient_noise(source)
        audio = recognizer.listen(source)
    response = recognize_speech_from_mic(recognizer, source, service, source_language)
    if response["success"]:
        if response["detected_language"] != target_language:
            translated_text = translator.translate(response["transcription"], src=source_language, dest=target_language).text
