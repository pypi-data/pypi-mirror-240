...
class Series:
    ...
    @staticmethod
    def round(series: pd.Series, unit: str, decimals: float = 0) -> pd.Series:
        """
...
class Dataframe:
    ...
    def m_as(dataframe: pd.DataFrame, units: str):
        ...
        """
        dataframe_units = dataframe.astype(units)
        with warnings.catch_warnings():
            # .pint.m_as not available for df
            warnings.simplefilter(
                action="ignore",
                category=pint.UnitStrippedWarning)
...
class InterpolationQuantity:
    ...
    def __init__(self, y: pd.Series, x: pd.Series = None, **kwargs):
        """
        ...
        self.interpolationQuantity = InterpolationQuantity.\
            __quantity_interpolation(x=x.pint.magnitude, y=y.pint.magnitude,
                                     **kwargs)
...
def change_unit(quantity: Type[pint.Quantity or pd.Series], unit: str):
    """
