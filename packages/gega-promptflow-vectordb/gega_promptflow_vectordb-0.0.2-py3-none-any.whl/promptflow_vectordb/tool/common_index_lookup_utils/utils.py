    ...
    def get(subscription_id, resource_group, workspace_name):
        if (subscription_id, resource_group, workspace_name) not in CallbackContext.__instances:
            with CallbackContext.__instances_lock:
                if (subscription_id, resource_group, workspace_name) not in CallbackContext.__instances:
                    CallbackContext.__instances[(subscription_id, resource_group, workspace_name)] = CallbackContext(subscription_id, resource_group, workspace_name)

        return CallbackContext.__instances[(subscription_id, resource_group, workspace_name)]
    
    @property
    def arm_id(self):
        return f'/subscriptions/{self.subscription_id}/resourceGroups/{self.resource_group}/providers/Microsoft.MachineLearningServices/workspaces/{self.workspace_name}'
