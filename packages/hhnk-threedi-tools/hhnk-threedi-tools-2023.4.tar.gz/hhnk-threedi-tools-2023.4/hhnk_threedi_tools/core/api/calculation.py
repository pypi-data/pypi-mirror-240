            ...
            for index, row in hrt.sqlite_table_to_df(
                ...
                for entry in action_table_string.split("#"):
                    try:
                        ...
                        print(
                            f"""Problem with '{entry}' at index {action_table_string.index(entry)} of the action_table_string for
{row}
"""
                        )
                        raise e
    ...
    def _get_aggregation_from_sqlite(self) -> list:
        ...
        for index, row in hrt.sqlite_table_to_df(
            ...
            data = {
                ...
                "flow_variable": apply_translate_dict(
                    row["flow_variable"], translate_dict=translate_dict_flow_variable
                ),  # string
...
class Simulation:
    ...
    def add_structure_control(self):
        for control_data in self.data.structure_control:
            self._add_to_simulation(
                self.threedi_api.simulations_events_structure_control_table_create,
                simulation_pk=self.id,
                data=control_data,
            )
    ...
    def add_laterals(self):
        ...
        for lateral in self.data.laterals:
            self._add_to_simulation(
                self.threedi_api.simulations_events_lateral_timeseries_create,
                simulation_pk=self.id,
                data=lateral,
                async_req=False,
            )
    ...
    def add_boundaries(self):
        ...
        if self.data.boundaries != []:
            bc_upload = self._add_to_simulation(
                self.threedi_api.simulations_events_boundaryconditions_file_create,
                simulation_pk=self.id,
    ...
    def add_basic_post_processing(self):
        self._add_to_simulation(
            ...
            simulation_pk=self.id,
            data=self.data.basic_processing,
        )
    ...
    def add_damage_post_processing(self):
        self._add_to_simulation(
            ...
            simulation_pk=self.id,
            data=self.data.damage_processing,
        )
    ...
    def add_arrival_post_processing(self):
        self._add_to_simulation(
            ...
            simulation_pk=self.id,
            data=self.data.arrival_processing,
        )
    ...
    def start(self, extra_name=""):
        ...
        if self.structure_control_valid:
            ...
            apicall_txt = (
                ...
                / f"apicall{extra_name}_simid{self.simulation.id}_date{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
