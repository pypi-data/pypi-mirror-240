...
class HinaCloudSDK(object):
    ...
    @staticmethod
    def init(url, batch=1, enable_log=False, flush_max_time=15):
        with INSTANCE_LOCK:
    ...
    def close(self):
        ...
        if self._config.batch > 1:
            ...
            # 循环发送，直到队列和发送缓存都为空
            while not self._config.queue.empty() or len(self._config.flush_buffer) != 0:
        ...
        with FLUSH_LOCK:
            if len(self.flush_buffer) == 0:
                for _ in range(max(self.batch, 100)):
                    if self.queue.empty():
                        break
                    self.flush_buffer.append(self.queue.get_nowait())
            if len(self.flush_buffer) > 0:
                flush_success = self.send(self.flush_buffer)
            if flush_success:
                self.flush_buffer = []
