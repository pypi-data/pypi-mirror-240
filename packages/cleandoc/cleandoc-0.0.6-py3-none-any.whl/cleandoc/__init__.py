...
def cleandoc_all(
    ...
    logger = ch.config_log("cleandoc_log.txt", level=loglevel)
...
def clean_all(
    ...
    for i, pyfile in enumerate(pyfilelist):
        ...
        headerstr = f"{ch.format_header(header, repeat_char='o')}\n"
        logger.info(headerstr)
        summary = clean_pyfile(pyfile, write=write, loglevel=loglevel)
...
def gen_docs(
    pkgpath: str, changed: bool = True, release: str = "", loglevel=logging.INFO
):
