from abstract_gui import make_component,sg,ensure_nested_list
from abstract_utilities import create_new_name
from abstract_utilities import ThreadManager
...
class GUIManager:
    def __init__(self,window_mgr):
        ...
        self.event_threads={}
    def long_running_operation(self,window_name,event_handler):
        # Simulate a long-running operation

            # Here you would have some mechanism to send data back to the GUI
        return 'good'
    def start_long_operation_thread(self,window_name,event_handler):
        name = self.thread_manager.add_thread(name=window_name,target_function=self.long_running_operation,function_args={"window_name":window_name,"event_handler":event_handler}, daemon=True)
        self.event_threads[name]=False
        self.thread_manager.start(name)
        
    def start_run(self,window_name,event_handler):
        name = self.thread_manager.add_thread(name=window_name,target_function=self.run,function_args={"window_name":window_name,"event_handler":event_handler}, daemon=True)
        self.event_threads[name]=False
        results = self.thread_manager.start(name)
        self.run(window_name,event_handler)
        self.thread_manager.stop(name,results)
        return name
    def run(self,window_name,event_handler=None):
        self.event_threads[window_name] = True
        self.window=self.window_mgr.get_window_method(window_name)
        while self.event_threads[window_name]:
            event, values = self.window_mgr.read_window(timeout=100)  # Non-blocking read with a timeout
            if event == sg.WIN_CLOSED or self.window_mgr.exists(window_name=window_name)==False:
                self.event_threads[window_name] = False
            # ... handle other events ...
            if event_handler:
                event_handler(event, values,self.window)
        # Cleanup
...
class AbstractWindowManager:
    ...
    def add_window(self,title=None,layout=None, window_name=None,  default_name=True, 
        ...
        if layout:
           layout=ensure_nested_list(layout)
            ...
            if kwargs_layout:
                ...
                del args[num]
        self.global_windows.append({"name":window_name,"method":make_component('Window', title=title,layout=layout,*args, **kwargs),"event":False,"values":False,"current":current,**sizes})
        return window_name
    def while_window(self,window_name=None,window=None,event_handler=None):
        window_enumeration = self.get_any_enumeration(window_name=window_name,window=window)
        if window_enumeration !=None:
            name = self.global_windows[window_enumeration]["name"]
            self.gui_mgr.start_long_operation_thread(window_name=name,event_handler=event_handler)
            self.gui_mgr.run(name,event_handler)
