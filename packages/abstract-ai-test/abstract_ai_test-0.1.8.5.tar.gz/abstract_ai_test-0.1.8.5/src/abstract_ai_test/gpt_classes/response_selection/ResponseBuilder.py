...
class SaveManager:
    ...
    def save_to_file(self, data, file_path):
        write_to_file(contents=json.dumps(data),file_path=file_path)
        #with open(file_path, 'w', encoding='utf-8') as file:
            #json.dump(data, file, ensure_ascii=False, indent=4)
            #write_to_file(contents=json.dumps(data),filepath=file_path)
    @staticmethod
    def read_saved_json(file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            data = safe_json_loads(file.read())
        return data
class ResponseManager:
    ...
    def re_initialize_query(self):
        ...
        self.response_keys = ["api_response","abort","additional_responses","suggestions","notation","generate_title","request_chunks","token_adjustment","prompt_as_previous"]
    ...
    def get_abort(self):
       self.abort_js = {"abort":None,"additional_responses":None,"request_chunks":None}
       for key,value in self.prompt_mgr.instruction_mgr.instructions_js.items():
    ...
    def send_query(self):
        ...
        """
        self.prompt = self.prompt_mgr.create_prompt(dist_number=self.i_query,
                                                    bot_notation=self.notation,
        ...
        self.save_manager = SaveManager(data=json.dumps(self.response),title='responsejson.json',directory=self.directory,model=self.model)
        try:
            self.response = self.response.json()
            self.save_manager = SaveManager(data=json.dumps(self.response),title='dotjson.json',directory=self.directory,model=self.model)
        except:
            pass
        ...
        if self.content:
            for i,key in enumerate(self.response_keys):
                value = get_any_value(self.content,key)
                setattr(self,key,value)
                if key == 'prompt_as_previous' and value:
                    self.prompt_as_previous_clone=self.api_response
        
        error_response = get_any_value(self.query_js["response"],'error')
        self.model = get_any_value(self.query_js["response"],'model') or 'default'
        if error_response:
            self.model = 'error'
        self.title = self.original_title or self.generate_title or get_any_value(self.query_js["response"],'created')
        self.save_manager = SaveManager(data=self.query_js,title=self.title,directory=self.directory,model=self.model)
        self.query_js['file_path']=self.save_manager.file_path
