...
class GptManager:
    ...
    def __init__(self):
 	...
 	"""
        ...
        self.display_number_tracker={'instructions':0,'request':0,'prompt_data':0,'chunk':0,'query':0,}
    ...
    def update_text(self,**kwargs):
        self.update_instruction_mgr()
        self.update_prompt_mgr(**kwargs)
    def fill_lists(self):
        if self.prompt_data_list[-1] != '' or self.request_data_list[-1] != '':
            print('fill lists prompt_data_list = ',self.prompt_data_list)
            self.prompt_data_list.append('')
            self.request_data_list.append('')
            self.instructions_list.append({})
            self.update_text()
            print('fill lists done prompt_data_list = ',self.prompt_data_list)
#ApiManagement:
...
#PromptManagement
	...
	"""
        self.prompt_mgr = PromptManager(instruction_mgr=self.instruction_mgr,
                                   ...
                                   request=request_data_list or self.request_data_list,
    ...
    def update_request_data_display(self):
        spec_section_number = self.get_spec_section_number_display(variable_header='request')
        self.window_mgr.update_value(key=text_to_key('request'),value=self.request_data_list[spec_section_number])
    ...
    def update_prompt_data_list(self,data=None):
        ...
        if self.event == text_to_key('prompt_data data') or data:
            if data == None:
    ...
    def update_prompt_data_display(self):
        spec_section_number = self.get_spec_section_number_display(variable_header='prompt_data')
        self.window_mgr.update_value(key=text_to_key('prompt_data data'),value=self.prompt_data_list[spec_section_number])
    ...
    def update_chunk_info(self,chunk_token_distribution_number=0,chunk_number=0):
        if self.chunk_token_distributions:
            if chunk_token_distribution_number < len(self.chunk_token_distributions) and chunk_token_distribution_number >=0:
    ...
    def adjust_chunk_display(self,event_type,chunk_token_distribution_number,chunk_number=0):
        """
 	...
 	"""
        ...
        #update chunk data
        self.window_mgr.update_value(text_to_key('chunk sectioned data'),
                                     self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]['chunk']['data'])
        #get query
        self.window_mgr.update_value(key='-QUERY-',
                                     value=self.prompt_mgr.create_prompt(chunk_token_distribution_number=chunk_token_distribution_number,chunk_number=chunk_number))
        #update chunk info
        self.update_chunk_info(chunk_token_distribution_number,
                               chunk_number)
        ...
        #update chunk keys
        self.update_chunk_info(chunk_token_distribution_number=chunk_token_distribution_number,chunk_number=chunk_number)
        return chunk_token_distribution_number
    ...
    def add_to_chunk(self,content):
 	...
 	"""
        ...
        if prompt_data:
            content = prompt_data+'\n\n'+content
        self.prompt_data_list[self.get_prompt_data_section_number()] = eatAll(content,['\n'])
    ...
    def chunk_event_check(self):
 	...
 	"""
        chunk_query_sub_navigation_keys = [
                          text_to_key("chunk forward"),
                          text_to_key("chunk back"),
                          text_to_key("query forward"),
                          text_to_key("query back")]
        instruction_request_prompt_navigation_keys = [text_to_key('instructions section back'),
        ...
        chunk_number = 0
        event_type = self.event.split('_')[-1][:-1].lower()
        if self.event in instruction_request_prompt_navigation_keys+chunk_query_sub_navigation_keys:

            prompt_number=get_prompt_section_number(event_type,prompt_number,self.prompt_data_list)
            for header in ['instructions','request','prompt_data','chunk','query']:
                self.window_mgr.update_value(text_to_key(f"{header} section number"),prompt_number)
                self.display_number_tracker[header]=prompt_number
            self.update_request_data_display()
            self.update_prompt_data_display()
            if self.event in chunk_query_sub_navigation_keys:
                chunk_number = self.get_chunk_number()
                chunk_number= get_prompt_section_number(event_type,chunk_number,self.chunk_token_distributions[prompt_number])
            ...
            if self.event == '-CLEAR_CHUNKS-':
                ...
                self.add_to_chunk(self.window_mgr.get_values()['-URL_TEXT-'])
                ...
                self.add_to_chunk(self.window_mgr.get_values()[self.script_event_js['-FILE_TEXT-']])
        ...
        chunk_token_distribution_number=self.adjust_chunk_display(event_type,prompt_number,chunk_number)
        self.update_text()
        return True
