...
async def chat_completion(request: ChatCompletionRequest):
    ...
    try:
        if "single" in model_name:
            # LOAD GNP SINGLE DATAFRAME
            df_path = os.path.join(os.path.expanduser("~"),
                                   ".cache/embeddings/gnp_single",
                                   "df_embeddings.bin")
            df = pd.read_pickle(df_path)
            answer = rag.ask_manually(messages, 
                                      qualtop_llmapi.llm,
                                      df)
        elif "collection" in model_name:
            answer = rag.ask(messages, qualtop_llmapi.llm, "collection")
        else:
            answer = single_prompt.ask(formatted_messages, 
                                       qualtop_llmapi.llm)
