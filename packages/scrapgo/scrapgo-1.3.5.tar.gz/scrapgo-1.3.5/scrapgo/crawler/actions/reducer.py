...
class ReduceMixin:
    ...
    @assingles
    def dispatch_renderer(self, action, response, responsemap, prev_results, context):
    ...
    @assingles
    def dispatch_payloader(self, action, responsemap, prev_results, context):
        if not hasattr(action, 'payloader') or not action.payloader:
            return {}
        else:
            return self.dispatch(
                'payloader', action.payloader,
                responsemap=responsemap, prev=prev_results, **context
            )
    ...
    def dispatch_extractor(self, action, meta, response, prev_results, results_cache, context):
        extractset = {}
        if module := action.extractor:
            ...
            for g, func in find_function(module, pat):
                extracted = select_kwargs(
                    ...
                    soup=meta.soup,
                    prev=prev_results,
                    results_set=results_cache, **context
                )
                extractset[g('ext')] = self.validate_extracted(extracted, func, meta)
        return extractset

    def dispatch_parser(self, action, response, extracted, meta, prev_results, results_cache, context):
    ...
    def dispatch_urlfilter(self, action, url, responsemap, prev_results, context):
        ...
        return self.dispatch(
            ...
            url=url, responsemap=responsemap, prev=prev_results, **context
        )
    ...
    def dispatch_onfailure(self, action, exception, response, responsemap, prev_results, context, **requests_kwargs):
        self.dispatch(
