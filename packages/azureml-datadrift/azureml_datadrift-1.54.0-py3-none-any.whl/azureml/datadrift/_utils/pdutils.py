...
def get_inferred_fillna_columns(summaries):
    return [c.name for c in summaries
            ...
            and c.invalid_row_count != 0 and c.invalid_row_count / c.total_row_count < PD_INVALID_PERCENT_THRESHOLD]


def get_inferred_drop_columns(summaries):
    return [c.name for c in summaries
            if (pd.api.types.is_numeric_dtype(c.inferred_dtype)
                and c.invalid_row_count / c.total_row_count >= PD_INVALID_PERCENT_THRESHOLD)
            or (not pd.api.types.is_numeric_dtype(c.inferred_dtype)
...
def get_pandas_df_summary(df):
    ...
    for col in df.columns:
        ...
        if (pd.api.types.is_numeric_dtype(inferred_dtype)
