...
import simplepygraph
from simplepygraph.constantes import *
...
class Dijkstra:
    ...
    def diaporama(self, html_directory=".", filename="index"):
        # fix temporaire pour inclusion dans Capytale ; a priori on n'en n'a plus besoin
        ...
        tmp_directory = str(simplepygraph.__path__[0])
        
        #filename = open_file+"/exported/"+filename + ".html"
        filename = f'{html_directory}/exported/{filename}.html'
        css_dir = f'{html_directory}/exported/css'
        js_dir = f'{html_directory}/exported/js'
        img_dir = f'{html_directory}/exported/img'
        ...
        while self.__solved != True:
            self.next()
            self.__graph.write(img_dir+str(i), view=False)
            if os.path.exists(str(i)):
                os.remove(str(i))
            header += self.make_section(f"Step : {i}", f"img/{i}.svg")
            i += 1
        if self.__solved:
            self.dijkstra_path()
            ...
            self.__graph.write(img_dir+str(i), view=False)
            header += self.make_section(f"Step : {i}", f"img/{i}.svg")
        shutil.copyfile(tmp_directory +'/res/left.svg', f"{img_dir}/left.svg")
        shutil.copyfile(tmp_directory +'/res/right.svg', f"{img_fir}/right.svg")
        shutil.copyfile(tmp_directory +'/res/css.css', f"{css_dir}/css.css")
        shutil.copyfile(tmp_directory +'/res/script.js', f"{js_dir}/script.js")
    ...
    def cost_between(self, start, end):
        ...
        weight = self.graph.edge_view(start, end).weight
        return weight if weight is not None else 1
    ...
    def dsatur(self):
        ...
        def select_node():
            ...
            return max((node_id for node_id in g.node_ids() if g.node_view(node_id).color_id == -1),
                       key=lambda s: (dsat(s), g.degree(s)))
