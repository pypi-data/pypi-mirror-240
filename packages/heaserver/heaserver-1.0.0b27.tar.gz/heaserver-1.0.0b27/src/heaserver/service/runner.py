...
def get_application(package_name: str | None = None,
        ...
        if package_name is not None:
            ...
            async with client_session(connector=TCPConnector(), connector_owner=True, raise_for_status=True) as session:
                ...
                if component is not None:
                    ...
                    logger.debug('Registering external URL %s for component %s', external_base_url, package_name)
                    try:
                    ...
                    logger.warn('The external URL for component %s is not registered with the HEA registry service', package_name)
        else:
    ...
    async def _background_tasks(app: web.Application) -> AsyncGenerator:
        ...
        try:
            ...
            logger.warn('Timed out trying to end background tasks: %s', background_tasks)
        finally:
