...
from tests.conftest import EXAMPLE_CARD_ID, EXAMPLE_FEEDBACKS, EXAMPLE_EMPTY_FEEDBACK_RESPONSE, EXAMPLE_FEEDBACK_RESPONSE, EXAMPLE_CARD_RESPONSE, EXAMPLE_PARAMS, EXAMPLE_REVIEW_RATING, EXAMPLE_RATING, EXAMPLE_ROOT_ID, EXAMPLE_NAME
from pytest_httpx import HTTPXMock
from analyzer.const import WB_CARD_URL, WB_FEEDBACK_URLS


def test_wb_parser_get_card(httpx_mock: HTTPXMock):
    ...
    wb = WBParser(q)
    card = wb.get_card(EXAMPLE_CARD_ID)
    assert card.state == 0
...
def test_wb_parser_get_product_feedbacks(httpx_mock: HTTPXMock):
    httpx_mock.add_response(
        url=f"{WB_CARD_URL}{EXAMPLE_CARD_ID}",
        json=EXAMPLE_CARD_RESPONSE
    )
    ...
    wb = WBParser(q)
    card = wb.get_card(EXAMPLE_CARD_ID)
    feedbacks = wb.get_feedbacks_for_card(card)
...
def test_wb_parser_get_product_feedbacks_empty(httpx_mock: HTTPXMock):
    httpx_mock.add_response(
        url=f"{WB_CARD_URL}{EXAMPLE_CARD_ID}",
        json=EXAMPLE_CARD_RESPONSE
    )
