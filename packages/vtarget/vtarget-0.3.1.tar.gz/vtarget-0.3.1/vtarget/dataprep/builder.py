class Builder:
    ...
    def set_skip_childs_recursive(self, flow_id: str, node_key: str, data: dict):
        for child in self.nodes[node_key]["childs"]:
            ...
            for port_name in data["nodeDataArray"][child["to_id"]]["meta"]["ports_map"]["pout"]:
                data["nodeDataArray"][child["to_id"]]["meta"]["ports_map"]["pout"][port_name][
                    "head"
                ] = []
                data["nodeDataArray"][child["to_id"]]["meta"]["ports_map"]["pout"][port_name][
                    "rows"
    ...
    def analyzer(
        ...
        while num_nodes_ok < len(self.nodes):
            ...
            for node_key, node_data in self.nodes.items():
                ...
                ) or (disable_all_email_nodes and (node_data["type"] in [NodeType.EMAIL.value])):
                ...
                if is_loaded:
                    ...
                    # --------------------------------------------------
                    node_settings = (
                        cache_handler.settings[flow_id][node_key]
                    ...
                    if (
                        ...
                        == json.dumps(
                            data["nodeDataArray"][node_idx]["meta"]["config"], sort_keys=True
                        )
                        and (
                            "ports_config" not in node_settings
                            or (
                                "ports_config" in node_settings
                        ...
                        if node_data["type"] not in [
                            ...
                            for _, pin_name in enumerate(
                                ...
                                if (
                                    ...
                                    cache_handler.load(
                                        flow_id, to_load["node_key"], to_load["port_name"]
                                    )
                                    ...
                                    if (
                                        ...
                                        dtypes_list = data["nodeDataArray"][node_idx]["meta"][
                                            "ports_map"
                                        ]["pin"][pin_name]["dtypes"]
        ...
        try:
            ...
            if flow_id in cache_handler.cache and node_key in cache_handler.cache[flow_id]:
                ...
                if "pout" in cached_node and port_name in cached_node["pout"]:
                    ...
                    return {"flow_id": flow_id, "node_key": node_key, "node": node, "success": False, "error": f"The flow has not been run yet"}
        except Exception as e:
            return {"flow_id": flow_id, "node_key": node_key, "node": node, "success": False, "error": str(e)}
    ...
    def run_node(self, flow_id: str, node_key: str, node: dict, links_to: list):
        ...
        for ld in links_to:
            ...
            parent_cache = (
                cache_handler.cache[flow_id][parent_key]
                if flow_id in cache_handler.cache and parent_key in cache_handler.cache[flow_id]
                else dict()
            )

            if parent_cache and "pout" in parent_cache and parent_port in parent_cache["pout"]:
                input_port[node_port] = parent_cache["pout"][
                    parent_port
                ]  # actualiza pin del nodo con la salida correspondiente del padre
