...
def test_run_raster():
    ...
    results = run(data)
    assert [r if r is None else round(r, 2) for r in results] == [81, 1.04, 1.31, 296.81, 296.25]
...
def test_run_raster_missing_data():
    ...
    results = run(data)
    assert [r if r is None else round(r, 2) for r in results] == [15, 1, 1, 0.05, 1, 0, 0.2, 1.59, None, 0.29, 0, None]
