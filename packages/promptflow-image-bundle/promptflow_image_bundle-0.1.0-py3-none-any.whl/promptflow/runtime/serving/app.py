...
from promptflow.runtime.serving.utils import enable_monitoring, get_cost_up_to_now, on_stream_end, on_stream_start
...
def finish_monitoring(response):
    ...
    flow_id = g.get("flow_id", app.model_name)
    if data and flow_result and flow_result.output:
...
def score():
    ...
    streaming_start_func = None
    streaming_end_func = None
    if g.streaming:
        streaming_start_func = on_stream_start(app.metric_recorder, flow.id, g.start_time)
        streaming_end_func = on_stream_end(app.metric_recorder, flow.id, g.start_time)

    response_creator = ResponseCreator(
        flow_run_result=result,
        accept_mimetypes=request.accept_mimetypes,
        stream_start_callback_func=streaming_start_func,
        stream_end_callback_func=streaming_end_func,
    )
