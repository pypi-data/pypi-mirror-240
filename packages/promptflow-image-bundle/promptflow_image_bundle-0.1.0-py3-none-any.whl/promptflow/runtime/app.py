...
def kill_child_process(parent_pid):
    """Terminate child process."""
    logger.info("Terminate child process with parent pid %s", parent_pid)
    parent = psutil.Process(parent_pid)
    children = parent.children(recursive=True)
    for child in children:
        child.kill()
        logger.info("Successfully killed child process with pid %s", child.pid)

    parent.kill()


def signal_handler(signum, frame):
    logger.info(f"Runtime stopping, pid: {os.getpid()}")
    kill_child_process(os.getpid())
    ...
    try:
        ...
        if flow_request is not None and isinstance(run_tracker, LegacyRunTracker):
...
def submit_bulk_run():
    ...
    try:
        req = BulkRunRequestV2.deserialize(payload)
        req_id = request.headers.get("x-ms-client-request-id")
        with get_log_context_from_v2_request(req):
            # Please do not change it, it is used to generate dashboard.
            logger.info(
...
def submit():
    ...
    try:
        ...
        with get_log_context(flow_request), setup_contextvar(active_flow_request_context, flow_request):
            # Please do not change it, it is used to generate dashboard.
            logger.info(
...
def dynamic_list():
    ...
    return jsonify(gen_dynamic_list(func_path, func_kwargs, ws_triple))
