...
class ArtifactSystemError(SystemErrorException):
...
class ArtifactClient:
    ...
    @retry(ArtifactSystemError, tries=10, logger=logger)
    def register_artifact(self, run_id, datastore_name, relative_path, path):
        try:
            ...
            logger.info(f"Creating Artifact for Run {run_id}...")
            ...
            if response.status_code != 200:
                logger.error(
                    "Failed to create Artifact for Run %s. Code=%s. Message={customer_content}",
                    run_id,
            ...
            if response.status_code == 401 or response.status_code == 403:
                ...
                raise Exception(
                    "Failed to create Artifact for Run {}. Code={}. Message={}".format(
                        run_id, response.status_code, response.text
                    )
                )
        except UserErrorException:
            logger.exception("Create Artifact for Run %s failed with user error.", run_id)
            raise
        except Exception as ex:
            logger.error(
                "Create Artifact for Run %s failed. exception={customer_content}",
                run_id,
                extra={"customer_content": ex},
                exc_info=True,
            )
            raise ArtifactSystemError(f"Failed to create Artifact for {run_id}: {str(ex)}") from ex
