...
from promptflow.runtime._errors import (
    ...
    RetriableTableStorageError,
    RunInfoNotFoundInStorageError,
    RunNotFoundInTable,
    RunStorageConfigMissing,
...
from promptflow.runtime.constants import (
...
from promptflow.runtime.storage.run_storage import AbstractRunStorage, deserialize_flow_run_info
...
from promptflow.runtime.utils._utils import get_mlflow_tracking_uri, get_string_size, is_in_ci_pipeline
...
class AzureMLRunStorage(AbstractRunStorage):
    ...
    def init_azure_table_service_client(self, credential, storage_account_name):
        ...
        try:
            ...
            error_status_code = get_status_code(e)
            error_error_code = get_error_code(e)
            raise TableInitResponseError(
                target=ErrorTarget.AZURE_RUN_STORAGE,
                message_format=(
                    "Failed to get or create table due to HttpResponseError with "
                    f"status_code: {error_status_code}, error_code: {error_error_code}. "
                    f"The exception details: {{inner_ex}}."
                ),
                inner_ex=msg,
            ) from e
            ...
            msg = to_string(e)
            error_message = (
                "An issue occurred while attempting to retrieve or create the storage "
                "due to a transient ServiceRequestError. Please retry the operation. Exception: {customer_content}"
            )
            logger.error(
                error_message,
                extra={"customer_content": msg},
            )
            error_type = get_exception_type(e)
            error_status_code = get_status_code(e)
            error_error_code = get_error_code(e)
            raise RetriableTableStorageError(
                target=ErrorTarget.AZURE_RUN_STORAGE,
                message_format=(
                    "An issue occurred while attempting to retrieve or create the storage due to a "
                    "transient ServiceRequestError. Please retry the operation. "
                    f"exception type: {error_type}, status code: {error_status_code}, error_code: {error_error_code}, "
            ...
            msg = to_string(e)
            error_message = "Failed to get or create table due to Unexpected exception: {customer_content}"
            logger.error(
                error_message,
                extra={"customer_content": msg},
            )
            ...
            raise TableStorageInitError(
                target=ErrorTarget.AZURE_RUN_STORAGE,
                message_format=(
                    "Failed to get or create table due to Unexpected exception. "
                    f"exception type: {error_type}, status code: {error_status_code}, error_code: {error_error_code}, "
                    f"exception: {{exception_str}}."
                ),
                exception_str=msg,
    ...
    def _write_azure_table(self, record_dict: dict, table_operation_method: Callable):
        ...
        try:
            ...
            raise TableStorageWriteError(
                target=ErrorTarget.AZURE_RUN_STORAGE,
                message_format=(
                    "Failed to perform azure table operations. "
                    f"exception type: {error_type}, error code: {error_status_code}, exception: {{exception_str}}."
                ),
                exception_str=msg,
            ) from e
    ...
    def cancel_flow_child_runs(self, flow_id: str, run_id: str):
        run_infos = self._get_flow_child_runs(flow_id, run_id)
        for run_info in run_infos:
            if not Status.is_terminated(run_info.status):
                run_info.status = Status.Canceled
                self._create_or_update_flow_run_info(run_info)
    ...
    def _get_flow_child_runs(self, flow_id: str, run_id: str) -> List[FlowRunInfo]:
        run_infos = []
        try:
            # Query the table using the partition key
            query_filter = (
                "(PartitionKey eq '{flow_id}') and ((root_run_id eq '{run_id}') or (parent_run_id eq '{run_id}'))"
            ).format(flow_id=flow_id, run_id=run_id)
            records = self.flow_table_client.query_entities(query_filter=query_filter)
        except Exception as error:
            error_message = to_string(error)
            content = f"Partition_key={flow_id}, run_id={run_id}, error_message={error_message}"
            refined_error_msg = "Failed to get flow child runs. {customer_content}"
        ...
        for record in records:
            ...
            if rowkey != run_id:
                run_record = self._convert_entity_to_run_info(record)
                run_infos.append(deserialize_flow_run_info(run_record))

        logger.info(f"Successfully get {len(run_infos)} child runs.")
        return run_infos
    ...
    def _init_azure_run_storage(storage_account_name):
        ...
        from promptflow._legacy.azureml_run_storage import AzureMLRunStorage
        from promptflow.runtime.utils._utils import _get_default_credential
    ...
    def get_api_call_cred(self):
        try:
            ...
            raise InvalidClientAuthentication(
                message="Failed to get mlflow credential", target=ErrorTarget.AZURE_RUN_STORAGE
            ) from ex
        except Exception as e:
