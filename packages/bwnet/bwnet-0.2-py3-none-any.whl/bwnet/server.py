...
def _serve_client(skey: selectors.SelectorKey, mask: int):
    ...
    if mask & selectors.EVENT_READ:
        ...
        if cl_sock is None:
            ...
            if len(b_msg) == 0:
                _logger.warning(f'Null response from the client process {cl_proc}, it will be disconnected')
    ...
    if mask & selectors.EVENT_WRITE:
        if not cl_is_losing_conn and any(_cl_buffs):  # If some buffer is filled, make sure it is not this client's.
...
def init(host: str, port: int, max_clients: int, buffer_size: int = 256, loop_fps: int = 0, show_logs: bool = False) -> int:
    global _sel, _lis_sock, _loop_iter_dur, _max_clients, _buffer_size, _cl_socks, _cl_procs, _cl_buffs, is_initialized
    if is_initialized:
    ...
    threading.Thread(target=_handle_selector).start()
...
def close() -> int:
    ...
    if is_initialized:
        for cl_sock in filter(lambda s: s is not None, _cl_socks):
            _forget_client(sock=cl_sock)
