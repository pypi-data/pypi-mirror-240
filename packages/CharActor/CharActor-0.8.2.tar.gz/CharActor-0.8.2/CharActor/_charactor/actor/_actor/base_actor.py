...
from CharObj import Armory, Goods
...
class BaseCharacter(BaseActor):
    ...
    def move(self, direction: str = None, cell: object | str = None):
        ...
        if direction is not None and direction in {
            ...
            self._update_nearby()
            return f'{self.actions["move"][self.grid_entity.movements-1]["from"]} --> {self.actions["move"][self.grid_entity.movements-1]["to"]}'
    ...
    def look_around(self):
        ...
        if len(self._nearby_items) > 8:
            ...
            {self._see_item(item, direction) for item, direction in zip(self._nearby_items.values(), directions)}
