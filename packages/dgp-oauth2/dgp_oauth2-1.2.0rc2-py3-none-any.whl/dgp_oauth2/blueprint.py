...
from .credentials import \
    ...
    private_token, db_connection_string

def register_blueprint(controllers, register):
    def func(app, options, *args, **kwargs):        
        controllers.oauth.init_app(app)
        return register(app, options, *args, **kwargs)
    return func



def make_blueprint(external_address, engine=None):
    """Create blueprint.
    """

    models = Models(db_connection_string, engine=engine)
    ...
    def oauth_callback_():
        ...
        return redirect(oauth_callback_controller(state, callback_url(), private_token, set_session))
