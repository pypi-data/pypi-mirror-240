...
def industry_stock_sw(industry='801270',top=5,printout=False):
    ...
    """
    if printout:
        title_txt="申万行业指数成分股排名与权重："+industry_sw_name(industry)+'('+industry+')'
...
def find_peers_china(industry='',top=5,rank=20,sw_level='2'):
    """
    功能：
    当industry = ''，显示申万三级行业分类
    当industry 1=''，显示申万三级行业分类的头top个股票及其代码，返回这些股票代码列表
    
    itype：申万一级行业'I'，二级行业'T'，三级行业'3'
    """
    ...
    if industry == '':
        ...
        print_industry_sw(itype=itype,numberPerLine=5,colalign='left')
        return None
