...
def process_device(input_device):
    ...
    nd_results = nd.get_port_data(device['ip'])
    ...
    for port in nd_results:
        ...
        if r_device.model.startswith("EX") and r_device.os_version.startswith('12'):
            ...
            # names to descriptions and move on to the next port
                ...
                nonels_port_desc[port['port']] = port['name']
                continue
        ...
        if port['time_last']:
            active_port = True
            host_last_seen = port['time_last'].strftime("%m/%d/%Y %H:%M:%S")

            # if the user provided a time range to check for, set a port to
            # non-active if the last MAC was seen before the time frame
            if(args.time_range):
                time_delta = (now - port['time_last'])
                if int(time_delta.total_seconds() / 3600) > args.time_range:
                    active_port = False
        
        # if we've never seen a MAC on the port, it is not active
        else:
            host_last_seen = "Never"
            active_port = False

        # adding to our per-vlan active port count
        if active_port:
            if port['vlan'] not in vlan_data:
                vlan_data[port['vlan']] = 0
            vlan_data[port['vlan']] += 1
    ...
    if nonels_port_desc:
        for r in results:
            if r['port'] in nonels_port_desc:
                r['description'] = nonels_port_desc[r['port']]

    results.sort(key=interface_sort)
    gs.create_or_overwrite_worksheet(device['name'].replace(".umnet.umich.edu.",""), results)
...
if __name__ == '__main__':
    ...
    vlan_data_list = [{'vlan':k, 'count':v} for k,v in vlan_data.items()]
