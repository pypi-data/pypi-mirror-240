...
class Umnetdisco(UMnetdb):
    ...
    def get_port_data(self, name_or_ip):
        '''
        Pulls physical port details for a particular switch by name or IP.
        Ignores ports with switch neighbors.
        ...
        - description
        - vlan
        - last mac seen on the port
        - date last mac was seen
        '''

        # first we need to figure out what type of switch this is so we know
        # how to identify physical ports only
        nd_device = self.get_device(name_or_ip)
        if not(nd_device):
            raise LookupError(f'Could not find {nd_device} in Netdisco')
        

        # Our basic sql builder doesn't support 'select distinct on' so we're building
        # the whole query string by hand
        sql = f'''
select distinct on (dp.port, dp.name)
dp.port, dp.name, dp.descr,
REPLACE(dp.up,'lowerLayerDown','down') as oper_status,
dp.up_admin as admin_status,
COALESCE(NULLIF(dp.vlan,'0'), node.vlan) as vlan,
dp.remote_ip,
node.mac,
node.time_last,
node_ip.ip
...
where dp.ip='{nd_device['ip']}'
order by dp.port, dp.name, node_ip.time_last
'''
