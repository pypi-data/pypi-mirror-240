...
class GymEnvironment(Environment):
    def __init__(
        ...
        **gym_kwargs,
        ...
        if reward_space is not None and reward_space != "":
            self.reward_space = eval(reward_space)
        else:
            self.reward_space = Box(float("-inf"), float("inf"), (1, 1))
            LOG.warning(
                "No 'reward_space' found in params, defaulting to Box(float('-inf'), float('inf'), (1,1))"
            )
    ...
    def start_environment(self) -> EnvironmentBaseline:
        ...
        return EnvironmentBaseline(
            sensors_available=[
                obs_to_sensors(
                    obs, self.env.space, self.conversion
                )
            ],
            actuators_available=[
                space_to_actuator(self.env.space, self.conversion)
            ],
        )
    ...
    def update(self, actuators: List[ActuatorInformation]) -> EnvironmentState:
        ...
        return EnvironmentState(
            ...
            rewards=[
                RewardInformation(reward, self.reward_space, "gym.reward")
            ],
            done=done,
        )
