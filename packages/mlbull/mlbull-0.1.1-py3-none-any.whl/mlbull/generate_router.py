...
from .utils import get_decorators, get_module_from_filename, sanitize_module_name
...
def generate_router(filename: str):
    ...
    try:
        if filename.endswith(".py"):
            ...
            module, logger, notebook_html = get_notebook_as_module(path.name, str(path))
        else:
            raise NotValidPackageException(f"{filename} is not something that looks like python")
