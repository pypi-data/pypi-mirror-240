...
# 	Copyright (c) 2007, 2008, 2009, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023 R.Dreas Nielsen
...
class CommandList(object):
	...
	def run_and_increment(self):
		...
		if compiling_loop:
			...
			subvars.add_substitution("$CURRENT_TIME_UTC", datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M"))
...
class DisplayUI(object):
	...
	def __init__(self, title, message, button_list, selected_button=0, no_cancel=False, column_headers=None,
		...
		if self.free:
			...
			ok_btn.grid(column=0, row=0, sticky=tk.E, padx=3)
		else:
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+MESSAGE\s+"(?P<message>(.|\n)*)"\s+DISPLAY\s+', r'(?:\s+(?P<free>FREE))?\s*$'), x_prompt)
