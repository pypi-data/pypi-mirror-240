...
from .keys import ATTRIBUTES, BUCKET_NAME, FILE_NAME, IMAGES, IMAGE, ANNOTATIONS, BBOX, BOX_BOUNDARIES_AND_DIMENSIONS, JOB_ID, STORAGE_NAME, STORAGE_PATH, VALUE, X, Y, W, H, POINTS, LABEL, \
    ...
    def upload_annotation_json(
        ...
        if "isSuccess" in label_references:
            if label_references["isSuccess"] == False:
                raise Exception(f'Error in retrieving datalake label references: {label_references["message"]}')
        ...
        if (upload_type == AnnotationUploadType.BY_FILE_NAME or upload_type == AnnotationUploadType.BY_FILE_NAME_OR_UNIQUE_NAME):
            if unique_id is not None and unique_id != "":
                collection_object = self._client.get_collection_details(unique_id, {"storagePath":True})
            ...
            if unique_id is not None and unique_id != "":
                ...
                raise Exception(f'job id is required for upload annotation data by job id')
