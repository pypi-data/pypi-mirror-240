...
class StudioClient:
    ...
    def create_project(self, project_name, selection_id, fps, frames_per_task, assign_to_all, send_email, default_shape_type, content_type):
        if project_name == '':
            print('Project name is empty')
            return None
        #print('project details: ', project_name, selection_id)
        _project = Project(client=self)
        response = _project.create_project(project_name, selection_id, fps, frames_per_task, assign_to_all, send_email, default_shape_type, content_type)
        return response
    
    """
    update the project
    """
    def update_project(self, project_id, selection_id, fps, frames_per_task, assign_to_all, send_email, default_shape_type, content_type):
        #print('project details: ', project_id, selection_id)
        _project = Project(client=self)
        response = _project.update_project(project_id, selection_id, fps, frames_per_task, assign_to_all, send_email, default_shape_type, content_type)
        return response
