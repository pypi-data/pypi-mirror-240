...
class TesseractU(PyApiB):
    ...
    def image_to_num_by_folder(self, imgU:ImgU, folderPath:str):
        ...
        for iii in range(1,len(result)):
            if result[iii][0]-result[iii-1][0] < 3:
                # 矩离太近了不要了
                pass
            elif result[iii][0]-result[iii-1][0] < charW+3:
        ...
        for x, y, w, h in result:    
            ...
            ImgU().initImg(digit).save(f"{zzz}.png")
            
            zzz+=1
            res = []
            for key in temps:
                score = cv2.matchTemplate(digit, temps[key], cv2.TM_CCORR_NORMED)
                # TODO 改为计算相同的数量比
                res.append((key, score[0]))
