...
def _create_table_function_expression(
    ...
    **named_args: ColumnOrName,
    ...
    table_function_expression.partition_spec = (
        TableFunctionPartitionSpecDefinition(over, partition_by, order_by) if over else None
    )
...
def _get_cols_after_join_table(
    ...
    current_plan: TrinoPlan,
    join_plan: TrinoPlan,
) -> Tuple[List, List]:
    def get_column_names_from_plan(plan: TrinoPlan) -> List[str]:
