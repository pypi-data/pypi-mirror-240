...
class Component:
    ...
    def __init__(self, name, stylesheet=None, script=None):
        ...
        try:
            ...
            self.script = (
                '<script src="' + script + '"></script>' if script != None else ""
            )
            self.components = {}
        finally:
            ...
            for index in range(0, len(components)):
                if type(components[index]) == object:
                    ...
                    for index2 in range(0, len(inner_list)):
                        try:
                            self.components[
                                components[index][index2].name
                            ] = components[index][index2]
                        except AttributeError:
                            raise TypeError(
                                f"Lemon.Component: add() must be a list of components -> {components[index][index2]}"
                            )
                else:
                    try:
                        self.components[components[index].name] = components[index]
                        ...
                        raise TypeError(
                            "Lemon.Component: add() must be a list of components"
                        )
        else:
    ...
    def parse(self, Root, prop: list):
        ...
        def parse_html(component, root):
            ...
            for line in component:
                ...
                try:
                    component_props = (
                        lines.split(" ", 1)[1] if len(lines.split(" ")) > 1 else ""
                    )
                    if component_props != "":
    ...
    def render(self, app: str):
        ...
        for component in app:
            if component != "":
                ...
                component_props = (
                    component.split(" ", 1)[1] if len(component.split(" ")) > 1 else ""
                )
                if component_props != "":
                    component_props = re.split(r"\s(?=\w+=)", component_props)
                    props = {}
