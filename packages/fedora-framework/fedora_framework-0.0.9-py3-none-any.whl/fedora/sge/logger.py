...
def save_progress_to_file(data):
    with open('%s/run_%d_%d/progress_report.csv' % (params['EXPERIMENT_NAME'], params['RUN'], params['SEED']), 'a') as f:
        f.write(data + '\n')
...
def save_step(generation, population):
    ...
    open('%s/run_%d_%d/iteration_%d.json' % (params['EXPERIMENT_NAME'], params['RUN'], params['SEED'], generation), 'a').write(c)


def save_parameters():
    params_lower = dict((k.lower(), v) for k, v in params.items())
    c = json.dumps(params_lower)
    open('%s/run_%d_%d/parameters.json' % (params['EXPERIMENT_NAME'], params['RUN'], params['SEED']), 'a').write(c)


def prepare_dumps():
    try:
        os.makedirs('%s/run_%d_%d' % (params['EXPERIMENT_NAME'], params['RUN'], params['SEED']))
