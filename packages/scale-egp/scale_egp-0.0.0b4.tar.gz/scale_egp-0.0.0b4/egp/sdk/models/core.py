...
from pydantic import (
    Field, validator, ValidationError,
)

from egp.sdk.enums import (
    TestCaseSchemaType, EvaluationStatus, ArtifactSource, UploadJobStatus,
)
from egp.sdk.models.sub import (
    EmbeddingConfig, ChunksStatus, EvaluationConfig, TestCaseData, GenerationTestCaseResultData,
    S3DataSourceConfig,
    SharePointDataSourceConfig, GoogleDriveDataSourceConfig, LocalChunksSourceConfig,
    CharacterChunkingStrategyConfig, CrossEncoderRankStrategy, RougeRankStrategy, ArtifactsStatus,
    GenerationTestCaseData, ModelParameters, SharePointDataSourceAuthConfig,
    ChunkToUpload,
)
from egp.utils.model_utils import (
    BaseModel, Entity, RootModel,
)
...
class TestCaseSchemaValidator:
    ...
    def validate(cls, schema_type: TestCaseSchemaType, data: Dict[str, Any]):
        try:
            ...
            raise ValueError(
                f"Test case data does not match schema type {schema_type}"
            ) from e
...
class ChunkSynthesisResponse(BaseModel):
    response: str = Field(
        ..., description="Natural language response addressing the query."
    )
