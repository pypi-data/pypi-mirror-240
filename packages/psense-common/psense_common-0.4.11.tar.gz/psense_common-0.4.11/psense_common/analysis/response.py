...
class StepResponse(object):
    ...
    def compute_sensor_response(self, channels=[]):
        ...
        for channel in channels:
            ...
            cum_deriv = clip - clip.iloc[0]  # np.cumsum(deriv)
            goal_start = 0.1 * cum_deriv.iloc[-1]
            goal_end = 0.9 * cum_deriv.iloc[-1]
        ...
        for channel in channels:
            ...
            for start, finish in zip(tm_start, tm_finish):
                ...
                if sr.is_valid():
                    ...
                    self.result[channel] = pd.concat(
                        [self.result[channel], sr.output],
                        axis=0,
                        join="outer",
                        ignore_index=True,
                    )
