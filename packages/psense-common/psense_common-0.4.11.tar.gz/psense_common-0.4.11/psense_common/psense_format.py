...
class psense_exp_title:
    ...
    def validate(self, experiment_str: str):
        name_metadata = self.decode(experiment_str)
        try:
            assert isinstance(name_metadata, dict)
            assert isinstance(name_metadata["study"], dict)
            assert isinstance(name_metadata["sensor"], dict)

            assert name_metadata["study"]["type"]
            assert len(name_metadata["study"]["run"]) < 3
            assert len(name_metadata["study"]["device"]) > 2

            sensor_code = name_metadata["sensor"]["code"].split("-")
            assert len(sensor_code) == 3
            assert len(sensor_code[0]) == 4
            assert len(sensor_code[1]) == 3
            assert len(sensor_code[2]) == 2

            assert len(name_metadata["sensor"]["vsn"]) == 2  # 2-digit vsn

            assert isinstance(
                name_metadata["sensor"]["electrodes"], list
            )  # design config defined
            # then check for either:
            we_count = name_metadata["sensor"]["count"]
            if we_count.isnumeric():
                assert len(name_metadata["sensor"]["electrodes"]) == int(we_count)
                assert len(experiment_str) == 25 + len(
                    name_metadata["study"]["device"]
                ) + 3 * int(
                    we_count
                )  # three letters per electrode, no more or less, or ...
            else:
                assert len(name_metadata["sensor"]["electrodes"]) == 1
                assert len(experiment_str) == 29 + len(
                    name_metadata["study"]["device"]
                )  # 32 letters in the old experiment id format

            assert isinstance(name_metadata["sensor"]["electrodes"][0], dict)

        except BaseException:
            # some validation check failed, return false
            return False

        # passed all validation checks, return true
        return True
