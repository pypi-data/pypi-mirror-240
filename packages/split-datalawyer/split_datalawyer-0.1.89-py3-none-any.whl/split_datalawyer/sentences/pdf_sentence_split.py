...
from split_datalawyer.utils.pdf_utils import clean_text, is_upper, strip_punctuation, is_cnj_number, \
    ...
    merge_split_words, merge_split_sections, load_pdf_lines_from_path
...
class PDFSentenceSplit(BaseSentenceSplit):
    ...
    def get_sentences_from_path(self, pdf_path: Union[str, Path]) -> List[str]:
        return self.get_sentences_from_lines(lines=load_pdf_lines_from_path(pdf_path))

    def get_sentences(self, text: str):
        text = self._preprocess_text(text)
        if text is not None:
            return self.get_sentences_from_lines(text.splitlines())
        else:
            return []

    def get_sentences_from_lines(self, lines: List[str]):
        ...
        for idx, line in enumerate(lines):
            if line in count and count[line] >= max_occurrences:
                continue
            elif self._remove_stop_phrase and self._is_stop_phrase(line):
                continue
            if not line.strip():  # Verifica se a linha está em branco
                ...
                if is_upper(strip_punctuation(line)) or is_cnj_number(line):
                    if current_paragraph:
                        paragraphs.append(current_paragraph.strip())
                        current_paragraph = ""
                    paragraphs.append(line.strip())

                elif any([line.endswith(punctuation) for punctuation in final_punctuations]) or (
                    ...
                    if current_paragraph:
                        paragraphs.append(current_paragraph.strip())
                        current_paragraph = ""
                    if any([line.endswith(punctuation) for punctuation in final_punctuations]):
                        paragraphs.append(line.strip())
                    else:
                        current_paragraph += line + " "
        ...
        return [
            ...
            merge_split_sections(split_by_sentence(
                ...
                split_parenthesis_ending=self._split_parenthesis_ending,
