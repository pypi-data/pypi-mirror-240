...
def generate_file_name(output_chunks: str, chunked_document: ChunkedDocument, file_extension: str) -> str:
    file_name = hashlib.sha256(chunked_document.source.filename.replace("\\", "_").replace("/", "_").encode("utf-8")).hexdigest()
...
def main(args, logger, activity_logger):
    ...
    extension_loaders = get_extension_loaders_with_document_intelligence(doc_intel_connection_id=args.doc_intel_connection_id)
