...
def read_chunks_into_documents(files: Iterator[pathlib.Path], chunk_format: str = "csv") -> Iterator[Document]:
    ...
    files = [file for file in files if not file.is_dir() and file.name.lower().endswith((".jsonl", ".csv"))]
    num_files = len(files)
...
def load_embeddings_container(embeddings_model_uri: str, connection_args: dict, logger, embeddings_container_path: Optional[str] = None, snapshot_dir_name: Optional[str] = None, metadata_only: bool = False, documents_only: bool = True):
    ...
    if embeddings_container_path is not None:
        with track_activity(logger, "init.load_embeddings_container") as activity_logger:
            ...
            if snapshot_dir_name:
                embeddings_container = EmbeddingsContainer.load(snapshot_dir_name, pathlib.Path(embeddings_container_path), metadata_only=metadata_only, documents_only=documents_only)
            else:
                embeddings_container = EmbeddingsContainer.load_latest_snapshot(pathlib.Path(embeddings_container_path), metadata_only=metadata_only, documents_only=documents_only, activity_logger=activity_logger)
...
def chunk_embedder(worker_id: int, chunk_queue: mp.Queue, results_queue: mp.Queue, embeddings_model_uri: str, connection_args: dict, output: str, embeddings_container: Optional[str] = None, snapshot_dir_name: Optional[str] = None, metadata_only: bool = False, logger_queue: Optional[Any] = None):
    ...
    try:
        ...
        embedder = load_embeddings_container(embeddings_model_uri, connection_args, logger, embeddings_container, snapshot_dir_name=snapshot_dir_name, metadata_only=metadata_only)
...
def _merge_and_log_worker_results(results_queue, merged_results: dict, activity_logger, results_callback: Optional[Callable[[Dict], None]] = None):
    got_results = False
    while True:
        try:
    ...
    if got_results and results_callback:
        results_callback(merged_results, activity_logger)
...
def create_embeddings(chunks: Iterator[Document],
                      ...
                      output: str,
                      snapshot_dir_name: Optional[str] = None,
                      metadata_only: bool = False,
                      activity_logger=None,
                      results_callback: Callable[[Dict, Any], None] = None,
                      batch_size: int = 100,
                      num_workers: int = -1,
    ):
    """
    ...
    `results_callback` is called with a dictionary containing two entries:
        - "documents_embedded": number of documents embedded
        - "documents_reused": number of documents reused
    and an `activity_logger`
    """
    ...
    with mp.Manager() as manager:
        try:
            ...
            with mp.Pool(processes=num_workers) as pool:
                ...
                embedder_futures = [pool.apply_async(chunk_embedder, (i, chunk_queue, results_queue, embeddings_model_uri, connection_args, output, embeddings_container, snapshot_dir_name, metadata_only, logger_queue)) for i in range(num_workers)]
                ...
                for chunk in chunks:
                    ...
                    if len(chunk_batch) == batch_size:
                        ...
                        while True:
                            ...
                            merged_results = _merge_and_log_worker_results(results_queue, merged_results, activity_logger, results_callback)
                            try:
...
def main(args, logger, activity_logger):
    ...
    def log_embeddings_results(results: dict, activity_logger=None):
        write_status_log(
            ...
            total_units=results["documents_embedded"],
        ...
        write_status_log(
            ...
            total_units=results["documents_reused"],
            processed_units=results["documents_reused"],
            ...
            try:
                ...
                with rslex_uri_volume_mount(args.embeddings_container, f"{os.getcwd()}/embeddings_container", options=mnt_options) as mount_context:
                    try:
                        create_embeddings(
                            chunks=chunks,
                            embeddings_model_uri=args.embeddings_model,
                            connection_args=connection_args,
                            embeddings_container=mount_context.mount_point,
                            output=args.output,
