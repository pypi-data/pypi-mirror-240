...
def list_mlindex_assets(client, workspace: Workspace):
    data_assets = client.mlindex.list(
...
def register_new_data_asset_version(client, run: Run, asset_name: str, uri: str, properties: dict, output_type: str = 'UriFolder'):
    workspace = run.experiment.workspace
    output_type = output_type
    try:
        create_dto = DataVersion(
            data_type=output_type, data_uri=uri, run_id=run.id, properties=properties)
        ...
        if hasattr(ex, 'model') \
            ...
            and ('OutputName is invalid' in ex.model.error.message
                    or 'does not have permissions for Microsoft.MachineLearningServices/workspaces/datasets/registered/write actions' in ex.model.error.message):
            raise UserErrorException(
                f'Data asset creation API failed with user error:{ex.model.error.message}')
        raise RuntimeError(
            f'Data asset creation API failed with system error:{ex}')
...
def _save_asset_output_lineage(run: Run, asset_id: str, asset_type: str, output_name: str, rest_client):
    #with _tracer.start_as_current_span('data-capability._save_asset_output_lineage'):
        logger.info('saving asset output lineage for output', extra={
        ...
        try:
            ...
            update_dto = CreateRun(
                run_id=run.id, outputs=outputs, experiment_id=run.experiment.id)
            ...
            logger.error('Cannot update output asset lineage',
