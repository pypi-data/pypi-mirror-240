...
def generateLogPlot(
    outpath, centrality, clusterNr, centralityDF, clusterDF,
    additionalDF=False, save=False, upperLimitBin=1.0, lowerLimitBin=0.0000001
):
    ...
    cluster = clusterDF.query(
        'centrality == @centrality and cluster == @clusterNr and bin > @lowerLimitBin and bin <= @upperLimitBin'
    ).drop('centrality', axis=1).drop('cluster', axis=1)

    mergedcluster = cluster.merge(
        cent,
        left_on=['year', 'bin'],
        right_on=['year', 'bin'],
        how='inner'
    ).set_index(['year', 'bin']).sum(axis=1).reset_index()
    ...
    if additionalDF is not False:
        clusteraddition = additionalDF.query(
            'centrality == @centrality and cluster == @clusterNr and bin > @lowerLimitBin and bin <= @upperLimitBin'
        ).drop('centrality', axis=1).drop('cluster', axis=1)
        mergedaddition = clusteraddition.merge(
            mergedcluster,
            left_on=['year', 'bin'],
            right_on=['year', 'bin'],
            how='inner'
        ...
        if mergedaddition.shape[0] > 0:
            ...
            z = mergedaddition[0]
            ax.scatter(x, y, z, color='red', marker="d", linewidth=2, antialiased=True, alpha=0.6)
   
    ax.scatter(X, Y, Z, color='black', marker='.', linewidth=1, antialiased=True, alpha=0.4)
    ax.plot_trisurf(baseX, baseY, baseZ, cmap="coolwarm", linewidth=0.4, antialiased=True, alpha=0.2)
