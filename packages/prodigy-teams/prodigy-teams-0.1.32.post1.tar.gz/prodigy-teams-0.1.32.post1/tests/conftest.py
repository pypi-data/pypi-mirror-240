...
ALL_COMMANDS = list(cli.commands.values())
for cmd in cli.subcommands.values():
    ALL_COMMANDS.extend(cmd.values())
ALL_COMMANDS_NO_PLACEHOLDERS = [c for c in ALL_COMMANDS if not c.is_placeholder]
...
@pytest.fixture(scope="session")
def subcommands() -> List[Command]:
    return ALL_COMMANDS


@pytest.fixture(params=ALL_COMMANDS, scope="session")
def command(request) -> Command:
    return request.param


@pytest.fixture(params=ALL_COMMANDS_NO_PLACEHOLDERS, scope="session")
def command_no_placeholder(request) -> Command:
    return request.param


# @pytest.fixture
# def command_module(command_name: str) -> ModuleType:
...
def req_mock_auth_broker(tmp_path, monkeypatch):
    monkeypatch.setenv("_PTC_TESTING_API_HOST", "http://localhost:8000")
    import prodigy_teams.commands._state as state

    with state.cli_context(config_dir=tmp_path / "config"):
        yield

    # TODO FIXME:
    # - finish switching over to httpx
    # - replace with httpx mocking
    # - add cli.json endpoint response, since that's now mandatory for anything
    #   involving auth
    #
    # config = CLIConfig(
    ...
    #     m.post(f"{config.api_host}/api/v1/login", json=identity_response)
    #     m.get(f"{broker_address}/api/v1/broker/meta", json=broker_meta_response)
    #     m.get(
    #         f"{broker_address}/api/v1/broker/info",
    #         json={"data_bucket": "broker-data", "version": "0.4.12"},
    #     )

    #     # Sources
    #     m.post(
    #         f"{broker_address}/api/v1/sources/",
    #         json={"id": 1337, "title": "New Source"},
    #     )
    #     m.post(f"{broker_address}/api/v1/sources/1337/upload", status_code=200)
    ...
    #     m.post(f"{broker_address}/api/v1/tasks/", json=identity_response["tasks"])
