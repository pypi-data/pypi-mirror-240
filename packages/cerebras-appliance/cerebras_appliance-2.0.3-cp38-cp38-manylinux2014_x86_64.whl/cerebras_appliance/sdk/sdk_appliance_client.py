...
class SdkContext:
    ...
    def __init__(self, sdk_mode: SdkMode, **kwargs):
        ...
        coord_address = kwargs.get("coord_address", None)
    ...
    def _init_mgmt_client(self, **kwargs):
        ...
        coord_cpu = int(kwargs.get("coord_cpu", 24000))
        coord_mem = int(kwargs.get("coord_mem", 67 << 30))
        ...
        compile_memory_bytes = coord_mem  # current default ram memory for coord
        compile_cpu_millicore = (
            coord_cpu  # current default num of cpus for coord
        )
        ...
        if response['service_url'].count(':') == 1:
            self._coord_address = response['service_url']
        else:
            second_colon_idx = response['service_url'].rfind(':')
            self._coord_address = response['service_url'][:second_colon_idx]
    ...
    def _init_appliance_stub(self):
        ...
        wrk_address = self._coord_address
    ...
    def _set_credentials_path(self, value):
        ...
        if self._credentials_path:
            with open(self._credentials_path, 'r', encoding="utf8") as f:
...
class SdkClient:
    ...
    def __init__(self, sdk_mode: SdkMode, **kwargs):
        ...
        if os.path.exists("sdk_test_params.json"):
            with open("sdk_test_params.json", "r", encoding="utf8") as fparam:
                ...
                # kwargs["authority"] = data["authority"]
                # kwargs["credentials_path"] = data["credentials_path"]
                kwargs["coord_mem"] = data["coord_mem"]
                kwargs["coord_cpu"] = data["coord_cpu"]
                kwargs["relative_compile_dir"] = data["relative_compile_dir"]

        self._stub = None

        # coord_address = "localhost:50051"
        # kwargs["coord_address"] = coord_address
    ...
    def _check_sdk_response(
        self,
        response: sdk_appliance_pb2.SdkResponse, preamble: str
    ) -> None:
...
class SdkRuntime(SdkClient):
    ...
    def __init__(self, artifact_id: str, **kwargs,) -> None:
        ...
        self._check_sdk_response(response, "SdkRuntime creation")
    ...
    def stop(self):
        ...
        if len(self._nonblock_task) > 0:
            LOGGER.debug(
                "SdkRuntime.stop() is waiting for unfinished commands")
    ...
    def _check_task_status(self, task: Task, wait: bool) -> bool:
        ...
        if task.is_done:
            ...
            if wait:
                ...
                response_iterator = self._stub.sdk_runtime_is_task_done(
                    request)
...
class SdkCompiler(SdkClient):
    ...
    def compile(self, app_path: str, csl_main: str, options: str,) -> str:
        ...
        Returns the artifact_id corresponding to the compiled program
        """
        ...
        if response.code != sdk_common_pb2.StatusCode.SDK_RT_ALREADY_COMPILED:
            ...
            self._check_sdk_response(
                response, "SdkCompiler.compile() failed."
            )
        else:
            LOGGER.info("Application was found in the compile cache.")

        artifacts_dir = compile_hash
