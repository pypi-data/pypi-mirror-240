...
from cerebras_appliance.cluster_client import (
    ClusterJobInitError,
...
# temporary client side sparsity mask applicator
def _sparsity_group_closure(tensors: List[TensorSendPayload]):
    mask = tensors[0].tensor
    for tensor in tensors[1:]:
        tensor.tensor[~mask] = float('nan')
    # Clear mask, its not used.
    tensors[0].tensor = np.array([], dtype=np.bool)
...
class TensorGroup:
    ...
    Args:
        ...
        closure: An optional closure to apply to the tensors after loading
            them, modifying them in-place
    """

    tensors: List[TensorSendPayload]
    closure: Optional[Callable[[List[TensorSendPayload]], None]] = None
...
class ApplianceManager(ABC, ClassLogger):
    ...
    def __init__(
        ...
        self._set_credentials_path(self._cs_config.credentials_path)
        ...
        self._mgmt_client_args = {
            ...
            "client_lease_strategy": self._debug_args.debug_mgr.client_lease_strategy,
        ...
        if use_cs_grad_accum:
            set_ini(self._debug_args, ws_opt_disable_grad_accum=False)
        ...
        self.logger.verbose(f"Credentials path: {self._credentials_path}")
    ...
    def _set_credentials_path(self, value):
        """Sets the credentials path, loading the credentials from the file."""
    ...
    def grpc_client(self):
        ...
        self.logger.debug(
            ...
            f"{self._credentials}, {self._default_authority}"
        )
    ...
    def request_execute_job(
        ...
        Returns:
            reponse: Protobuf message ExecuteJobResponse as a dict appended with the
                'credentials_path' field which the caller can use to configure the coordinator grpc
                channel.
        """
    ...
    def stage_execute_coordinator(
        ...
        self._set_credentials_path(resource_response["credentials_path"])
    ...
    def compile_cluster(
        ...
        with ClusterManagementClient(**self._mgmt_client_args) as mgmt_client:
            ...
            self._set_credentials_path(response["credentials_path"])
    ...
    def send_weights(
        ...
        if any(len(g.tensor_names) > 1 for g in tensor_rt_groups):
            ...
            def server_grouper(
                ...
                for group in tensor_rt_groups:
                    closure = None
                    if (
                        group.group_type == "sparsity"
                        and len(group.tensor_names) > 1
                    ):
                        # Actual sparsity group!
                        # SW-92522 HACK: apply sparsity mask client side
                        closure = _sparsity_group_closure
        ...
        with short_temp_dir(), limit_mp_threads(), Pool(
        ...
        try:
            if group.closure:
                ...
                for tensor in group.tensors:
                    ...
                    tensor.tensor, _ = self._get_numpy_tensor(tensor, False)

                group.closure(group.tensors)

                # Send each sequentially (for now).
                for tensor in group.tensors:
                    self._grpc_client.send_weight(
                        0, tensor.rt_name, tensor.tensor
                    )
            else:
                # We dont want to load all tensors in memory upfront as there is no closure needed
                # Helps save memory usage where large tensors come in a group
                for tensor in group.tensors:
                    # Assign the numpy tensor back into the TensorSendPayload.
...
class TensorTransferTracker(ClassLogger):
    ...
    def add(self, tensor_name: str, outcome: Union[bool, Exception]) -> None:
        ...
        if outcome is True:
            ...
            # Since we raise the first error's exception later, we make this
            # error the first element of 'errors' list.
            if not self.errors:
                self.errors.append(outcome)
            else:
                self.errors[0] = outcome
        elif isinstance(outcome, Exception):
            self.logger.error(
                f"Ran into error when transferring '{tensor_name}'. {str(outcome)}"
            )
            # Storing large number of expections with stack traces blows up
            # memory. We only store the first error's exception (to raise later)
            # and store the names for other failed tensors.
            if not self.errors:
                self.errors.append(outcome)
            else:
                self.errors.append(tensor_name)
