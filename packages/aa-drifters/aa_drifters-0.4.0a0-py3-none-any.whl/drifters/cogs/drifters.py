...
class Drifters(commands.Cog):
    ...
    async def list_complex_wormholes(
        ...
        for wormhole in Wormhole.objects.filter(complex=complex):
            embed.add_field(
    ...
    async def list_system_wormholes(
        ...
        for wormhole in Wormhole.objects.filter(system=evesolarsystem):
            embed.add_field(
    ...
    async def list_system_jumps_wormholes(
        ...
        for wormhole in Wormhole.objects.filter(system__in=systems_range(evesolarsystem.id, jumps)):
            embed.add_field(
    ...
    async def list_region_wormholes(
        ...
        for wormhole in Wormhole.objects.filter(region=everegion):
            embed.add_field(
                name=wormhole.complex, value=f"{wormhole.mass}, {wormhole.formatted_lifetime}, Updated:<t:{wormhole.updated_at.timestamp():.0f}:R>", inline=False
            )
    ...
    async def status_region(
        ...
        for system in EveSolarSystem.objects.filter(eve_constellation__eve_region=everegion, name__in=DRIFTERS_JOVE_OBSERVATORIES):
            if Wormhole.objects.filter(system=system):
                for wormhole in Wormhole.objects.filter(system=system):
    ...
    async def route(
        ...
        for step in route_path_gates(from_evesolarsystem.id, to_evesolarsystem.id, edges=include_drifters_driftercomplexes()):
            step_from, step_to, step_type = step
            if step_type == "drifter_k":
                wh = Wormhole.objects.filter(system_id=step_from, complex=get_complex_name_from_id(step_to)).order_by("-created_at").first()
                ...
                wh = Wormhole.objects.filter(system_id=step_to, complex=get_complex_name_from_id(step_from)).order_by("-created_at").first()
