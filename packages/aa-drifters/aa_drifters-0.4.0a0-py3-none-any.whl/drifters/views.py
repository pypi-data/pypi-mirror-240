...
def render_complex(request, complex: Wormhole.Complexes) -> str:
    context = {
        "complex": complex,
        "wormholes": Wormhole.objects.filter(complex=complex),
...
def generate_channel_motd_2() -> str:
    ...
    for complex in Wormhole.Complexes:
        ...
        if Wormhole.objects.filter(complex=complex).exists():  # Exit early if we have no Wormholes belonging to a Complex
            ...
            for motd_system in DriftersConfiguration.get_solo().motd_systems.all():
                ...
                for hole in Wormhole.objects.filter(system__in=systems_range(motd_system.id, DRIFTERS_MOTD_SYSTEM_SEARCH_RANGE, include_source=True), complex=complex):
...
def scout_region_render(request, region: EveRegion) -> str:
    ...
    count = Wormhole.objects.filter(system__eve_constellation__eve_region=region).count()
    update_older_than_4h = Wormhole.objects.filter(system__eve_constellation__eve_region=region).exclude(updated_at__range=(four_hours_ago,timezone.now())).count()
    update_older_than_8h = Wormhole.objects.filter(system__eve_constellation__eve_region=region).exclude(updated_at__range=(eight_hours_ago,timezone.now())).count()
    update_older_than_12h = Wormhole.objects.filter(system__eve_constellation__eve_region=region).exclude(updated_at__range=(twelve_hours_ago,timezone.now())).count()
...
def scout_complex_render(request, complex: Wormhole.Complexes) -> str:
    ...
    eight_hours_ago = timezone.datetime.now() - timezone.timedelta(hours=8)
    twelve_hours_ago = timezone.datetime.now() - timezone.timedelta(hours=12)
    count = Wormhole.objects.filter(complex=complex).count()
    update_older_than_4h = Wormhole.objects.filter(complex=complex,).exclude(updated_at__range=(four_hours_ago,timezone.now())).count()
    update_older_than_8h = Wormhole.objects.filter(complex=complex,).exclude(updated_at__range=(eight_hours_ago,timezone.now())).count()
    update_older_than_12h = Wormhole.objects.filter(complex=complex,).exclude(updated_at__range=(twelve_hours_ago,timezone.now())).count()
    context = {
        'complex': complex,
        'count': count,
        'update_older_than_4h': update_older_than_4h,
...
def scout_system_render(request, system: EveSolarSystem) -> str:
    ...
    formset = WormholeFormSet(queryset=Wormhole.objects.filter(system=system), initial=[{"system": system}], prefix=system.id, form_kwargs={'system': system})
...
def scout_region(request, region_id) -> HttpResponse:
    ...
    if request.method == "POST":
        for system in EveSolarSystem.objects.filter(eve_constellation__eve_region=region, name__in=DRIFTERS_JOVE_OBSERVATORIES):
            formset = WormholeFormSet(request.POST, queryset=Wormhole.objects.filter(system=system), initial=[{"system": system}], prefix=system.id, form_kwargs={'region': region})
            if formset.is_valid():
                formset.save()
...
def scout_complex(request, complex) -> HttpResponse:
    ...
    if request.method == "POST":
        formset = WormholeFormSet(request.POST, queryset=Wormhole.objects.filter(complex=complex), initial=[{"complex": complex}])
        if formset.is_valid():
            formset.save()
    else:
        formset = WormholeFormSet(queryset=Wormhole.objects.filter(complex=complex), initial=[{"complex": complex}])
