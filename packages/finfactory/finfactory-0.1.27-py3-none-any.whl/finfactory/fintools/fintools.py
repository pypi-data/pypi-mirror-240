...
# @print_used_time
def get_merge1min_candle(df1m, func_merge, n=50,
                         new_freq=15, **kwargs_merge):
    '''
    | 用1分钟K线合成更低频K线
    ...
    '''
    df = df1m.sort_values('time')
    df = df.tail(min(int((n+2)*new_freq), df.shape[0]))
    df = func_merge(df, new_freq, **kwargs_merge)
    return df.iloc[-n:, :]
...
def cal_ic(data, col_factor, col_price='close',
           col_time=None, col_code=None, lag=1,
           ic_type='rank'):
    '''
    | 计算因子IC值
    ...
    if isnull(col_code):
        df['code'] = 'code0'
        col_code = 'code'
    df = df[[col_code, col_time, col_price, col_factor]].copy()
    df_ = []
    def _get_pct(df):
...
def cross_plot(data, col, col_base, col_price='close',
               ...
               **kwargs_plot):
    '''在 :func:`finfactory.fintools.fintools.cross` 基础上增加画图'''
    df = data.copy()
