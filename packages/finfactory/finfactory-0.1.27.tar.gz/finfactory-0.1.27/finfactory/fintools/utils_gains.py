...
from finfactory.fintools.fundnet import get_fundnet
...
def cal_beta(values_target, values_base, gain_type='pct', pct_cost0=1):
    ...
    | gain_type和pct_cost0同 :func:`finfactory.fintools.utils_gains.cal_gain_pcts` 中的参数
    | 参考：
...
def cal_return_period_by_gain_pct(gain_pct, n, nn=250, rtype='exp',
    ...
    '''
    if gain_pct_type.lower() in ['log', 'ln', 'lg']:
...
def get_maxup(values, return_idx=True, abs_val=False):
    '''
    最大盈利计算（与最大回撤 :func:`finfactory.fintools.utils_gains.get_maxdown` 
    相对应，即做空情况下的最大回撤）
    '''
...
def get_gains_act(df_settle, act_gain_pct_method=2):
    ...
    df = df[['累计转入', '累计转出', '累计净流入', '累计投入',
             '累计总值', '累计盈亏', '实际总盈亏%']].copy()
    return df


def get_fundnet0(df_settle, when='before', restart=False):
    '''
    ...
    for k in range(0, df.shape[0]):
        if k == 0:
            df.loc[k, '新增份额'] = df.loc[k, '净流入']
            df.loc[k, '份额'] = df.loc[k, '新增份额']
            df.loc[k, '净值'] = x_div_y(df.loc[k, '资产总值'],
                                df.loc[k, '份额'], v_y0=1, v_xy0=1)
        elif restart and df.loc[k, 'restart'] == 1:
            df.loc[k, '新增份额'] = df.loc[k, '净流入'] + \
            ...
            if when == 'before':
                ...
                df.loc[k, '份额'] = df.loc[df.index[k-1], '份额'] + \
                                    df.loc[k, '新增份额']
                df.loc[k, '净值'] = x_div_y(df.loc[k, '资产总值'],
                                    df.loc[k, '份额'], v_y0=1, v_xy0=1)
            else:
                total = df.loc[k, '资产总值'] - df.loc[k, '净流入']
                df.loc[k, '净值'] = x_div_y(total,
...
def get_gains(df_settle, gain_types=['act', 'fundnet'],
              ...
              when_in=None, when_out=None, outtype=1, intype=1,
              fundnet_func=0):
    '''
    | 不同类型的盈亏情况统计
    ...
    if 'fundnet' in gain_types:
        ...
        if fundnet_func == 0:
            df_net = get_fundnet0(df_gain, when=when, restart=restart)
        else:
            df_net = get_fundnet(df_settle, when_in=when_in, when_out=when_out,
                             outtype=outtype, intype=intype, restart=restart)
        df_gain = merge_df(df_gain, df_net, same_keep='left', how='left',
...
def cal_sig_gains(data, sig_col, sig_type=1, shift_lag=0,
                  ...
                  dy_loss=None, dy_gain=None,
                  add_loss_pct=None, add_gain_pct=None,
                  add_loss_val=None, add_gain_val=None,
    ...
    fee : float
        单向交易综合成本比例（双向收费）
    max_loss : float, str
        止损比例，默认为比例，若要设金额绝对值(平均)，写成格式如'val_30'
    max_gain : float, str
        止盈比例，默认为比例，若要设金额绝对值(平均)，写成格式如'val_30'
    max_down : float, str
        平仓最大回撤比例，默认为比例，若要设金额绝对值(平均)，写成格式如'val_30'
    dy_loss : float, str
        追踪止损比例，默认为比例，若要设金额绝对值(平均)，写成格式如'val_30'
    dy_gain : float, str
        追踪止盈比例，默认为比例，若要设金额绝对值(平均)，写成格式如'val_30'
        
        .. note::
              ``dy_loss`` 和 ``dy_gain`` 优先级高于别的止盈止损方式
    add_loss_pct : float
    ...
    add_gain_pct : float
        ...
        加仓方式由 ``func_vol_add`` 决定
    add_loss_val : float, int
        将 ``add_loss_pct`` 改为金额(平均)，意义相同
    add_gain_val : float
        将 ``add_gain_pct`` 改为金额(平均)，意义相同
    stop_sig_order : str
    ...
    - 止盈止损计算初始点位可选第一次开仓而不是最近一次操作
    - 最大回撤平仓添加以点位记回撤的方式
    - 计算变量中增加止盈止损价格
    - 止盈止损、最大回撤、持仓最大盈利等考虑增加最高最低价判断条件
    - 单独写函数每次交易操作前确定交易价格（止盈止损的交易价格可能由最高最低价判断）
    - 增加根据固定盈亏点位加仓（比如IF赚20个点或亏20个点加仓）
    - 止盈止损信号增加动态确定函数(止盈止损或最大回撤百分比不固定，根据持仓时间和盈亏等情况确定)
    - 止盈止损增加根据总盈利额条件（目前是评价盈利）
    ...
    - 采用字典或列表记录构成当前持仓的所有操作记录（每次操作价格，数量，方向，距离当前的时间等）
    - 固定持仓时间平仓
    - 固定时间加减仓（类似定投）
    - fee买入和卖出分开设置
    - fee设置成函数
    ...
    def get_AddTradeVol(Price, func_vol_add, hold_vol, cash):
        ...
        if isinstance(func_vol_add, str) and 'base' in func_vol_add:
            x = float(func_vol_add.split('_')[-1])
    ...
    def get_SubStopTradeVol(Price, VolF_SubStop, hold_vol, hold_cost, cash):
        ...
        if isinstance(VolF_SubStop, str) and 'hold_base' in VolF_SubStop:
            x = float(VolF_SubStop.split('_')[-1])
    ...
    assert (isnull(dy_loss) and isnull(dy_gain)) or (not isnull(dy_loss) and not isnull(dy_gain))
    def _var_tmp_check(var_tmp):
        if isnull(var_tmp):
            return True
        if isinstance(var_tmp, float):
            return True
        if isinstance(var_tmp, str):
            try:
                float(var_tmp.replace('val_', ''))
                return True
            except:
                return False
        return False
    for var_tmp in [max_loss, max_gain, max_down, dy_gain, dy_loss]:
        assert _var_tmp_check(var_tmp)
    ...
    df['holdPreGainPct'] = 0 # 持仓盈亏比例（交易完成前）
    df['holdPreGainVal'] = 0 # 持仓盈亏金额（交易完成前）
    df['holdPreGainPctMax'] = 0 # 持仓达到过的最高收益比例（交易完成前）
    df['holdPreLossPctMax'] = 0 # 持仓达到过的最大亏损比例（交易完成前）
    df['holdPreMaxDown'] = 0 # 持仓最大回撤比例（交易完成前）    
    df['holdPreGainValMax'] = 0 # 持仓达到过的最高收益金额（交易完成前）
    df['holdPreLossValMax'] = 0 # 持仓达到过的最大亏损金额（交易完成前）
    df['holdPreMaxDownVal'] = 0 # 持仓最大回撤金额（交易完成前）    
    ...
    df['stopGainPrice'] = np.nan # 止盈价格（每期交易完成后计算）
    df['stopLossPrice'] = np.nan # 止损价格（每期交易完成后计算）
    ...
    for k in range(0, df.shape[0]):
        ...
        if k == 0:
            ...
            if holdVol_pre == 0:
                ...
                holdPreGainVal = holdVal_pre - holdCost_pre
                ...
                if last_act == 0:
                    ...
                    holdPreGainValMax = max(holdPreGainVal,
                                    df.loc[df.index[k-1], 'holdPreGainValMax'])
                    holdPreLossValMax = min(holdPreGainVal,
                                    df.loc[df.index[k-1], 'holdPreLossValMax'])
                else:
                    holdPreGainPctMax = max(holdPreGainPct, 0)
                    holdPreLossPctMax = min(holdPreGainPct, 0)
                    holdPreGainValMax = max(holdPreGainVal, 0)
                    holdPreLossValMax = min(holdPreGainVal, 0)
                # 最大回撤
                ...
                holdPreMaxDown = abs(cal_gain_pct(holdValMax_pre, holdVal_pre,
                                                  pct_cost0=0))
                holdPreMaxDownVal = abs(holdPreGainValMax - holdPreGainVal)
                
                # 止盈止损
                if (not isnull(dy_loss)) and (not isnull(dy_gain)):
                    # 动态追踪平仓
                    act_stop = 0
                    stopLossPrice = df.loc[df.index[k-1], 'stopLossPrice']
                    if holdVol_pre > 0:
                        if Price_settle <= stopLossPrice:
                            if holdPreGainPct > 0:
                                act_stop = 1.5 # 做多止盈
                            else:
                                act_stop = 0.5 # 做多止损
                    ...
                    # 固定比率|金额止盈止损
                        ...
                        holdPreGainVal_mean = holdPreGainVal / abs(holdVol_pre)
                        # 止损
                        if (not isnull(max_loss)) and \
                           ((isinstance(max_loss, float) and holdPreGainPct <= -max_loss) or \
                            (isinstance(max_loss, str) and holdPreGainVal_mean <= -float(max_loss.replace('val_', '')))):
                            if holdCost_pre > 0:
                                act_stop = 0.5 # 做多止损
                            elif holdCost_pre < 0:
                                act_stop = -0.5 # 做空止损
                                ...
                                act_stop = 0
                        # 止盈
                        elif (not isnull(max_gain)) and \
                             ((isinstance(max_gain, float) and holdPreGainPct >= max_gain) or \
                              (isinstance(max_gain, str) and holdPreGainVal_mean >= float(max_gain.replace('val_', '')))):
                            if holdCost_pre > 0:
                                act_stop = 1.5 # 做多止盈
                            elif holdCost_pre < 0:
                                act_stop = -1.5 # 做空止盈
                    ...
                    # 最大回撤平仓
                        ...
                        if (isinstance(max_down, float) and (holdPreMaxDown < max_down)) or \
                           (isinstance(max_down, str) and holdPreMaxDownVal/abs(holdVol_pre) < float(max_down.replace('val_', ''))):
                            act_stop = 0
                        else:
                            if holdCost_pre > 0:
                                if holdPreGainPct < 0:
                ...
                # 加减仓
                # 没有加仓
                if holdVol_pre == 0 or last_acted == 0:
                    act_add = 0
                elif (isnull(add_loss_pct) and isnull(add_gain_pct)) and \
                     (isnull(add_loss_val) and isnull(add_gain_val)):
                    act_add = 0
                else:
                    if (not isnull(add_loss_pct)) or (not isnull(add_gain_pct)):
                        # 比上次同向操作时价格涨跌幅
                        ...
                        if holdVol_pre > 0 and last_acted == -1 and \
                            ...
                            act_add = 0
                    elif (not isnull(add_loss_val)) or (not isnull(add_gain_val)):
                        # 比上次同向操作时价格涨跌额
                        chg_lastacted = Price_settle - last_actPrice
                        # 浮盈加仓|浮亏加仓
                        if holdVol_pre > 0 and last_acted == -1 and \
                              not isnull(add_gain_val) and chg_lastacted >= add_gain_val:
                            act_add = -1 # 做多浮盈加仓
                        elif holdVol_pre < 0 and last_acted == 1 and \
                              not isnull(add_gain_val) and chg_lastacted <- add_gain_val:
                            act_add = 1 # 做空浮盈加仓
                        elif holdVol_pre > 0 and last_acted == -1 and \
                              not isnull(add_loss_val) and chg_lastacted <= -add_loss_val:
                            act_add = -1 # 做多浮亏加仓
                        elif holdVol_pre < 0 and last_acted == 1 and \
                              not isnull(add_loss_val) and chg_lastacted >= add_loss_val:
                            act_add = 1 # 做空浮亏加仓
        ...
        df.loc[df.index[k], 'holdPreGainVal'] = holdPreGainVal
        ...
        df.loc[df.index[k], 'holdPreGainValMax'] = holdPreGainValMax
        df.loc[df.index[k], 'holdPreLossValMax'] = holdPreLossValMax
        df.loc[df.index[k], 'holdPreMaxDownVal'] = holdPreMaxDownVal
        ...
        # 止盈止损价格
        if (not isnull(dy_loss) and not isnull(dy_gain)) and (holdVol != 0):
            # 追踪止盈止损
            holdCost_mean  = df.loc[df.index[k], 'holdCost_mean']
            # 若前一次有操作，则止盈止损基于前一次成本价
            if act == 0:
                stopGainPrice = df.loc[df.index[k-1], 'stopGainPrice']
                stopLossPrice = df.loc[df.index[k-1], 'stopLossPrice']
                # TODO: 改为采用最高价个最低价判断是否触及止盈止损
                if holdVol > 0:
                    if Price_settle >= stopGainPrice:
                        if isinstance(dy_gain, float):
                            stopGainPrice = stopGainPrice * (1 + dy_gain)
                        else:
                            stopGainPrice = stopGainPrice + float(dy_gain.replace('val_', ''))
                        if isinstance(dy_loss, float):
                            stopLossPrice = stopGainPrice * (1 - dy_loss)
                        else:
                            stopLossPrice = stopGainPrice - float(dy_loss.replace('val_', ''))
                elif holdVol < 0:
                    if Price_settle <= stopGainPrice:
                        if isinstance(dy_gain, float):
                            stopGainPrice = stopGainPrice * (1 - dy_gain)
                        ...
                        if isinstance(dy_loss, float):
                            ...
                            stopLossPrice = stopGainPrice + float(dy_loss.replace('val_', ''))
                df.loc[df.index[k], 'stopGainPrice'] = stopGainPrice
                df.loc[df.index[k], 'stopLossPrice'] = stopLossPrice
            else:
                # TODO: holdCost_mean为0的情况怎么处理？（极端情况下可能会出现）
                if holdCost_mean > 0:
                    if isinstance(dy_gain, float):
                        stopGainPrice = holdCost_mean * (1 + dy_gain)
                    else:
                        stopGainPrice = holdCost_mean + float(dy_gain.replace('val_', ''))
                    if isinstance(dy_loss, float):
                        stopLossPrice = holdCost_mean * (1 - dy_loss)
                    else:
                        stopLossPrice = holdCost_mean - float(dy_loss.replace('val_', ''))
                    df.loc[df.index[k], 'stopGainPrice'] = stopGainPrice
                    df.loc[df.index[k], 'stopLossPrice'] = stopLossPrice
                elif holdCost_mean < 0:
                    if isinstance(dy_gain, float):
                        stopGainPrice = abs(holdCost_mean) * (1 - dy_gain)
                    else:
                        stopGainPrice = abs(holdCost_mean) - float(dy_gain.replace('val_', ''))
                    if isinstance(dy_loss, float):
                        stopLossPrice = abs(holdCost_mean) * (1 + dy_loss)
                    else:
                        stopLossPrice = abs(holdCost_mean) + float(dy_loss.replace('val_', ''))
                    df.loc[df.index[k], 'stopGainPrice'] = stopGainPrice
                    df.loc[df.index[k], 'stopLossPrice'] = stopLossPrice
...
def get_yield_curve(data, sig_col, nn=252, ext_type=1,
    ...
    if net_type == 'prod':
        ...
        gain_type = 'pct'
        rtype = 'mean'
    elif net_type == 'maxused_act':
        df_gain['净值'] = df_gain['AccountValue_act_net']
    elif net_type == 'maxused_final':
        df_gain['净值'] = df_gain['AccountValue_maxUsed_net']
    ...
    if plot:
        ...
        xparls_info = {'净值': [(1,)]}
        xparls_info, kwargs_plot = get_update_kwargs('xparls_info', xparls_info, kwargs_plot)
...
def get_yield_curve2(data, col_gain, col_cost, col_price=None, nn=252,
    ...
    if net_type == 'prod':
        ...
        df['净值'] = df['AccountValue_act_net']
    elif net_type == 'maxused_final':
        df['净值'] = df['AccountValue_maxUsed_net']
    MDabsV = True if net_type == 'sum' else False
    ...
    gain_info = {'年化收益': return_ann,
                 ...
                 '总盈利额': sumGain,
                 '总亏损额': sumLoss,
                 '平均盈利额': x_div_y(sumGain, Nhit, v_x0=0, v_y0=0, v_xy0=0),
    ...
    if plot:
        ...
        if not isnull(col_price):
            cols_styl_up_left.update(
                    {'value_mkt': ('-k', '市场净值(价格/初始价格)')})
        plot_series(df, cols_styl_up_left,
                    xparls_info={'基金净值': [(1,)]},
                    **kwargs_plot)
...
if __name__ == '__main__':
    ...
    # beta
    df_beta = load_csv('../_test/000001.XSHG_qfq.csv').set_index('time')
    df_beta = df_beta[['close']].rename(columns={'close': 'base'})
    tmp = load_csv('../_test/600519.XSHG_qfq.csv').set_index('time')
    ...
    # test9
    data9 = pd.DataFrame(
        {'close': [100, 90, 80, 70, 80, 90, 100, 110],
         'signal': [1, 0, 0, 0, 0, 0, 0, 0]})
    trade_gain_info9, df_gain9 = get_yield_curve(
                        data9, 'signal', sig_type=1,
                        base_money=None, base_vol=1, init_cash=0,
                        dy_loss='val_9', dy_gain='val_9', fee=0/100,
                        max_down=None, stop_sig_order='both',
                        add_sig_order='sig_only',
                        sos_money=10, force_final0='settle')
    
    # test10
    data10 = pd.DataFrame(
        {'close': [70, 80, 90, 100, 110, 100, 90, 80],
         'signal': [-1, 0, 0, 0, 0, 0, 0, 0]})
    trade_gain_info10, df_gain10 = get_yield_curve(
                        data10, 'signal', sig_type=1,
                        base_money=None, base_vol=1, init_cash=0,
                        dy_loss='val_9', dy_gain='val_9', fee=0/100,
                        max_down=None, stop_sig_order='both',
                        add_sig_order='sig_only',
