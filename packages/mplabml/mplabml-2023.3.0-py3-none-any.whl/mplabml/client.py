...
from mplabml.base.snippets import Snippets, function_help
from mplabml.connection import Connection
from mplabml.datamanager import (
...
from mplabml.datamanager.captures import CaptureExistsError
from mplabml.datamanager.knowledgepack import (
    KnowledgePack,
    delete_knowledgepack,
    get_knowledgepack,
)
from mplabml.datasets import DataSets
from mplabml.dclproj.upload import upload_project, upload_project_dcli

# from mplabml.base.exceptions import *
from mplabml.pipeline import Pipeline

config_dir = user_config_dir(__name__.split(".")[0], False)

SERVER_URL="https://www.mplabml.microchip.com/"

AUTH2_PRIMARY=False

logger = logging.getLogger("MPLABML")
...
def print_list(func):
    """This is a wrapper for printing out lists of objects stored in MPLABML Cloud"""
    ...
    def wrapper(*args, **kwargs):
        ...
        if len(result.keys()) == 0 and kwargs.get("silent", False) == False:
            print(
                "No {} stored on MPLABML Cloud for this project.".format(
...
class Client(object):
    ...
    def validate_client_version(self, skip_validate=True):
        ...
        if response_data.get("MPLABML_Python_Library_Windows_Minimum") is None:
            return True

        if version.parse(
            response_data["MPLABML_Python_Library_Windows_Minimum"]
        ) > version.parse(__version__):
            print(
                f"Error: The MPLABML Python SDK is out of date.\n\n\tCurrent installed version = {__version__}. \n\tMinimum supported version = {response_data['MPLABML_Python_Library_Windows_Minimum']}."
            )
            print(
                "\nTo update the MPLABML Python SDK run \n\n\tpip install mplabml -U\n\nIf you are in a notebook, you can execute the following in a cell then restart the shell.\n\n\t!pip install mplabml -U"
            )
            print(
    ...
    def list_projects(self, get_objects=False, silent=False):
        """Lists all of the projects on MPLABML Cloud

        Returns:
            DataFrame: projects on MPLABML Cloud
        """
    ...
    def get_knowledgepack(self, uuid, get_flatbuffer=True):
    ...
    @print_list
    def list_captures(self, get_objects=False, force=False):
    ...
    @print_list
    def refresh_capture_list(self, get_objects=False):
    ...
    @project_set
    def get_captures(self, force: bool = True):
    ...
    def upload_data_file(
        ...
        print('Uploading file "{}" to MPLABML Cloud.'.format(name))
        if name[-4:] != ".csv":
    ...
    def download_pipeline_python(self, fmt="py"):
    ...
    def download_pipeline_notebook(self):
    ...
    def list_background_captures(self, get_objects=False):
        """List all captures for the active project
    ...
    def download_background_capture_by_uuid(
        self, uuid: str, output_filename: str = "", output_folder="./"
    ):
