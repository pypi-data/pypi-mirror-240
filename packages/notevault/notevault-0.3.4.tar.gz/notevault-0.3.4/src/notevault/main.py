    ...
    def save(self):
        ...
        if not document:
            ...
            for obj in self.parsed_objects:
                ...
                is_list = next(
                    s["section"]["is_list"]
                    for s in self.document_spec["Sections"]
                    if s["section"]["type"] == obj_type
                )
                instrumented_list = obj_type.lower() + "s"
                if not is_list:
                    print(f"{obj_type}: {sqlalchemy_instance}")
                    setattr(
                        document, instrumented_list, [sqlalchemy_instance]
                    )  # 1:1 relationship
                else:
                    current_value = getattr(
                        document, instrumented_list
                    )  # 1:n relationship -> InstrumentedList
                    headings = [m.name for m in current_value]
                    if sqlalchemy_instance.name in headings:
                        idx = headings.index(sqlalchemy_instance.name)
                        print(
                            f"{obj_type} {sqlalchemy_instance.name} already exists {idx=}."
                        )
