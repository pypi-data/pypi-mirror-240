...
@pytest.fixture
def markdown_text_with_list_items():
    return """
# List
- start: 07:30, duration: 1, breaks: 0:30
- start: 17:30, duration: 2, breaks:, data: "adsfadfasdf, asdfasdf"
"""


@pytest.fixture
def mock_document_structure_for_list():
    return {
        "Sections": [
            {"section": {"heading": "# List", "type": "ListModelMock", "is_list": True}}
        ]
    }


@pytest.fixture
def mock_generated_classes_for_list():
    return {"ListModelMock": ListModelMock}


def test_parse_markdown_list(
    markdown_text_with_list_items,
    mock_document_structure_for_list,
    mock_generated_classes_for_list,
):
    parsed_models = parse_markdown(
        markdown_text_with_list_items,
        mock_document_structure_for_list,
    ...
    assert len(parsed_models) == 2
    assert parsed_models[0].start == time(7, 30)
    assert parsed_models[0].duration == 1
    assert parsed_models[0].breaks == timedelta(minutes=30)
    assert parsed_models[0].data is None
    assert parsed_models[1].start == time(17, 30)
    assert parsed_models[1].duration == 2
    assert parsed_models[1].breaks is None
    assert parsed_models[1].data == "adsfadfasdf, asdfasdf"
