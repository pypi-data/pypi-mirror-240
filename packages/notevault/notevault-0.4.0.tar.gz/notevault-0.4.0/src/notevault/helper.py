...
def string_to_yaml(s: str) -> str:
    ...
    current_line = f"{key.strip()}: {value.strip()}"

    for pair in pairs[1:]:
        if ":" in pair:
            # Start of a new key-value pair
            yaml_lines.append(current_line)
            key, value = pair.split(":", 1)
            current_line = f"{key.strip()}: {value.strip()}"
        else:
            # Continuation of the current value
            current_line += f", {pair.strip()}"

    yaml_lines.append(current_line)  # Add the last line

    # Add quotes around values where necessary
    formatted_lines = []
