...
def test_field_type_mapping(sample_yaml_spec):
    generated_classes = generate_models_from_yaml(sample_yaml_spec)
    assert str(generated_classes["General"].model_fields["start"].annotation) == 'typing.Optional[datetime.time]'
    assert str(generated_classes["Meeting"].model_fields["name"].annotation) == "<class 'str'>"
