...
class LoginMixin(PreLoginFlowMixin, PostLoginFlowMixin):
    ...
    def login(
        ...
        request_data_2fa: Optional[dict] = None
    ) -> bool:
        """
        Login
        ...
        verification_method: str
            0 - email, 1 - sms, 3 - authenticator
        request_data_2fa: Optional[dict]
            last request data for 2fa
        ...
        try:
            ...
            if request_data_2fa:
                self.request_data_2fa = {
                    **request_data_2fa,
                    "verification_code": self.verification_code,
                    "verification_method": self.verification_method,
                }
            else:
                self.request_data_2fa = data

            logged = self.private_request(
                "accounts/two_factor_login/", self.request_data_2fa, login=True
            )
            self.authorization_data = self.parse_authorization(
                self.last_response.headers.get("ig-set-authorization")
            )
