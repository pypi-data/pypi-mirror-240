...
class ChallengeResolveMixin:
    ...
    def challenge_resolve_contact_form(self, challenge_url: str) -> bool:
        ...
        #     # SEND CODE
        #     time.sleep(WAIT_SECONDS)
        result = session.post(challenge_url, {"security_code": self.verification_code}).json()
    ...
    def challenge_resolve_simple(self, challenge_url: str) -> bool:
        ...
        if step_name == "delta_login_review" or step_name == "scraping_warning":
            ...
            #     time.sleep(wait_seconds)
            print(
                f'Code entered "{self.verification_code}" for {self.username} (attempts by {wait_seconds} seconds)'
            )
            self._send_private_request(challenge_url, {"security_code": self.verification_code})
