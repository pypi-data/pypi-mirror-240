...
class AMF:
    ...
    fonction = {
        'setAdress' : "@ADDR=#", #Define the adress of the product
        ...
        'slowMode' : "-", #Set the slow mode (only for the RVMFS)
        'fastMode' : "+", #Set the fast mode (only for the RVMFS)
        ...
        'getValveAdress' : "?26", #Get the valve or pump adress
        ...
        'getSuplyVoltage' : "*", #Get the supply voltage
        ...
        'home' : "ZR", #Moov to the home position
        'enforcedShortestPath' : "B#R", #Moov to the shortest path make 360° if target port is the current position
        'shortestPath' : "b#R", #Moov to the shortest path no moov if target port is the current position
        'enforcedIncrementalMoov' : "I#R", #Moov to the clockwise path make 360° if target port is the current position
        'incrementalMoov' : "i#R", #Moov to the clockwise path no moov if target port is the current position
        'enforcedDecementalMoov' : "O#R", #Moov to the counter clockwise path make 360° if target port is the current position
        'decrementalMoov' : "o#R", #Moov to the counter clockwise path no moov if target port is the current position
        'hardStop' : "T", #Stop the pump (only for SPM et LSPOne)
        'powerOff' : "@POWEROFF", #Power off the product (only for SPM et LSPOne)
        'forceAndHome' : "Y#R", #set force and go to home position (only for SPM et LSPOne)
    ...
    def __init__(self, product, autoconect = True, portnumber: int = None, syringeVolume : int = None, productAdress : int = 1, type : str = None, serialBaudrate : int = None) -> None:
        """
        INPUTS:
            product: # product to connect to. Can be a serial port, a serial number or a Device objectZ
            portnumber: int # Number of ports on the product. Default: 6
            syringeVolume: int # Volume of the syringe in µL.
            serialBaudrate: int # Baudrate to use for the serial connection. Default: 9600
            productAdress: int # Address of the product to connect to. Default: 1
        ...
        if productAdress != 1:
            self.productAddress = productAdress
        if type is not None:
    ...
    def connect(self, serialBaudrate : int = serialBaudrate, serialTimeout : float = serialTimeout) -> bool:
        ...
        try :
            ...
            if self.portnumber is None:
                self.portnumber = self.getPortNumber(integer = True)
            return True
...
#                                                                                                          #
############################################################################################################
    def setAdress(self, adress : int) -> None:
        """
        INPUTS:
            adress: int # Adress of the  [[1; 9]]
        OUTPUTS:
            None
        """
        if adress < 1 or adress > 9:
            raise ValueError("Adress must be between 1 and 9")
        self.productAddress = adress
        self.send(self.prepareCommand('setAdress', adress))
    ...
    def setAcelerationRate(self, rate : int) -> None:
        """
        INPUTS:
            rate: int [[100; 59590]]
        OUTPUTS:
    ...
    def setPumpStrenghAndHome(self, strengh : int, block : bool = True) -> None:
        """
        INPUTS:
            None
        ...
        self.send(self.prepareCommand('forceAndHome', strengh))
...
############################################################################################################
    ...
    def getValveAdress(self) -> int:
        ...
        OUTPUTS:
            int
        """
        self.productAddress = self.send(self.prepareCommand('getValveAdress'), integer = True, force_aws=True)
    ...
    def getSuplyVoltage(self) -> float:
        """
        INPUTS:
            None
        OUTPUTS:
            float
        """
        self.send(self.prepareCommand('getSuplyVoltage'), force_aws=True)
    ...
    def getDeviceInformation(self) -> object:
        ...
        if self.typeProduct == "RVMFS" :
            ...
            try: info["valveAdress"] = self.getValveAdress()
            except : info["valveAdress"] = None
...
############################################################################################################
    ...
    def valveIncrementalMoov(self, target: int, enforced : bool = False, block : bool = True) -> None:
        """
        INPUTS:
            target: int
            enforced: bool
        ...
        if target < 1 or target > self.portnumber:
            raise ValueError("Target must be between 1 and "+str(self.portnumber))
        if enforced:
            self.send(self.prepareCommand('enforcedIncrementalMoov', target))
        else:
            self.send(self.prepareCommand('IncrementalMoov', target))

        if block: self.pullAndWait()
    
    def valveClockwiseMoov(self, target: int, enforced : bool = False, block : bool = True) -> None:
        self.valveIncrementalMoov(target, enforced= enforced, block = block)

    def valveDecrementalMoov(self, target: int, enforced : bool = False, block : bool = True) -> None:
        """
        INPUTS:
            target: int
            enforced: bool
        ...
        if target < 1 or target > self.portnumber:
            raise ValueError("Target must be between 1 and "+str(self.portnumber))
        if enforced:
            self.send(self.prepareCommand('enforcedDecementalMoov', target))
        else:
            self.send(self.prepareCommand('DecrementalMoov', target))

        if block: self.pullAndWait()

    def valveCounterClockwiseMoov(self, target: int, enforced : bool = False, block : bool = True) -> None:
        self.valveDecrementalMoov(target, enforced = enforced, block = block)

    def valveMoov(self, target: int, mode:int = 0, enforced = False, block : bool = True):
        """
        INPUTS:
            target: int
            mode: int # 0: ShortestPath, 1: IncrementalMoov, 2: DecrementalMoov
            enforced: bool
        OUTPUTS:
            None
        """
        if mode == 0:
            ...
            self.valveIncrementalMoov(target, enforced)
        elif mode == 2:
            self.valveDecrementalMoov(target, enforced)
        else:
            raise ValueError("Mode must be between 0 and 2")
...
class utile:
    def getProductList(specified_type = None) -> list:
        ...
        if os.name == 'posix':
            list_com_port = serial.tools.list_ports.comports()
            for com in list_com_port:
                if "P100" in com.serial_number or "P200" in com.serial_number or "P201" in com.serial_number:
...
if __name__ == "__main__":
    ...
    list_product = utile.getProductList()
