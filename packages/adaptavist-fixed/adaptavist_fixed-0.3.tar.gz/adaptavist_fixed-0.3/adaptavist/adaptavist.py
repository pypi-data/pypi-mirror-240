...
from .const import PRIORITY_NORMAL, STATUS_APPROVED, STATUS_NOT_EXECUTED, STEP_TYPE_BY_STEP, TEST_CASE, TEST_PLAN, \
    TEST_RUN
...
class Adaptavist:
    ...
    def __init__(self, jira_server: str, jira_username: str = None, jira_password: str = None, jira_token: str = None):
    ...
    def get_users(self) -> List[str]:
        ...
        i = 0
        while True:
            request_url = f"{self.jira_server}/rest/api/2/user/search?username=.&startAt={i}&maxResults=2000"
    ...
    def get_folders(self, project_key: str, folder_type: str) -> List[str]:
        ...
        request_url = f"{self.jira_server}/rest/tests/1.0/project/{project_id}/foldertree/{folder_type.replace('_', '').lower()}?startAt=0&maxResults=2000"
    ...
    def edit_test_plan(self, test_plan_key: str, **kwargs: Any) -> bool:
        ...
        update_field(response.get("labels", []), request_data, "labels", labels)
        update_field([test_run["key"] for test_run in response.get("testRuns", [])], request_data, "testRuns",
                     test_runs)
        update_field(response.get("issueLinks", []), request_data, "issueLinks", issue_links)
    ...
    def create_test_results(
            self, test_run_key: str, results: List[Dict[str, Any]], exclude_existing_test_cases: bool = True,
            **kwargs: Any
    ) -> List[int]:
        """
    ...
    def edit_test_script_status(self, test_run_key: str, test_case_key: str, step: int, status: str,
                                **kwargs: Any) -> bool:
        """
