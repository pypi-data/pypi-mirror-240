...
class InDirect(GenericCRUD):
    def __init__(self) -> None:
        super().__init__(schema_name="api_call",default_table_name="api_call_table",default_id_column_name="api_call_id")
    ...
    def before_call_api(self, external_user_id:int,api_type_id: int,endpoint:str, outgoing_body:str, outgoing_header:str) :
        ...
        arr,outgoing_body_significant_fields_hash=api_management.check_cache(api_type_id,outgoing_body)
        if arr==None:
            is_network=None
        ...
        api_call_id=api_call._insert_api_call_dict(api_call_data_dict=api_call_data_dict)
    ...
    def after_call_api(self, external_user_id:int,api_type_id: int,endpoint:str, outgoing_body:str, outgoing_header:str,http_status_code:int,response_body:str,incoming_message:str,api_call_id:int,used_cache:bool):
        ...
        update_data={'external_user_id':external_user_id,'endpoint':endpoint,'outgoing_body':str(outgoing_body),'outgoing_header':str(outgoing_header),'http_status_code':http_status_code,'response_body':str(response_body),'incoming_message':str(incoming_message),'is_network':is_network}
        self.update_by_id(id_column_value=api_call_id,json_data=update_data)
