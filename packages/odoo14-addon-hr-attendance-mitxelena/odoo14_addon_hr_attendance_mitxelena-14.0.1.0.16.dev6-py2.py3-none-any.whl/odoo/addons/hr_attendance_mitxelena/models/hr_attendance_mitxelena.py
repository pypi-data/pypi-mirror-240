...
class HrAttendanceMitxelena(models.Model):
    ...
    ATTENDANCE_WINDOW = int(config.get("hr_attendance_window", 12))
        ...
        day_worked_time = self.env["hr.attendance"].search(
            [
                ...
                (
                    "check_out",
                    ">=",
                    self.check_in - timedelta(hours=self.ATTENDANCE_WINDOW),
                ),
    ...
    def process_attendance_overtime(self):
        ...
        if self.no_recompute or not self.check_out:
            return
        ...
        dt_actual_check_out = fields.Datetime.from_string(self.check_out)
        attendance_window = dt_actual_check_out - timedelta(
            hours=self.ATTENDANCE_WINDOW
        )
        ...
        if (
            ...
            updated_entry.message_post(body=f"Updated entry: Inheriting values from previous attendance record: {previous_attendance.id}")
            updated_entry.message_post(body=f"Updated entry: {self.id} Original values for entry: Extra time: {self.extra_time} \n - Extra time with factor: {self.extra_time_with_factor}")
            return

        # Check if is the last attendance record of the day
        ...
        # If it's not the last attendance record of the day, skip it
        if last_attendance.id != self.id:
        ...
        new_checkout = dt_actual_check_out - timedelta(hours=day_extra_time)
        ...
        self.message_post(body=f"Original values for entry: Check_in: {original_entry['check_in']} \n - Check_out: {original_entry['check_out']} \n New entry: {new_entry.id}")
        new_entry.message_post(body=f"Created from entry: {self.id} Original values for entry: Check_in: {original_entry['check_in']} \n - Check_out: {original_entry['check_out']}")        

    def action_process_attendance_overtime(self):
        self.ensure_one()
        self.process_attendance_overtime()
