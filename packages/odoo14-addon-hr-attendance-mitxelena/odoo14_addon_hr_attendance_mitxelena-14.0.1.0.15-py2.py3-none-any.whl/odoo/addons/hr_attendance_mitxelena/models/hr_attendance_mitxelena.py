...
class HrAttendanceMitxelena(models.Model):
    ...
    extra_time = fields.Float(compute="_compute_entry_extra_time", store=True, default=0)
    ...
    def _compute_entry_extra_time(self):
        for record in self:
            ...
            extra_time = max(self.compute_day_extra_time(
    ...
    def process_attendance_overtime(self):        
            ...
            self.write(
                {
                    ...
                    "extra_time_with_factor": self.compute_day_extra_time_with_factor(
                        self.worked_hours, self.entry_type.factor
                    ),
                }
            )
            return
        ...
        if (
            last_attendance
            and last_attendance.is_overtime
            and last_attendance.check_out >= attendance_window
        ):
            self.write(
                {
                    "entry_type": last_attendance.entry_type.id,
                    "shift_type": last_attendance.shift_type,
                    "is_relevo": last_attendance.is_relevo,
                    "consecutive_days": last_attendance.consecutive_days,
                    "time_changed_manually": False,
                }
            )
            return
