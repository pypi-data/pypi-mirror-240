...
class lattice(np.ndarray):
    ...
    def fast_vis(self, plot, show_outline: bool = True, show_centroids: bool = True, color = "#ff8fa3", opacity=0.3):
        ...
        ** Usage Example: **
        ...
        grid.point_arrays["values"] = self.flatten(
    ...
    def to_csv(self, filepath: str):
        ...
        # Open the CSV file in binary mode to prevent line terminator issues
        with open(filepath, 'wb') as df_out:

            # Write metadata without line terminator
            metadata.to_csv(df_out, index=False,
                            header=True, float_format='%g')

            # Explicitly write a newline separator
            df_out.write(b'\n')

            # Write volume data without line terminator
            vol_df.to_csv(df_out,
                        index=False,
                        float_format='%g')

            # Explicitly write a newline separator at the end
            df_out.write(b'\n')
