...
def _get_non_paged_result(path: str, max_results: int, filters: str) -> str:
    if max_results > 0:
        return api_client._post(
            path=f"{path}?size={max_results}&order_by=versjon&asc=false", body_json=filters
        )
    ...
    while len(items) < total:
        response = api_client._post(
            path=f"{path}?page={page}&size=100&order_by=versjon&asc=false", body_json=filters
        )
...
def _get_paged_result(path, paging, filters) -> str:
    return api_client._post(
        path=f"{path}?page={paging.page}&size={paging.size}&order_by=versjon&asc=false", body_json=filters
    )
...
def get_skjema_by_ra_nummer(
    ...
    try:
        ...
        content: str
        if paging is None:
            content = _get_non_paged_result(path="/skjemadata/skjema_paged", max_results=max_results, filters=filters)
        else:
            content = _get_paged_result(path="/skjemadata/skjema_paged", paging=paging, filters=filters)
...
def create_skjema(
    ...
    try:
        ...
        content: str = api_client._post(path="/skjemadata/skjema", body_json=body)
...
def delete_skjema(*, skjema_id: int, context: SuvOperationContext = None) -> OperationResult:
    try:
        content: str = api_client._delete(path=f"/skjema/{skjema_id}")
