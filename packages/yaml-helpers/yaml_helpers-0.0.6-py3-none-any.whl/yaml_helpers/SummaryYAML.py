...
class SummaryYAML:
    ...
    def get_entity_code(self, reduced=True):
        ...
        entity_code += ''.join(lines[last_line:end_line + 1]).rstrip()
    ...
    def get_function_names_from_path(self, code_lines=None):
        if code_lines:
            if len(code_lines) > 1:
            ...
            entities = self._get_nested_entities_names(file_entities)
        else:
            entities = [e[0]['name'] for e in self.get_entities_in_current_file()]
        return entities

    def _get_nested_entities_names(self, file_entities):
        entities = []
        for e in file_entities:
            entities.append(e["name"])
            if e['nested']:
                self._get_nested_entities_names(e['nested'])
        return entities
