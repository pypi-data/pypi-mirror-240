...
class recset(Generic[T]):
    ...
    def __initRows(self, columns:list = None):
    ...
    def joinfilterExpr(self, joinTable:dict, columns:tuple, isouter, w:saWhere, s:saOrderBy):
    ...
    def joinFilter(self, j:saJoin, isouter, w:saWhere, s:saOrderBy):
        ...
        # クエリ実行
        rowIndex = 0
        q = self.joinfilterExpr(j.joinTable, j.columns, isouter, w, s).all()
        for row in q:
            ...
            # メインテーブルの行情報コピー
            mainRow = copy.deepcopy(row[0])
            # 行情報のメンバ取得
            attributes = inspect.getmembers(row[0], lambda x: not(inspect.isroutine(x)))
            rowInfoMember = list(filter(lambda x: not(x[0].startswith("__") or x[0].startswith("_") or x[0] == "metadata" or x[0] == "registry"), attributes))
            # 抽出列以外はメンバを削除
            for attr in rowInfoMember:
                if attr[0] in fldList: continue
                delattr(mainRow, attr[0])

            # 行状態をnoChangedに変更
            mainRow.__rowState = self.rowState.noChanged
    ...
    def merge(self, srcRecset, sort = True):
