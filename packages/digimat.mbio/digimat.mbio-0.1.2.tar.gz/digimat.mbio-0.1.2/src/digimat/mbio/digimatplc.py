...
class MBIODeviceDigimatPLC(MBIODevice):
    NBCHANNELDI=4
    NBCHANNELDO=4
    NBCHANNELAI=4
    NBCHANNELAO=4
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        self.config.update('boards', xml.getInt('boards', vmin=1))
        ...
        for board in range(self.config.boards):
            ...
            for channel in range(self.NBCHANNELDO):
                ...
                if value.config.default is not None:
                    ...
                    data |= (0x10 << 2*channel)
            self.writeRegisterIfChanged(configRegister0, data)
    ...
    def poweronAI(self):
        for board in range(self.config.boards):
            ...
            self.writeRegisterIfChanged(configRegister0+0, data)
    ...
    def poweronAO(self):
        for board in range(self.config.boards):
            ...
            for channel in range(self.NBCHANNELAO):
                ...
                if value.config.default is not None:
                    data |= (0x1 << channel)
                    self.writeRegisterIfChanged(configRegister0+1,
                        self.AO_state2raw(value, value.config.default))

            self.writeRegisterIfChanged(configRegister0+2, data)
