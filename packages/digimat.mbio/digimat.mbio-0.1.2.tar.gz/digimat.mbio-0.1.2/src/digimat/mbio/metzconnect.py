...
class MBIODeviceMetzConnectMRDO4(MBIODevice):
    NBCHANNEL=4
    ...
    def onInit(self):
        ...
        for channel in range(self.NBCHANNEL):
            value=self.valueDigital('do%d' % channel, writable=True)
    ...
    def poweron(self):
        self.writeRegisterIfChanged(66, self.config.watchdog*100)
        data=0x0
        for channel in range(self.NBCHANNEL):
            value=self.DO[channel]
            ...
            if state:
                data |= (0x1 << channel)
        self.writeRegisterIfChanged(1, data)
        return True
...
class MBIODeviceMetzConnectMRDI10(MBIODevice):
    NBCHANNEL=10

    def onInit(self):
        self.DI=[]
        for channel in range(self.NBCHANNEL):
            value=self.valueDigital('di%d' % channel)
...
class MBIODeviceMetzConnectMRAI8(MBIODevice):
    NBCHANNEL=8
    ...
    def poweron(self):
        for channel in range(self.NBCHANNEL):
            ...
            self.writeRegisterIfChanged(16+channel, data)
        return True
                ...
                if data is not None:
                    if data<999999:
                        try:
                            ...
                            data=value.config.x0+data/10.0*dx
                        except:
                            pass
...
class MBIODeviceMetzConnectMRAOP4(MBIODevice):
    NBCHANNEL=4
    ...
    def onInit(self):
        ...
        for channel in range(4):
            value=self.value('ao%d' % channel, unit='%', writable=True)
    ...
    def poweron(self):
        ...
        for channel in range(self.NBCHANNEL):
            value=self.AO[channel]
            state=value.config.default
            self.writeRegisterIfChanged(self.NBCHANNEL+channel,
