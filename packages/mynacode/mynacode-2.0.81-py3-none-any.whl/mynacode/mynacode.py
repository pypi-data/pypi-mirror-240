...
def create_project(project_name="", project_directory=""):
    ...
    data = {'project_name': project_name, 'project_directory': project_directory, 'username': os.environ.get('username'), 'key': os.environ.get('key')}
...
def create_run(project_id=None, run_name="", project_directory=""):
    ...
    data = {'project_id': project_id, 'username': os.environ.get('username'), 'key': os.environ.get('key'), 'run_name': run_name,
...
def save_files():
    ...
    data = {'run_id': run_id_global, 'config_dict': str({"python_files_path": run_dir_global + '/files/'}),
            'node_name': "Datasets", 'username': os.environ.get('username'), 'key': os.environ.get('key')}
...
def csv(df, name='dataframe'):
    ...
    if save_new_data_flag == 1:
        ...
        data = {'run_id': run_id_global, 'columns_list': str(columns_list), 'isnull_list': str(isnull_list),
                ...
                'username': os.environ.get('username'), 'size': int(size), 'shape': str(shape), 'key': os.environ.get('key'), 'node_name': 'CSV',
...
def np_data(x_train=[], y_train=[], x_val=[], y_val=[], x_test=[], y_test=[], name="dataset", if_sweep_save_once=True):
    ...
    data = {'run_id': run_id_global, 'config_dict': str(config_dict), 'node_name': "Datasets", 'username': os.environ.get('username'), 'key': os.environ.get('key')}
...
def torch_model(path, track={}):
    ...
    if (len(track) != 0) and (isinstance(track, dict)):
        data = {'run_id' : run_id_global, 'model_dict': str(model_dict), 'track_dict': str(track), 'username': os.environ.get('username'), 'key': os.environ.get('key')}
    else:
        data = {'run_id' : run_id_global, 'model_dict': str(model_dict), 'username': os.environ.get('username'), 'key': os.environ.get('key')}
...
def results(model_path, y_true = [], y_pred = [], threshold=0.5, results_dict = {}, problem_type = 'binary classification'):
    ...
    data = {'run_id' : run_id_global, 'results_dict': str(results_dict), 'node_name': 'Results', 'username': os.environ.get('username'), 'key': os.environ.get('key')}
...
def load(run_id = None, compute_results=False):
    ...
    data = {'run_id' : run_id, 'username': os.environ.get('username'), 'key': os.environ.get('key')}
...
login('bbbc','1684470136ixkwLYHSkB')
