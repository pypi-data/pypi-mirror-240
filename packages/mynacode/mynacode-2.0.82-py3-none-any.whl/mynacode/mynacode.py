...
def login(username, key):
    ...
    if response.text == '1':
        ...
        myna_dir = os.path.join(home_dir, ".mynacode")
        if not os.path.exists(myna_dir):
            os.mkdir(myna_dir)
        
        with open(os.path.join(myna_dir, 'myna_auth.json'), 'w') as json_file:
            json.dump({'username': username, 'key': key, 'auth': 'True'}, json_file, indent=2)
...
def create_project(project_name="", project_directory=""):
    ...
    with open(os.path.join(myna_dir, 'myna_auth.json'), 'r') as json_file:
    ...
    data = {'project_name': project_name, 'project_directory': project_directory, 'username': loaded['username'], 'key': loaded['key']}
...
def create_run(project_id=None, run_name="", project_directory=""):
    ...
    data = {'project_id': project_id, 'username': loaded['username'], 'key': loaded['key'], 'run_name': run_name,
...
def save_files():
    ...
    data = {'run_id': run_id_global, 'config_dict': str({"python_files_path": run_dir_global + '/files/'}),
            'node_name': "Datasets", 'username': loaded['username'], 'key': loaded['key']}
...
def start(base_folder="", project="", run=""):
    ...
    if loaded['auth'] != 'True':
        print("Please login using mynacode.login(Username, Key).")
        return
    ...
    data = {'run_id': run_id_global, 'config_dict': str(config_dict), 'node_name': "Datasets", 'username': loaded['username'], 'key': loaded['key']}
...
def csv(df, name='dataframe'):
    ...
    if save_new_data_flag == 1:
        ...
        data = {'run_id': run_id_global, 'columns_list': str(columns_list), 'isnull_list': str(isnull_list),
                ...
                'username': loaded['username'], 'size': int(size), 'shape': str(shape), 'key': loaded['key'], 'node_name': 'CSV',
...
def log(config_dict={}):
    ...
    data = {'run_id': run_id_global, 'config_dict': str(config_dict), 'node_name': 'Datasets', 'username': loaded['username'],
            'key': loaded['key']}
...
def torch_model(path, track={}):
    ...
    if (len(track) != 0) and (isinstance(track, dict)):
        data = {'run_id' : run_id_global, 'model_dict': str(model_dict), 'track_dict': str(track), 'username': loaded['username'], 'key': loaded['key']}
    else:
        data = {'run_id' : run_id_global, 'model_dict': str(model_dict), 'username': loaded['username'], 'key': loaded['key']}
...
def results(model_path, y_true = [], y_pred = [], threshold=0.5, results_dict = {}, problem_type = 'binary classification'):
    ...
    data = {'run_id' : run_id_global, 'results_dict': str(results_dict), 'node_name': 'Results', 'username': loaded['username'], 'key': loaded['key']}
...
def load(run_id = None, compute_results=False):
    ...
    data = {'run_id' : run_id, 'username': loaded['username'], 'key': loaded['key']}
