...
class Dxlfunc:
    ...
    def __init__(self):
        ...
        self._one_byte_values = [self.Address.ID, self.Address.BaudRate, self.Address.ReturnDelayTime,
                                 ...
                                 self.Address.TorqueEnable, self.Address.LED,
                                 self.Address.StatusReturnLevel, self.Address.HardwareErrorStatus,
        ...
        if MotorID == 'ALL':
            for i in self.IDs:
                if INPUT_Address in self._one_byte_values:
                    self._packetHandler.write1ByteTxRx(self._portHandler, i, INPUT_Address[self.MotorSeries[i]], value)
                elif INPUT_Address in self._two_byte_values:
                    self._packetHandler.write2ByteTxRx(self._portHandler, i, INPUT_Address[self.MotorSeries[i]], value)
                elif INPUT_Address in self._four_byte_values:
                    self._packetHandler.write4ByteTxRx(self._portHandler, i, INPUT_Address[self.MotorSeries[i]], value)
        else:
            if INPUT_Address in self._one_byte_values:
                self._packetHandler.write1ByteTxRx(self._portHandler, MotorID, INPUT_Address[self.MotorSeries[MotorID]], value)
            elif INPUT_Address in self._two_byte_values:
                self._packetHandler.write2ByteTxRx(self._portHandler, MotorID, INPUT_Address[self.MotorSeries[MotorID]], value)
            elif INPUT_Address in self._four_byte_values:
                self._packetHandler.write4ByteTxRx(self._portHandler, MotorID, INPUT_Address[self.MotorSeries[MotorID]], value)

    def read(self, MotorID, INPUT_Address):
        ret = 0
        if INPUT_Address in self._one_byte_values:
            ret, _, _ = self._packetHandler.read1ByteTxRx(self._portHandler, MotorID, INPUT_Address[self.MotorSeries[MotorID]])
            if ret >= np.power(2, 7):
                ret = ret - np.power(2, 8)
        elif INPUT_Address in self._two_byte_values:
            ret, _, _ = self._packetHandler.read2ByteTxRx(self._portHandler, MotorID, INPUT_Address[self.MotorSeries[MotorID]])
            if ret >= np.power(2, 15):
                ret = ret - np.power(2, 16)
        elif INPUT_Address in self._four_byte_values:
            ret, _, _ = self._packetHandler.read4ByteTxRx(self._portHandler, MotorID, INPUT_Address[self.MotorSeries[MotorID]])
            if ret >= 2**31:
                ret = ret - 2**32
        return ret
