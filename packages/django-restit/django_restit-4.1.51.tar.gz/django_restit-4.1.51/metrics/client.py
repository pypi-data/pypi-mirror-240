...
def get_metrics(slugs, since=None, granularity="daily", group=None):
    """Create/Increment a metric."""
    return get_r().get_metric_history_chart_data(slugs, since, granularity, group=group)
...
class R(object):
    ...
    def build_periods(self, slug, since, to=None, granularity="daily"):
        periods = []
        for date in utils.date_range(granularity, since, to):
            periods.append(
                utils.slug_to_label(
    ...
    def get_metric_history_chart_data(self, slugs, since=None, granularity='daily', group=None):
        ...
        """
        if since is None:
            since = utils.start_by_granularity(granularity)
        if group:
        ...
        periods = self.build_periods(slugs[0], since, granularity=granularity)
