...
class ChannelApi:
    ...
    CHANNELS_URI = 'api/rest/v1/channels'
    CHANNEL_URI = 'api/rest/v1/channels/%s'
    ...
    def get(self, code: str) -> dict[str, any]:
        response = self.resource_client.get_resource(self.CHANNEL_URI, [code])

        return json.loads(response.content)

    def all(self, page_size: int = 10, query_params: dict = {}) -> Iterable[list]:
        response = self.resource_client.get_resources(self.CHANNELS_URI, [], query_params, page_size)
    ...
    def create(self, code: str, data: dict = {}) -> None:
        ...
        self.resource_client.create_resource(self.CHANNELS_URI, [], DictSerialize(data))

    def upsert(self, code: str, data: dict = {}) -> None:
        self.resource_client.upsert_resource(self.CHANNEL_URI, [code], DictSerialize(data))

    def upsert_list(self, data: list[dict]) -> list[dict]:
        batch = LineSerialize()
        batch.add_items(data)

        response = self.resource_client.upsert_list_resource(self.CHANNELS_URI, [], batch)
