...
async def get_mltclassifier_agent():
...
async def qna_run(request_id):
    ...
    try:
        ...
        details = await get_mltclassifier_agent()

        if details is None:
            query_update_result(request_id, {
                'status': "failure",
                "message": f"LLM Multiclassifier Agent could not be found/built for {label}",
                "result": {}
            })
            return
        ...
        # Now run the query
        success, result = await run_in_threadpool(lambda: agent.label(query, label_spec=get_profilespec()))
