...
class LLMDataExplainer(object):
    ...
    def json_to_text(self, json_obj, policy):
        ...
        for key, item in json_obj.items():
            ...
            prompt_spec = spec.get("keys", {}).get(key)
            if not prompt_spec:
                self.logger.debug(f"No prompt defined for [{key}], skipping",
                             extra={
            ...
            if success:
                ...
                self.logger.debug(f"Something went wrong generating text for [{key}]",
                             extra={
                                 'source': self.agent_name,
