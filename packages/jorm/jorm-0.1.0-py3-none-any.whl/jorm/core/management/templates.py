...
from jorm.core.management.base import BaseCommand, CommandError
from jorm.core.management.utils import (
    find_formatters,
    handle_extensions,
    run_formatters,
)
from jorm.template import Context, Engine
from jorm.utils import archive
from jorm.utils.http import parse_header_parameters
...
class TemplateCommand(BaseCommand):
    ...
    :param style: A color style object (see jorm.core.management.color).
    ...
    def handle_template(self, template, subdir):
        ...
        Use jorm.__path__[0] as the default because the Django install
        directory isn't known.
        """
        if template is None:
            return os.path.join(jorm.__path__[0], "conf", subdir)
        else:
    ...
    def download(self, url):
        ...
        opener = build_opener()
        opener.addheaders = [("User-Agent", f"Django/{jorm.__version__}")]
        try:
