...
class PluginEntrypoint:
    ...
    def __init__(
        ...
        self._plugin_instance._snowflake = _snowflake  # pylint: disable=protected-access
    ...
    def sync(self, sync_request: Dict):
        ...
        connection_parameters._snowflake = self._snowflake  # pylint: disable=protected-access
        ...
        if request.sync_direction == "outbound":
            ...
            parameters._snowflake = self._snowflake
            if request.oauth_secret_name is not None:
    ...
    def get_secrets(
        ...
        if oauth_secret_name is not None:
            connection_secrets["access_token"] = StoredConfigurationValue(
                value=self._snowflake.get_oauth_access_token(oauth_secret_name)
            )
        if other_secrets_name is not None:
            try:
                secret_string_content = self._snowflake.get_generic_secret_string(
                    other_secrets_name
                )
    ...
    def api_limits(self,
        ...
        connection_parameters._snowflake = self._snowflake
        if oauth_secret_name is not None:
