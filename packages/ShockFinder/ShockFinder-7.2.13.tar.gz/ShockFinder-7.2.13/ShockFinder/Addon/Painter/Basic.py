...
def pol_to_xoy(r, phi, v=None):
    ...
    if v is not None:
        values = v.flatten()
        grid_V = griddata(points, values, (grid_X, grid_Y), method='linear', fill_value=0.1 * np.min(v))
...
def rot_to_xoz(r,theta,v=None):
    ...
    if v is not None:
        values = v.flatten()
        grid_V = griddata(points, values, (grid_X, grid_Z), method='linear', fill_value=0.1 * np.min(v))
...
def rzp_to_xyz(r, z, phi, v=None):
    ...
    if v is not None:
        values = v.flatten()
        grid_V = griddata(points, values, (grid_X, grid_Y, grid_Z), method='linear', fill_value=0.1 * np.min(v))
