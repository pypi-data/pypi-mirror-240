...
def gradient(Dataobj,quantity_name):
	...
	if Dataobj.quantities[quantity_name].ndim==1:#1d
		quantities["Gradient_"+quantity_name+"_x1"]=np.gradient(Dataobj.quantities[quantity_name],Dataobj.grid["x1"])
...
def divergence_sph(F, r, theta=None, phi=None):
	...
	if F.ndim == 1:  # 1D (just Radial)
		...
		dFtheta_dtheta = np.gradient(np.sin(theta).reshape(1,len(theta))*F, theta, axis=1)
		return (1/r.reshape(len(r),1)**2) * drFr_dr + (1/(r.reshape(len(r),1) * np.sin(theta).reshape(1,len(theta)))) * dFtheta_dtheta
		
	elif F.ndim == 3:  # 3D
		drFr_dr = np.gradient(r.reshape(len(r),1,1)**2*F, r, axis=0)
		dFtheta_dtheta = np.gradient(np.sin(theta).reshape(1,len(theta),1)*F, theta, axis=1)
		dFphi_dphi = np.gradient(F, phi, axis=2)
		return (1/r.reshape(len(r),1,1)**2) * drFr_dr + (1/(r.reshape(len(r),1,1) * np.sin(theta).reshape(1,len(theta),1))) * (dFtheta_dtheta + dFphi_dphi)
...
def result(quantity_name=None,anafname=None):
	if anafname=="Gradient":
		return ["Gradient_"+i+"_x1" for i in quantity_name.split(",")]#+["Gradient_"+i+"_x2" for i in quantity_name.split(",")]+["Gradient_"+i+"_x3" for i in quantity_name.split(",")]
