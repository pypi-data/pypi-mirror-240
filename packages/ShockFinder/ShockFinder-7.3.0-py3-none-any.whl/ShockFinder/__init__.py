...
def args_cut(args,pnum):
	...
	for i in args:
		...
		if pp==pnum:
			pp=0
	return result
def test_database(num=None,tindex=None,printfun=print,geometry="SPHERICAL",grids=AnalysisLib["TestData"].grids_default_num,grids_map=AnalysisLib["TestData"].grids_default["SPHERICAL"]):
	db=DataBase(Default,printfun)
	if num==None:
		num=1
		tindex=[0]
	db.build_test_data(AnalysisLib["TestData"].get,num=num,geometry=geometry,grids=grids,grids_map=grids_map)
