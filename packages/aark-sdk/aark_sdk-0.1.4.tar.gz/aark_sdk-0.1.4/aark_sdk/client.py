...
class Client:
    ...
    def get_limit_orders(self, market_id: Optional[int]):
        response = requests.get(
            f"{self.BACKEND_API_URL}/order/futures/limit?status=0&user={self.wallet.address}{'&marketId='+str(market_id) if market_id else ''}")
    ...
    def __trigger_order(self, symbol: str, order_object: str, nonce: Decimal,  trigger_price: Decimal, is_limit: bool):
        ...
        current_epoch = self.__get_epoch()
        trigger_price_applied_decimals = (
            Decimal(trigger_price) * (10 ** self.PRICE_DECIMALS)).__floor__()
        msg_hash = keccak.new(
            digest_bits=256).update(encode(['address', 'uint256', 'uint256', 'uint256', 'uint256', 'uint256'], [
                self.wallet.address, int(order_object, 16), current_price, trigger_price_applied_decimals, nonce, current_epoch])).hexdigest()
