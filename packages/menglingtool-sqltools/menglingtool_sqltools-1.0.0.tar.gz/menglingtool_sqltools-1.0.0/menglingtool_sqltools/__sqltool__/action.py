...
class Action(Body):
    ...
    def _auto_add_lie(self, lies, table, dataclass, other):
        ...
        sql = '''ALTER TABLE {table} {add} {other}
                '''.format(table=self.getTablestr(table),
                           add=','.join(f'ADD {lie} {dataclass}' for lie in self.getLiestrs(not_liest)),
                           other=other)
        return self.run(sql)
    ...
    def create_insert(self, table, dts: list,
                      ...
                      hz: str = None,
                      in_hz='',
                      auto_lie_class=None, **null):
        sdata = SqlData(dts)
        self.createTable(table, sdata.getLies(), sdata.getAutoColMap(self.class_map, **colmap), key=key, hz=hz, **null)
        self.insert(table, sdata, in_hz=in_hz, auto_lie_class=auto_lie_class, **null)
