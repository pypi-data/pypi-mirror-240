...
def MinMaxScaledDifference(SimResultObjects=[], AttributesOrKeys=[], TimeSteps=None, Interpolate=False):
    ...
    _verbose(verbosity, 1,
             ' < MinMaxDiff > found ' + str(len(MatchingKeys)) + ' Keys and ' + str(len(Atts)) + ' Attributes, ')
    ...
    _verbose(verbosity, 1, ' < MinMaxDiff > totalizing ' + str(
    ...
    if TimeSteps is None:
        ...
        TimeSteps = list()
        for obj in SimResultObjects:
            _verbose(verbosity, 0, ' < MinMaxDiff > getting TimeSteps from ' + str(
                obj.get_Name()) + '\n < MinMaxDiff > extracted ' + str(
        ...
        _verbose(verbosity, 1, ' < MinMaxDiff > ' + str(len(TimeSteps)) + ' TimeSteps found, from ' + str(
            TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.')

    else:
        _verbose(verbosity, 1, ' < MinMaxDiff > ' + str(len(TimeSteps)) + ' TimeSteps received, from ' + str(
            TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.')
        ...
        if Interpolate == False:
            ...
            if len(TimeSteps) > 0:
                _verbose(verbosity, 1, ' < MinMaxDiff > ' + str(
            ...
            if len(TimeSteps) - len(MatchingTimeSteps) > 0:
                _verbose(verbosity, 2, ' < MinMaxDiff > WARNING: data will be interpolated for ' + str(
                    len(TimeSteps) - len(
    ...
    _verbose(verbosity, -1,
             ' < MinMaxDiff >  resulting total MinMaxScaledDifference is: ' + str(abs(sumDF).mean(axis=1).mean(axis=0)))
    return (abs(sumDF).mean(axis=1, skipna=True).mean(axis=0, skipna=True), abs(sumDF).mean(axis=0, skipna=True),
            pd.DataFrame(data=abs(sumDF).mean(axis=1, skipna=True), index=abs(sumDF).index,
                         columns=['MinMaxScaledDifference']), sumDF, dictOfDF)
