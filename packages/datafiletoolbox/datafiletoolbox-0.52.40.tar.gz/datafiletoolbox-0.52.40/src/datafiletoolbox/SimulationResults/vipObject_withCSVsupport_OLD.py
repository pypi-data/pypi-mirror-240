...
class VIP(SimResult):
    ...
    def loadSSS(self, SSSFilePath):
        if type(SSSFilePath) == str:
            ...
            self.name = extension(SSSFilePath)[1]
            for file in self.SSSfiles:
                self.results[extension(file)[1] + extension(file)[0]] = self.SSSread(file)
            self.strip('NAME')
            self.set_FieldTime()
            self.get_Vector('DATE')
            self.get_Wells(reload=True)
    ...
    def CSVread(self, CSVFilePath):
        ...
        while row < len(CSVlines):
            ...
            if cell0 == '[S3INFO]':
                ...
                if '[' in CSVlines[row].split(', ')[0].split('=')[0][0] and ']' in \
                        CSVlines[row].split(', ')[0].split('=')[0][-1]:
                    section = CSVlines[row].split(', ')[0].split('=')[0]
                else:
                    CSVdict[section][cell0] = [CSVlines[row].split(', ')[0].split('=')[1]] + CSVlines[row].split(', ')[
                                                                                             1:]
            row += 1
        return CSVdict
    ...
    def CSVextractHeaders(self, CSVname=''):
        ...
        for i in range(len(self.CSV[CSVname]['[HEADERS]']['VARIABLE'])):
            if len(self.CSV[CSVname]['[HEADERS]']['MEMBER'][i].strip()) > 0:
                self.units[
                    self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i] + ':' + self.CSV[CSVname]['[HEADERS]']['MEMBER'][i]] = \
                self.CSV[CSVname]['[HEADERS]']['UNITS'][i]
            ...
            SimResult.CSV_Variable2Verbose[self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i]] = \
            self.CSV[CSVname]['[HEADERS]']['VERBOSE'][i]
            SimResult.CSV_Verbose2Variable[self.CSV[CSVname]['[HEADERS]']['VERBOSE'][i]] = \
            self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i]
            CSVkeys += [
                self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i] + ':' + self.CSV[CSVname]['[HEADERS]']['MEMBER'][i]]
            ECLkey = fromCSVtoECL(variableORkey=self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i],
                                  CLASStype=self.CSV[CSVname]['[HEADERS]']['CLASS'][i],
                                  MEMBER=self.CSV[CSVname]['[HEADERS]']['MEMBER'][i], speak=self.speak)
            if ECLkey is not None:
                ECLkeys += [ECLkey]
                VIPkey, keyType, keyName = fromECLtoVIP(ECLkey, self.speak)
                VIPkeys += [VIPkey + ':' + keyName]

            fullName = self.CSV[CSVname]['[HEADERS]']['CLASS'][i] + ':' + self.CSV[CSVname]['[HEADERS]']['MEMBER'][
                i] + ':' + self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i]
            self.pandasColumns[fullName] = [self.CSV[CSVname]['[HEADERS]']['CLASS'][i],
                                            self.CSV[CSVname]['[HEADERS]']['MEMBER'][i],
                                            self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i],
                                            self.CSV[CSVname]['[HEADERS]']['UNITS'][i],
                                            self.CSV[CSVname]['[HEADERS]']['VERBOSE'][i]]
    ...
    def CSVextractVectors(self, CSVname):
        ...
        for i in range(numHeaders):
            if len(self.CSV[CSVname]['[HEADERS]']['MEMBER'][i].strip()) > 0:
                CSVkey = self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i] + ':' + self.CSV[CSVname]['[HEADERS]']['MEMBER'][
                    i]
            else:
    ...
    def CSVloadVector(self, key, VIPkey='', keyType='', keyName='', CSVname=None):
        ...
        for CSVname in CSVnames:
            numHeaders = len(self.CSV[CSVname]['[HEADERS]']['VARIABLE'])
            verbose(self.speak, -1, ' looking for vector for key: ' + str(
                key) + ' where variable=' + Variable + ', class=' + Class + ' or ' + keyType + ' and member=' + Member + ' or ' + keyName)
            for col in range(numHeaders):
                if (self.CSV[CSVname]['[HEADERS]']['CLASS'][col] == keyType or self.CSV[CSVname]['[HEADERS]']['CLASS'][
                    col] == Class) and \
                        (self.CSV[CSVname]['[HEADERS]']['MEMBER'][col] == keyName or
                         self.CSV[CSVname]['[HEADERS]']['MEMBER'][col] == Member) and \
                        (self.CSV[CSVname]['[HEADERS]']['VARIABLE'][col] == Variable or
                         self.CSV[CSVname]['[HEADERS]']['VARIABLE'][col] == keyword):
                    verbose(self.speak, -1, ' found vector for key: ' + str(key) + ' where variable=' +
                            self.CSV[CSVname]['[HEADERS]']['VARIABLE'][col] + ', class=' +
                            self.CSV[CSVname]['[HEADERS]']['CLASS'][col] + ' and member=' +
                            self.CSV[CSVname]['[HEADERS]']['MEMBER'][col] + '.')
                    if len(self.CSV[CSVname]['[HEADERS]']['MEMBER'][col]) > 0:
                        CSVkey = self.CSV[CSVname]['[HEADERS]']['VARIABLE'][col] + ':' + \
                                 self.CSV[CSVname]['[HEADERS]']['MEMBER'][col]
                    else:
                        CSVkey = self.CSV[CSVname]['[HEADERS]']['VARIABLE'][col]
                    ECLkey = fromCSVtoECL(variableORkey=self.CSV[CSVname]['[HEADERS]']['VARIABLE'][col],
                                          CLASStype=self.CSV[CSVname]['[HEADERS]']['CLASS'][col],
                                          MEMBER=self.CSV[CSVname]['[HEADERS]']['MEMBER'][col], speak=self.speak)
    ...
    def CSVgenerateResults(self):
        for CSVname in list(self.CSV.keys()):
            ...
            verbose(self.speak, 3,
                    ' generating raw data dictionary from CSV table, \n  > preparing results dictionary\n    ... please wait ...')
            for sss in list(set(self.CSV[CSVname]['[HEADERS]']['CLASS'])):
                if sss not in self.results.keys():
                    self.results[str(sss) + '@' + extension(self.path)[1] + extension(self.path)[0]] = (
                    str(sss), {'Data': {}, 'Units': {}})
            ...
            for i in range(numHeaders):
                self.results[
                    self.CSV[CSVname]['[HEADERS]']['CLASS'][i] + '@' + extension(self.path)[1] + extension(self.path)[
                        0]][1]['Units'][self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i]] = \
                self.CSV[CSVname]['[HEADERS]']['UNITS'][i]
            ...
            for i in range(numHeaders):
                ...
                self.results[
                    self.CSV[CSVname]['[HEADERS]']['CLASS'][i] + '@' + extension(self.path)[1] + extension(self.path)[
                        0]][1]['Data'][self.CSV[CSVname]['[HEADERS]']['VARIABLE'][i] + ':' + Name] = Vector
        ...
        for CSV in list(self.results.keys()):
            ...
            if max(KEYsLenght) == min(KEYsLenght):
                verbose(self.speak, 3,
                        '  > ' + str(CSV) + ' properly created with ' + str(numHeaders) + ' columns and ' + str(
                            max(KEYsLenght)) + ' rows.')
            else:
                print(max(KEYsLenght), min(KEYsLenght), numRows)
                verbose(self.speak, -1, '  > ' + str(CSV) + ' issue: ' + str(numHeaders) + ' columns and ' + str(
                    max(KEYsLenght)) + ' rows.')
                OK = False
    ...
    def SSSparts(self, SSSFilePath):
        ...
        if extension(SSSFilePath)[0].upper() == '.SSS':
        ...
        else:  # if extension(SSSFilePath)[0] != '.SSS' :
            SSSroot = extension(SSSFilePath)[2] + extension(SSSFilePath)[1]
            for Case in expectedParts:
                for part in Case:
                    if os.path.isfile(SSSroot + part):
                        SSSfiles.append(SSSroot + part)
    ...
    def SSSread(self, sssPath):
        ...
        if self.speak != 0:
            ...
            for each in sssDict['Data']:
                verbose(self.speak, 1, '  > ' + str(each) + str(' ' * (16 - len(str(each)))) + ' with ' + str(
                    ...
                    for sss in list(self.results.keys()):
                        if self.results[sss][0] == keyType:
                            if VIPkey in self.results[sss][1]['Data'].keys():
                                ...
                                verbose(self.speak, 1,
                                        'extracted ' + VIPkey + ' from ' + keyType + ' with lenght ' + str(len(RawCol)))
                                try:
                        ...
                        if self.results[sss][0] == keyType:
                            if VIPkey in self.results[sss][1]['Data'].keys():
                                ...
                                if type(keyName) == str:
                                    ...
                                    verbose(self.speak, 1, 'extracting ' + VIPkey + ' with lenght ' + str(
                                    ...
                                    CleanCol = np.extract(np.char.equal(NameCol, keyName), RawCol)
                                    CleanTime = np.extract(np.char.equal(NameCol, keyName), TimeCol)
                                    verbose(self.speak, 1, 'cleaned ' + VIPkey + ' with lenght ' + str(
                                        len(CleanCol)) + ' for item ' + keyName + '.')
                                else:
                                    verbose(self.speak, 2, 'multiple ( ' + str(
    ...
    def get_Dates(self):
        try:
            ...
            DateVector = strDate(list(self.loadVector('DATE', 'FIELD', True)), formatIN='DD-MM-YYYY',
                                 speak=(self.speak == 1))
                    ...
                    if user in ['Y', 'YES', 'SI', 'SÍ', 'OUI']:
                        if ':' in ECLkey:
                            if self.is_Key('WBP:' + ECLkey.split(':')[1]):
                                ...
                                verbose(self.speak, -1, " the corresponding well for the key '" + mainKey(
