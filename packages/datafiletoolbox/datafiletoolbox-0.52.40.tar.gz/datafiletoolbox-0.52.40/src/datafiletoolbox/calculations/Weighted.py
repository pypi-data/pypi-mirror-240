# -*- coding: utf-8 -*-
"""
Created on Wed May 13 15:03:38 2020
...
def WeightedDifference(SimResultObjects=[], AttributesOrKeys=[], TimeSteps=None, Interpolate=False):
    ...
    _verbose(verbosity, 1, ' < WeightDiff > sarting with verbosity set to ' + str(verbosity))

    _verbose(verbosity, 1, ' < WeightDiff > the following objects will be used:')
    if verbosity > 0:
        for obj in SimResultObjects:
            _verbose(verbosity, 1, ' < WeightDiff > ' + str(obj.get_Name()))

    # split keys from attributes
    _verbose(verbosity, 1, ' < WeightDiff > checking AttributesOrKeys list:')
    _verbose(verbosity, 0, ' < WeightDiff >   Keys          Attributes')
    ...
    for each in AttributesOrKeys:
        if ':' in each:
            ...
            _verbose(verbosity, 0, ' < WeightDiff >   ' + each.strip())
    ...
    _verbose(verbosity, 1,
             ' < WeightDiff > found ' + str(len(MatchingKeys)) + ' Keys and ' + str(len(Atts)) + ' Attributes, ')
    ...
    if TimeSteps is None:
        _verbose(verbosity, 1, ' < WeightDiff > TimeSteps not received, all the matching timesteps will be used.')
        TimeSteps = list()
        for obj in SimResultObjects:
            _verbose(verbosity, 0, ' < WeightDiff > getting TimeSteps from ' + str(
                obj.get_Name()) + '\n < WeightDiff > extracted ' + str(
                len(list(obj.get_Vector(TimeKind)[TimeKind]))) + ' ' + TimeKind + '.')
            TimeSteps += list(obj.get_Vector(TimeKind)[TimeKind])
        _verbose(verbosity, 1, ' < WeightDiff > ' + str(len(TimeSteps)) + ' TimeSteps found.')
        TimeSteps = list(set(TimeSteps))
        TimeSteps.sort()
        _verbose(verbosity, 1, ' < WeightDiff > ' + str(len(TimeSteps)) + ' sorted TimeSteps found.')
        if Interpolate == False:
            for obj in SimResultObjects:
                _verbose(verbosity, 1, ' < WeightDiff > looking for matching TimeSteps for ' + obj.get_Name())
                TimeSteps = list(set(TimeSteps).intersection(set(list(obj.get_Vector(TimeKind)[TimeKind]))))
        _verbose(verbosity, 1, ' < WeightDiff > ' + str(len(TimeSteps)) + ' TimeSteps found, from ' + str(
            TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.')

    else:
        _verbose(verbosity, 1, ' < WeightDiff > ' + str(len(TimeSteps)) + ' TimeSteps received, from ' + str(
            TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.')
        if type(TimeSteps[0]) == type(SimResultObjects[0].get_Vector('DATES')['DATES'][0]):
            ...
            TimeKind = 'TIME'
        _verbose(verbosity, 1, ' < WeightDiff > TimeSteps found to be of type ' + TimeKind)
        if Interpolate == False:
            for obj in SimResultObjects:
                TimeSteps = list(set(TimeSteps).intersection(set(obj.get_Vector(TimeKind)[TimeKind])))
            if len(TimeSteps) > 0:
                _verbose(verbosity, 1, ' < WeightDiff > ' + str(
                    len(TimeSteps)) + ' TimeSteps found matching the simulation data TimeSteps, from ' + str(
                    TimeSteps[0]) + ' to ' + str(TimeSteps[-1]) + '.')
            else:
                _verbose(verbosity, 2, ' < WeightDiff > ' + 'No matching TimeSteps in the input profiles.')
        else:
            for obj in SimResultObjects:
            ...
            if len(TimeSteps) - len(MatchingTimeSteps) > 0:
                _verbose(verbosity, 2, ' < WeightDiff > WARNING: data will be interpolated for ' + str(
                    len(TimeSteps) - len(
    ...
    _verbose(verbosity, -1,
             ' < WeightDiff >  resulting total WeightedDifference is: ' + str(abs(sumDF).mean(axis=1).mean(axis=0)))
    return (abs(sumDF).mean(axis=1, skipna=True).mean(axis=0, skipna=True), abs(sumDF).mean(axis=0, skipna=True),
            pd.DataFrame(data=abs(sumDF).mean(axis=1, skipna=True), index=abs(sumDF).index,
                         columns=['WeightedDifference']), sumDF, dictOfDF)
