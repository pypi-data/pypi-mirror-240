...
class NetdotAPIDataclass(CSVDataclass):
    ...
    def _attempt_auto_update_if_enabled(self, name, value):
        """__setattr__ helper function"""
        all_updatable_fields = self._updatable_fields(
            xlink_suffix=True
        ) + self._updatable_fields(xlink_suffix=False)
        if (
            ...
            and name
            in all_updatable_fields  # Only try to run updates if this is an updatable field
        ):
    ...
    @classmethod
    def _updatable_fields(cls, xlink_suffix=True) -> List[str]:
        ...
        return list(
            filter(
                ...
                and field_name
                not in cls.foreign_key_field_names(remove_suffix=xlink_suffix),
    ...
    def to_DTO(self) -> Dict:
        ...
        dto = {field: getattr(self, field) for field in self._updatable_fields()}
