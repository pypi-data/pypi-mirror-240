...
def mdapy_csp(pos, box):
    CSP = mp.CentroSymmetryParameter(12, pos, box)
    CSP.compute()


def test_csp_average_time(ave_num=3):
    time_list = []
    print("*" * 30)
    for num in [5, 25, 45, 65, 85, 105, 125]:
        ...
        for turn in range(ave_num):
            ...
            start = time()
            data.apply(CentroSymmetryModifier(num_neighbors=12))
            end = time()
            ovito_t += end - start
...
def plot(time_list, title=None, savefig=True):
    ...
    plt.ylabel("Time (s)")
    if savefig:
        plt.savefig("CSP.png", dpi=300, bbox_inches="tight", transparent=True)
    plt.show()


if __name__ == "__main__":
    mp.init()
    time_list = test_csp_average_time(ave_num=3)
    np.savetxt("time_list.txt", time_list)
    time_list = np.loadtxt("time_list.txt")
    plot(time_list, "Calculate CSP")
