    ...
    def upload_groundtruth_from_json(
        ...
        if annotation_geometry == BOX_ANNOTATION:
            _groundTruth.upload_groundtruth_json(
                unique_id, operation_id, file_path, BOX_ANNOTATION, is_normalized, version, bucket_name, upload_type)
        elif annotation_geometry == POLYGON_ANNOTATION:
            _groundTruth.upload_groundtruth_json(
                unique_id, operation_id, file_path, POLYGON_ANNOTATION, is_normalized, version, bucket_name, upload_type)
        elif annotation_geometry == LINE_ANNOTATION:
            _groundTruth.upload_groundtruth_json(
                unique_id, operation_id, file_path, LINE_ANNOTATION, is_normalized, version, bucket_name, upload_type)
        elif annotation_geometry is None:
            _groundTruth.upload_groundtruth_json(
                unique_id, operation_id, file_path, None, is_normalized, version, bucket_name, upload_type)
        else:
            datalake_logger.debug(
                f'unsupported annotation_geometry={annotation_geometry}')
