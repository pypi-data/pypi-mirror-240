...
def setup_logger(name, log_file, level=logging.INFO):
    ...
    # Using ConcurrentRotatingFileHandler instead of standard FileHandler
    handler = ConcurrentRotatingFileHandler(log_file, "a", maxBytes=5*1024*1024, backupCount=5)
...
class WorkerProcess(Process):  # /# have to have "Process" here to enable worker.start()
    ...
    def __init__(self, index,
                 ...
                 total_reads_5p_no_3p, reads_too_short,
                 ultra_mode,
        ...
        self._total_demultiplexed = manager.list([0])
        self._total_reads_assigned = manager.list([0])
        self._total_reads_qtrimmed = manager.list([0])
        self._total_reads_adaptor_trimmed = manager.list([0])
        ...
        self._reads_too_short = reads_too_short  # a queue which keeps track of how many reads are too short to be assigned
    ...
    def run(self):
        ...
        while True:  # /# once spawned, this keeps running forever, until poison pill recieved
            ...
            # LOG reads too short
            prev_total = self._reads_too_short.get()
            new_total = prev_total + reads_too_short
            self._reads_too_short.put(new_total)
...
def start_workers(n_workers, input_file, need_work_queue, adapter,
                  ...
                  total_reads_qtrimmed, total_reads_adaptor_trimmed, total_reads_5p_no_3p, reads_too_short,
    ...
    for index in range(n_workers):
        ...
        worker = WorkerProcess(index=index,
                               ...
                               save_name=save_name,
total_demultiplexed = manager.list([0])
total_reads_assigned = manager.list([0])
total_reads_qtrimmed = manager.list([0])
total_reads_adaptor_trimmed = manager.list([0])
...
total_reads_adaptor_trimmed = manager.list([0])
    ...
    # /# wait for all workers to finish, close all the queues
    need_work_queue.put('STOP')
    total_demultiplexed.put('STOP')
    total_reads_assigned.put('STOP')
    total_reads_qtrimmed.put('STOP')
    total_reads_adaptor_trimmed.put('STOP')
    total_reads_5p_no_3p.put('STOP')
    reads_too_short.put('STOP')
    #logger.join()
...
total_qtrim = manager.list([0])
total_qtrim_percent = manager.list([0])
total_adaptortrim = manager.list([0])
total_adaptortrim_percent = manager.list([0])
total_5p_no3 = manager.list([0])
total_5p_no3_percent = manager.list([0])
total_ass = manager.list([0])
total_ass_percent = manager.list([0])
    ...
    shortmsg = str(reads_too_short.get()) + " reads are too short after processing to be written"
    logger.info(assmsg)
