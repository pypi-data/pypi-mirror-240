...
def netconf(mode):
    ...
    while True:
        ...
        if mode == 'netconf' :
            ...
            #choosing FW name
            uf = prompt(f'CLI > {mode} >', completer=coms_obj.netconf()).strip() 
        ...
        if uf != 'return':
            ...
            if mode == 'netconf' :
                ...
                try:
                    # Cridential as list for the selected device ['192.168.1.201','root','Password']
                    cridential_as_list = devices.FWs[uf]
                except Exception as e:
