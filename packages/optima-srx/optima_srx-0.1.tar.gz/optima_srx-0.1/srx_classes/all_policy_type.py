...
def make_new_address(address,ip,dev=None,file_content=None):
    if ip.find(' to ') != -1:
        send_address=f'address\t{address}\t\t{ip[0]}\t{ip[1]}' 
    else:
...
def get_address_ip(address,dev=None,file_content=None):
    ...
    if len(retuen_match_in_list_set)== 0 : # not found as address set 
        ...
        add = re.search('address (.*?)$',retuen_match_in_list_set[0]).group(1)
        if dev != None:
...
def create_new_app(protocol,port): #['app',['command1','command2']] or [False,False]
    ...
    command=[
        f'set applications application {app} protocol {protocol.lower()}',
    ...
    for each_src in Src_Zon_Group:
        for each_dst in Dst_Zon_Group:
            ...
            if True_if_idp(dev=dev ,file_content=file_content):
                ...
                commands.append(f'set security policies from-zone {m(each_src[0])} to-zone {m(each_dst[0])} policy {m(rule_name)} match url-category none')
                ...
                commands.append(f'set security policies from-zone {m(each_src[0])} to-zone {m(each_dst[0])} policy {m(rule_name)} then ' + m('permit'))
            commands.append(f'set security policies from-zone {m(each_src[0])} to-zone {m(each_dst[0])} policy {m(rule_name)} then log ' + m('session-close'))
            commands.append(f'set security policies from-zone {m(each_src[0])} to-zone {m(each_dst[0])} policy {m(rule_name)} then ' + m('count'))
    commands.append('')
    return commands
...
def values_after(mylist,after_value):
    ...
    search_range = []
    if after_value == 'Src': search_range = ['Src. Address','Dst. Address']
    if after_value == 'Dst': search_range = ['Dst. Address','Ser.']
    if after_value == 'Ser': search_range = ['Ser.','Src. Address']
    
    
    # Iterate through the list
    for item in mylist:
        if item == search_range[0]:
            found_Item = True
        elif item == search_range[1]:
            found_Item = False
        elif found_Item and item != '':
            found_values_between.append(item)
