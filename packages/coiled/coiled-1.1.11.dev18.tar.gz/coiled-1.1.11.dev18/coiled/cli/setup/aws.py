...
def aws_setup(
    ...
    account: str | None,
    iam_user: str,
    setup_policy: str | None,
    ongoing_policy: str | None,
    update_policies: bool,
    update_instance_policy: bool,
    cloudshell_link: bool | None,
...
def make_access_key(iam, user, *, retry=0, accept_delete: bool = False) -> tuple[str | None, str | None]:
    try:
...
def get_session(aws_profile: str | None, region_name: str | None, skip_telemetry: bool = False):
    try:
...
def request_quota_increase(quota_client, region: str, quota: dict, desired_value: int | float) -> tuple[bool, str]:
    quota_label = quota["class"]
...
def get_support_case_link(case_id: str | None) -> str:
    if case_id:
