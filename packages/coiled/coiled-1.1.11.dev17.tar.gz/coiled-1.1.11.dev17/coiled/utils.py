...
def get_account_membership(user_dict: dict, memberships: list, account: "str" | None = None) -> dict | None:
    if account is None:
...
def scheduler_ports(protocol: str | list[str]):
...
async def get_worker_creation_notification_error(cloud, account: str | None) -> str | None:
...
def parse_requested_memory(
    memory: int | (str | (float | (list[str] | (list[int] | list[float])))) | None,
    min_memory: str | (int | float) | None,
) -> dict:
...
def any_gpu_instance_type(type_list: str | list[str] | None) -> bool:
...
def validate_backend_options(backend_options):
    ...
    all_key_types = {
        **get_type_hints(AWSOptions),
        **get_type_hints(GCPOptions),
    }
    keys_by_type = {
...
def validate_vm_typing(vm_types: list[str] | (tuple[str] | None)):
...
def parse_bytes_as_gib(size: str | int | None) -> int | None:
