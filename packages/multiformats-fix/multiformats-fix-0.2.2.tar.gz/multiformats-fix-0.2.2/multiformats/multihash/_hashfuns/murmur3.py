...
def _murmur3(version: str, digest_bits: int) -> Hashfun:
    try:
        ...
        raise ImportError(
            "Module 'mmh3' must be installed to use 'murmur3' hash functions. Consider running 'pip install mmh3'."
        ) from e
    if version == "32":
        ...
        def hashfun(data: BytesLike, size: Optional[int] = None) -> bytes:
            ...
            if not isinstance(data, bytes):
                data = bytes(data)
            d: bytes = mmh3.hash(data, signed=False).to_bytes(
                4, byteorder="big"
        ...
        def hashfun(data: BytesLike, size: Optional[int] = None) -> bytes:
            ...
            if not isinstance(data, bytes):
                data = bytes(data)
            d: bytes = mmh3.hash128(data, signed=False).to_bytes(
                16, byteorder="big"
            )  # pylint: disable = c-extension-no-member
            return d if size is None else d[:size]
...
def _jit_register_murmur3(m, register) -> bool:  # type: ignore
    ...
    if register is not None:
        register(
            f"murmur3-x64-{digest_bits}", _murmur3("x64", digest_bits), digest_bits // 8
        )
    return True
