...
from .manifest_utils import _load_manifest, _get_tests_and_nodes
from .transform_utils import _create_transform


def deploy_tests_cmd(manifest_file, hostname, data_service, dataflow):
    
    # Load manifest file
    manifest = _load_manifest(manifest_file)

    # Get nodes and dependencies
    test_data, nodes_with_test_data = _get_tests_and_nodes(manifest)
    ...
    applier.apply(data_service_id=data_service, dataflow=dataflow_def, delete=False, dry_run=False)


def delete_tests_cmd(manifest_file, hostname, data_service, dataflow):
    # Create Ascend Client
    client = Client(hostname)
    ...
    # Get test nodes
    test_data, _ = _get_tests_and_nodes(manifest)
    ...
    try:
        client.delete_component_group(data_service_id=data_service, dataflow_id=dataflow, id="dbt_tests")
    except Exception as e:
        print(f"An error occurred while deleting dbt_tests group: {e}")

def check_test_results_cmd(manifest_file, hostname, data_service, dataflow):
    """
    ...
    for test_node in nodes_to_be_tested:
        try:
            test_results = client.get_transform_records(data_service_id=data_service, dataflow_id=dataflow, id=test_node, limit=1).data
    ...
    for test_node in nodes_to_be_tested:
        try:
            test_results = client.get_transform_quality_test_results(data_service_id=data_service, dataflow_id=dataflow, id=test_node).data
