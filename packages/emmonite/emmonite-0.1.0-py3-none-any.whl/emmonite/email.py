...
def send_email(email_sender, email_password, smtp_server, smtp_port,
               subject, body_path=None, body_str=None, variables=None, lista_correos=None, attachment_data=None, filename=""):
    """
    Envia un correo.
    ...
    try:
        if body_path:
            # Leer el contenido del archivo HTML y reemplazar las variables
            with open(body_path, 'r', encoding='utf-8') as file:
                html_string = file.read()
                html_template = Template(html_string)
                if variables:
                    body = html_template.safe_substitute(variables)
