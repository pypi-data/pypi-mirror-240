...
from awsiot.greengrasscoreipc.model import QOS, PayloadFormat
...
class GGv2ModbusConnector:
    ...
    def setup_modbus(self):
        ...
        if not self.check_modbus() \
            ...
            self.close_modbus()
            self.modbus_client = self.connection \
                .get(get_host, get_port, get_max_retry, get_wait_sec)
    ...
    def _publish_to(self, target, topic, model):
        _logger_.info(f"{time.ctime()} {topic}: {model} via {target}")
        if self.ggv2_ipc is not None:
            message = json.dumps(model)
            if self.IOT_CORE == target:
