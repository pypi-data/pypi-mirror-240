...
from .TestInjectorDemo import (
    ...
    Position3Service, Position4Service, box, FatherService
)
...
class InjectorTest(unittest.TestCase):
    ...
    def test_position_param_no_exist_has_kwargs(self):
        ...
        self.assertEqual(self.box.position1_service.show_params(), 1, msg='注入参数命名 与 宿主类 __init__ 参数不匹配')
    ...
    def test_position_param_args_kwargs1(self):
        ...
        self.assertEqual(self.box.position4_service.show_params1(), 100, '客户端修改参数')
        self.box.reset('position4_service')
        self.assertEqual(self.box.position4_service.show_params1(), 99, '重置时， 构造参数重载')
        self.assertEqual(self.box.position4_service.show_args(), (1, 2, 3, 4))
        self.assertEqual(self.box.position4_service.show_kwargs(), dict(params2=88))
    ...
    def test_service_api_inject(self):
        """ 服务中根据依赖服务的接口注入 """
        f_srv = FatherService('fa_params_1', fa_params_2=2)
        print(f'pre \n{"="*10}\n{f_srv.box} \n{"="*10}\n ')
        print(f_srv.child1_service)
        print(f'after \n{"="*10}\n{f_srv.box} \n{"="*10}\n ')
