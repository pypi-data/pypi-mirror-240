...
- Replaced ``miles_util`` with ``sps_util``, a more general and flexible
  function that can handle different stellar population synthesis (SPS) models.
  For now, I distribute (with permission) the ``E-MILES`` (Vazdekis+2016),
  ``GALAXEV`` (Bruzual & Charlot 2003) and ``fsps`` (Conroy+2009,2010) stellar
  population templates. One can now switch SPS by just changing the filename.
  
  IMPORTANT: I do not include the SPS models in the ``pPXF`` package any more,
  due to file size limitations. One must download the SPS templates after the
  ``pPXF`` installation, as illustrated in all the ``pPXF`` examples.
- I adapted all the examples to use the new ``sps_util``. 
...
- ``ppxf_example_kinematics_sdss`` and ``ppxf_example_kinematics_sauron``:
  Showed how to use ``varsmooth`` and ``log_rebin`` to convolve and rebin a
  full array of spectra.
