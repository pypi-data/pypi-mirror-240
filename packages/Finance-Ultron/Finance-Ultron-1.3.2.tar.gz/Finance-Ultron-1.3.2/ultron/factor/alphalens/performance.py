...
import ultron.factor.empyrical as ep
from pandas.tseries.offsets import BDay
from scipy import stats
from statsmodels.regression.linear_model import OLS
from statsmodels.tools.tools import add_constant
from ultron.factor.alphalens import utils
...
def quantile_turnover(quantile_factor, quantile, period=1):
    ...
    quant_name_sets = quant_names.groupby(
        level=['date']).apply(lambda x: set(x.index.get_level_values('asset')))
...
def create_pyfolio_input(factor_data,
    ...
    # returns
    #
    cumrets = factor_cumulative_returns(factor_data, period, long_short,
                                        group_neutral, equal_weight, quantiles,
                                        groups)
    ...
    #
    positions = factor_positions(factor_data, period, long_short,
                                 group_neutral, equal_weight, quantiles,
                                 groups)
    ...
    if capital is not None:
        positions = positions.mul(cumrets.reindex(positions.index) * capital,
                                  axis=0)
    ...
    if benchmark_period in fwd_ret_cols:
        ...
        benchmark_rets = factor_cumulative_returns(benchmark_data,
                                                   ...
                                                   group_neutral=False,
                                                   equal_weight=True)
        benchmark_rets = benchmark_rets.resample('1D').last().fillna(
            method='ffill')
