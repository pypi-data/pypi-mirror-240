...
def infer_trading_calendar(factor_idx, prices_idx):
    ...
    for day, day_str in enumerate(days_of_the_week):
        ...
        all_weekdays = pd.date_range(
            full_idx.min(),
            full_idx.max(),
            freq=CustomBusinessDay(weekmask=day_str)).normalize()
...
def compute_forward_returns(factor,
    ...
    for period in sorted(periods):
        ...
        for i in range(30):
            ...
            if p_idx is None or p_idx < 0 or (p_idx + period) >= len(
                    prices.index):
                continue
    ...
    df.set_index(pd.MultiIndex.from_product([factor_dateindex, prices.columns],
                                            names=['date', 'asset']),
                 inplace=True)
...
def save_file(table, name=None, fmt=None, path=None, handle=None):
    if isinstance(handle, str):
        table.to_csv(os.path.join(handle, "{0}.csv".format(name)))
    else:
        table.to_excel(handle, sheet_name=name)
...
def get_clean_factor(factor,
    ...
    if groupby is not None:
        if isinstance(groupby, dict):
            diff = set(factor_copy.index.get_level_values('asset')) - set(
                groupby.keys())
            if len(diff) > 0:
                raise KeyError("Assets {} not in group mapping".format(
                    list(diff)))
        ...
        if groupby_labels is not None:
            ...
            if len(diff) > 0:
                raise KeyError("groups {} not in passed group names".format(
                    list(diff)))
    ...
    quantile_data = quantize_factor(merged_data, quantiles, bins,
                                    binning_by_group, no_raise, zero_aware)
...
def get_clean_factor_and_forward_returns(factor,
    ...
    factor_data = get_clean_factor(factor,
                                   forward_returns,
                                   groupby=groupby,
...
def diff_custom_calendar_timedeltas(start, end, freq):
    ...
    if weekmask is not None and holidays is not None:
        ...
        actual_days = np.busday_count(
            ...
            np.array(end).astype('datetime64[D]'), weekmask, holidays)
    else:
        # default, it is slow
