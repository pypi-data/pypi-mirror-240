...
class GeneticMutationFactors(object):
    ...
    def ga_generate_ori(self, ori_field, add_field):
        ## 选取原始特征过程
        ori_list = (np.random.uniform(0, 1, (1, len(ori_field))) >
                    self._del_prob).tolist()[0]
        add_list = (np.random.uniform(0, 1, (1, len(add_field))) <
                    self._add_prob).tolist()[0]
    ...
    def ga_cross_next_group(self, ori_group, dict_score='ori', generation=0):
        ...
        cross_group = np.random.choice(g,
                                       size=int(len(g) / 2),
                                       p=p,
                                       replace=False)
        for (fa, mo) in itertools.combinations(cross_group, 2):
            ...
            if prob < self._change_prob:
                ...
                new1[pos] = np.random.uniform(2,
                                              len(accumulators_pool) + 1,
                                              count)
    ...
    def gevent_calc_factor(self, params):
        ...
        if i > 0:
            ...
            sub_data = accumulator.transform(
                factor_data.set_index('trade_date'),
                category_field='code',
                dropna=False)
    ...
    def gevent_evalue_group(self, params):
        ...
        for i in sub_group:
            if i > 0:
                ...
                factor_data = total_data[[
                    str(factor_name), 'code', 'trade_date'
                ]]
                jobs.append(
                    gevent.spawn(self.gevent_calc_factor,
        ...
        score = self._objective(sub_data.replace([np.inf, -np.inf], np.nan),
                                cols)
    ...
    def apply_calc_factor(self, data):
        ...
        if calc_acc > 0:
            ...
            sub_data = accumulator(factor_name).transform(
                data.fillna(0).set_index('trade_date'),
                name=name,
                category_field='code',
                dropna=False)
    ...
    def process_evalue_group(self, params):
        ...
        sub_factor_data = total_data.set_index([
            'trade_date', 'code'
        ])[evalue_cols[[True if i > 0 else False for i in sub_group]]].T
    ...
    def ga_evalue_group(self, sub_group, total_data, evalue_cols, diff_filed):
        ...
        for g, code in sub_group.items():
            jobs.append([sub_group[g], evalue_cols, total_data, diff_filed, g])
        ...
        for values in values_list:
            ...
            score_dict[g] = score
            cols_dict[g] = cols
            sub_data.sort_values(by=['trade_date', 'code'],
                                 ascending=True,
                                 inplace=True)
    ...
    def ga_kill_group(self,
        ...
        """
        # 生成新一代特征
        sub_group = self.ga_cross_next_group(ori_group,
                                             dict_score=dict_score,
                                             generation=generation)
        # 生成对应数据
        sub_data, ori_score_dict, cols_dict = self.ga_evalue_group(
            sub_group, total_data, evalue_cols, diff_filed)
        ori_score_se = pd.Series(ori_score_dict)
        MLog().write().info('Number of population:%d, Population average:%f' %
                            (len(ori_score_se), ori_score_se.mean()))
        #真实分数
        fact_se = ori_score_se.sort_values(ascending=False)[:self._group_num]
        #比例分数
        score_se = ori_score_se.sort_values(
            ascending=False)[:self._group_num] / (ori_score_se.sort_values(
                ascending=False)[:self._group_num].sum())

        liv_group = {}
        for k in score_se.keys():
            liv_group[k] = np.array([
                1 if i in cols_dict[k] else 0
                for i in np.array(sub_data.columns)
            ])

        factor_columns = sub_data.columns
        for diff in diff_filed:
            sub_data[diff] = total_data[diff].copy(
            ).values  #pandas.core.series.Series拷贝时根据索引排序
        return sub_data, liv_group, score_se, dict(fact_se), np.array(
            factor_columns), cols_dict

    def genetic_run(self,
                    total_data,
                    diff_filed,
                    sequence=['trade_date', 'code'],
                    factors_filed=None,
                    strong_field=None,
                    weak_field=None,
                    is_best=True):
        ...
        #初始种群
        ori_group = {
            i: self.ga_generate_ori(ori_field, add_field)
        ...
        for i in range(self._generations):
            ...
            best_fact_score = pd.Series(fact_se).sort_values(
                ascending=False)[0:1]
            ...
            now_best_score = (best_fact_score.values[0]
                              if best_fact_score.values[0] > last_score else
                              last_score)
            MLog().write().info(
                'Multiplicaint number:%d, Best score:%f,%d(%f) And %d(%f) Best group difference:%f,%d best and wores difference:%f'
                % ((i + 1), now_best_score, i + 1, best_fact_score.values[0],
                   i, last_score, diff_fact_score, i + 1, diff_score))
            if diff_fact_score < self._conver_prob and diff_score < self._conver_prob:
                break
            last_score = best_fact_score.values[0]

        if is_best:
            best_code = pd.Series(fact_se).sort_values(
                ascending=False)[0:1].index[0]
