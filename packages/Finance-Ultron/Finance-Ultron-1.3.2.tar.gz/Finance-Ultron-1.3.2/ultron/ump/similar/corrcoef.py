...
from ultron.kdutils.decorator import consume_time
from ultron.ump.core.fixes import rankdata
from ultron.ump.core.helper import pd_rolling_corr
...
def rolling_corr(df, ss=None, window=g_rolling_corr_window):
    ...
    if window < 1 or window > df.shape[0]:
        raise TypeError('window out of index, must in [{},{}]'.format(
            1, df.shape[0]))
    rolling_window = list(
        zip(np.arange(df.shape[0] - window + 1),
            np.arange(window, df.shape[0] + 1)))
    """
