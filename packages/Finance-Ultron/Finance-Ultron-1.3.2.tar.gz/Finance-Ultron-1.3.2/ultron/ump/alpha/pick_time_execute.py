...
from ultron.ump.alpha.pick_time_worker import PickTimeWork
from ultron.ump.alpha import trade_proxy
...
def _do_pick_time_work(capital,
                       buy_factors,
                       sell_factors,
                       kl_pd,
                       benchmark,
    ...
    pick_timer_worker = PickTimeWork(capital, kl_pd, benchmark, buy_factors,
                                     sell_factors)
    if show_pg:
    ...
    orders_pd, action_pd, _ = trade_proxy.trade_summary(
        pick_timer_worker.orders, kl_pd, show_info=show_info)
...
@add_process_env_sig
def do_symbols_with_same_factors(target_symbols,
                                 benchmark,
                                 buy_factors,
                                 sell_factors,
                                 capital,
    ...
    def _batch_symbols_with_same_factors(p_buy_factors, p_sell_factors):
        ...
        with MulPidProgress(len(target_symbols),
                            'pick times complete',
                            show_progress=show_progress) as progress:
            for epoch, target_symbol in enumerate(target_symbols):
                ...
                try:
                    ...
                    ret, fit_error = _do_pick_time_work(
                        capital,
                        p_buy_factors,
                        p_sell_factors,
                        kl_pd,
                    ...
                    kd_logger.exception("{0}:{1}".format(target_symbol, e))
                    continue
                ...
                if ret is None and back_target_symbols is not None:
                    ...
                    while True:
                        ...
                        kl_pd = kl_pd_manager.get_pick_time_kl_pd(
                            target_symbol)
                        ret, fit_error = _do_pick_time_work(capital,
                                                            p_buy_factors,
                                                            p_sell_factors,
                                                            kl_pd,
                                                            benchmark,
                ...
                r_orders_pd = ret[0] if r_orders_pd is None else pd.concat(
                    [r_orders_pd, ret[0]])
                r_action_pd = ret[1] if r_action_pd is None else pd.concat(
                    [r_action_pd, ret[1]])
        return r_orders_pd, r_action_pd, r_all_fit_symbols_cnt

    orders_pd, action_pd, all_fit_symbols_cnt = _batch_symbols_with_same_factors(
