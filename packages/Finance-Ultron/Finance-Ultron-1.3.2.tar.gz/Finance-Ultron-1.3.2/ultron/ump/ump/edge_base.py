...
"""在第二轮的相似度匹配中使用的方法，传递给UltronCorrcoef.corr_xy函数"""
...
class EdgeBase(Base):
    ...
    @classmethod
    def ump_edge_clf_dump(cls, orders_pd_train, market_name=None):
        """
        类方法，通过交易训练集orders_pd_train构造UltronUmpEdgeBase子类对象, 使用fit方法对训练集进行特征采集，后进行dump_clf即
        本地序列化存贮等工作
        ...
        :return: UltronUmpEdgeBase子类对象实例
        """
    ...
    def fit(self):
        ...
        win_top = len(self.fiter.df['profit_cg']) - len(
            self.fiter.df['profit_cg']) * K_CG_TOP_RATE
        """
            eg:
                len(self.fiter.df['profit_cg']) == 100
                -> win_top = 100 - 100 * 0.236
        ...
        self.fiter.df['rk'] = np.where(self.fiter.df['p_rk_cg'] > win_top,
                                       EEdgeType.E_STORE_TOP_WIN.value,
                                       self.fiter.df['rk'])
        # noinspection PyTypeChecker
        self.fiter.df['rk'] = np.where(self.fiter.df['p_rk_cg'] < loss_top,
    ...
    def dump_file_fn(self):
        """
            边裁本地缓存的存储路径规则：
            UltronEnv.g_project_data_dir ＋ 'ump/ump_edge_' ＋ market_name + self.class_unique_id()
        """
        ...
        unique_ump_name = 'ump/ump_edge_{}_{}'.format(self.market_name,
    ...
    def dump_clf(self):
        """
            ...
            通过UltronFileUtil.dump_pickle进行保存
        """
    ...
    def predict(self, **kwargs):
        ...
        df_x_dict = EdgeBase.dump_clf_manager.get_ump(self)

        # 从df_x_dict['fiter_df'].columns中筛选特征列
        feature_columns = df_x_dict['fiter_df'].columns.drop(
            ['profit', 'profit_cg', 'p_rk_cg', 'rk'])
        """
        ...
        distances_cx = pairwise_distances(con_x[0].reshape(1, -1),
                                          con_x[1:],
                                          metric='euclidean')
        ...
        n_top = K_N_TOP_SEED if len(distances_cx) > K_N_TOP_SEED else len(
            distances_cx)
        ...
        similar_cx = {
            arg: corr_xy(con_x[0], con_x[arg + 1], g_similar_type)
            for arg in distances_sort
        }
        """
            eg: similar_cx
        ...
        # 相似度大到小排序
        similar_sorted = sorted(zip(similar_cx.values(),
                                    similar_cx.keys()))[::-1]
        """
            eg: similar_sorted
        ...
        similar_filters = list(
            filter(lambda sm: sm[0] > K_SIMILAR_THRESHOLD, similar_sorted))
        """
            eg: similar_filters
            [(1.0, 0), (0.9936229414412624, 26), (0.92424298737490296, 1), (0.9197507467964976, 19)]
        """
        ...
        elif int(top_loss_cluster_cnt *
                 K_EDGE_JUDGE_RATE) > top_win_cluster_cnt:
            """
