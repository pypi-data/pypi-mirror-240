...
from ultron.factor.experimental.portfolio import er_portfolio_analysis as longshort_er_portfolio_analysis
from ultron.factor.analysis.factor_analysis import er_portfolio_analysis as onlylong_er_portfolio_analysis
from ultron.strategy.executor.naive import NaiveExecutor
from ultron.optimize.model.linearmodel import ConstLinearModel
from ultron.optimize.constraints import LinearConstraints, create_box_bounds, BoundaryType
...
class Optimize(object):
    ...
    def _build_setting(self, neutralized_styles, effective_industry,
        ...
        _boundary = {
            ...
            'maxabsrel': BoundaryType.MAXABSREL,
            'minabsrel': BoundaryType.MINABSREL
        }
        ...
        for name in total_risk_names:
            ...
            elif name in riskstyle:
                b_type.append(
                    _boundary[setting_params['riskstyle']['boundary']])
                l_val.append(setting_params['riskstyle']['lower'])
                u_val.append(setting_params['riskstyle']['upper'])
            #else:
    ...
    def _calculate_pos(self,
        ...
        epa = onlylong_er_portfolio_analysis if self.category == 'onlylong' \
            else longshort_er_portfolio_analysis
        try:
            target_pos, _ = epa(
                er=er,
    ...
    def create_positions(self, params, total_data):
        ...
        for ref_date, this_data in total_data.groupby('trade_date'):
    ...
    def backtest(self, positions, rate):
        ...
        if self.index_returns is not None:
            index_returns = self.index_returns.rename(
                columns={'returns': 'index'})
            returns = pd.concat(
                [ret_df, index_returns.set_index('trade_date')], axis=1)
            returns['benchmark_returns'] = returns['index']
            ret_df = returns[[
                'returns', 'turn_over', 'leverage', 'benchmark_returns'
    ...
    def empyrical(self, ret_df):
        ...
        for r in ['returns', 'benchmark_returns', 'excess_return']:
            ...
            mat['name'] = r
            rets.append(mat)
        return pd.DataFrame(rets) if len(rets) > 0 else None

    def run(self, params, rate=0.0):
        positions = self.rebalance_positions(params=params)
        returns = self.backtest(positions=positions, rate=rate)
