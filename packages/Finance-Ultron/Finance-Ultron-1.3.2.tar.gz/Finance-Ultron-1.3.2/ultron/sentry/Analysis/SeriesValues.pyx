...
cdef class SeriesValues(object):
    ...
    @cython.wraparound(False)
    @cython.cdivision(True)
    cpdef SeriesValues winsorize(self, int nums_stds, SeriesValues groups=None):
        cdef np.ndarray[double, ndim=1] data
        cdef np.ndarray[long long, ndim=1] order
        cdef np.ndarray[long long, ndim=1] index_diff
        cdef long long diff_loc
        ...
        if groups:
            ...
            for diff_loc in index_diff:
                curr_idx = order[start:diff_loc + 1]
                curr_values = data[curr_idx]
                ubound = nanmean(curr_values) + nums_stds * nanstd(curr_values)
                lbound = nanmean(curr_values - nums_stds * nanstd(curr_values))
                data[curr_idx] = where(
                    curr_values < lbound, lbound,
                    where(curr_values > ubound, ubound, curr_values))
                start = diff_loc + 1
            data[isnan(values)] = NAN
        else:
            data = values.copy()
            ubound = nanmean(data) + nums_stds * nanstd(data)
            lbound = nanmean(data - nums_stds * nanstd(data))
            data = where(values < lbound, lbound,
                            where(values > ubound, ubound, values))
            data[isnan(values)] = NAN
        return SeriesValues(data, self.name_mapping)
    
    cpdef SeriesValues neut(self, SeriesValues right):
        ...
        cdef double beta = nansum(x * y) / nansum(x * x)
        cdef double intercept = y_bar - beta * x_bar
        
        return SeriesValues(y - intercept - (beta * x), self.name_mapping)

    cpdef SeriesValues proj(self, SeriesValues right):
        cdef np.ndarray[double, ndim=1] y = self.values
        cdef np.ndarray[double, ndim=1] x = right.values
        cdef y_bar = nanmean(y)
        cdef x_bar = nanmean(x)
        ...
        return SeriesValues(intercept + (beta * x), self.name_mapping)


    cpdef SeriesValues sign_power(self, SeriesValues right):
        cdef np.ndarray[double, ndim=1] values
        
        values = self.values ** right.values
        
        values = where(self.values > 0, values, -1 * values)
        values[~np.isfinite(values)] = NAN

        return SeriesValues(values, self.name_mapping)


    cpdef SeriesValues floor_div(self, SeriesValues right):
        cdef np.ndarray[double, ndim=1] values

        values = floor(self.values / right.values)
        values[~np.isfinite(values)] = NAN
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef SeriesValues demean(self, SeriesValues groups=None):
        cdef np.ndarray[double, ndim=1] data
        cdef np.ndarray[long long, ndim=1] order
        cdef np.ndarray[long long, ndim=1] index_diff
        cdef long long diff_loc
        ...
        if groups:
            ...
            for diff_loc in index_diff:
                curr_idx = order[start:diff_loc + 1]
                data[curr_idx] = data[curr_idx] - nanmean(data[curr_idx])
                start = diff_loc + 1
            data[isnan(self.values)] = NAN
        else:
            data = self.values - nanmean(self.values)
            data[isnan(self.values)] = NAN
        return SeriesValues(data, self.name_mapping)

    cpdef SeriesValues softmax(self, SeriesValues groups=None):
        cdef np.ndarray[double, ndim=1] data
        cdef np.ndarray[long long, ndim=1] order
        cdef np.ndarray[long long, ndim=1] index_diff
        cdef long long diff_loc
        ...
        if groups:
            ...
            for diff_loc in index_diff:
                curr_idx = order[start:diff_loc + 1]
                curr_values = data[curr_idx]
                data[curr_idx] = np.exp(curr_values - np.max(curr_values)) / nansum(curr_values)
                start = diff_loc + 1
            data[isnan(self.values)] = NAN
        else:
            data = values.copy()
            data = np.exp(data - np.max(data)) / nansum(data)
            data[isnan(values)] = NAN
        return SeriesValues(data, self.name_mapping) 
    ...
    cpdef SeriesValues std(self, SeriesValues groups=None):
        cdef np.ndarray[double, ndim=1] data
        cdef np.ndarray[long long, ndim=1] order
        cdef np.ndarray[long long, ndim=1] index_diff
        cdef long long diff_loc
        ...
        if groups:
            ...
            for diff_loc in index_diff:
                curr_idx = order[start:diff_loc + 1]
                data[curr_idx] = nanstd(values[curr_idx])
                start = diff_loc + 1
            data[np.isnan(values)] = NAN
        else:
            data = np.ones_like(values) * nanstd(values)
            data[np.isnan(values)] = NAN
        return SeriesValues(data, self.name_mapping)

    cpdef SeriesValues skew(self, SeriesValues groups=None):
        cdef np.ndarray[double, ndim=1] data
        cdef np.ndarray[long long, ndim=1] order
        cdef np.ndarray[long long, ndim=1] index_diff
        cdef long long diff_loc
        ...
        if groups:
            ...
            for diff_loc in index_diff:
                curr_idx = order[start:diff_loc + 1]
                data[curr_idx] = skew(values[curr_idx])
                start = diff_loc + 1
            data[np.isnan(values)] = NAN
        else:
            data = np.ones_like(values) * skew(values)
            data[np.isnan(values)] = NAN
        return SeriesValues(data, self.name_mapping)

    cpdef SeriesValues sum(self, SeriesValues groups=None):
        cdef np.ndarray[double, ndim=1] data
        cdef np.ndarray[long long, ndim=1] order
        cdef np.ndarray[long long, ndim=1] index_diff
        cdef long long diff_loc
        ...
        if groups:
            ...
            for diff_loc in index_diff:
                curr_idx = order[start:diff_loc + 1]
                data[curr_idx] = nansum(values[curr_idx])
                start = diff_loc + 1
            data[np.isnan(values)] = NAN
        else:
            data = np.ones_like(values) * nansum(values)
            data[np.isnan(values)] = NAN
        return SeriesValues(data, self.name_mapping)
