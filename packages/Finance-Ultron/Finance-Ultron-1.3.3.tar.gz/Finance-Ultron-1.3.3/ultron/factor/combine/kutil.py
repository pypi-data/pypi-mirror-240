...
def calc_ic(factor_df,
            return_df,
            factor_list,
            return_col_name='target_return',
            ic_type='spearman'):
    """
    计算因子IC值, 本月和下月因子值的秩相关
    params:
            factor_df: DataFrame, columns=['code', 'trade_date', factor_list]
            return_df: DataFrame, colunms=['code, 'trade_date'， return_col_name], 预先计算好的未来的收益率
    ...
    for factor_name in factor_list:
        tmp_factor_ic = merge_df.groupby(['trade_date']).apply(lambda x: x[
            [factor_name, return_col_name]].corr(method=ic_type).values[0, 1])
