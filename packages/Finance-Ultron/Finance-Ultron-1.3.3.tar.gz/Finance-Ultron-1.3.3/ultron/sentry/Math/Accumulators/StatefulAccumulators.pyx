...
cdef class WAverage(StatefulValueHolder):
    def __init__(self, window, x):
        super(WAverage, self).__init__(window)
        self._inner  = build_holder(x)
        self._window = self._inner.window + window
        self._inner_window = window
        self._dependency = deepcopy(self._inner.dependency)
        self._average = 0.0
        self._weight =  1 / (0.5 * self._inner_window * (self._inner_window + 1))
    ...
    cpdef push(self, dict data):
        ...
        self._average += self._inner_window * self._weight * value - self._runningSum * self._weight
        popout = self._deque.dump(value, 0.)
        self._runningSum += value - popout
        self._isFull = self._isFull or self._inner.isFull()
    
    def __str__(self):
        return "\\mathrm{{WAverage}}({0}, {1})".format(self._inner_window,
                                                       str(self._inner))
...
cdef class MovingMedian(SortedValueHolder):
    def __init__(self, window, x):
        super(MovingMedian, self).__init__(window, x)


    cpdef double result(self):
        if self._sortedArray:
            return statistics.median(self._sortedArray)
        else:
            return NAN
...
cdef class MovingPercentage(SortedValueHolder):
    def __init__(self, window, x):
        super(MovingPercentage, self).__init__(window, x)

    cpdef double result(self):
        return self._cur_pos / self.size()

    def __str__(self):
        return "\\mathrm{{MPercent}}({0}, {1})".format(self._window, str(self._x))
...
cdef class MovingMaxDiff(SortedValueHolder):
    ...
    def __init__(self, window, x):
        super(MovingMaxDiff, self).__init__(window, x)
    
    
    cpdef double result(self):
        if self._sortedArray:
            return self._lastx - self._sortedArray[-1]
        else:
            return NAN

    def __str__(self):
        return "\\mathrm{{MMaxDiff}}({0}, {1})".format(self._window,
                                                       str(self._x))

cdef class MovingMinDiff(SortedValueHolder):
    
    def __init__(self, window, x):
        super(MovingMinDiff, self).__init__(window, x)
    
    cpdef double result(self):
        if self._sortedArray:
            return self._lastx - self._sortedArray[0]
        else:
            return NAN

    def __str__(self):
        return "\\mathrm{{MMinDiff}}({0}, {1})".format(self._window,
                                                       str(self._x))

cdef class MovingMinMaxCps(SortedValueHolder):
    
    def __init__(self, window, x, n=2.0):
        super(MovingMinMaxCps, self).__init__(window, x)
        self._n = n

    cpdef double result(self):
        if self._sortedArray:
            min_value = self._sortedArray[0]
            max_value = self._sortedArray[-1]
            return (min_value + max_value) - self._n * self._lastx

    def __str__(self):
        return "\\mathrm{{MCPS}}({0}, {1})".format(self._window,
                                                       str(self._x))

cdef class MovingMinMaxDiff(SortedValueHolder):
    
    def __init__(self, window, x, n=0.5):
        super(MovingMinMaxDiff, self).__init__(window, x)
        self._n = n
    ...
    cpdef double result(self):
        if self._sortedArray:
            min_value = self._sortedArray[0]
            max_value = self._sortedArray[-1]
            return self._lastx - self._n * (min_value + max_value)
...
cdef class MovingAverageDiff(SingleValuedValueHolder):
    ...
    def __init__(self, window, x):
        super(MovingAverageDiff, self).__init__(window, x)
        self._runningSum = 0.0
        self._lastx = NAN
    ...
    cpdef double result(self):
        ...
        if size:
            return self._lastx - self._runningSum / size
        else:
            return NAN
    
    def __str__(self):
        return "\\mathrm{{MAVDiff}}({0}, {1})".format(self._window, str(self._x))
...
cdef class MovingIR(SingleValuedValueHolder):
    ...
    def __init__(self, window, x, isPopulation=False):
        super(MovingIR, self).__init__(window, x)
        self._runningSum = 0.0
        self._runningSumSquare = 0.0
        self._isPop = isPopulation
        if not self._isPop:
    ...
    @cython.cdivision(True)
    cpdef double result(self):
        cdef size_t size = self._deque.size()
        cdef double tmp
        cdef double std
        cdef double mean
        ...
        mean = self._runningSum / size
        tmp = self._runningSumSquare - self._runningSum * self._runningSum / size
        ...
        if self._isPop:
            ...
            if size >= 2:
                ...
                std = NAN
        return std if isnan(std) else (mean / std)


cdef class MovingKurtosis(SingleValuedValueHolder):
    def __init__(self, window, x, isPopulation=False):
        super(MovingKurtosis, self).__init__(window, x)
        self._runningSum = 0.0
        self._runningSumFourthPower = 0.0
        self._isPop = isPopulation
        if not self._isPop:
            pyFinAssert(window >= 2, ValueError, "sampling standard deviation can't be calculated with window size < 2")
    ...
    cpdef push(self, dict data):
        ...
        # 更新滚动的一阶和四阶累积和
        self._runningSum += value - popout
        self._runningSumFourthPower += value**4 - popout**4
    ...
    cpdef double result(self):
        ...
        cdef double mean
        cdef double sum_diff_fourth_power
        cdef int n
        ...
        ### 计算方差
        variance = self._runningSumFourthPower / size - mean**4
        ...
        # 计算四阶中心矩和峰度
        sum_diff_fourth_power = self._runningSumFourthPower - (n +
                                                               1) * variance**2
                                
        return (sum_diff_fourth_power / (n * variance**2)) - 3
    
    def __str__(self):
        return "\\mathrm{{MKURT}}({0}, {1})".format(self._window, str(self._x))

cdef class MovingSkewness(SingleValuedValueHolder):
    
    def __init__(self, window, x, isPopulation=False):
        super(MovingSkewness, self).__init__(window, x)
        self._runningSum = 0.0
        self._runningSumSquare = 0.0
        self._runningSumCube = 0.0
        self._isPop = isPopulation
        if not self._isPop:
            pyFinAssert(window >= 2, ValueError, "sampling standard deviation can't be calculated with window size < 2")
    ...
    cpdef push(self, dict data):
        ...
        #更新滚动的三次方和
        self._runningSumCube += value**3 - popout**3
    ...
    cpdef double result(self):
        ...
        # 计算滚动方差
        cdef double variance = self._runningSumSquare - self._runningSum * self._runningSum / size

        # 计算滚动偏度
        cdef double skewness = (self._runningSumCube -
                    3 * self._runningSum * variance) / (size * variance**1.5)

        return skewness

    def __str__(self):
        return "\\mathrm{{MSKEW}}({0}, {1})".format(self._window, str(self._x))

cdef class MovingZScore(SingleValuedValueHolder):

    def __init__(self, window, x, isPopulation=False):
        super(MovingZScore, self).__init__(window, x)
        self._runningSum = 0.0
        self._runningSumSquare = 0.0
        self._lastx = NAN
        self._isPop = isPopulation
    ...
    cpdef double result(self):
        ...
        return std if isnan(std) else ((self._lastx - mean) / std)        
...
cdef class MovingProduct(SingleValuedValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MPRO}}({0}, {1})".format(self._window, str(self._x))


cdef class MovingConVariance(StatefulValueHolder):

    def __init__(self, window, x, y):
        super(MovingConVariance, self).__init__(window)
        self._runningSumLeft = 0.0
        self._runningSumRight = 0.0
        self._runningSum = 0.0
        self._runningSumSquareLeft = 0.0
    ...
    cpdef push(self, dict data):
        ...
        self._runningSumLeft += x - headLeft
        self._runningSumRight += y - headRight

        self._runningSum += x * y - headLeft * headRight

        self._runningSumSquareLeft += x * x - headLeft * headLeft
        self._runningSumSquareRight += y * y - headRight * headRight
    ...
    cpdef double result(self):
        ...
        cov_xy += self._runningSum - self._runningSumLeft * self._runningSumRight / length

        if self._isPop:
            return cov_xy / length
        else:
            if length >= 2:
                return cov_xy / (length - 1)
            else:
                return np.NAN
    
    def __str__(self):
        return "\\mathrm{{MCONVARIANCE}}({0}, {1})".format(self._window, str(self._x))
...
cdef class MACD(Accumulator):
    ...
    def __str__(self):
        return "\\mathrm{{MACD}}({0})".format(self._window)
...
cdef class MovingLogReturn(SingleValuedValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MLogReturn}}({0}, {1}, {2})".format(self._window, str(self._x))
...
cdef class MovingSharp(StatefulValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MSharp}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))
...
cdef class MovingSortino(StatefulValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MSortino}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))
...
cdef class MovingDrawdown(StatefulValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MDrawdown}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))
...
cdef class MovingMaxDrawdown(StatefulValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MMaxDrawdown}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))
...
cdef class MovingResidue(StatefulValueHolder):
    ...
    def __str__(self):
        return "\\mathrm{{MRes}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))

cdef class MovingMeanResidue(StatefulValueHolder):

    def __init__(self, window, x, y):
        super(MovingMeanResidue, self).__init__(window)
        self._cross = 0.
        self._xsquare = 0.
        self._runningx = 0.
        self._runningy = 0.
    ...
    cpdef push(self, dict data):
        ...
        self._runningx += x - head_x
        self._runningy += y - head_y
    ...
    @cython.cdivision(True)
    cpdef double result(self):
        cdef meanx = self._runningx / self.size()
        cdef meany = self._runningy / self.size()
        cdef double coef = self._cross / self._xsquare
        return meany - coef * meanx
...
cdef class MovingCoef(StatefulValueHolder):
    def __init__(self, window, x, y):
        super(MovingCoef, self).__init__(window)
        self._cross = 0.
        self._xsquare = 0.
        self._lasty = NAN
        self._lastx = NAN
    ...
    def __str__(self):
        return "\\mathrm{{MCoef}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))



cdef class MovingRSquared(StatefulValueHolder):
    def __init__(self, window, x, y):
        super(MovingRSquared, self).__init__(window)
        self._cross = 0.
        self._xsquare = 0.
        self._ysquare = 0.
        self._lastx = np.NAN
    ...
    cpdef push(self, dict data):
        ...
        self._cross += x * y - head_x * head_y
        self._xsquare += x * x - head_x * head_x
        self._ysquare += y * y - head_y * head_y
    ...
    @cython.cdivision(True)
    cpdef double result(self):
        cdef double numerator = self._cross * self._cross
        cdef double denominator = self._xsquare * self._ysquare
    ...
    def __str__(self):
        return "\\mathrm{{MRSquared}}({0}, {1}, {2})".format(self._window, str(self._x), str(self._y))