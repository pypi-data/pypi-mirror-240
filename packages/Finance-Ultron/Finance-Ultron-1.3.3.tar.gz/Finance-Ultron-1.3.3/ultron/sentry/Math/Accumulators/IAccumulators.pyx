...
from libc.math cimport log,log2,log10
...
from ultron.sentry.Math.MathConstants cimport NAN,ZERO
...
cdef class IAccumulator(object):
    ...
    def __mod__(self, right):
        return ModedValueHolder(self, right)
...
cdef class ModedValueHolder(CombinedValueHolder):
    def __init__(self, left, right):
        super(ModedValueHolder, self).__init__(left, right)
...
cdef class Log2(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(Log2, self).__init__(x, orig_value)

    cpdef double result(self):
        return log2(self._origValue)

    def __str__(self):
        return "\\log10({0})".format(str(self._inner))


cdef class Log10(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(Log10, self).__init__(x, orig_value)

    cpdef double result(self):
        return log10(self._origValue)
...
cdef class Frac(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(Frac, self).__init__(x, orig_value)

    cpdef double result(self):
        return sign(self._origValue) * (abs(self._origValue) - floor(self._origValue))


cdef class SigLog2Abs(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(SigLog2Abs, self).__init__(x, orig_value)

    cpdef double result(self):
        return sign(self._origValue) * log2(1 + abs(self._origValue))

    def __str__(self):
        return "\\mathrm{{SigLog2Abs}}({0})".format(str(self._inner))


cdef class SigLog10Abs(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(SigLog10Abs, self).__init__(x, orig_value)

    cpdef double result(self):
        return sign(self._origValue) * log10(1 + abs(self._origValue))

    def __str__(self):
        return "\\mathrm{{SigLog10Abs}}({0})".format(str(self._inner))

cdef class SigLogAbs(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(SigLogAbs, self).__init__(x, orig_value)
    
    cpdef double result(self):
        return sign(self._origValue) * log(1 + abs(self._origValue))

    def __str__(self):
        return "\\mathrm{{SigLogAbs}}({0})".format(str(self._inner))

cdef class SigSqrtAbs(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(SigSqrtAbs, self).__init__(x, orig_value)

    cpdef double result(self):
        return sqrt(abs(self._origValue)) * sign(self._origValue)

    def __str__(self):
        return "\\mathrm{{SigSqrtAbs}}({0})".format(str(self._inner))
...
cdef class Relu(BasicFunction):
    def __init__(self, x, orig_value=NAN):
        super(Relu, self).__init__(x, orig_value)

    cpdef double result(self):
        return max(self._origValue, ZERO)