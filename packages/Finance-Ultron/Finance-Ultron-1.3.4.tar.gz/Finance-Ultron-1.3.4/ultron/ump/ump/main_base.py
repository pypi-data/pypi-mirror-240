...
# 1. 在sub_ncs的聚类范围内[40~80],遍历对进行聚类。
# 2. 聚类后对df进行分组
# 3. 计算每组的失败率和成功率即 result为0和1所占概率
# 4. 通过threshold过滤掉失败率低的组
# 5. 记录聚类数， 此聚类模型，此聚类符合阈值的组号及df值
# 6. 返回clf_component_dict 和 cluster_df_dict
def _do_gmm_cluster(sub_ncs, x, df, threshold):
    ...
    with Progress(len(sub_ncs), 'gmm fit') as progress:
        for epoch, component in enumerate(sub_ncs):
            ...
            if len(cluster_ind) > 0:
                ...
                clf_component_dict[component] = (
                    clf, cluster_ind)  # 聚类对应 --> (聚类模型,符合失败率分组编号 cluster_ind)
                """
    ...
    return clf_component_dict, cluster_df_dict
    #clf_component_dict: key: 聚类数量， value:(聚类模型，聚类后的符合阈值的组号)
    #cluster_df_dict:key: 聚类数量_符合阈值的组号， values:符合阈值的组号的df数据，特征+标记
...
class MainBase(Base):
    ...
    # 1. 计算 聚类数_分类簇的 lcs:交易个数，lrs:分类簇中失败亏损的交易数量/分类簇中总交易数量(失败率), lms:分类簇中的交易的总收益比例(总收益率), lps:分类簇中的交易的平均收益(平均收益)
    # 2. 分类簇对应订单信息计算收益率
    def _fit_cprs(self):
        """
    ...
    def _fit_brust_min(self, brust_min):
        ...
        if brust_min:
            # 使用全局最优参数计算brust_min
            # 使用lrs，lps，lms的最优组合，针对最优结果effect_num即提升次数最多，因优化器是最小，所以实现中取负数
