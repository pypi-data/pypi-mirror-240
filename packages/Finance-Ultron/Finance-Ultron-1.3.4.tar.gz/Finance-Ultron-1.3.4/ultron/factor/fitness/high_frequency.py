...
class HighFrequencyWeighted(object):
    ...
    def __init__(self):
        self._industry_styles = [
            'Bank', 'RealEstate', 'Health', 'Transportation', 'Mining',
            'NonFerMetal', 'HouseApp', 'LeiService', 'MachiEquip', 'BuildDeco',
            'CommeTrade', 'CONMAT', 'Auto', 'Textile', 'FoodBever',
            'Electronics', 'Computer', 'LightIndus', 'Utilities', 'Telecom',
            'AgriForest', 'CHEM', 'Media', 'IronSteel', 'NonBankFinan',
            'ELECEQP', 'AERODEF', 'Conglomerates'
        ]
        self._risk_styles = [
    ...
    def returns(self, factor_se, forward_returns):
        weights = factor_se.groupby(level=['trade_date']).apply(
            self.to_weights)
        weighted_returns = forward_returns.multiply(weights, axis=0)

        factor_ret_se = weighted_returns.groupby(level='trade_date').sum()
        turnover_se = weights.unstack().diff().abs().sum(axis=1)
        long_count = factor_se.groupby(level=['trade_date']).apply(
            self.to_ls_count)
        short_count = factor_se.groupby(level=['trade_date']).apply(
            lambda x: self.to_ls_count(x, long=False))
        #计算方向
        forward_returns.name = 'nxt1_ret'
        factor_se.name = 'factor'
        forward_returns.reset_index().merge(factor_se.reset_index(),
                                            on=['trade_date', 'code'])
        ic_mean = forward_returns.reset_index().merge(
            factor_se.reset_index(),
            on=['trade_date',
                'code']).groupby('trade_date').apply(lambda x: np.corrcoef(
                    x['factor'], x['nxt1_ret'])[0, 1]).mean()
    ...
    def run(self,
        ...
        factors_se = factor_data.merge(risk_data.reset_index(),
                                       on=['code', 'trade_date'])
        ndiff_field = [
            i for i in list(set(factors_se.columns)) if i not in fac_list
        ]
        ...
        for k, g in grouped:
            if is_neutralize:
                ret_preprocess = neutralize(
                    g[self._industry_styles +
                      risk_styles].values.astype(float),
                    factor_processing(
                        g[fac_list].fillna(0).values,
            ...
            for k in ndiff_field:
                f[k] = g[k].values
            alpha_res.append(f)
        factor_se = pd.concat(alpha_res).set_index(['trade_date',
                                                    'code'])[factor_name]
