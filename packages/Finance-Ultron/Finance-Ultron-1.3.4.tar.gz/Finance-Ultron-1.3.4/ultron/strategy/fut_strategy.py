...
class Strategy(object):
    ...
    def prepare_risk_model(self, name='unshrunk', window=20):
        models = {}
        returns_data = self.total_data[['trade_date', 'code', 'prev1_ret']]
    ...
    def backtest(self, positions, yields, rate):
        ...
        if self.index_return is not None:
            index_return = self.index_return.rename(
                columns={'returns': 'index'})
            returns = pd.concat(
                [ret_df, index_return.set_index('trade_date')], axis=1)
