...
__jac_import__(target='impl.cmds_impl', base_path=__file__)
from impl.cmds_impl import *
import impl.cmds_impl
...
@cmd_reg.register
def enter(filename: str, entrypoint: str, args: list):
    if filename.endswith('.jac'):
        [base, mod] = os.path.split(filename)
        base = './' if not base else base
        mod = mod[:-4]
...
@cmd_reg.register
def ast_tool(tool: str, args: list=[]):
